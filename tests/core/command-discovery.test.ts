...
const EXPECTED_GROUPS = ["allowlist", "gasless", "gasless transfer", "bridge", "oneclick", "oneclick source", "circle", "circle approval", "circle source", "policy", "mcp", "doctor", "wallet", "wallet permission", "wallet policy", "x402", "x402 fetch", "pay", "pay transfer", "operation", "receipt"];
const EXPECTED_COMMANDS = [
  ...
  "allowlist inventory", "allowlist resolve",
];
...
test("group help renders exact subgroup usages and complete leaf synopses", () => {
  assert.equal(renderHelp(["wallet"]), [
    ...
    "  apn wallet import --profile <profile> --key-file <absolute-path> --key-name <env-name> --expected-address <checksummed-address> — Import a private key from an owner-only file into a new encrypted local wallet profile.",
...
test("manifest validator and compatibility gate reject every declared structural or semantic drift class", () => {
  ...
  previousVersion.product_version = "0.3.0";
  ...
  assert.doesNotThrow(
    ...
    "schema-compatible 0.3.0 to 0.3.1 evolution is permitted",
  );
