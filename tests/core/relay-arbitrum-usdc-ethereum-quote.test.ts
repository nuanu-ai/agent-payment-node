...
test("order, recipient, source token, bounds, signature, and unsigned transactions fail closed", async () => {
  const edits: Array<[string, (q: any) => void]> = [
    ...
    ["approval action", q => { q.steps[0].action = "Sign arbitrary authorization"; }],
    ["deposit action", q => { q.steps[1].action = "Send transaction elsewhere"; }],
    ...
    ["details expected output", q => { q.details.currencyOut.amount = "999999999"; }],
    ["details source minimum", q => { q.details.currencyIn.minimumAmount = "1"; }],
