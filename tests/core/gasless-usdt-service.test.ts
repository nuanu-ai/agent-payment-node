...
test("gasless USDT service binds profile, preserves canonical records, and keeps status/resume read-only", async t => {
  ...
  const repository = new UsdtOperationRepository(temporary.root), service = new GaslessUsdtOperationService(repository).forProfile(PROFILE);
  ...
  await assert.rejects(() => new GaslessUsdtOperationService(repository).forProfile("c".repeat(64)).status(operation.operationId), { code: "APN_OPERATION_NOT_FOUND" });
});

test("gasless USDT service never exposes effect-capable approval, signer, dispatch, or recovery", async t => {
  const temporary = await temporaryState(); t.after(temporary.cleanup);
  const service = new GaslessUsdtOperationService(new UsdtOperationRepository(temporary.root)).forProfile(PROFILE);
...
test("repository-bound adapter refuses unscoped profile access", async t => {
  const temporary = await temporaryState(); t.after(temporary.cleanup);
  const service = new GaslessUsdtOperationService(new UsdtOperationRepository(temporary.root));
  await assert.rejects(() => service.status("b".repeat(64)), /profile binding is required/u);
});
