...
const REQUEST = { command: "x402.fetch.prepare" as const, profile: "default", chainId: 1 as const, url: X402_URL, idempotencyKey: "eth-x402-synthetic" };
for (const body of [undefined, null, "", Buffer.from('{"planId":"synthetic","idempotencyKey":"merchant-one"}').toString("base64")]) {
  test(`Ethereum actual encrypted signer completes ${body === undefined ? "legacy GET" : body === null ? "absent POST" : body === "" ? "empty POST" : "JSON POST"} without Base allowance or replacement`, async (context) => {
    const temporary = await temporaryState(); context.after(temporary.cleanup);
    const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
    ...
    assert.equal(original.network, "eip155:1"); assert.equal(original.token, x402Network(1).token.toLowerCase());
    ...
    assert.equal(payload.accepted.network, "eip155:1");
    ...
    assert.equal(stored.settlementEvidence?.chainId, "1");
...
test("Ethereum never inherits Base policy; lowering current cap blocks new signing and first exposure but not reconciliation", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await fixture.core.wallet.ensure("default");
  ...
  await fixture.core.wallet.policySet({ command: "wallet.policy.set", profile: "default", chainId: 1, ...NETWORK_LIMITS });
  const prepared = await fixture.core.execute(REQUEST); assert.equal(prepared.ok, true, JSON.stringify(prepared));
  const operationId = (prepared.operation as { operationId: string }).operationId;
  const lower = { command: "wallet.policy.set" as const, profile: "default", chainId: 1 as const, ...NETWORK_LIMITS, maxX402AmountAtomic: "1" };
...
test("Ethereum offer selection refuses other networks, assets, mechanisms and early Arbitrum", () => {
  const required = { ...X402_PAYMENT_REQUIRED, accepts: [{ ...X402_REQUIREMENTS, network: "eip155:1" as const, asset: x402Network(1).token }] };
  assert.equal(inspectCandidates(required, X402_URL, 1).length, 1);
  assert.equal(inspectCandidates(required, X402_URL).length, 0);
  for (const override of [{ network: "eip155:42161" as const }, { asset: x402Network(8453).token }, { extra: { ...X402_REQUIREMENTS.extra, assetTransferMethod: "permit2" } }, { extra: { assetTransferMethod: "erc7710", facilitatorAddresses: ["0x2222222222222222222222222222222222222222"] } }]) {
    assert.equal(inspectCandidates({ ...required, accepts: [{ ...required.accepts[0]!, ...override }] }, X402_URL, 1).length, 0);
  }
});

for (const bodyBase64 of [undefined, null, "", Buffer.from('{"planId":"synthetic"}').toString("base64")]) {
  test(`Ethereum fresh processes recover encrypted authorization and ambiguous POST ${JSON.stringify(bodyBase64)} without replacement`, async (context) => {
    const temporary = await temporaryState(); context.after(temporary.cleanup);
    const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
    fixture.http.outcomes[0] = networkChallenge(1, true);
...
for (const failure of ["overfunded", "insufficient", "domain", "chain"] as const) test(`Ethereum current ${failure} blocks first exposure without losing the signed operation`, async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
  const prepared = await fixture.core.execute(REQUEST); assert.equal(prepared.ok, true, JSON.stringify(prepared));
  const operationId = (prepared.operation as { operationId: string }).operationId;
...
test("Ethereum settlement rejects Base token logs and foreign evidence even with resealed hashes", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
...
test("Ethereum CLI/MCP policy and x402 share one encrypted state and exact chain handoff", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await fixture.core.wallet.ensure("default");
  ...
  const setInput = { profile: "default", chain: "eip155:1", max_balance_usdc_atomic: NETWORK_LIMITS.maxBalanceUsdcAtomic, max_x402_amount_atomic: NETWORK_LIMITS.maxX402AmountAtomic };
  ...
  assert.match(JSON.stringify(handoff.error?.details), /set-network/); assert.match(JSON.stringify(handoff.error?.details), /eip155:1/);
  ...
  assert.equal(configured.ok, true, JSON.stringify(configured)); assert.equal(fixture.policyApprovals[0]?.x402Network?.chainId, 1);
  const shown = await invoke("apn_wallet_policy_show_network", { profile: "default", chain: "eip155:1" });
  ...
  const input = { profile: "default", chain: "eip155:1", url: X402_URL, idempotency_key: REQUEST.idempotencyKey, rpc_url: "https://rpc.example", method: "POST", body_base64: "" };
...
test("Ethereum expiry requires finalized selected-network absence, not a timeout or Base balance", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
  const prepared = await fixture.core.execute(REQUEST); assert.equal(prepared.ok, true);
  const operationId = (prepared.operation as { operationId: string }).operationId;
  ...
  const stored = (await fixture.state.findX402Operation(operationId))!;
  assert.equal(stored.unusedExpiryEvidence?.chainId, "1"); assert.equal(stored.unusedExpiryEvidence?.token, frozen.token);
  assert.equal(fixture.http.calls.length, 1); assert.equal(stored.attempts.length, 0);
});

test("Ethereum first-exposure policy reads inherit the caller's bounded deadline", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
  const prepared = await fixture.core.execute(REQUEST); assert.equal(prepared.ok, true);
...
test("Ethereum rechecks receipt and safe block after authorization-state reads before accepting settlement", async (context) => {
  const temporary = await temporaryState(); context.after(temporary.cleanup);
  const fixture = networkFixture(temporary.root); await initializeNetworkFixture(fixture);
