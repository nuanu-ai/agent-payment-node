...
import { RelayArbitrumDepositPreflightReader, preflightRelayArbitrumDeposit } from
  "../../src/relay/arbitrum-deposit-preflight.js";
...
test("deposit requires canonical allowance and deposit gas quote at the pinned block", async t => {
  const { temporary, operation } = await prepared(); t.after(temporary.cleanup);
  let index = 0;
  const insufficient = await preflightRelayArbitrumDeposit(operation, active(), owner, "0", now,
    { now: () => now, batch: async () => rows(0n)[index++]! });
  assert.equal(index, 3);
  assert.equal(insufficient.readOnlyConditionsSatisfied, false);
  assert.deepEqual(insufficient.reasons, ["allowance_below_principal"]);
  index = 0;
  const ready = await preflightRelayArbitrumDeposit(operation, active(), owner, "0", now,
    { now: () => now, batch: async () => rows(500_000n)[index++]! });
  assert.equal(ready.readOnlyConditionsSatisfied, true);
  assert.equal(ready.quotedMaxFeePerGas, (operation.arbitrumDraft!.rawQuote as any).steps[1].items[0].data.maxFeePerGas);
  index = 0;
  await assert.rejects(preflightRelayArbitrumDeposit(operation, active(), owner, "0", now,
    { now: () => now, batch: async () => index++ < 2 ? rows(500_000n)[index - 1]! :
      ["0xa4b1", { ...head, hash: `0x${"34".repeat(32)}` }, "0x7", "0x7"] }),
  { code: "APN_OPERATION_BLOCKED" });
});

test("deposit reader shares one 24 POST guard and stops on 429", async t => {
  const { temporary, state, operation } = await prepared(); t.after(temporary.cleanup);
  let index = 0, tick = 1000;
  const guard = new EvmDirectRpcGuard(state, 24, () => tick, async milliseconds => { tick += milliseconds; });
  const reader = new RelayArbitrumDepositPreflightReader("https://arb-rpc.example", state,
    { batchCall: async () => rows(500_000n)[index++]! }, () => guard, async () => {});
  assert.equal((await reader.read(operation, active(), owner, "0", now, () => now)).readOnlyConditionsSatisfied, true);
  assert.equal(guard.physicalRequests, 3);
  const rateLimited = new RelayArbitrumDepositPreflightReader("https://arb-rpc-429.example", state,
