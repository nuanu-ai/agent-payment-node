...
After the approval is safely observed at the exact principal and before that deadline, execution obtains a new quote and snapshots its hash, source block and 60 second local expiry. The integrity-bound `postApprovalQuote` also freezes the current pool and token code, route finality policy, Executor native-drop cap, policy binding, source token/native balances, exact allowance, pending nonce, fee quote, fresh finalized destination balance baseline, exact `eth_call`, exact `eth_estimateGas`, bridge envelope and recomputed maximum debit. Amount, owner minimum output, quoted loss, native drop, mechanism, fee ceilings, bridge gas ceiling and total native-debit cap are re-applied. The preparation quote is never used to sign this bridge. A restart reuses the one stored post-approval snapshot; a missing snapshot can be recreated through read-only quoting, while a stored snapshot can never be rewritten. The bridge must be signed and durably marked before the stored post-approval quote expires.
