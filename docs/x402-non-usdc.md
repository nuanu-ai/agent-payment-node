...
There is no signer port, approval UI, approve transaction, send RPC, settlement dispatcher, custody, journal transition, CLI/MCP route, or payment response codec in this change. The production read port requires an explicitly configured RPC source, active owner profile, authenticated local account resolver and usage ledger. It rechecks that account after reading. It is not installed on a CLI or MCP route. Receipt verification records evidence only; it never claims that an HTTP response or facilitator assertion completed payment.

The adapter requires an Avalanche `finalized` block no older than 30 seconds and uses that exact block number for every contract read. Its RPC call dependency must be bound by the caller to one trusted Avalanche endpoint; the port cannot independently prove the remote endpoint's identity. PayAI `/supported` advertises the v2 exact network and EIP-2612 extension, but does not name the token or Permit2 method. The token and proxy capability remain pinned from the prior read-only evidence, so a changed provider implementation can still refuse later. There is no live merchant or paid acceptance in this change.
