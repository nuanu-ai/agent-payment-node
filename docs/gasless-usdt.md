...
The separate `UsdtBoundOperationRepository` and `GaslessUsdtOperationService.prepareBound` persist the complete preparation as a private, integrity-checked local record. `statusBound` reads it back; `resumeBound` rechecks the active owner policy, usage admission, paymaster expiry and safe chain snapshot, returning a read-only classification. Revocation, policy changes and nonce drift fail closed. A later safe block without proof of the original block's canonicality requires recovery. The older dormant v1 journal remains readable for existing status callers and does not contain the complete binding. Neither journal reserves usage, signs or dispatches. A future runtime adapter must authenticate the safe block and policy, reserve shared usage atomically with its operation lifecycle, and recheck before any signature or effect. This package has no CLI or MCP execution path for the new bound preparation and no fresh mainnet transfer acceptance.
