...
This package contains Ethereum USDT gasless capability checks and a guarded library send core. It contains the exact asset, chain and sponsor registry; quote, paymaster, UserOperation and receipt codecs; nonce, balance and allowance-slot reads; stable refusal classification; and a read-only rehearsal fixture. The `gasless usdt prepare` CLI and MCP commands save an unsigned, policy-bound operation. `gasless usdt status` reads that saved binding.

`LocalUsdtSigningService` signs the saved binding in local custody. `UsdtExecutionJournal` reserves common asset usage and persists the may-have-sent marker. `GuardedUsdtSendService` can issue one `eth_sendUserOperation` call through the fixed keyless Pimlico endpoint after fresh guards and a durable hash-bound marker. The send core is library-only: the CLI and MCP still expose unsigned prepare and read-only status, and no settlement or payment response is wired. The rehearsal loads no key, signs nothing and sends nothing.
