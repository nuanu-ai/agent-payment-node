...
One prepare permits at most **19 physical public RPC attempts**, with no retry, polling or batch fallback. Ethereum uses 14: chain ID, safe block identity, four pinned code hashes, four contract invariants, owner code, USDT balance, EntryPoint nonce and EOA nonce. Account and invariant reads use the canonical safe block hash. The EOA nonce is needed for the unsigned EIP-7702 authorization stub when the account is undelegated. The fixed sponsor uses five: quote twice, gas price twice and paymaster data once. The second quote and gas price reads enforce the existing post-paymaster drift check. The command has a 60-second total read deadline; each endpoint family shares one persisted in-flight slot, at least 750 ms between starts and a fail-closed 429 cooldown. Public RPC execution and live payment acceptance are still unverified.
