...
Set credential-free `APN_ETHEREUM_RPC_URL` and `APN_UNICHAIN_RPC_URL`, then use `apn stargate native prepare`, `execute`, `observe`, `status`, and `receipt`. `status` is a local journal read. `observe` is the explicit network operation that may advance an already attempted operation from safe chain evidence; it never signs or resends. Set `APN_STARGATE_NATIVE_PREPARE_BATCH=1` to opt into strict read-only JSON-RPC batching for this Ethereum to Unichain native prepare. The default uses scalar requests. A rejected or malformed batch aborts preparation without retry or scalar fallback. This changes transport grouping only; the final `quoteSend` fee is still rechecked before the envelope is frozen. The synthetic success model is 38 logical reads in 16 physical POSTs, including the destination baseline reads. The scalar path now makes 36 POSTs because the source quote block and destination baseline block are rechecked after their dependent reads. Provider support and the live count have not been verified.
