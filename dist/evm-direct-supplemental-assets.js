...
export const DIRECT_EVM_SUPPLEMENTAL_ASSETS = SUPPLEMENTAL.map((entry) => ({
...
export function directEvmSupplementalAsset(chain, identifier) {
...
export function resolveDirectPolicyAsset(input, inventory) {
    if (input.rail === "direct" && input.kind === "token" && input.identifier !== undefined && input.chain.startsWith("eip155:")) {
        let canonical;
        try {
            canonical = getAddress(input.identifier);
        }
        catch { /* The frozen resolver classifies invalid identities. */ }
...
//# sourceMappingURL=evm-direct-supplemental-assets.js.map