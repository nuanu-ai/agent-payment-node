...
export async function retryDirect(method, _params, oneAttempt, wait = async (milliseconds) => {
...
export async function submitDirect(method, params, oneAttempt) {
    try {
...
export async function withEndpointRole(read, endpointRole) {
    try {
        return await read;
    }
    catch (error) {
...
export function isReceiptFallbackError(error) {
    if (!(error instanceof ApnError))
        return false;
...
export function historicalReceiptUnavailable(value, chainId, target, method) {
    let error;
    try {
...
export function knownPublicNodeReceiptCapability(chainId, target, method, body) {
...
export function missingHistoricalArchive() {
...
export function rpcBodyMethod(body) {
    try {
...
export function decodeAtomicBatchResponse(response, requests) {
...
export function isArchiveBatchItem(method, params) {
...
export function isReceiptBatchShape(items) {
    if (items.length !== 2)
        return false;
...
export function rpcArchiveChainValue(chainId) {
    return (value) => {
...
export function rpcFallbackChainValue(chainId, role) {
    return (value) => {
        const observed = evmRpcQuantity(value);
        if (observed !== BigInt(chainId))
...
export function rpcReceiptFallbackValue(expected) {
    return (value) => {
        if (value === null)
            return null;
...
export function assertMatchingHeader(raw, expected) {
...
export function quantity(n) { return `0x${n.toString(16)}`; }
export function eip7702Delegation(code) { return /^0xef0100[0-9a-f]{40}$/iu.test(code); }
