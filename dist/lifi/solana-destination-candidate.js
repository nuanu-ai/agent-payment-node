...
export function solanaJsonAccountKeys(resultValue) {
    ...
    if (version !== undefined && version !== "legacy" && version !== 0)
        protocolFailure();
    ...
    if (version === 0) {
        ...
        for (const item of rpcArray(wire.addressTableLookups, 64)) {
            ...
            if (new Set([...writableIndexes, ...readonlyIndexes]).size !== writableIndexes.length + readonlyIndexes.length)
                protocolFailure();
            writableCount += writableIndexes.length;
            readonlyCount += readonlyIndexes.length;
        }
        if (writableCount !== writable.length || readonlyCount !== readonly.length)
            protocolFailure();
    }
    else if (writable.length !== 0 || readonly.length !== 0 || wire.addressTableLookups !== undefined)
        protocolFailure();
