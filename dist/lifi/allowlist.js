...
export class BridgeAllowlistGate {
    ...
    async follow(op, target) {
        ...
        const move = async (state, outcome = false) => {
        ...
        if (target === "unknown_finality") {
            ...
            if (current.state === "submitted")
                await move("unknown_finality");
            return;
        }
        if (target === "failed_before_effect") {
            if (current.state !== "reserved")
                bridgeFailure("APN_STATE_CORRUPT", "bridge_usage_release_after_effect");
    ...
    async active(profile, owner) {
        ...
        if (active.accounts.evm !== owner)
            refuse("allowlist_account_mismatch", "The active owner allowlist policy names a different EVM account.");
        return active;
    }
}
...
export function bridgeUsageTarget(op) {
    ...
    if (op.state === "failed_after_approval" || op.state === "failed_confirmed_revert")
...
function bridgeSubject(account, request) {
    ...
    return { account, chain: `eip155:${request.fromChainId}`, asset: row.kind === "native"
            ? { kind: "native", identifier: null } : { kind: "token", identifier: row.address },
...
function evaluate(registry, input) {
    try {
        return evaluateAssetPolicy(registry, input);
    }
    catch (error) {
        return mapCap(error);
    }
...
function mapCap(error) {
    if (error instanceof ApnError && error.code === "APN_OPERATION_BLOCKED") {
        ...
        if (error.message.includes("per-transfer cap"))
            refuse("allowlist_per_transfer_cap_exceeded", error.message);
    }
    throw error;
}
function refuse(reason, message) { throw new ApnError("APN_ALLOWLIST_REFUSED", message, { reason, rail: "bridge" }); }
