...
import { bridgeIntentBinding, bridgeSnapshot, sealBridgeOperation } from "./operation-model.js";
...
export const BASE_DEPLOYMENT_MIGRATION_SCHEMA = "apn.bridge-base-usdc-deployment-migration.v1";
...
const sourceBlock = { numberAtomic: "25991578", hash: "0xa5d09b09f95cb83bd60cac35e4da453286c69a3123e1ea80ce23f8d75c3ff770", timestampAtomic: "1789580279" };
const destinationBlock = { numberAtomic: "51395469", hash: "0x74f557f8da50f60b81ab32f4871dc619ddf92b08f505edd8c9a56ef8169f2222", timestampAtomic: "1789580285" };
...
const verifiedSourceDeployment = { ...oldSourceDeployment, rpcOrigin: "https://eth.drpc.org", block: sourceBlock };
const newDestinationDeployment = { ...oldDestinationDeployment, rpcOrigin: "https://base.drpc.org", block: destinationBlock };
export const BASE_DEPLOYMENT_MIGRATION_CANDIDATE = Object.freeze({ schemaVersion: BASE_DEPLOYMENT_MIGRATION_SCHEMA,
    ...
    sourceSafeBlock: { numberAtomic: "26024956", hash: "0x5e404085d24ddc013b6312c58725ced1473ce5a984b3fe3dce46f3ecab91ac88", timestampAtomic: "1789982423" },
    destinationSafeBlock: { numberAtomic: "51596747", hash: "0x5a7faf1f747fdf165ed9267831df50e8e69782d601db422d87a6bef92b5bb937", timestampAtomic: "1789982841" },
...
export function assertBaseDeploymentMigrationCandidate(operation) {
...
export function assertBaseDeploymentMigrationProof(operation, source, sourceDeployment, destination, destinationDeployment, sourceFinalityBlock, destinationFinalityBlock) {
    ...
    if (destination.transaction.chainId !== 8453 || destination.transaction.transactionHash !== c.destinationTransactionHash ||
        ...
        hashObject({ ...destination.transaction, safeBlock: null }) !== c.destinationTransactionProofHash)
        blocked("destination_transaction_mismatch");
    if (!same(destinationDeployment, c.newDestinationDeployment) || !same(destinationDeployment.block, destination.transaction.block))
    ...
    try {
        parsedDestination = bridgeDestinationProof(parsedSource, operation.intent.materialization, operation.intent.decoded, destination.receipt);
    }
    catch {
        return blocked("destination_correlation_mismatch");
    }
...
export function assertBaseDeploymentMigrationSourceProof(operation, source, sourceDeployment) {
    const c = BASE_DEPLOYMENT_MIGRATION_CANDIDATE;
    const finalized = (proof, minimum) => proof.status === "success" && proof.safeBlock !== null &&
        BigInt(proof.safeBlock.numberAtomic) >= BigInt(minimum.numberAtomic) &&
        (proof.safeBlock.numberAtomic !== minimum.numberAtomic || same(proof.safeBlock, minimum));
    if (source.transaction.chainId !== 1 || source.transaction.transactionHash !== c.sourceTransactionHash ||
        ...
        blocked("source_transaction_mismatch");
    const savedSource = operation.effects.find((effect) => effect.role === "bridge")?.safeProof;
    if (savedSource === null || savedSource === undefined || !same({ ...source.transaction, safeBlock: null, rpcOrigin: null }, { ...savedSource, safeBlock: null, rpcOrigin: null }))
        blocked("source_transaction_identity_mismatch");
    if (!same(sourceDeployment, c.verifiedSourceDeployment) || !same(sourceDeployment.block, source.transaction.block))
        blocked("source_deployment_mismatch");
    let parsedSource;
    try {
        parsedSource = bridgeSourceProof(operation.intent.materialization, operation.intent.decoded, source.receipt);
    }
    catch {
        return blocked("source_correlation_mismatch");
    }
    if (!same(parsedSource, operation.sourceProof) || parsedSource.sourceAmountAtomic !== c.route.amountAtomic ||
        ...
        parsedSource.correlation.outputAmountAtomic !== c.route.outputAmountAtomic || parsedSource.correlation.recipient !== word(c.route.recipient))
        blocked("source_correlation_mismatch");
}
export function migrateBaseDeploymentOperation(operation, destinationProof) {
    ...
    if (destinationProof === undefined || destinationProof.transactionHash !== BASE_DEPLOYMENT_MIGRATION_CANDIDATE.destinationTransactionHash ||
        destinationProof.transactionProofHash !== BASE_DEPLOYMENT_MIGRATION_CANDIDATE.destinationTransactionProofHash)
        blocked("destination_proof_missing");
...
function hasCurrentShape(operation) {
...
function assertOldDigest(operation) {
    const c = BASE_DEPLOYMENT_MIGRATION_CANDIDATE;
    if (operation.fingerprint !== c.oldFingerprint || operation.integrityHash !== c.oldIntegrityHash ||
        operation.transitions.at(-1)?.transitionHash !== c.oldTransitionRoot)
        blocked("old_digest_mismatch");
}
function assertCandidate(operation, current) {
    ...
    if (operation.operationId !== c.operationId || operation.profileHash !== c.profileHash ||
        (current ? !operation.terminal || operation.state !== "completed" : operation.terminal || operation.state !== "unknown_finality"))
        blocked("operation_identity_mismatch");
    if (r.fromChainId !== c.route.fromChainId || r.toChainId !== c.route.toChainId || r.fromToken !== c.route.fromToken ||
        ...
        operation.intent.materialization.tool !== c.route.tool)
    ...
    if (!same(operation.intent.sourceDeployment, expectedSource) || !same(operation.intent.destinationDeployment, expectedDestination) ||
        operation.intent.sourceRpcOrigin !== expectedSource.rpcOrigin || operation.intent.destinationRpcOrigin !== expectedDestination.rpcOrigin)
        blocked("deployment_mismatch");
    if (!current)
        assertOldDigest(operation);
    if ((current ? operation.destinationProof?.transactionHash !== c.destinationTransactionHash : operation.destinationProof !== null) ||
        ...
        operation.providerObservation.destinationTransactionHash !== c.destinationTransactionHash)
    ...
    if (current && (operation.fingerprint !== c.newFingerprint || operation.integrityHash !== c.newIntegrityHash ||
        operation.transitions.at(-1)?.transitionHash !== c.newTransitionRoot))
        blocked("new_digest_mismatch");
}
function resealBase(operation, current, destinationProof) {
    const c = BASE_DEPLOYMENT_MIGRATION_CANDIDATE, draft = structuredClone(operation);
    if (!current) {
        const restored = draft.transitions.at(-2);
        if (restored === undefined)
            blocked("migration_transition_missing");
        draft.transitions.pop();
        draft.updatedAt = restored.at;
        draft.terminal = false;
        for (const key of ["state", "approval", "sourceProof", "destinationProof", "providerObservation", "destinationScan", "failure", "usageLease"]) {
    ...
    draft.intent.sourceDeployment = source;
    draft.intent.destinationDeployment = destination;
    draft.intent.sourceRpcOrigin = source.rpcOrigin;
    draft.intent.destinationRpcOrigin = destination.rpcOrigin;
    draft.intent.sourceAccount.rpcOrigin = source.rpcOrigin;
    if (current) {
        draft.intent.allowlist = null;
        draft.usageLease = null;
    }
    else {
        delete draft.intent.allowlist;
        delete draft.usageLease;
    }
    ...
    const transitions = draft.transitions.map((entry) => {
        ...
        if (current)
            body.usageLease = null;
        else
            delete body.usageLease;
    ...
    if (current) {
        ...
        draft.terminal = true;
        draft.destinationProof = destinationProof;
        draft.failure = { reason: "delivery_correlated", residualAllowance: null };
        const snapshot = bridgeSnapshot(draft), body = { ...snapshot, at: draft.updatedAt, previousHash };
        const transition = { ...body, transitionHash: hashObject(body) };
    ...
    if (current)
        ...
        validateLegacyBridgeOperation(migrated);
    return migrated;
}
function auditFor(oldOperation, newOperation) {
    const body = { schemaVersion: BASE_DEPLOYMENT_MIGRATION_SCHEMA, operationId,
        ...
        touchedFields: ["intent.destinationDeployment", "intent.destinationRpcOrigin", "intent.allowlist", "usageLease", "transitions[].usageLease", "state",
            ...
            "transitions[].previousHash", "transitions[].transitionHash", "transitions[]", "integrityHash"] };
    return { ...body, auditDigest: hashObject(body) };
}
function word(address) { return `0x${address.slice(2).toLowerCase().padStart(64, "0")}`; }
//# sourceMappingURL=base-deployment-migration.js.map