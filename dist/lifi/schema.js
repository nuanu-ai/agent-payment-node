...
export const addressSchema = z.string().refine((v) => { try {
    return bridgeAddress(v) === v;
}
catch {
    return false;
} });
export const isoSchema = z.string().refine((v) => { try {
    return bridgeIso(v) === v;
}
catch {
    return false;
} });
...
export const originSchema = z.string().max(256).refine((v) => { try {
    const u = new URL(v);
...
const observationRpcFailureSchema = z.strictObject({
    ...
    stage: z.enum(["source_observation", "source_transaction", "source_receipt", "source_included_block", "source_safe_head",
        ...
        "destination_fee_evidence", "destination_deployment", "destination_protocol_evidence"]),
    ...
    reason: z.enum(["bridge_RPC_HTTP_status", "bridge_RPC_response", "receipt_transaction_membership",
        ...
        "bridge_archive_block_mismatch", "bridge_prepared_block_reorg", "transaction_chain_or_hash",
        "transaction_signature", "transaction_parity", "transaction_signature_reconstruction",
