...
const PRE_SIGN_RPC_METHODS = new Set(["eth_chainId", "eth_getBlockByNumber", "eth_getBalance", "eth_getCode",
    "eth_getStorageAt", "eth_getTransactionCount", "eth_call", "eth_estimateGas", "eth_maxPriorityFeePerGas", "debug_traceTransaction"]);
async function preSignRpc(input, work) {
    try {
        return await work();
    }
    catch (error) {
        ...
        const method = typeof candidate === "string" && PRE_SIGN_RPC_METHODS.has(candidate)
            ? candidate : null;
        const context = { schemaVersion: "apn.bridge-presign-rpc-failure.v1", phase: "pre_sign_guard", ...input, method };
...
function rpcBoundary(role, stage, chainRole, chainId, category) {
    return { effectRole: role, stage, chainRole, chainId, category };
}
export async function guardBridgeEffect(op, role, source, destination, now) {
    ...
    const [sourceDeployment, destinationDeployment] = await Promise.all([
        preSignRpc(rpcBoundary(role, "source_deployment_refresh", "source", source.chainId, "deployment_refresh"), async () => await source.deployment(m.tool, m.request.toChainId, m.request.fromToken)),
        preSignRpc(rpcBoundary(role, "destination_deployment_refresh", "destination", destination.chainId, "deployment_refresh"), async () => await destination.deployment(m.tool, m.request.fromChainId, m.request.toToken)),
    ]);
    ...
    const estimate = await preSignRpc(rpcBoundary(role, "source_execution_simulation", "source", source.chainId, "simulation"), async () => await source.estimate({ chainId: envelope.chainId, from: envelope.from, to: envelope.to, data: envelope.data,
    ...
    for (const e of op.effects) {
        if (e.submissionAttempts === 1) {
            ...
            const quote = await preSignRpc(rpcBoundary(role, "source_fee_quote", "source", source.chainId, "fee_quote"), async () => await source.feeQuote(e.envelope));
