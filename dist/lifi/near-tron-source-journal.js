...
const ROUTE = "base_usdc_to_tron_usdt_lifi_near_intents";
function fail(reason) { return bridgeFailure("APN_OPERATION_BLOCKED", `near_tron_journal_${reason}`); }
export function bindNearTronSourcePreparationToJournal(input) {
    ...
    if (draft.route !== ROUTE || draft.profileHash !== input.profileHash || draft.operationId !== input.operationId ||
        ...
        !isoSchema.safeParse(input.createdAt).success)
        fail("draft_identity");
    ...
    if (!isPlainRecord(p) || !exactKeys(p, ["kind", "executionAdmitted", "evidenceTrust",
        ...
        "maxSourceNativeDebitWei", "preparationDigest"]))
        fail("preparation_shape");
    if (p.kind !== "read_only_near_tron_source_preparation" || p.executionAdmitted !== false ||
        p.evidenceTrust !== "untrusted_quote_and_rpc")
        fail("preparation_kind");
    ...
    if (!hashSchema.safeParse(p.preparationDigest).success || hashObject(preparedBody) !== p.preparationDigest ||
        ...
        !wordSchema.safeParse(p.quoteId).success || !wordSchema.safeParse(p.preflightBlockHash).success)
        fail("preparation_digest_or_quote");
    ...
    if (!isPlainRecord(c) || !exactKeys(c, ["chainId", "from", "to", "valueAtomic", "data",
        ...
        "maxPriorityFeePerGasAtomic", "accessList"]))
        fail("source_call_shape");
    if (!addressSchema.safeParse(c.from).success || !addressSchema.safeParse(c.to).success ||
        ...
        !uintSchema.safeParse(c.maxPriorityFeePerGasAtomic).success)
        fail("source_call_shape");
    if (p.quoteId !== inspection.quoteId || p.quoteId !== draft.provider.quoteId ||
        ...
        draft.provider.stepId !== bridgeRecord(q.includedSteps[1]).id ||
        ...
        c.gasLimitAtomic !== BigInt(tx.gasLimit).toString() ||
        !Array.isArray(c.accessList) || c.accessList.length !== 0)
        fail("source_call");
    ...
    if (tip > maxFee || gas * maxFee + value > cap ||
        p.maxSourceNativeDebitWei !== draft.maxSourceNativeDebitWei)
        fail("envelope_or_cap");
    ...
    if (!addressSchema.safeParse(admission.backendSigner).success ||
        ...
        typeof admission.note !== "string" || admission.note.length < 1 || admission.note.length > 256)
        fail("synthetic_admission");
    ...
    const binding = Object.freeze({ profileHash: input.profileHash, operationId: input.operationId,
        draftIntegrityHash: draft.integrityHash, route: ROUTE, createdAt: input.createdAt,
        sourceCall: Object.freeze({ ...c, accessList: Object.freeze([]) }),
...
//# sourceMappingURL=near-tron-source-journal.js.map