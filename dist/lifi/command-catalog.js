...
const output = { contract: "apn.cli.v1", success_exit: 0, failure_exit: 1,
    ...
    failures: ["Classified APN error; no provider fallback or automatic replacement effect."] };
const bridgeStates = { terminal: ["completed", "failed_before_effect", "failed_after_approval", "failed_confirmed_revert"],
    non_terminal: ["awaiting_approval", "execution_pending", "source_pending", "destination_pending", "unknown_finality"] };
...
export function includeBridgeRecovery(commands) {
...
function bridgeLegToken(value) {
    if (value === "native")
        return BRIDGE_ZERO_ADDRESS;
    const token = bridgeAddress(value, "APN_INVALID_INPUT");
    if (token === BRIDGE_ZERO_ADDRESS)
        bridgeFailure("APN_INVALID_INPUT", "native_leg_is_named_native");
    return token;
}
export function bindBridgeCommand(path, o) {
    if (path === "bridge capabilities")
        return { command: "bridge.capabilities", ...(o["--profile"] === undefined ? {} : { profile: o["--profile"] }) };
    if (path === "bridge inventory")
        return { command: "bridge.inventory" };
    if (path === "bridge prepare")
        return { command: "bridge.prepare", profile: o["--profile"], quote: bridgeHash(o["--quote"], "APN_INVALID_INPUT"),
    ...
    const request = validateBridgeRequest({ fromChainId, toChainId, fromToken, toToken,
        ...
        maxRouteFeeAtomic: bridgeDecimal(o["--max-route-fee"], decimals), slippageBps: slippage > 1000n ? -1 : Number(slippage) });
