...
const output = { contract: "apn.cli.v1", success_exit: 0, failure_exit: 1,
    success: "Saved exact Circle V2 Base approval state.", failures: ["Classified APN failure; no automatic resend."] };
...
function v(o, key) { return o[key]; }
function uint(o, key) { return bridgeUint(v(o, key), false, "APN_INVALID_INPUT").toString(); }
export function bindCircleCommand(path, o) {
    if (path === "circle approval execute" || path === "circle approval status")
        return {
    ...
    if (path === "circle approval prepare")
        return { command: "circle.approval.prepare", profile: v(o, "--profile"),
    ...
    if (path !== "circle source submit")
        throw new Error("Unknown Circle command");
    const setup = v(o, "--recipient-setup");
    if (setup !== "existing_ata" && setup !== "create_ata")
        throw new ApnError("APN_INVALID_INPUT", "Unknown Circle recipient setup.");
...
//# sourceMappingURL=circle-command-catalog.js.map