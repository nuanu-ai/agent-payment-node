...
export function verifyBnbCompositeTrace(raw, transactionHash, message, call, transaction) {
    ...
    try {
        ...
        if (executorArgs[0] !== call.transactionId || executorArgs[1].length !== 1 || executorArgs[2] !== BNB_COMPOSITE.weth || executorArgs[3] !== call.finalReceiver)
            mismatch();
        const swap = executorArgs[1][0];
        if (swap.callTo !== BNB_COMPOSITE.flyRouter || swap.approveTo !== BNB_COMPOSITE.flyRouter || swap.sendingAssetId !== BNB_COMPOSITE.weth ||
            swap.receivingAssetId !== "0x0000000000000000000000000000000000000000" || String(swap.fromAmount) !== call.inputAmountAtomic ||
            swap.callData !== call.flyCalldata || swap.requiresDeposit !== true)
            mismatch();
        if (executor.error !== null) {
            if (receiverEffects.length !== 4)
                mismatch();
            if (walk(executor).some((frame) => frame.type === "CALL" && frame.to === call.finalReceiver && frame.value > 0n && frame.error === null))
                mismatch();
        ...
        if (v[0].poolId !== BNB_COMPOSITE.poolId || Number(v[0].kind) !== 0 || v[0].assetIn !== BNB_COMPOSITE.weth ||
            ...
            v[1].toInternalBalance !== false || v[2] !== 0n || v[3].toString() !== call.deadlineAtomic)
            mismatch();
        if (!/^0x[0-9a-f]{64}$/u.test(vault.output))
            mismatch();
        const vaultOutput = BigInt(vault.output);
        if (vaultOutput <= 0n)
            mismatch();
        if (vault.calls.length !== 3)
            mismatch();
        const pool = vault.calls[0];
        if (pool === undefined || pool.type !== "STATICCALL" || pool.to !== BNB_COMPOSITE.pool || pool.error !== null || pool.value !== 0n ||
            pool.output !== vault.output)
            mismatch();
        const poolArgs = decode(pool.input, "onSwap"), request = poolArgs[0];
        if (Number(request.kind) !== 0 || request.tokenIn !== BNB_COMPOSITE.weth || request.tokenOut !== BNB_COMPOSITE.wbnb ||
            ...
            request.to !== BNB_COMPOSITE.core || request.userData !== "0x" || BigInt(request.lastChangeBlock) <= 0n || poolArgs[1] <= 0n || poolArgs[2] <= 0n)
            mismatch();
        ...
        if (error instanceof TraceMismatch)
            return { outcome: "protocol_mismatch", transactionHash, inputAmountAtomic: call.inputAmountAtomic,
                vaultOutputAtomic: null, deliveredAmountAtomic: "0", retainedAmountAtomic: "0", traceHash };
        throw error;
    }
}
...
function mismatch() { throw new TraceMismatch(); }
function retainedOutput(actual, expected, retentionBps) {
    return actual <= expected ? actual : [expected, actual - actual * retentionBps / 10000n].reduce((a, b) => a > b ? a : b);
}
function traceNode(raw, bounded, depth) {
    if (depth > 32 || ++bounded.count > 1024 || raw === null || typeof raw !== "object" || Array.isArray(raw))
        fail("bnb_trace_shape");
    const r = raw, allowed = new Set(["type", "from", "to", "value", "input", "output", "error", "revertReason", "calls", "gas", "gasUsed"]);
    if (Object.keys(r).some((key) => !allowed.has(key)) || (r.type !== "CALL" && r.type !== "STATICCALL"))
        fail("bnb_trace_shape");
    const calls = r.calls === undefined ? [] : r.calls;
    if (!Array.isArray(calls) || calls.length > 256)
        fail("bnb_trace_shape");
    let from, to, input, output, value;
    try {
        from = getAddress(String(r.from));
        to = getAddress(String(r.to));
        input = bridgeHex(r.input ?? "0x", 24_576);
        output = bridgeHex(r.output ?? "0x", 24_576);
        value = BigInt(String(r.value ?? "0x0"));
    }
    catch {
        return fail("bnb_trace_shape");
    }
    if (value < 0n || (r.error !== undefined && typeof r.error !== "string"))
        fail("bnb_trace_shape");
...
function decode(input, name) {
    try {
        const d = decodeFunctionData({ abi: TRACE_ABI, data: input });
