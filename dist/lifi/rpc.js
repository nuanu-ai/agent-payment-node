...
const BASE_ECOTONE_EIP6780_TIMESTAMP = 1710374401n; // 2024-03-14T00:00:01Z
...
function deploymentAggregateItem(rows, tag) {
...
export function bridgeRpcCall(chainId, environment, options = {}) {
    ...
    const batchAttempt = async (target, body, now = Date.now(), endpointRole = "primary", telemetrySession) => {
    ...
    const sessionBatchCall = (session) => async (items, route = "primary") => {
        ...
        const attempt = async (body) => await batchAttempt(target, body, session.currentTime(), role, session);
    ...
    const sessionCall = (session) => async (method, params) => {
        ...
        if (isHistoricalStateRead(method, params)) {
            if (distinctArchive === null)
                return missingHistoricalArchive();
            const archiveAttempt = (m, p) => oneAttempt(distinctArchive, m, p, session.currentTime(), "archive", session);
...
export class BridgeRpc {
    ...
    async deployment(tool, peerChainId, token, block, exactHashPin = false) {
        ...
        const codeRows = [...contract.code, ...feeContract.code], verificationCodeRows = compactDeployment
            ? [...codeRows, { address: getAddress(MULTICALL3_ADDRESS), codeHash: multicallHash }] : codeRows, readRows = [...contract.reads, ...feeContract.reads], aggregateGroups = compactDeployment ? [contract.reads, feeContract.reads].map((rows) => rows.filter((row) => row.kind === "call" && deploymentMulticallEligible(row.data))).filter((rows) => rows.length > 0) : [], aggregateRows = aggregateGroups.flat(), directRows = readRows.filter((row) => !aggregateRows.includes(row));
        const extraItems = [];
        ...
        for (const row of verificationCodeRows) {
            ...
            if (bytes === "0x" || eip7702Delegation(bytes) || keccak256(bytes) !== row.codeHash)
        ...
        for (const rows of aggregateGroups) {
            ...
            for (let index = 0; index < rows.length; index += 1)
                observedByRead.set(hashObject(rows[index]), aggregate[index]);
        }
    ...
    async refreshDeployment(tool, peerChainId, token, frozen) {
        ...
        const items = [
            ...
            ...feeContract.code.map((row) => ({ method: "eth_getCode", params: [row.address, tag], cachePolicy: "immutable",
                decoder: rpcHexValue(128 * 1024) })),
            ...directRows.map((row) => ({ method: row.kind === "storage" ? "eth_getStorageAt" : "eth_call",
                params: row.kind === "storage" ? [row.address, row.data, tag] : [{ to: row.address, data: row.data }, tag],
                cachePolicy: "immutable", decoder: rpcHexValue(64 * 1024) })),
        ...
        const configuration = readRows.map((row) => {
            ...
            if (observed === undefined)
                return bridgeFailure("APN_RPC_PROTOCOL", "bridge_multicall_response");
            if (observed !== row.expected)
                return bridgeFailure("APN_PROVIDER_PROTOCOL", "bridge_deployment_configuration_changed");
            return { ...row, expected: observed };
        });
        const recheck = values[offset++], preparedRecheck = values[offset];
        if (evmRpcQuantity(recheck.number).toString() !== at.numberAtomic || evmRpcHex(recheck.hash, 32) !== at.hash)
            bridgeFailure("APN_RPC_PROTOCOL", "bridge_block_reorg");
    ...
    async basePinnedFeeValues(gas, block, safe) {
        if (this.batchCall === undefined)
            bridgeFailure("APN_RPC_CONFIG", "bridge_batch_unavailable");
        ...
        if (multicallHash === undefined || multicallHash === null)
            bridgeFailure("APN_RPC_CONFIG", "bridge_multicall_unreviewed");
        const codeRows = [...BASE_FEE_CONTRACT.code, { address: getAddress(MULTICALL3_ADDRESS), codeHash: multicallHash }], directRows = BASE_FEE_CONTRACT.reads.filter((row) => row.kind !== "call" || !deploymentMulticallEligible(row.data)), aggregateRows = BASE_FEE_CONTRACT.reads.filter((row) => row.kind === "call" && deploymentMulticallEligible(row.data));
        const operatorData = `0x275aedd2${gas.toString(16).padStart(64, "0")}`;
        const items = [
            { method: "eth_chainId", params: [], cachePolicy: "immutable", decoder: rpcExpectedChainValue(this.chainId) },
            ...codeRows.map((row) => ({ method: "eth_getCode", params: [row.address, tag], cachePolicy: "immutable", decoder: rpcHexValue(128 * 1024) })),
            ...directRows.map((row) => ({ method: "eth_getStorageAt", params: [row.address, row.data, tag], cachePolicy: "immutable", decoder: rpcHexValue(64 * 1024) })),
        ...
        for (const row of directRows)
            if (values[offset++] !== row.expected)
                bridgeFailure("APN_RPC_PROTOCOL", "Base_fee_configuration_identity");
        const aggregate = values[offset++];
        for (let index = 0; index < aggregateRows.length; index += 1) {
            if (aggregate[index] !== aggregateRows[index].expected)
                bridgeFailure("APN_RPC_PROTOCOL", "Base_fee_configuration_identity");
        }
        const rawReturn = values[offset++], rawScalar = values[offset++], rawConstant = values[offset++];
        assertMatchingHeader(values[offset++], block);
        assertMatchingHeader(values[offset], safe);
    ...
    async observeCanonical(hash, expected, nativeDelivery, includeFees) {
        ...
        const fees = includeFees ? await bridgeActualFees(this.chainId, r, block, this.call, this.chainId === 8453 && this.batchCall !== undefined ? async (gas, pinned) => await this.basePinnedFeeValues(gas, pinned, safe) : undefined) : null;
...
function eip7702Delegation(code) { return /^0xef0100[0-9a-f]{40}$/iu.test(code); }
