...
function fail(reason) { return bridgeFailure("APN_PROVIDER_PROTOCOL", `circle_v2_source_preparation_${reason}`); }
...
function quantity(value) { return bridgeUint(value); }
export async function prepareCircleV2BaseSourceReadOnly(draft, transport, readBase, limits, now = Date.now) {
    ...
    if (copy.kind !== "circle_v2_preflighted_draft" || copy.state !== "preflighted_unsubmitted" || copy.executionAdmitted !== false ||
        copy.sourceChainId !== 8453 || integrityDigest !== digest(unsignedDraft))
        fail("draft_integrity");
    ...
    if (principal === 0n || fee >= principal || fee > quantity(copy.maxSourceFeeAtomic) || requiredDebit > (1n << 256n) - 1n)
        fail("quote_fee");
    ...
    if (newNumber < oldNumber || newNumber - oldNumber > 20n ||
        (newNumber === oldNumber && bridgeHex(copy.preflight.blockHash, 32, 32) !== bridgeHex(preflight.blockHash, 32, 32)))
        fail("stale_or_reorged_preflight");
    let state;
    try {
        state = await readBase({ payer, token: BASE_USDC, spender: tx.to, to: tx.to, data: tx.data, valueAtomic: tx.valueAtomic,
    ...
    if (state.chainId !== 8453 || bridgeAddress(state.payer) !== payer ||
        ...
        quantity(state.blockNumber) !== newNumber || bridgeHex(state.blockHash, 32, 32) !== bridgeHex(preflight.blockHash, 32, 32))
        fail("base_identity");
    ...
    if (balance < requiredDebit)
        fail("usdc_balance");
    if (maxAllowance < requiredDebit || maxAllowance > balance || maxAllowance === (1n << 256n) - 1n ||
        allowance < requiredDebit || allowance > maxAllowance || allowance === (1n << 256n) - 1n)
        fail("bounded_allowance");
    ...
    if (gas === 0n || gas > BRIDGE_MAX_GAS || gas > quantity(limits.maxGasLimitAtomic) || maxFee === 0n || priority > maxFee ||
        maxFee > quantity(limits.maxFeePerGasWei) || priority > quantity(limits.maxPriorityFeePerGasWei))
        fail("gas_or_fee_cap");
    ...
    if (nativeDebit > (1n << 256n) - 1n)
        fail("native_debit_overflow");
    if (nativeDebit > quantity(limits.maxNativeDebitWei) || quantity(state.nativeBalanceWei) < nativeDebit)
        fail("native_balance_or_cap");
    const current = now();
    if (!Number.isSafeInteger(current) || current < 0 || !Number.isSafeInteger(limits.ttlMs) ||
        limits.ttlMs < BRIDGE_MIN_REMAINING_MS || limits.ttlMs > BRIDGE_TTL_MS)
        fail("ttl");
    const expiry = bridgeRecord(response.expiry);
    let expiresAt = current + limits.ttlMs;
    if (expiry.mode === "TIMESTAMP")
        expiresAt = Math.min(expiresAt, Number(expiry.expiresAt) * 1000);
    else if (expiry.mode !== "BLOCK_NUMBER" || newNumber >= BigInt(Number(expiry.expiresAtBlock)))
        fail("quote_expiry");
    if (!Number.isSafeInteger(expiresAt) || expiresAt - current < BRIDGE_MIN_REMAINING_MS)
        fail("ttl_or_quote_expiry");
    const fields = { kind: "circle_v2_base_source_preparation", executionAdmitted: false,
        quoteAuthenticityVerified: false, baseStateSourceVerified: false,
        blockers: ["The Circle quote, validation response, and Base state are supplied through caller-controlled inputs or transports; this artifact does not authenticate their origin",
            ...
            "The expiry timestamp is a local preparation limit; a BLOCK_NUMBER quote can expire sooner and must be checked again before any separately authorized submission"],
        ...
        transaction: { type: "eip1559", chainId: 8453, from: payer, to: bridgeAddress(tx.to), data: bridgeHex(tx.data),
            valueAtomic: "0", nonceAtomic: latest.toString(), gasLimitAtomic: gas.toString(), maxFeePerGasWei: maxFee.toString(),
...
//# sourceMappingURL=circle-v2-source-preparation.js.map