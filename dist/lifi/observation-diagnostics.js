...
function safeReason(error) {
    ...
    if (typeof detail === "string" && REASONS.has(detail))
        return detail;
    const transport = error.details?.transportReason;
    if (typeof transport === "string" && REASONS.has(transport))
        return transport;
    const match = /^Bridge validation failed: ([A-Za-z0-9_]+)\.$/u.exec(error.message);
    return match !== null && REASONS.has(match[1]) ? match[1] : undefined;
}
...
function stage(chainRole, reason, method) {
                    ...
                    : method === "eth_getTransactionReceipt" || reason?.startsWith("receipt_") ? "receipt"
                        : method === "eth_getLogs" || reason?.startsWith("destination_log_") ? "logs"
                            : method === "eth_getBlockByNumber" ? "safe_head"
                                : "observation";
    return `${chainRole}_${boundary}`;
}
//# sourceMappingURL=observation-diagnostics.js.map