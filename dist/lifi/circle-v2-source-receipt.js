...
const SOLANA_TOKEN_MESSENGER_BYTES32 = "0xa65fc81d0fefa8860cb3b83f089b0224be8a6687b7ae49f594c0b9b4d7e93893";
...
function word(address) { return `0x${"0".repeat(24)}${address.slice(2).toLowerCase()}`; }
...
export function decodeCircleV2BaseSourceReceiptOffline(intent, transactionValue, receiptValue) {
    ...
    if (tx.chainId !== 8453 || receipt.chainId !== 8453 || bridgeHex(tx.hash, 32, 32, "APN_RPC_PROTOCOL") !== hash ||
        ...
        bridgeAddress(tx.to, "APN_RPC_PROTOCOL") !== BASE_CCTP_V2_TOKEN_MESSENGER_WITH_FEES)
    ...
    if (blockHash === ZERO || !Array.isArray(receipt.logs) || receipt.logs.length > 512)
        fail("receipt_shape");
    ...
    if (burns.length !== 1 || messages.length !== 1)
        fail("event_count");
    if (logs.indexOf(messages[0]) >= logs.indexOf(burns[0]))
        fail("event_order");
    const burn = burns[0], sent = messages[0];
    if (burn.topics.length !== 4 || sent.topics.length !== 1)
        fail("event_topics");
    let b, message;
    try {
        b = decodeEventLog({ abi: BURN_ABI, eventName: "DepositForBurn", data: burn.data, topics: burn.topics, strict: true }).args;
        message = decodeEventLog({ abi: MESSAGE_ABI, eventName: "MessageSent", data: sent.data, topics: sent.topics, strict: true }).args.message;
    }
    catch {
        return fail("event_decode");
    }
    if (encodeAbiParameters(BURN_ABI[0].inputs.filter(i => !("indexed" in i)), [b.amount, b.mintRecipient, b.destinationDomain, b.destinationTokenMessenger, b.destinationCaller, b.maxFee, b.hookData]).toLowerCase() !== burn.data.toLowerCase() ||
        encodeAbiParameters([{ type: "bytes" }], [message]).toLowerCase() !== sent.data.toLowerCase())
        fail("noncanonical_event");
...
//# sourceMappingURL=circle-v2-source-receipt.js.map