...
const VERSION = "apn.bridge-effect-envelope.v1";
export class BridgeEffectStore extends SecureStateStore {
    ...
    async load(op, role) {
        ...
        if (wrapping === null)
            bridgeFailure("APN_PROVIDER_EFFECT_UNAVAILABLE", "bridge_wrapping_secret_missing");
        const salt = base64(envelope.kdf.salt, 32), nonce = base64(envelope.cipher.nonce, 12), tag = base64(envelope.cipher.tag, 16), ciphertext = base64(envelope.cipher.ciphertext);
        ...
        try {
            ...
            if (canonicalJson(value) !== text)
                bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_canonical_plaintext");
    ...
    async seal(op, material) {
        await validateMaterial(material, op, material.role);
        const existing = await this.load(op, material.role);
        if (existing !== null) {
            if (!bridgeSame(existing, material))
                bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_already_sealed");
            return existing;
        }
        const wrapping = await this.wrappingSecret.load();
        if (wrapping === null)
        ...
        const header = { schemaVersion: VERSION, profileHash: op.profileHash, operationId: op.operationId,
        ...
        try {
            ...
            await this.ensureDirectory(`bridge-effects/${op.profileHash}`);
    ...
    path(op, role) {
        stateIdentifier(op.profileHash, "bridge effect profile");
        stateIdentifier(op.operationId, "bridge effect operation");
        if (role !== "approval" && role !== "bridge")
            bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_role");
        return `bridge-effects/${op.profileHash}/${op.operationId}-${role}.json`;
    }
}
export async function validateMaterial(value, op, role) {
    ...
    if (e === undefined || r.schemaVersion !== "apn.bridge-effect.v1" || r.profileHash !== op.profileHash || r.operationId !== op.operationId ||
        r.role !== role || r.fingerprint !== op.fingerprint || r.envelopeHash !== e.envelope.envelopeHash)
        bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_identity");
    bridgeHash(r.materialHash);
    const { materialHash, ...body } = r;
    if (materialHash !== hashObject(body))
        bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_hash");
    ...
    if ((e.transactionHash !== null && e.transactionHash !== tx) || (e.sealedMaterialHash !== null && e.sealedMaterialHash !== materialHash))
        bridgeFailure("APN_STATE_CORRUPT", "bridge_committed_material_identity");
    await verifyBridgeSigned(raw, tx, e.envelope);
    return r;
}
function parseEnvelope(value, op, role) {
    ...
    if (e.schemaVersion !== VERSION || e.profileHash !== op.profileHash || e.operationId !== op.operationId || e.role !== role ||
        ...
        [k.salt, c.nonce, c.ciphertext, c.tag].some((v) => typeof v !== "string"))
        bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_envelope");
    return e;
}
function headerOf(e) { return { ...e, cipher: { name: e.cipher.name, nonce: e.cipher.nonce } }; }
function deriveKey(wrapping, salt, e) {
...
function base64(value, length) {
    ...
    if (b.length === 0 || b.length > 64 * 1024 || b.toString("base64") !== value || (length !== undefined && b.length !== length)) {
        b.fill(0);
        bridgeFailure("APN_STATE_CORRUPT", "bridge_effect_encoding");
    }
    return b;
}
