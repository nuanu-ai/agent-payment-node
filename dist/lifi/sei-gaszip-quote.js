...
function token(value, chainId, symbol) {
...
function sameAction(value, fromChainId, toChainId, amount, sender, recipient, toSymbol) {
    ...
    for (const key of ["destinationCall", "destinationCalls", "destinationCallData", "contractCalls", "permit", "permit2", "authorization"])
        if (action[key] !== undefined)
            fail("action_extension");
    token(action.fromToken, 1, "ETH");
    token(action.toToken, toChainId, toSymbol);
...
export function inspectSeiGasZipQuote(value, fetchedAtMs) {
    ...
    if (q.type !== "lifi" || q.tool !== destination.tool || q.integrator !== "lifi-api" ||
        object(q.toolDetails).key !== destination.tool)
        fail("provider_tool");
    sameAction(action, 1, destination.chainId, SOURCE_AMOUNT, owner, owner, "SEI");
    if (action.slippage !== 0.005)
        fail("slippage");
    const quoted = bridgeUint(estimate.toAmount, true), minimum = bridgeUint(estimate.toAmountMin, true), feeRows = estimate.feeCosts;
    if (estimate.tool !== destination.tool || estimate.fromAmount !== SOURCE_AMOUNT ||
        ...
        !Array.isArray(feeRows) || feeRows.length !== 1)
        fail("estimate_binding");
    const fee = object(feeRows[0]);
    token(fee.token, 1, "ETH");
    ...
    if (fee.name !== "LIFI Fixed Fee" || fee.included !== true || feeAmount !== 500000000000n || bridgeAmount <= 0n ||
        estimate.skipApproval !== true)
        fail("fee_binding");
    const split = object(fee.feeSplit);
    if (split.lifiFee !== feeAmount.toString() || split.integratorFee !== "0" ||
        !Array.isArray(split.recipients) || split.recipients.length !== 1)
        fail("fee_split");
    const recipient = object(split.recipients[0]);
    if (recipient.name !== "lifi" || recipient.type !== "FIXED" || recipient.fee !== feeAmount.toString())
        fail("fee_split");
    ...
    if (collection.type !== "protocol" || collection.tool !== "feeCollection" ||
        ...
        object(collection.toolDetails).key !== "feeCollection" || object(cross.toolDetails).key !== destination.tool)
        fail("effect_graph");
    ...
    for (const step of [collection, cross])
        if ((step.executionType !== undefined && step.executionType !== "transaction") ||
            (step.gasless !== undefined && step.gasless !== false))
            fail("execution_mode");
    ...
    if (collected.tool !== "feeCollection" || collected.fromAmount !== SOURCE_AMOUNT ||
        ...
        !Array.isArray(bridged.feeCosts) || bridged.feeCosts.length !== 0)
        fail("step_economics");
    ...
    if (Object.keys(tx).some((key) => !txKeys.includes(key)) ||
        ...
        (tx.type !== undefined && tx.type !== 2 && tx.type !== "0x2"))
        fail("transaction_fields");
    if (tx.chainId !== 1 || bridgeAddress(tx.from) !== owner || bridgeAddress(tx.to) !== BRIDGE_DIAMOND ||
        quantity(tx.value) !== source || quantity(tx.gasLimit) <= 0n || quantity(tx.gasLimit) > 5000000n)
...
export function revalidateSeiGasZipQuote(saved, value, nowMs) {
    const now = time(nowMs), fetchedAt = Date.parse(saved.fetchedAt);
    if (!Number.isSafeInteger(fetchedAt) || now < fetchedAt || now - fetchedAt >= SEI_GASZIP_LOCAL_MAX_AGE_MS)
        fail("local_quote_stale");
    const current = inspectSeiGasZipQuote(value, fetchedAt);
    if (canonicalJson(saved) !== canonicalJson(current))
        fail("quote_mutated");
    return current;
}
//# sourceMappingURL=sei-gaszip-quote.js.map