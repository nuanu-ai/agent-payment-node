...
function validateIntent(op) {
    ...
    op.effects.forEach((effect, index) => {
        ...
        if (effect.role === "bridge") {
            if (e.to !== m.transaction.to || e.data !== m.transaction.data || e.valueAtomic !== m.transaction.valueAtomic ||
                e.economics.gasLimitAtomic !== m.transaction.gasLimitAtomic || e.provisionalGas !== (a.allowanceAtomic === "0"))
                bridgeCorrupt();
        }
    ...
    if (native > BigInt(r.maxNativeDebitWei) || native > BigInt(a.nativeBalanceWei))
        bridgeCorrupt();
    const included = m.feeCosts.filter((f) => f.included).reduce((sum, f) => sum + BigInt(f.amountAtomic), 0n);
    if (included + BigInt(m.quotedOutputAtomic) + BigInt(i.implicitProtocolFeeAtomic) !== BigInt(r.amountAtomic))
        bridgeCorrupt();
}
...
function validateSnapshot(op, s) {
