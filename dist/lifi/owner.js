...
export async function bridgeOwner(state, profileInput) {
    ...
    if (binding !== null && binding.provider_id !== "local")
        bridgeFailure("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "bridge_profile_adapter_unavailable");
    const wallet = await state.loadWallet(profileHash);
    if (wallet === null)
        bridgeFailure("APN_WALLET_POLICY_REQUIRED", "existing_local_wallet_required");
    ...
    if (provider.drift.state !== "bound" || provider.public_address !== wallet.address || provider.account_binding_hash !== wallet.bindingHash ||
        provider.profile !== profile || provider.profile_hash !== profileHash)
        bridgeFailure("APN_PROFILE_DRIFT", "bridge_profile_binding");
    return {
...
export async function assertBridgeOwner(state, expected) {
    const current = await bridgeOwner(state, expected.owner.profile);
    if (!bridgeSame(current, { owner: expected.owner, providerBinding: expected.providerBinding }))
        bridgeFailure("APN_PROFILE_DRIFT", "bridge_owner_changed");
