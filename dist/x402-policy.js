...
export function selectPrepareOffer(challenge, underCap, evidence, wallet, context, transferMethod = "eip3009") {
    ...
    for (const candidate of underCap) {
        if (candidate.assetTransferMethod !== transferMethod)
            continue;
        ...
        return {
            ...
            selectedOffer: candidate.assetTransferMethod === "eip3009" ? {
                ...
                resolved: {
                    ...
                    facilitatorAddresses: candidate.facilitatorAddresses,
                },
                offerHash: domainHash("apn.x402.offer.v1", declaredCanonicalJson),
            },
    ...
    if (!hasMatchingMethod)
        throw new ApnError("APN_X402_UNSUPPORTED_OFFER", "No fresh seller offer matches the payer transfer method.");
...
function validatePrepareEvidence(evidence, wallet, context, transferMethod) {
    ...
    if (transferMethod === "eip3009" && (typeof evidence.tokenName !== "string" || evidence.tokenName.length === 0 || Buffer.byteLength(evidence.tokenName, "utf8") > 128 ||
