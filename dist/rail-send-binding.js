...
export function validateRailSendBinding(value, prepared) {
    if (!isPlainRecord(value) || !exactKeys(value, ["blockReference", "lastValidBlockHeight", "observedBlockHeight", "acquiredAt", "simulation"]))
        corrupt();
    if (prepared.rail !== "solana" || prepared.lastValidBlockHeight === null)
        corrupt();
    if (typeof value.blockReference !== "string" || !BLOCK_REFERENCE.test(value.blockReference))
        corrupt();
    const lastValid = atomic(value.lastValidBlockHeight, true);
    const observed = atomic(value.observedBlockHeight, true);
    // The re-acquired window must open later than the frozen one and must not already be closing.
    if (lastValid < atomic(prepared.lastValidBlockHeight) || lastValid - observed < SOLANA_MIN_SEND_BLOCKS)
        corrupt();
    isoDate(value.acquiredAt);
    if (value.acquiredAt < prepared.preparedAt || value.acquiredAt > prepared.expiresAt)
        corrupt();
    validateRailSimulation(value.simulation);
    return value;
}
function validateRailSimulation(value) {
    if (!isPlainRecord(value) || !exactKeys(value, ["outcome", "slotAtomic", "unitsConsumedAtomic", "signatureVerified", "payloadHash"]))
        corrupt();
    if (value.outcome !== "would_succeed" || value.signatureVerified !== false)
        corrupt();
    atomic(value.slotAtomic, true);
    atomic(value.unitsConsumedAtomic);
    if (typeof value.payloadHash !== "string" || !HASH.test(value.payloadHash))
        corrupt();
}
/**
 ...
 */
export function railSendLifetime(prepared, send) {
...
export function railSendReason(error, fallback) {
...
export function railSendTransient(error) {
...
//# sourceMappingURL=rail-send-binding.js.map