        ...
        switch (request.command) {
            ...
            case "relay.prepare": {
                ...
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay prepare runtime is unavailable.");
            ...
            case "relay.native.prepare": {
                const service = this.context.relayPrepare;
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay native prepare runtime is unavailable.");
            ...
            case "relay.native.execute": {
                ...
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay native execute runtime and foreground confirmation are required.");
            ...
            case "relay.retire": {
                ...
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay retirement runtime is unavailable.");
            ...
            case "relay.base.observe": {
                ...
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay Base observe runtime is unavailable.");
                return dataOutcome(await service.observe(request.operationId), "read_only_rpc_observation");
            }
            case "relay.arbitrum.observe": {
                const service = this.context.relayArbitrumObserve;
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay Arbitrum source observer is unavailable.");
                return dataOutcome(await service.observe(request.operationId), "read_only_rpc_observation");
            }
            ...
            case "relay.arbitrum.deposit-dispatch": {
                ...
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Relay Arbitrum deposit dispatch is unavailable.");
                return dataOutcome(await service.execute(request.profile, request.operationId), "source_effect_journal");
            }
            ...
            case "stargate.native.receipt": {
                const service = this.context.stargateNative;
                if (service === undefined)
                    throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Stargate native runtime is unavailable.");
                if (request.command === "stargate.native.execute")
                    return operationOutcome(await service.execute(request.operationId));
                if (request.command === "stargate.native.observe")
                    return operationOutcome(await service.observe(request.operationId));
                if (request.command === "stargate.native.status")
            ...
            case "swap.uniswap-token.execute":
            case "swap.uniswap-token.cleanup": return await executeUniswapCommand(request, this.context);
            case "swap.sunswap.inventory":
            case "swap.sunswap.quote":
            case "swap.sunswap.prepare":
            case "swap.sunswap.status":
            case "swap.sunswap.approve":
            case "swap.sunswap.execute": return await executeSunSwapCommand(request, this.context);
            case "swap.jupiter.inventory":
            case "swap.jupiter.quote":
            case "swap.jupiter.prepare":
            ...
            case "x402.fetch.prepare": {
                ...
                if ((request.chainId ?? 8453) === 8453)
                    await this.providerWallet.assertPaymentAvailable(request.profile, "x402", request.idempotencyKey);
            ...
            case "operation.resume": {
                if (request.observeOnly !== undefined && request.observeOnly !== true)
                    throw new ApnError("APN_INVALID_INPUT", "observeOnly must be true.");
                ...
                if (operation.kind === "metamask_gasless_transfer") {
                    if (request.waitSeconds !== undefined)
                        throw new ApnError("APN_INVALID_INPUT", "MetaMask gasless recovery performs one bounded observation; omit --wait-seconds.", { reason: "mm_gasless_input" });
                ...
                if (operation.kind === "bridge_route") {
                    if (request.waitSeconds !== undefined)
                        throw new ApnError("APN_INVALID_INPUT", "Bridge recovery performs one bounded observation; omit --wait-seconds.");
            ...
            case "operation.status": {
                ...
                if (operation.kind === "gasless_transfer")
                    return operationOutcome(await this.gasless.status(request.operationId));
                if (operation.kind === "metamask_gasless_transfer")
                    return operationOutcome(await this.metaMaskGasless.status(request.operationId));
                if (operation.kind === "rail_transfer")
                    return operationOutcome(await this.operations.status(request.operationId));
            ...
            case "receipt.get": {
                ...
                if (operation.kind === "gasless_transfer")
                    return receiptOutcome(await this.gasless.receipt(request.operationId));
                if (operation.kind === "metamask_gasless_transfer")
                    return receiptOutcome(await this.metaMaskGasless.receipt(request.operationId));
                if (operation.kind === "rail_transfer")
