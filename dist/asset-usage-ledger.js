...
export class AssetUsageLedger extends SecureStateStore {
    ...
    async hasMatchingRelayReservation(account, policyDigest, amountAtomic) {
        if (policyDigest !== undefined)
            digest(policyDigest, "Policy digest");
        await this.ready();
        for (const entry of await this.readDirectory("asset-usage")) {
            if (!entry.isDirectory() || entry.isSymbolicLink() || !DIGEST.test(entry.name))
                corrupt("The usage ledger bucket is invalid.");
            for (const item of await this.readDirectory(`asset-usage/${entry.name}`)) {
                if (!item.isFile() || item.isSymbolicLink() || !item.name.endsWith(".json") || !DIGEST.test(item.name.slice(0, -5)))
                    corrupt("The usage ledger entry is invalid.");
                ...
                if (record.reservationId !== item.name.slice(0, -5) || this.bucketDirectory(exactIdentity(record)) !== `asset-usage/${entry.name}`)
                    corrupt("The usage reservation path is invalid.");
...
function reservationIdFor(identity, idempotencyHash) {
