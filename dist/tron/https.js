...
export const tronHttpsFetch = async (input, init) => {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const finish = (response) => {
            ...
            if (response === undefined)
                reject(new ApnError("APN_RPC_PROTOCOL", "The bounded TRON HTTPS transport did not return valid evidence."));
            else
                resolve(response);
        };
...
async function resolveBounded(endpoint, signal) {
    let abort;
    try {
        return await Promise.race([resolvePublicAddresses(endpoint, "APN_RPC_CONFIG", "TRON RPC endpoint"), new Promise((_resolve, reject) => {
