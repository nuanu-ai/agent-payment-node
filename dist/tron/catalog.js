...
const option = (name, constraints, required = true) => ({
    name, type: "string", constraints, required, default: { kind: "none" }, sensitivity: "operator_input",
});
const profile = { ...option("--profile", ["explicit_canonical_profile"]), type: "profile" };
...
const output = { contract: "apn.cli.v1", success_exit: 0, failure_exit: 1,
    success: "Exact TRON mainnet identity, frozen resource bounds and classified recovery.", failures: ["Classified APN error; no provider or native-funding fallback."] };
const states = { terminal: ["completed", "classified_failure"], non_terminal: [] };
export const TRON_COMMANDS = [
