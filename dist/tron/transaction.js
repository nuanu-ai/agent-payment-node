...
export function buildTronTransaction(input) {
    tronHash(input.blockId);
    const type = input.token ? "TriggerSmartContract" : "TransferContract";
...
export function unsignedTronPrepared(prepared) {
    const snapshot = prepared.resources;
    if (prepared.rail !== "tron" || snapshot === undefined || prepared.unsignedPayload === null)
        mismatch();
    ...
    let parsed;
    try {
        parsed = JSON.parse(prepared.unsignedPayload);
    }
    catch {
        return mismatch();
    }
    if (canonicalJson(expected) !== canonicalJson(parsed) || prepared.unsignedPayload !== canonicalJson(parsed) || expected.raw_data_hex.length / 2 !== Number(snapshot.rawDataBytesAtomic))
        mismatch();
    return expected;
}
export function validateTronChainTransaction(value, prepared) {
    const expected = unsignedTronPrepared(prepared);
    const record = tronRecord(value);
    if (Object.keys(record).some((key) => !["visible", "txID", "raw_data_hex", "raw_data", "signature", "ret"].includes(key)) || record.visible !== undefined && record.visible !== false || record.txID !== expected.txID || record.raw_data_hex !== expected.raw_data_hex)
        mismatch();
    ...
    if (Object.keys(raw).some((key) => !allowed.includes(key)) || raw.data !== undefined && raw.data !== "")
        mismatch();
    const contracts = tronArray(raw.contract, 1);
    if (contracts.length !== 1)
        mismatch();
    const contract = tronRecord(contracts[0]);
    if (Object.keys(contract).some((key) => !["type", "parameter", "Permission_id"].includes(key)) || tronAtomic(contract.Permission_id, true) !== 0n)
        mismatch();
    const parameter = tronRecord(contract.parameter);
    if (!exactKeys(parameter, ["type_url", "value"]))
        mismatch();
    ...
    if (Object.keys(parameters).some((key) => !keys.includes(key)) || typeof parameters.owner_address !== "string")
        mismatch();
    const normalized = { owner_address: tronHex(parameters.owner_address) };
    if (prepared.asset.kind === "native") {
        if (typeof parameters.to_address !== "string")
            mismatch();
        normalized.to_address = tronHex(parameters.to_address);
        normalized.amount = tronSafeNumber(tronAtomic(parameters.amount));
    }
    else {
        if (typeof parameters.contract_address !== "string" || typeof parameters.data !== "string")
            mismatch();
        for (const key of ["call_value", "call_token_value", "token_id"])
            if (tronAtomic(parameters[key], true) !== 0n)
                mismatch();
        normalized.contract_address = tronHex(parameters.contract_address);
        normalized.data = parameters.data;
    }
    ...
    if (canonicalJson(normalizedRaw) !== canonicalJson(expected.raw_data))
        mismatch();
    const signatures = tronArray(record.signature, 1);
    if (signatures.length !== 1 || typeof signatures[0] !== "string" || !/^[a-fA-F0-9]{130}$/u.test(signatures[0]))
        mismatch();
    const signature = signatures[0].toLowerCase();
    try {
        if (tronAddress(utils.crypto.ecRecover(expected.txID, signature)) !== prepared.sender)
            mismatch();
        if (utils.transaction.txPbToRawDataHex(utils.transaction.txJsonToPb({ visible: false, raw_data: normalizedRaw })).toLowerCase() !== expected.raw_data_hex)
            mismatch();
    }
    catch {
        return mismatch();
...
export function validateTronEffect(prepared, effect) {
    if (sha256(effect.rawPayload) !== effect.rawPayloadHash)
        mismatch();
    let value;
    try {
        value = JSON.parse(effect.rawPayload);
    }
    catch {
        return mismatch();
    }
    const transaction = validateTronChainTransaction(value, prepared);
    if (effect.transactionId !== transaction.txID || canonicalJson(transaction) !== effect.rawPayload)
        mismatch();
    return transaction;
}
export function signTronTransaction(prepared, seed) {
    const unsigned = unsignedTronPrepared(prepared);
    const bytes = [...seed];
    try {
        if (seed.length !== 32 || tronAddress(utils.crypto.getBase58CheckAddress(utils.crypto.getAddressFromPriKey(bytes))) !== prepared.sender)
            mismatch();
...
function mismatch() { throw new ApnError("APN_WALLET_MISMATCH", "TRON protobuf, transaction identity or signature does not match the frozen payment."); }
