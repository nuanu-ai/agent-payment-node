...
export function bindUniswapCommand(path, o) {
    ...
    if (action === "status")
        return { command: "swap.uniswap.status", operationId: o["--operation"] };
    if (action === "approve")
        return { command: "swap.uniswap.approve", operationId: o["--operation"] };
    if (action === "execute")
        return { command: "swap.uniswap.execute", operationId: o["--operation"] };
    if (action === "prepare")
        return { command: "swap.uniswap.prepare", profile: o["--profile"], quoteHash: o["--quote"], idempotencyKey: o["--idempotency-key"] };
