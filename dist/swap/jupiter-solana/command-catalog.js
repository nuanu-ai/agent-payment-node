...
export function bindJupiterCommand(path, options) {
    ...
    if (!isPlainRecord(options))
        invalid("Jupiter options must be a plain object.");
    if (action === "inventory") {
        exact(options, []);
        return { command: "swap.jupiter.inventory" };
    }
    if (action === "status" || action === "approve" || action === "execute") {
        exact(options, ["--operation"]);
        return { command: `swap.jupiter.${action}`, operationId: hash(options["--operation"]) };
    }
    if (action === "prepare") {
...
function exact(value, keys) {
    if (!exactKeys(value, keys))
        invalid("Jupiter options contain an unknown or missing field.");
}
function basisPoints(value) {
    if (value === undefined || !/^(?:0|[1-9][0-9]{0,4})$/u.test(value))
        invalid("Jupiter basis points must be canonical.");
    const parsed = Number(value);
    if (!Number.isSafeInteger(parsed) || parsed > 10_000)
        invalid("Jupiter basis points exceed 10000.");
    return parsed;
}
function positiveAtomic(value) {
    if (value === undefined || !/^[1-9][0-9]{0,77}$/u.test(value))
        invalid("Jupiter amount must be a positive canonical uint256.");
    return value;
}
function hash(value) {
    if (value === undefined || !/^[a-f0-9]{64}$/u.test(value))
        invalid("Jupiter hash must be 64 lowercase hexadecimal characters.");
    return value;
}
