...
export class UniswapTokenNonceStore extends SecureStateStore {
    async allocate(op, kind, pending) {
        await this.initialize();
        await this.ensureDirectory("uniswap-token-nonces");
        const account = canonical(op.account), path = this.path(account), raw = await this.readJson(path), current = raw === null ? { schemaVersion: "apn.uniswap-token-nonces.v1", account, nextNonce: pending.toString(), reservations: {} } : validate(raw, account), slot = domainHash("apn.uniswap-token-nonce-slot.v1", canonicalJson({ operationId: op.operationId, kind })), prior = current.reservations[slot];
        if (prior !== undefined)
            return prior;
        const nonce = (BigInt(current.nextNonce) > pending ? BigInt(current.nextNonce) : pending).toString();
        const next = { ...current, nextNonce: (BigInt(nonce) + 1n).toString(), reservations: { ...current.reservations, [slot]: nonce } };
        await this.writeJson(path, next, raw === null);
        return nonce;
    }
...
function validate(value, account) {
    if (!isPlainRecord(value) || !exactKeys(value, ["schemaVersion", "account", "nextNonce", "reservations"]) ||
        value.schemaVersion !== "apn.uniswap-token-nonces.v1" || value.account !== account || !decimal(value.nextNonce) || !isPlainRecord(value.reservations))
        corrupt("Uniswap token nonce record is invalid.");
    for (const [slot, nonce] of Object.entries(value.reservations))
        if (!/^[a-f0-9]{64}$/u.test(slot) || !decimal(nonce))
