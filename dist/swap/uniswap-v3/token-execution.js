...
export class UniswapTokenExecution {
    ...
    async execute(id) {
        let op = await this.required(id);
        if (["observed", "cleaned"].includes(op.phase))
            return op;
        if (op.phase === "cleanup_required")
            return await this.cleanup(id);
        ...
        if (op.phase === "approved") {
            ...
            if (allowance === op.route.amountIn)
                op = await this.persist(transitionUniswapToken(op, "approval_observed", {}, this.ports.now()));
            else
                op = await this.start(op, "approval");
        }
        if (["approval_submission_started", "approval_submitted", "approval_unknown_finality"].includes(op.phase))
            return await this.observeApproval(op);
        if (op.phase === "approval_observed") {
            if (await this.ports.currentAllowance(op) !== op.route.amountIn)
                return await this.cleanupRequired(op, "approval_drift");
            op = await this.start(op, "swap");
        }
        if (["submission_started", "submitted", "unknown_finality"].includes(op.phase))
    ...
    async start(op, kind) {
        ...
        let hash;
        try {
            hash = await this.ports.submit(op, kind, nonce);
        }
        catch {
            return op;
        }
        if (!/^0x[a-f0-9]{64}$/u.test(hash))
            return op;
        const submitted = kind === "approval" ? "approval_submitted" : kind === "swap" ? "submitted" : "cleanup_submitted";
    ...
    async observeApproval(op) {
        ...
        if (hash === null || hash === undefined)
            return op;
        const result = await this.ports.observe(op, "approval", hash);
    ...
    async observeSwap(op) {
        ...
        if (hash === null || hash === undefined)
            return op;
        const result = await this.ports.observe(op, "swap", hash);
        if (result === null || result.status === "pending")
            return op;
        if (result.status === "reverted")
            return await this.cleanupRequired(op, "swap_reverted");
        ...
        const approvalGasWei = op.accumulatedNativeDebitWei, cleanupGasWei = "0", swapGasWei = result.gasDebitWei, nativeDebitWei = debit(op, result.gasDebitWei);
        if (BigInt(nativeDebitWei) > BigInt(op.maximumNativeDebitWei))
            return await this.cleanupRequired(op, "native_debit_exceeded");
        const receiptBody = { schemaVersion: "apn.uniswap-token-receipt.v1", operationId: op.operationId, approvalGasWei, swapGasWei, cleanupGasWei,
            nativeDebitWei, inputDebitAtomic: op.route.amountIn, outputCreditAtomic: result.outputCreditAtomic, residualAllowanceAtomic: "0",
            transactionHash: hash, observedAt: this.ports.now().toISOString() };
    ...
    async observeCleanup(op) {
        ...
        if (hash === null || hash === undefined)
            return op;
        const result = await this.ports.observe(op, "cleanup", hash);
        if (result === null || result.status === "pending")
            return op;
        if (result.status !== "success" || result.allowanceAtomic !== "0")
            blocked("Cleanup did not prove zero allowance.", "uniswap_cleanup_failed");
    ...
    async cleanupRequired(op, reason) { return await this.persist(transitionUniswapToken(op, "cleanup_required", { cleanupReason: reason }, this.ports.now())); }
