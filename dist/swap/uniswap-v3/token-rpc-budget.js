...
export class UniswapTokenRpcBudgetJournal extends SecureStateStore {
    async reserve(binding, command, cap, requestSessionCap = cap, budgetClass = cap === 0 ? "recovery" : "approval_effect") {
        ...
        return await this.withLocks([`uniswap-token-rpc-budget:${binding}`], async () => {
            ...
            const row = { reservation, command, cap, requestSessionCap, budgetClass, physicalRequests: null, attempts: null, logicalItems: null,
    ...
    async settle(binding, reservation, telemetry, effects, primaryPool) {
        ...
        await this.withLocks([`uniswap-token-rpc-budget:${binding}`], async () => {
            ...
            if (current === null || !Number.isSafeInteger(effects) || effects < 0)
                corrupt();
            const reserved = current.rows.find((row) => row.reservation === reservation);
