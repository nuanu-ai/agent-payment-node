...
export interface SwapProviderBuilderPort {
    build(input: SwapQuoteInput): Promise<SwapQuoteSnapshot>;
}
export interface SwapChainSimulatorPort {
    simulate(unsignedTransactionPayloadHash: string): Promise<{
...
export interface SwapChainSignerPort {
    sign(operation: SwapOperationRecord): Promise<{
        readonly signedMaterialHandle: string;
    }>;
}
export interface SwapChainSenderPort {
...
export interface SwapChainObserverPort {
    observe(operation: SwapOperationRecord): Promise<SwapReceiptProof | null>;
}
