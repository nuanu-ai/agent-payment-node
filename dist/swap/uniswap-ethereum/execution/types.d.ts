...
export interface UniswapForegroundApprovalPort {
    confirm(request: UniswapExecutionApprovalRequest): Promise<{
...
export interface UniswapEffectStorePort {
    ...
    markSendOutcome(operation: SwapOperationRecord, binding: UniswapExecutionBinding, phase: "send_accepted" | "send_ambiguous", now: Date): Promise<UniswapExecutionEffect>;
}
export interface UniswapExecutionSignerPort {
    sign(operation: SwapOperationRecord, binding: UniswapExecutionBinding, admission: UniswapOwnerAdmission, now: Date): Promise<UniswapExecutionEffect>;
}
