...
export class LocalUniswapEthereumSigner {
    state;
    effects;
    wallets;
    constructor(state, wrapping, effects) {
        this.state = state;
        this.effects = effects;
        this.wallets = new EncryptedWalletStore(state, wrapping);
    }
    async sign(operationValue, bindingValue, admission, now) {
        ...
        if (operation.state !== "submitting" || operation.submissionMarker === null || at >= operation.quote.expiresAt ||
            ...
            admission.admissionHash !== binding.ownerAdmissionHash)
            blocked("Uniswap signing gate or owner admission changed.", "uniswap_signing_gate");
        return await this.state.withLocks([`uniswap-sign:${operation.operationId}`], async () => {
            const existing = await this.effects.load(operation, binding);
            if (existing !== null)
                return existing;
            const wallet = await this.wallets.describe(operation.quote.profile);
            if (wallet === null)
                blocked("The admitted encrypted EVM wallet is unavailable.", "uniswap_wallet_missing");
            try {
                ...
                const nonce = BigInt(binding.nonce);
                if (nonce > BigInt(Number.MAX_SAFE_INTEGER))
                    blocked("Uniswap nonce exceeds signer bounds.", "uniswap_nonce_bound");
                const account = privateKeyToAccount(wallet.secret.privateKey), e = binding.envelope;
                if (getAddress(account.address) !== binding.account)
                    throw new ApnError("APN_WALLET_MISMATCH", "Uniswap signer address changed.");
                ...
                return await this.effects.seal(operation, binding, newUniswapExecutionEffect({ operationId: operation.operationId,
                    ...
                    submissionMarkerHash: binding.submissionMarkerHash, rawTransaction: raw, transactionHash,
...
function instant(value) { if (!(value instanceof Date) || !Number.isFinite(value.getTime()))
    throw new ApnError("APN_INVALID_INPUT", "Uniswap signing time is invalid."); return value.toISOString(); }
