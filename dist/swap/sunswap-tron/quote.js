...
export function sunSwapQuoteUrl(input) {
    loadSunSwapPinCatalog();
    if (!isPlainRecord(input) || !exactKeys(input, ["inputAmountAtomic"]))
        failInput();
...
export function decodeSunSwapQuote(value, inputAmountAtomic) {
    ...
    if (!isPlainRecord(value) || !exactKeys(value, ["code", "message", "data"]) || value.code !== 0 || value.message !== "SUCCESS" ||
        !Array.isArray(value.data) || value.data.length < 1 || value.data.length > 3)
        fail();
    if (!dense(value.data))
        fail();
    return value.data.map((raw) => {
        ...
        if (!Array.isArray(raw.roadForAddr) || !Array.isArray(raw.roadForName) || !Array.isArray(raw.pool) || raw.roadForAddr.length < 2 ||
            ...
            raw.pool.some((item) => typeof item !== "string" || !/^[A-Za-z0-9._-]{1,32}$/u.test(item)))
            fail();
        const path = raw.roadForAddr;
        if (path[0] !== SUNSWAP_NATIVE_TRX || path.at(-1) !== SUNSWAP_USDT)
            fail();
        for (const key of ["amount", "inUsd", "outUsd", "fee"])
            if (typeof raw[key] !== "string" || !decimal(raw[key], false))
                fail();
        if (typeof raw.impact !== "string" || !decimal(raw.impact, true))
            fail();
        const amountOutAtomic = decimalToAtomic(raw.amount, 6);
        const body = { inputAmountAtomic: inputAmount, roadForAddr: path, roadForName: raw.roadForName, pool: raw.pool, amount: raw.amount,
            amountOutAtomic, inUsd: raw.inUsd, outUsd: raw.outUsd, impact: raw.impact, fee: raw.fee };
        return { ...body, routeHash: domainHash("apn.sunswap-tron-route.v1", canonicalJson(body)) };
    });
}
export function validateSunSwapQuoteRoute(value) {
    if (!isPlainRecord(value) || !exactKeys(value, ["inputAmountAtomic", "amount", "amountOutAtomic", "fee", "impact", "inUsd", "outUsd",
        "pool", "roadForAddr", "roadForName", "routeHash"]) || typeof value.routeHash !== "string")
        fail();
    const decoded = decodeSunSwapQuote({ code: 0, message: "SUCCESS", data: [{ roadForAddr: value.roadForAddr, roadForName: value.roadForName,
                pool: value.pool, amount: value.amount, inUsd: value.inUsd, outUsd: value.outUsd, impact: value.impact, fee: value.fee }] }, value.inputAmountAtomic)[0];
    if (decoded.amountOutAtomic !== value.amountOutAtomic || decoded.routeHash !== value.routeHash)
        fail();
    return value;
}
export function assertSunSwapDirectRoute(value, minimumOutputAtomic) {
...
export function createSunSwapQuoteSnapshot(input) {
    ...
    if (route.inputAmountAtomic !== input.inputAmountAtomic)
        throw new ApnError("APN_OPERATION_BLOCKED", "The quote route input amount does not match the frozen swap input.");
...
function positiveAtomic(value) {
    if (typeof value !== "string" || !/^[1-9][0-9]{0,77}$/u.test(value) || BigInt(value) > MAX_UINT256)
        failInput();
    return value;
}
...
function canonicalTronAddress(value) { try {
    return tronAddress(value) === value;
}
catch {
    return false;
} }
function decimal(value, signed) {
    if (value.length > 96 || !(signed ? /^-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?$/u : /^(?:0|[1-9][0-9]*)(?:\.[0-9]+)?$/u).test(value))
        return false;
    return value !== "-0";
}
...
function failInput() { throw new ApnError("APN_INVALID_INPUT", "SunSwap quote input must be a positive canonical atomic amount."); }
function fail() { throw new ApnError("APN_PROVIDER_PROTOCOL", "SunSwap returned an untrusted or unsupported quote shape."); }
