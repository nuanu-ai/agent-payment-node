...
export const SUNSWAP_COMMAND_GROUPS = [
...
export function bindSunSwapCommand(path, options) {
    ...
    if (!isPlainRecord(options))
        invalid("SunSwap options must be a plain object.");
    if (action === "inventory") {
        exact(options, []);
        return { command: "swap.sunswap.inventory" };
    }
    if (action === "status" || action === "approve" || action === "execute") {
        exact(options, ["--operation"]);
        return { command: `swap.sunswap.${action}`, operationId: hash(options["--operation"]) };
    }
    if (action === "prepare") {
...
function exact(value, keys) {
    if (!exactKeys(value, keys))
        invalid("SunSwap options contain an unknown or missing field.");
}
function basisPoints(value) {
    if (value === undefined || !/^(?:0|[1-9][0-9]{0,4})$/u.test(value))
        invalid("SunSwap basis points must be canonical.");
    const parsed = Number(value);
    if (!Number.isSafeInteger(parsed) || parsed > 10_000)
        invalid("SunSwap basis points exceed 10000.");
    return parsed;
}
...
function hash(value) {
    if (value === undefined || !/^[a-f0-9]{64}$/u.test(value))
        invalid("SunSwap hash must be 64 lowercase hexadecimal characters.");
    return value;
}
