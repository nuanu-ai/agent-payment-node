 ...
 */
export class SunSwapProtectedExecutionAdapter {
    storage;
    rpc;
    operations;
    binding;
    operation;
    fingerprint;
    constructor(storage, rpc, operations, operation, binding) {
    ...
    async sign(operationValue) {
        ...
        const signed = await this.storage.withSeed(this.binding.account, async (seed) => signSunSwapTransaction(this.binding.transaction, this.binding.intent.owner, seed));
        const rawPayload = canonicalJson(signed);
        const effect = { operationId: operation.operationId, fingerprint: this.fingerprint,
    ...
    async sendOnce(signedMaterialHandle, submissionMarkerHash) {
        if (signedMaterialHandle !== this.fingerprint || !/^[a-f0-9]{64}$/u.test(submissionMarkerHash))
            mismatch();
        ...
        if (durable === null || durable.state !== "submitting" || durable.submissionMarker?.markerHash !== submissionMarkerHash ||
            ...
            validateSunSwapExecutionBinding(durable, this.binding) !== this.fingerprint)
            mismatch();
        const effect = await this.effect();
        if (effect === null)
            mismatch();
        const transaction = validateSunSwapEffect(effect, this.binding.transaction, this.binding.intent.owner, this.operation.operationId, this.fingerprint);
        let response;
        try {
            response = await this.rpc.call("wallet/broadcasttransaction", transaction);
        }
        catch (error) {
        ...
        if (!isPlainRecord(response) || response.result !== true || response.txid !== transaction.txID ||
            Object.keys(response).some((key) => !["result", "txid", "code", "message"].includes(key)))
            ambiguous();
        return { transactionHash: transaction.txID };
    }
    async effect() {
        const account = await this.storage.account(this.binding.account.profile, "tron");
        if (account === null || canonicalJson(account) !== canonicalJson(this.binding.account))
            mismatch();
        const effect = await this.storage.effect(this.binding.account, this.operation.operationId, this.fingerprint);
        if (effect !== null)
            validateSunSwapEffect(effect, this.binding.transaction, this.binding.intent.owner, this.operation.operationId, this.fingerprint);
        return effect;
    }
}
export function validateSunSwapExecutionBinding(operationValue, bindingValue) {
    const operation = validateSwapOperation(operationValue);
    if (!isPlainRecord(bindingValue) || !exactKeys(bindingValue, ["account", "intent", "transaction", "simulation"]))
        mismatch();
    ...
    if (account.rail !== "tron" || account.network !== "mainnet" || account.provider !== "local" ||
        ...
        operation.approvalCapAtomic !== "0")
        mismatch();
    const transaction = validateSunSwapUnsignedTransaction(binding.transaction, binding.intent);
    if (sunSwapUnsignedPayloadHash(transaction) !== operation.quote.unsignedTransactionPayloadHash)
        mismatch();
...
export function signSunSwapTransaction(transactionValue, owner, seed) {
    const transaction = transactionValue;
    tronAddress(owner);
    const bytes = [...seed];
    try {
        if (seed.length !== 32 || tronAddress(utils.crypto.getBase58CheckAddress(utils.crypto.getAddressFromPriKey(bytes))) !== owner)
            mismatch();
...
export function validateSunSwapSignedTransaction(value, unsigned, owner) {
    if (!isPlainRecord(value) || !exactKeys(value, ["visible", "txID", "raw_data_hex", "raw_data", "signature"]) ||
        ...
        value.signature.length !== 1 || typeof value.signature[0] !== "string" || !/^[a-fA-F0-9]{130}$/u.test(value.signature[0]))
        mismatch();
    const signature = value.signature[0].toLowerCase();
    try {
        if (tronAddress(utils.crypto.ecRecover(unsigned.txID, signature)) !== owner)
            mismatch();
    }
    catch {
        return mismatch();
...
function validateSunSwapEffect(effect, unsigned, owner, operationId, fingerprint) {
    if (effect.operationId !== operationId || effect.fingerprint !== fingerprint || effect.transactionId !== unsigned.txID ||
        sha256(effect.rawPayload) !== effect.rawPayloadHash)
        mismatch();
    let parsed;
    try {
        parsed = JSON.parse(effect.rawPayload);
    }
    catch {
        return mismatch();
    }
    const transaction = validateSunSwapSignedTransaction(parsed, unsigned, owner);
    if (canonicalJson(transaction) !== effect.rawPayload)
        mismatch();
    return transaction;
}
function immutableOperationHash(operation) {
...
function mismatch() { throw new ApnError("APN_WALLET_MISMATCH", "SunSwap owner, transaction, signature or protected effect binding does not match the frozen operation."); }
function ambiguous() { throw new ApnError("APN_RPC_AMBIGUOUS", "TRON did not acknowledge the exact SunSwap transaction identity; observe only and never resend."); }
