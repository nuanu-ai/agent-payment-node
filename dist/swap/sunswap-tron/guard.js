...
const TAPOS_WINDOW_BLOCKS = 65536n;
/**
...
export class SunSwapExecutionGuard {
    ...
    async inspect(operationValue, materialValue) {
        ...
        if (material.quote.quoteHash !== operation.quote.quoteHash)
            blocked("Prepared SunSwap material changed after preparation.", "swap_material_drift");
        ...
        for (const { role, address } of SUNSWAP_PINNED_CONTRACTS)
            await verifySunSwapPinnedCode(this.rpc, address, SUNSWAP_V2_CODE_HASHES[role]);
        const head = await sunSwapHead(this.rpc), reference = BigInt(market.referenceBlock.number);
        if (head.number < reference)
            throw new ApnError("APN_RPC_PROTOCOL", "TRON head is behind the recorded SunSwap reference block.");
        if (head.number - reference >= TAPOS_WINDOW_BLOCKS)
            blocked("The SunSwap reference block left the TAPOS window.", "sunswap_reference_block_expired");
        if (head.timestamp >= BigInt(intent.expirationMs))
            blocked("The SunSwap transaction expiration has passed on chain.", "sunswap_deadline_expired");
        ...
        if (maximumDebit === undefined)
            throw new ApnError("APN_STATE_CORRUPT", "SunSwap maximum TRX debit is missing.");
...
function protocol() { throw new ApnError("APN_RPC_PROTOCOL", "TRON returned a SunSwap pre-send simulation with an invalid shape or binding."); }
