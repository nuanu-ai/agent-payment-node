...
export const UNISWAP_TOKEN_COMMANDS = [command("inventory", [], "Read immutable token route pins.", "none"), command("quote", quote, "Quote and simulate the exact token route.", "network_read"), command("prepare", [profile, opt("--quote", "string", ["64_lowercase_hex_quote_hash"]), opt("--idempotency-key", "idempotency_key", ["global_payment_key"])], "Prepare from one saved quote.", "payment_prepare"), command("status", [operation], "Observe only.", "local_read"), command("approve", [operation], "Foreground consent and exact approval.", "payment_submit", { class: "foreground_tty", when: "Exact token operation." }), command("execute", [operation], "Continue swap or explicit cleanup without replay.", "payment_submit", { class: "foreground_tty", when: "Previously approved operation." })];
export function bindUniswapTokenCommand(path, o) {
    ...
    if (["status", "approve", "execute"].includes(action)) {
        exact(o, ["--operation"]);
...
function exact(o, keys) { if (!exactKeys(o, keys))
    invalid(); }
function hash(v) { if (!v || !/^[a-f0-9]{64}$/u.test(v))
    invalid(); return v; }
function integer(v) { if (!v || !/^[1-9][0-9]*$/u.test(v) || !Number.isSafeInteger(Number(v)))
