...
export const ORCA_COMMAND_GROUPS = [
...
export const ORCA_COMMANDS = [
    ...
    command("quote", [profile, option("--account", "string", ["canonical_32_byte_base58_solana_address_of_the_profile"]),
        ...
        option("--compute-unit-price", "string", ["micro_lamports_per_compute_unit"])], "Quote from the pinned pool's on-chain state, build the instructions locally, and simulate the exact unsigned transaction. Uses APN_SOLANA_RPC_URL.", "network_read"),
    command("prepare", [profile, option("--quote", "string", ["64_lowercase_hex_quote_hash"]), option("--idempotency-key", "idempotency_key", ["global_payment_key"])], "Prepare only after separate owner admission of SOL and USDC with the exact keyless mechanism.", "payment_prepare"),
    ...
    command("approve", [operation], "Show the exact swap screen, take the typed approval code, re-simulate with a fresh blockhash, sign locally and send once.", "payment_submit", { class: "foreground_tty", when: "Every guarded swap; MCP returns the exact CLI handoff only." }),
...
export function bindOrcaCommand(path, options) {
    const action = path.slice("swap solana orca ".length);
    if (!isPlainRecord(options))
        invalid("Orca options must be a plain object.");
    if (action === "inventory") {
        exact(options, []);
    ...
    if (action !== "quote")
        invalid("Unsupported Orca action.");
    ...
    if (price === undefined || !/^(?:0|[1-9][0-9]{0,15})$/u.test(price))
        invalid("Orca compute unit price must be canonical micro-lamports.");
...
function exact(value, keys) {
    if (!exactKeys(value, keys))
        invalid("Orca options contain an unknown or missing field.");
}
...
function canonicalAccount(value) {
    if (value === undefined)
        invalid("Orca account is required.");
    return solanaAddress(value);
}
function hash(value) {
    if (value === undefined || !/^[a-f0-9]{64}$/u.test(value))
        invalid("Orca hash must be 64 lowercase hexadecimal characters.");
    return value;
}
