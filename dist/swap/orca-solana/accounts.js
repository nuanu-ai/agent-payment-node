...
import { protocolFailure, rpcArray, rpcAtomic, rpcRecord, solanaAddress } from "../../solana/rpc.js";
 ...
 */
export async function readRawAccounts(rpc, addresses, maximumBytes, slice) {
    if (addresses.length < 1 || addresses.length > 16 || new Set(addresses).size !== addresses.length)
        protocolFailure();
...
export function rawAccount(value, maximumBytes, slice) {
    const record = rpcRecord(value);
    if (Object.keys(record).some((key) => !["data", "executable", "lamports", "owner", "rentEpoch", "space"].includes(key)) ||
        typeof record.owner !== "string" || typeof record.executable !== "boolean")
        protocolFailure();
    solanaAddress(record.owner);
    const data = rpcArray(record.data, 2);
    if (data.length !== 2 || typeof data[0] !== "string" || data[1] !== "base64" || data[0].length > Math.ceil(maximumBytes / 3) * 4)
        protocolFailure();
    const bytes = Buffer.from(data[0], "base64");
    if (record.space === undefined && slice !== undefined)
        protocolFailure();
    const space = record.space === undefined ? bytes.length : Number(rpcAtomic(record.space));
    if (bytes.toString("base64") !== data[0] || !Number.isSafeInteger(space) || bytes.length > maximumBytes)
        protocolFailure();
...
export function decodeWhirlpool(poolAddress, account, program, discriminator) {
    if (account === null || account.owner !== program || account.executable || account.data.length !== WHIRLPOOL_ACCOUNT_BYTES ||
        ...
        blocked("The pinned Whirlpool account is missing or malformed.", "orca_pool_state");
    const data = account.data, key = (offset) => getAddressDecoder().decode(data.subarray(offset, offset + 32));
...
export function decodeTickArray(arrayAddress, account, program, discriminator) {
...
export async function whirlpoolTickArrayAddress(program, pool, startTickIndex) {
    if (!Number.isSafeInteger(startTickIndex))
        blocked("Tick array start index is invalid.", "orca_tick_array_state");
...
export async function whirlpoolOracleAddress(program, pool) {
    const [pda] = await getProgramDerivedAddress({ programAddress: address(program),
...
function u128(data, offset) { return data.readBigUInt64LE(offset) + (data.readBigUInt64LE(offset + 8) << 64n); }
function i128(data, offset) { const value = u128(data, offset); return value >= 1n << 127n ? value - (1n << 128n) : value; }
