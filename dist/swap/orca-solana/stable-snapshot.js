...
import { ORCA_SOLANA_CHAIN, TOKEN_PROGRAM, USDC_MINT, WHIRLPOOL_ACCOUNT_DISCRIMINATOR, WHIRLPOOL_PROGRAM, WHIRLPOOLS_CONFIG, TICK_ARRAY_ACCOUNT_DISCRIMINATOR, verifyOrcaProgramPins } from "./pins.js";
import { prepareOrcaStableUnsigned } from "./stable-prepare.js";
 ...
 */
export async function readOrcaStableSnapshotAndPrepare(rpc, request) {
...
export async function readOrcaStableSnapshotCore(rpc, request, verifyPins) {
    ...
    if (full.slot < initial.slot)
        blocked("Stable Orca snapshot slot regressed.", "orca_slot_regressed");
    ...
    if (tickArrayStart(pool.tickCurrentIndex, 1) !== start)
        blocked("Stable Orca pool moved across tick arrays.", "orca_tick_array_moved");
    ...
    if (full.slot > BigInt(Number.MAX_SAFE_INTEGER))
        blocked("Stable snapshot slot exceeds RPC minContextSlot bounds.", "orca_stable_snapshot_slot");
    ...
    if (contextSlot < full.slot)
        blocked("Stable Orca blockhash read is behind the snapshot.", "orca_slot_regressed");
    if (contextSlot > BigInt(Number.MAX_SAFE_INTEGER))
        blocked("Stable blockhash context slot exceeds RPC bounds.", "orca_stable_snapshot_slot");
    const lifetime = rpcRecord(latest.value), blockhash = solanaAddress(lifetime.blockhash);
    ...
    const input = { owner: request.owner, quote,
...
function quoteFromRaw(pool, rows, addresses, request, slot) {
    ...
    const ticks = rows.map((row, index) => decodeTickArray(addresses[index], row, WHIRLPOOL_PROGRAM, TICK_ARRAY_ACCOUNT_DISCRIMINATOR));
...
function pinnedPool(account) {
    ...
    if (pool.config !== WHIRLPOOLS_CONFIG || pool.mintA !== USDC_MINT || pool.mintB !== SOLANA_USDT ||
        ...
        pool.feeTierIndexSeed !== 1 || pool.feeRate !== 100)
        blocked("Stable Orca pool pin changed.", "orca_stable_pool_state");
    return pool;
}
function blocked(message, reason) { throw new ApnError("APN_OPERATION_BLOCKED", message, { reason }); }
//# sourceMappingURL=stable-snapshot.js.map