import { AccountRole, address, appendTransactionMessageInstructions, blockhash, compileTransaction, createNoopSigner, createTransactionMessage, getBase64EncodedWireTransaction, getCompiledTransactionMessageDecoder, setTransactionMessageFeePayerSigner, setTransactionMessageLifetimeUsingBlockhash, } from "@solana/kit";
...
import { ATA_PROGRAM, COMPUTE_BUDGET_PROGRAM, ORCA_SOL_USDC_POOL, ORCA_SOL_VAULT, ORCA_USDC_VAULT, sha256Hex, SYSTEM_PROGRAM, TOKEN_PROGRAM, USDC_MINT, WHIRLPOOL_PROGRAM, WHIRLPOOL_SWAP_DISCRIMINATOR, WSOL_MINT, } from "./pins.js";
export const ORCA_ALLOWED_PROGRAMS = [COMPUTE_BUDGET_PROGRAM, ATA_PROGRAM, SYSTEM_PROGRAM, TOKEN_PROGRAM, WHIRLPOOL_PROGRAM];
 ...
 */
export function orcaSwapInstructions(planValue) {
    ...
    limit[0] = 2;
    limit.writeUInt32LE(plan.computeUnitLimit, 1);
    const price = Buffer.alloc(9);
    price[0] = 3;
    price.writeBigUInt64LE(BigInt(plan.computeUnitPriceMicroLamports), 1);
    const swap = Buffer.alloc(42);
    Buffer.from(WHIRLPOOL_SWAP_DISCRIMINATOR, "hex").copy(swap, 0);
    swap.writeBigUInt64LE(amount, 8);
    swap.writeBigUInt64LE(BigInt(plan.minimumOutputAtomic), 16);
    swap[40] = 1;
    swap[41] = 1;
...
export function compileOrcaSwap(plan, lifetime) {
    solanaAddress(lifetime.blockhash);
    if (!/^[1-9][0-9]{0,19}$/u.test(lifetime.lastValidBlockHeight))
        invalid("Solana lastValidBlockHeight is invalid.");
    const message = appendTransactionMessageInstructions(orcaSwapInstructions(plan), setTransactionMessageLifetimeUsingBlockhash({ blockhash: blockhash(lifetime.blockhash), lastValidBlockHeight: BigInt(lifetime.lastValidBlockHeight) }, setTransactionMessageFeePayerSigner(createNoopSigner(address(plan.owner)), createTransactionMessage({ version: 0 }))));
 ...
 */
export function validateOrcaSwapMessage(messageBytes, planValue, expectedBlockhash) {
    ...
    try {
        ...
        return refuse("Orca swap message bytes do not decode.", "orca_message_decode");
    }
    if (message.version !== 0 || (message.addressTableLookups ?? []).length !== 0)
        refuse("Orca swap must be a v0 message without lookup tables.", "orca_message_shape");
    if (message.lifetimeToken !== expectedBlockhash)
        refuse("Orca swap lifetime is not the bound blockhash.", "orca_message_lifetime");
    ...
    if (header.numSignerAccounts !== 1 || header.numReadonlySignerAccounts !== 0 || staticAccounts[0] !== plan.owner ||
        new Set(staticAccounts).size !== total || header.numReadonlyNonSignerAccounts >= total)
        refuse("Orca swap has a signer other than the owner.", "orca_instruction_signer");
    const writableAt = (index) => index < header.numSignerAccounts - header.numReadonlySignerAccounts ||
    ...
    staticAccounts.forEach((key, index) => {
        if (writableAt(index) && !allowedWritable.has(key))
            refuse("Orca swap marks an unexpected account writable.", "orca_instruction_writable");
    });
    const expected = expectedInstructions(plan);
    if (message.instructions.length !== expected.length)
        refuse("Orca swap instruction count changed.", "orca_instruction_mismatch");
    message.instructions.forEach((instruction, position) => {
        ...
        if (program === undefined || !ORCA_ALLOWED_PROGRAMS.includes(program))
            refuse("Orca swap invokes a program outside the allowlist.", "orca_instruction_program");
        const indices = instruction.accountIndices ?? [];
        const accounts = indices.map((index) => {
            const key = staticAccounts[index];
            if (key === undefined)
                refuse("Orca swap account index is out of range.", "orca_message_shape");
...
function expectedInstructions(plan) {
...
export function validatePlan(plan) {
    if (typeof plan !== "object" || plan === null || !Array.isArray(plan.tickArrays) || plan.tickArrays.length !== 3)
        invalid("Orca swap plan is invalid.");
    ...
    if (new Set(keys).size !== keys.length || ORCA_ALLOWED_PROGRAMS.includes(plan.owner))
        invalid("Orca swap plan accounts must be distinct.");
    ...
    if (amount > U64_MAX || minimum > U64_MAX || !Number.isSafeInteger(plan.computeUnitLimit) || plan.computeUnitLimit < 1 ||
        plan.computeUnitLimit > ORCA_MAX_COMPUTE_UNITS || BigInt(plan.computeUnitPriceMicroLamports) > U64_MAX)
        invalid("Orca swap plan bounds are invalid.");
    return plan;
}
function unsigned(value, positive) {
    if (typeof value !== "string" || !(positive ? /^[1-9][0-9]{0,19}$/u : /^(?:0|[1-9][0-9]{0,19})$/u).test(value))
        invalid("Orca swap integer is invalid.");
    return BigInt(value);
}
function invalid(message) { throw new ApnError("APN_INVALID_INPUT", message); }
function refuse(message, reason) { throw new ApnError("APN_OPERATION_BLOCKED", message, { reason }); }
