...
export function createApnCore(bound, options = {}) {
    ...
    if (bound.request.command === "relay.arbitrum.approval-check") {
        ...
        const reader = new RelayArbitrumApprovalPreflightReader(url, state, options.relayArbitrumApprovalRpc, () => guard);
    ...
    if (bound.request.command === "relay.base.observe") {
        ...
        return new ApnCore({ state, relayBaseObserve: options.relayBaseObserve ??
                new RelayBaseObserveService(state, baseInvocation, new RelayKeylessStatusService(state, options.relayStatusFetch), source) });
    }
    ...
    const relayExecute = relayAuthorization === undefined ? undefined
        : createRelayEthereumSourceRuntime(state, wrappingSecret, bound.rpcUrl ?? "", { confirm: relayExecuteConfirmation }, clock, options.relayExecuteTransport);
    ...
    const relayNativeExecute = nativeRelayAuthorization === undefined ? undefined
        : createRelayNativeSourceRuntime(state, wrappingSecret, bound.rpcUrl ?? "", nativeRelayAuthorization.confirm.bind(nativeRelayAuthorization), clock, options.relayExecuteTransport);
    ...
    return new ApnCore({
        ...
        ...(bound.request.command === "relay.prepare" || bound.request.command === "relay.base.prepare" ||
            bound.request.command === "relay.arbitrum.prepare" || bound.request.command === "relay.native.prepare" ||
            options.relayPrepare !== undefined ? { relayPrepare: options.relayPrepare ?? new RelayUnsignedPrepareService(state, clock, undefined, options.relayPreparePorts) } : {}),
...
function uniswapTokenRpcBudget(command) {
    ...
    if (command === "swap.uniswap-token.quote")
        return { maxHttpRequests: (overhead === 0 ? 8 : 7 + overhead), deadlineMs: 60_000 };
    if (command === "swap.uniswap-token.prepare")
        return { maxHttpRequests: (overhead === 0 ? 9 : 8 + overhead), deadlineMs: 60_000 };
