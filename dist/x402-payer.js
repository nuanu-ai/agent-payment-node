...
export async function resolveX402Payer(context, profileHash) {
    ...
    if (provider === null || provider.provider_id === LOCAL_PROVIDER_ID) {
        ...
        return { wallet: wallet.address.toLowerCase(), policy: policyBinding(wallet) };
    }
    const capability = provider.capability_snapshot.x402;
    if (provider.drift.state !== "bound" || !capability.available ||
    ...
    if (adapter.provider_id !== provider.provider_id || adapter.capabilities.x402.mode !== capability.mode ||
    ...
    return {
        wallet: provider.public_address.toLowerCase(),
...
//# sourceMappingURL=x402-payer.js.map