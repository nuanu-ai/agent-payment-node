...
import { ALLOWLIST_POLICY_RECORD_SCHEMA, allowlistProfileHash, compileAllowlistPolicyOverlay, isoInstant, validateAllowlistPolicyRecord, } from "./allowlist-policy-overlay.js";
import { ALLOWLIST_POLICY_RECORD_SCHEMA_V2, compileAllowlistPolicyOverlayV2, parseAllowlistPolicyFile, sealAllowlistPolicyRecordV2, validateAllowlistPolicyRecordV2, } from "./allowlist-policy-v2.js";
...
export const ALLOWLIST_POLICY_ACTIVATION_SCHEMA = "apn.allowlist-policy-activation.v1";
...
export class AllowlistPolicyStore extends SecureStateStore {
    ...
    async prepare(input) {
        ...
        return await this.writeRevision(compiled.overlay.profile, expectedRevision, { [family]: compiled.overlay.account }, compiled.overlay.overlayVersion, async (revision) => {
    ...
    async stage(input) {
        ...
        const compiled = compileAllowlistPolicyOverlayV2({ ...policy, profile: input.profile,
            datasetVersion: inventory.dataset.version, datasetSha256: inventory.dataset.sha256, inventorySha256: inventory.inventorySha256 }, inventory);
        const preparedAt = instant(input.now);
        return await this.writeRevision(input.profile, input.expectedRevision, compiled.overlay.accounts, compiled.overlay.overlayVersion, async (revision) => sealAllowlistPolicyRecordV2({ schemaVersion: ALLOWLIST_POLICY_RECORD_SCHEMA_V2, revision,
    ...
    async appendDecision(profile, expectedHeadDigest, decision) {
        ...
        return await this.withLocks([lockKey(profileHash)], async () => {
            ...
            if ((head?.entryDigest ?? null) !== expectedHeadDigest)
                conflict("The allowlist activation state changed after the approval screen.");
    ...
    async writeRevision(profile, expectedRevision, accounts, overlayVersion, build) {
        const profileHash = allowlistProfileHash(profile);
        await this.ready();
        return await this.withLocks([lockKey(profileHash)], async () => {
            const state = await this.load(profile, profileHash);
    ...
    async load(profile, profileHash) {
        const records = [];
        ...
        if (records.some((record, index) => record.revision !== index + 1))
            corrupt("Allowlist policy revisions are not contiguous.");
        const entries = [];
...
export function validateStagedAllowlistPolicyRecord(value) {
    if (value === null)
        corrupt("Allowlist policy version disappeared during validation.");
    if (isPlainRecord(value) && value.schemaVersion === ALLOWLIST_POLICY_RECORD_SCHEMA_V2)
        return validateAllowlistPolicyRecordV2(value);
    try {
        return validateAllowlistPolicyRecord(value);
    }
    catch (error) {
...
export function stagedRecordAccounts(record) {
...
export function validateActivationEntry(value) {
    ...
    if (value.status === "active") {
        let registry;
        try {
            registry = validateAssetPolicyRegistry(value.registry);
        }
        catch {
            return corrupt("Allowlist active registry is invalid.");
        }
        if (registry.policyDigest !== value.policyDigest)
            corrupt("Allowlist active registry digest binding is invalid.");
    }
...
//# sourceMappingURL=allowlist-policy-store.js.map