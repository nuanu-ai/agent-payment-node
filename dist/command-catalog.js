...
const BASE_COMMANDS = [
    ...
    command(["operation", "abandon"], "apn operation abandon --operation <operation-id>", "Administratively terminalize one strictly eligible unknown provider or expired local rail effect.", [operationRequired], "local_write", "After exact owner confirmation, appends an abandonment transition and authoritative receipt; expired local rail eligibility uses read-only RPC checks and never contacts a wallet, signer or provider.", "foreground_tty", "Requires the exact operation-bound phrase and acknowledgement that the financial outcome remains unknown.", { terminal: ["abandoned_unknown"], non_terminal: ["ambiguous_effect", "unknown_finality", "submitted_pending"] }, [
