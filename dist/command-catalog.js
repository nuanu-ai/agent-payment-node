...
const BASE_COMMANDS = [
    ...
    command(["relay", "native", "execute"], "apn relay native execute --operation <operation-id> --rpc-url <bnb-rpc>", "Foreground-confirm and submit the exact saved BNB native Relay deposit once.", [operationRequired, rpcRequired], "payment_submit", "Requires a fixed buyer profile, exact saved BNB to Polygon quote, active native bridge policy, fresh funding and nonce, encrypted local wallet, and one durable send marker. Returns BNB source evidence only; Polygon delivery and paid acceptance remain unproven.", "foreground_tty", "After saved operation validation and before signing or source effects.", { terminal: ["confirmed", "failed"], non_terminal: ["pending", "signing_started", "sealed", "submitting"] }, [{ command_path: ["operation", "status"], when: "Inspect the saved unsigned quote." }], ["apn relay native execute --operation <operation-id> --rpc-url https://bsc-rpc.publicnode.com"]),
    command(["relay", "retire"], "apn relay retire --profile <default|evm-live-buyer> --operation <operation-id>", "Retire one untouched prepared Relay quote locally.", [profileRequired, operationRequired], "local_write", "Writes a durable retirement marker only when no Relay effect or usage reservation exists. Buyer profile accepts its exact BNB native to Polygon or Monad quote. Does not sign or use the network.", "none", "Never.", { terminal: ["retired"], non_terminal: [] }, [{ command_path: ["operation", "status"], when: "Inspect the retained quote and retirement proof." }], ["apn relay retire --profile default --operation <operation-id>", "apn relay retire --profile evm-live-buyer --operation <operation-id>"]),
