...
export const COMMAND_GROUPS = [...ALLOWLIST_COMMAND_GROUPS, ...UNISWAP_COMMAND_GROUPS, ...UNISWAP_TOKEN_COMMAND_GROUPS,
    ...
    { path: ["relay"], summary: "Prepare and execute a finite Ethereum USDC to BNB native Relay source operation.", kind: "group" },
...
const BASE_COMMANDS = [
    ...
    command(["relay", "native", "prepare"], "apn relay native prepare --profile <bnb-owner-profile> --recipient 0x0B4Dd0C3dA001Fa146EEd3f80B01860BEF6B8a14 --amount-atomic <bnb-wei> --min-output-atomic <pol-wei> --max-deposit-network-fee-wei <bnb-wei> --idempotency-key <key>", "Freeze one unsigned BNB native to Polygon native POL Relay quote.", [profileRequired, option("--recipient", "address", true, noDefault, ["canonical_evm_address"], "public"), option("--amount-atomic", "wei", true, noDefault, ["positive_integer"], "operator_input"), option("--min-output-atomic", "wei", true, noDefault, ["positive_integer"], "operator_input"), option("--max-deposit-network-fee-wei", "wei", true, noDefault, ["positive_integer"], "operator_input"), option("--idempotency-key", "idempotency_key", true, noDefault, ["8_to_128_safe_ascii_characters"], "operator_input")], "payment_prepare", "Reads active BNB native bridge policy and one public Relay quote; writes an unsigned operation. Source account and Polygon recipient are fixed. No preflight, signing or send path is admitted.", "none", "Never.", { terminal: [], non_terminal: ["prepared"] }, [{ command_path: ["operation", "status"], when: "Inspect saved unsigned quote." }], ["apn relay native prepare --profile evm-live-buyer --recipient <default-wallet-address> --amount-atomic 1500000000000000 --min-output-atomic 9000000000000000000 --max-deposit-network-fee-wei 30000000000000 --idempotency-key relay-bnb-pol-0001"]),
    ...
    command(["relay", "execute"], "apn relay execute --operation <operation-id> --rpc-url <ethereum-rpc>", "Foreground-confirm and run the exact saved Ethereum approval and deposit source effects.", [operationRequired, rpcRequired], "payment_submit", "Requires explicit foreground confirmation; may sign and submit the exact saved Ethereum source effects once per durable effect. Returns only the source effect journal; BNB delivery and paid acceptance remain unproven.", "foreground_tty", "Inside the source runtime, after operation validation and before any durable source effect or signing.", { terminal: ["confirmed", "failed"], non_terminal: ["pending", "signing_started", "sealed", "submitting", "submission_marked", "tx_known"] }, [{ command_path: ["operation", "status"], when: "Inspect saved source state without a new signing attempt." }], ["apn relay execute --operation <operation-id> --rpc-url https://rpc.example"]),
    command(["relay", "retire"], "apn relay retire --profile default --operation <operation-id>", "Retire one untouched prepared Relay quote locally.", [profileRequired, operationRequired], "local_write", "Writes a durable retirement marker only when no Relay effect or usage reservation exists. Does not sign or use the network.", "none", "Never.", { terminal: ["retired"], non_terminal: [] }, [{ command_path: ["operation", "status"], when: "Inspect the retained quote and retirement proof." }], ["apn relay retire --profile default --operation <operation-id>"]),
