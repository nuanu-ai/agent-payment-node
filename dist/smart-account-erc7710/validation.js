...
export async function validateErc7710Material(intent, payment) {
    ...
    if (wire.delegationManager.toLowerCase() !== intent.delegationManager ||
        wire.delegator.toLowerCase() !== intent.ownerAddress)
        protocol();
    let chain;
    try {
        chain = decodeDelegations(wire.permissionContext);
    }
    catch {
        return protocol();
    }
    ...
    const encodedRoot = encodeDelegations([root]).toLowerCase();
    const expectedRoot = intent.rootContext.toLowerCase();
    let actualSalt;
    try {
        actualSalt = BigInt(child.salt);
    }
    catch {
        return protocol();
    }
    if (encodedRoot !== expectedRoot || child.delegate.toLowerCase() !== ANY_BENEFICIARY.toLowerCase() ||
        ...
        encodeDelegations([child, root]).toLowerCase() !== wire.permissionContext.toLowerCase())
        protocol();
    let recovered;
    try {
    ...
    if (recovered.toLowerCase() !== intent.sessionAddress)
        protocol();
    validateCaveats(child.caveats, intent);
    const encodedChild = encodeDelegations([child]).toLowerCase();
    return {
        encodedRoot,
        encodedChild,
        permissionContext: wire.permissionContext.toLowerCase(),
        rootDelegationHash: hashDelegation(toDelegationStruct(root)).toLowerCase(),
        childDelegationHash: hashDelegation(toDelegationStruct(child)).toLowerCase(),
    };
}
function strictWire(value) {
    if (!isPlainRecord(value) || !exactKeys(value, ["delegationManager", "delegator", "permissionContext"]) ||
        ...
        !isHex(value.permissionContext) || value.permissionContext === "0x")
        protocol();
    return value;
}
function validateCaveats(caveats, intent) {
    ...
    if (expected.some(value => value === undefined) || caveats.length !== 5 ||
        new Set(caveats.map(value => value.enforcer.toLowerCase())).size !== 5 || expected.some((address, index) => caveats.filter(value => value.enforcer.toLowerCase() === address?.toLowerCase() &&
        value.args.toLowerCase() === args[index]).length !== 1))
        protocol();
    const get = (address) => caveats.find(value => value.enforcer.toLowerCase() === address?.toLowerCase());
    try {
