...
import { SA_FILE_LIMIT, SA_HISTORY_LIMIT, SA_MUTABLE_KEYS, SA_OPERATION_KIND, SA_OPERATION_VERSION, SA_TERMINAL, SA_TERMINAL_RESERVE_BYTES, SA_TERMINAL_RESERVE_TRANSITIONS, saImmutable, saMutable, saRequestHash } from "./operation-model.js";
import { saMaterialHash, saRequirementsHash, saSame } from "./integrity.js";
import { SA_REASON_CODES, saFail } from "./reasons.js";
import { saBlock, saBlockOrder, saCanonicalAddress, saExact, saHash, saHex, saIso, saStateCounters, saUint, validateSmartAccountGaslessIntent } from "./schema.js";
const TRANSITION_KEYS = ["at", "previousHash", ...SA_MUTABLE_KEYS, "transitionHash"];
const OPERATION_KEYS = ["schemaVersion", "kind", "profileHash", "operationId", "idempotencyHash", "requestHash",
    "fingerprint", "createdAt", "updatedAt", "terminal", "intent", ...SA_MUTABLE_KEYS, "transitions", "integrityHash"];
const EDGES = {
...
const MATERIAL_HASH_KEYS = ["encodedRootHash", "encodedChildHash", "permissionContextHash", "payloadHash",
    "requirementsHash", "materialHash", "rootDelegationHash", "childDelegationHash"];
function corrupt() { return saFail("sa_gasless_state_corrupt"); }
function capacity() { return saFail("sa_gasless_capacity"); }
function time(value) { return Date.parse(value); }
function within(value, lower, upper) {
    const result = saIso(value);
    if (time(result) < time(lower) || time(result) > time(upper))
        corrupt();
    return result;
}
...
function cursorSchema(value, intent) {
    ...
    if (!saSame(start, intent.initialSnapshot.preparationBlock) || next < BigInt(start.numberAtomic))
        corrupt();
    if (c.previousEndBlock === null) {
        if (next !== BigInt(start.numberAtomic))
            corrupt();
    }
    else {
        const end = saBlock(c.previousEndBlock);
        saBlockOrder(start, end);
        if (BigInt(end.numberAtomic) + 1n !== next)
            corrupt();
    }
    if (c.expiryBlock !== null) {
        const expiry = saBlock(c.expiryBlock);
        saBlockOrder(start, expiry);
        if (BigInt(expiry.timestampAtomic) < BigInt(intent.beforeUnix) || next > BigInt(expiry.numberAtomic) + 1n)
            corrupt();
        if (c.previousEndBlock !== null)
            saBlockOrder(saBlock(c.previousEndBlock), expiry);
    }
    hashList(c.candidateHashes);
    hashList(c.transferAnomalies);
    if (typeof c.childScanComplete !== "boolean" || typeof c.transferScanComplete !== "boolean")
        corrupt();
    if ((c.childScanComplete || c.transferScanComplete) && (c.expiryBlock === null ||
        next !== BigInt(saBlock(c.expiryBlock).numberAtomic) + 1n))
        corrupt();
    return c;
}
function descriptor(value, intent, operationId, fingerprint, approvedAt, at) {
    const d = saExact(value, [...MATERIAL_HASH_KEYS, "sealedAt"]);
    hashes(d, MATERIAL_HASH_KEYS.slice(0, 6));
    saHex(d.rootDelegationHash, 32);
    saHex(d.childDelegationHash, 32);
    ...
    if (d.encodedRootHash !== intent.binding.encodedRootHash ||
        ...
        d.requirementsHash !== saRequirementsHash(intent.requirements))
        corrupt();
    const { sealedAt: _at, materialHash, ...rest } = d;
    if (materialHash !== saMaterialHash(operationId, fingerprint, rest))
        corrupt();
    return d;
}
function proofFields(m, intent, material, cursor, lower, at) {
    if (m.settlement !== null) {
        if (material === null || m.state !== "completed" || m.unusedProof !== null)
            corrupt();
        ...
        saBlockOrder(cursor.startBlock, tx);
        saBlockOrder(tx, head);
        saHex(s.txHash, 32);
        ...
        if ((s.source !== "rpc_discovered" && s.source !== "provider_hint") ||
            ...
            BigInt(tx.timestampAtomic) >= BigInt(intent.beforeUnix) || BigInt(head.timestampAtomic) * 1000n > BigInt(time(observedAt)))
            corrupt();
        if (s.source === "provider_hint" && (m.submissionAttempts !== 1 || m.providerSettlement === null ||
            m.providerSettlement.transactionHash !== s.txHash))
            corrupt();
        for (const key of ["childSpentAtomic", "debitAtomic", "deliveredAtomic"])
            if (s[key] !== intent.request.grossAtomic)
                corrupt();
    ...
    if (m.unusedProof !== null) {
        if (material === null || m.state !== "expired_unused" || m.settlement !== null)
            corrupt();
        ...
        if (!saSame(start, cursor.startBlock) || !saSame(expiry, cursor.expiryBlock) ||
            ...
            cursor.candidateHashes.length !== 0 || cursor.transferAnomalies.length !== 0)
            corrupt();
    }
    else if (m.state === "expired_unused")
        corrupt();
}
function journalMutable(value, intent, operationId, fingerprint, at) {
    ...
    if ((m.signingAttempts === 0 && material !== null) || (m.state === "material_pending" && material !== null) ||
        (material === null && (m.state === "material_sealed" || m.exposureAttempts === 1)))
        corrupt();
    let exposedAt = null, dispatchAt = null;
    if (m.exposureAttempts === 0) {
        if (m.exposureStartedAt !== null)
            corrupt();
    }
    else {
        exposedAt = within(m.exposureStartedAt, material.sealedAt, at);
        if (time(exposedAt) >= time(intent.expiresAt))
            corrupt();
    }
    if (m.verification !== null) {
        const v = saExact(m.verification, ["observedAt", "payer", "isValid", "responseHash"]);
        if (exposedAt === null || v.isValid !== true || saCanonicalAddress(v.payer) !== intent.binding.ownerAddress)
            corrupt();
        within(v.observedAt, exposedAt, at);
        saHash(v.responseHash);
    }
    if (m.submissionAttempts === 0) {
        if (m.dispatchStartedAt !== null || m.providerSettlement !== null)
            corrupt();
    }
    else {
        if (m.verification === null)
            corrupt();
        dispatchAt = within(m.dispatchStartedAt, m.verification.observedAt, at);
        if (time(dispatchAt) >= time(intent.expiresAt))
            corrupt();
    }
    if ((m.state === "verified_pending" && m.verification === null) ||
        (m.state === "exposure_pending" && m.verification !== null))
        corrupt();
    if (m.providerSettlement !== null) {
        const p = saExact(m.providerSettlement, ["observedAt", "transactionHash", "responseHash"]);
        if (dispatchAt === null)
            corrupt();
        within(p.observedAt, dispatchAt, at);
        saHash(p.responseHash);
        if (p.transactionHash !== null)
            saHex(p.transactionHash, 32);
    }
    ...
    if (m.observation !== null) {
        ...
        if (o.reason !== null && (typeof o.reason !== "string" || !Object.hasOwn(SA_REASON_CODES, o.reason)))
            corrupt();
        if (o.candidateTxHash !== null)
            saHex(o.candidateTxHash, 32);
        if (o.evidenceHash !== null)
            saHash(o.evidenceHash);
        if ((o.phase === "success") !== (m.state === "completed") ||
            (o.phase === "expired_unused") !== (m.state === "expired_unused"))
            corrupt();
        if ((m.state === "completed" || m.state === "expired_unused") && (o.reason !== null || o.evidenceHash === null))
            corrupt();
    }
    else if (m.state === "completed" || m.state === "expired_unused")
        corrupt();
    const cursor = cursorSchema(m.cursor, intent);
    if (m.exposureAttempts === 0 && !saSame(cursor, initialMutable(intent).cursor))
        corrupt();
    proofFields(m, intent, material, cursor, exposedAt ?? intent.preparedAt, at);
    if (m.settlement !== null && m.observation.candidateTxHash !==
        m.settlement.txHash)
        corrupt();
    if (m.failure !== null) {
        const f = saExact(m.failure, ["code", "reason"]);
        if (typeof f.reason !== "string" || !Object.hasOwn(SA_REASON_CODES, f.reason) ||
            f.code !== SA_REASON_CODES[f.reason])
            corrupt();
    }
    if ((m.state === "failed_before_effect" || m.state === "unknown_finality") && m.failure === null)
        corrupt();
    if (m.state !== "failed_before_effect" && m.state !== "unknown_finality" && m.failure !== null)
        corrupt();
    return m;
}
function initialMutable(intent) {
...
function step(previous, next) {
    if (!EDGES[previous.state].includes(next.state) || time(next.at) < time(previous.at))
        corrupt();
    const once = ["approval", "material", "verification", "providerSettlement"];
    const stages = { approval: ["awaiting_approval", "execution_pending"], material: ["material_pending", "material_sealed"],
        verification: ["exposure_pending", "verified_pending"], providerSettlement: ["dispatch_pending", "submitted_pending"] };
    for (const key of once) {
        if (previous[key] !== null && !saSame(previous[key], next[key]))
            corrupt();
        if (previous[key] === null && next[key] !== null &&
            (previous.state !== stages[key][0] || next.state !== stages[key][1]))
            corrupt();
    }
    if (previous.approval === null && next.approval !== null && next.approval.approvedAt !== next.at)
        corrupt();
    if (previous.material === null && next.material !== null && time(next.material.sealedAt) < time(previous.at))
        corrupt();
    const markers = { signingAttempts: ["execution_pending", "material_pending"],
        exposureAttempts: ["material_sealed", "exposure_pending"], submissionAttempts: ["verified_pending", "dispatch_pending"] };
    for (const key of ["signingAttempts", "exposureAttempts", "submissionAttempts"]) {
        if (previous[key] > next[key] || (previous[key] === 0 && next[key] === 1 &&
            (previous.state !== markers[key][0] || next.state !== markers[key][1])))
            corrupt();
    }
    for (const [counter, marker] of [["exposureAttempts", "exposureStartedAt"], ["submissionAttempts", "dispatchStartedAt"]]) {
        if ((previous[counter] === 0 && next[counter] === 1 && next[marker] !== next.at) ||
            (previous[counter] === 1 && previous[marker] !== next[marker]))
            corrupt();
    }
    if (previous.observation !== null && (next.observation === null ||
        time(next.observation.observedAt) < time(previous.observation.observedAt)))
        corrupt();
    ...
    if (after < before || !saSame(p.startBlock, n.startBlock) ||
        ...
        (p.expiryBlock !== null && !saSame(p.expiryBlock, n.expiryBlock)))
        corrupt();
    if (p.previousEndBlock !== null && n.previousEndBlock !== null)
        saBlockOrder(p.previousEndBlock, n.previousEndBlock);
    for (const key of ["candidateHashes", "transferAnomalies"])
        if (!saSame(p[key], n[key].slice(0, p[key].length)))
            corrupt();
    for (const key of ["childScanComplete", "transferScanComplete"])
        if (p[key] && !n[key])
            corrupt();
    if (!saSame(p, n) && (next.observation === null || previous.exposureAttempts !== 1))
        corrupt();
}
function seal(body) {
    return { ...body, integrityHash: hashObject(body) };
}
function byteLength(value) { return Buffer.byteLength(canonicalJson(value), "utf8") + 1; }
function validate(value) {
    if (byteLength(value) > SA_FILE_LIMIT)
        corrupt();
    const r = saExact(value, OPERATION_KEYS);
    if (r.schemaVersion !== SA_OPERATION_VERSION || r.kind !== SA_OPERATION_KIND || typeof r.terminal !== "boolean")
        corrupt();
    ...
    const profileHash = r.profileHash, operationId = r.operationId, fingerprint = r.fingerprint;
    if (profileHash !== sha256(`profile\0${intent.profile}`) || createdAt !== intent.preparedAt || time(updatedAt) < time(createdAt))
        corrupt();
    const immutable = { schemaVersion: SA_OPERATION_VERSION, kind: SA_OPERATION_KIND, profileHash, operationId,
        idempotencyHash: r.idempotencyHash, requestHash: r.requestHash, createdAt, intent };
    if (fingerprint !== hashObject(saImmutable(immutable)) || r.requestHash !== saRequestHash(profileHash, intent) ||
        !Array.isArray(r.transitions) || r.transitions.length < 1 || r.transitions.length > SA_HISTORY_LIMIT)
        corrupt();
    const transitions = [];
    for (const raw of r.transitions) {
        ...
        if (prior === undefined) {
            if (previousHash !== fingerprint || at !== createdAt || !saSame(saMutable(current), initialMutable(intent)))
                corrupt();
        }
        else {
            if (previousHash !== prior.transitionHash)
                corrupt();
            step(prior, current);
        }
    ...
    if (last.at !== updatedAt || !saSame(saMutable(last), current) || r.terminal !== SA_TERMINAL.includes(current.state))
        corrupt();
    ...
    if (integrityHash !== hashObject(body))
        corrupt();
    return operation;
}
export function validateSmartAccountGaslessOperation(value) {
    try {
...
export function validateSmartAccountGaslessContinuity(previous, next) {
    const before = validateSmartAccountGaslessOperation(previous), after = validateSmartAccountGaslessOperation(next);
    if (saSame(before, after))
        return;
    if (before.terminal || before.fingerprint !== after.fingerprint || !saSame(saImmutable(before), saImmutable(after)) ||
        after.transitions.length !== before.transitions.length + 1 || !saSame(before.transitions, after.transitions.slice(0, -1)))
        corrupt();
}
export function newSmartAccountGaslessOperation(identityInput, intent) {
...
export function advanceSmartAccountGaslessOperation(operation, patch, atInput) {
    const current = validateSmartAccountGaslessOperation(operation), at = saIso(atInput);
    if (time(at) < time(current.updatedAt) || Object.keys(patch).some(key => !SA_MUTABLE_KEYS.includes(key)))
        corrupt();
    const mutable = { ...saMutable(current), ...patch };
    if (saSame(saMutable(current), mutable))
        return current;
    if (current.terminal)
        corrupt();
    ...
    if (current.observation !== null && mutable.observation !== null &&
        time(mutable.observation.observedAt) < time(current.observation.observedAt))
        corrupt();
    if (sameObservation(saMutable(current), mutable))
        return current;
    ...
    if (next.transitions.length > SA_HISTORY_LIMIT || byteLength(next) > SA_FILE_LIMIT ||
        (!terminal && (next.transitions.length > SA_HISTORY_LIMIT - SA_TERMINAL_RESERVE_TRANSITIONS ||
            byteLength(next) > SA_FILE_LIMIT - SA_TERMINAL_RESERVE_BYTES)))
        capacity();
    validateSmartAccountGaslessContinuity(current, next);
    return next;
}
export function smartAccountGaslessAtTransition(operation, index) {
    const current = validateSmartAccountGaslessOperation(operation), entry = current.transitions[index];
    if (!Number.isSafeInteger(index) || index < 0 || entry === undefined)
        corrupt();
...
export function assertSmartAccountGaslessCapacity(operation) {
    ...
    if (op.transitions.length >= SA_HISTORY_LIMIT - SA_TERMINAL_RESERVE_TRANSITIONS ||
        byteLength(op) > SA_FILE_LIMIT - SA_TERMINAL_RESERVE_BYTES)
        capacity();
}
