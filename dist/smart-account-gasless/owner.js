import { accountBindingHash, capabilityHash, metamaskSmartAccountX402CapabilitySnapshot, validateProviderProfile } from "../provider-profile.js";
import { canonicalProfile } from "../wallet-policy.js";
import { SA_ZERO_ADDRESS } from "./model.js";
import { saFail } from "./reasons.js";
import { saAddress, saBinding } from "./schema.js";
export async function smartAccountGaslessOwner(state, input, expected) {
    ...
    if (p.provider_id !== "metamask-smart-account" || p.trust_class !== "external_owner_delegated_local_session" ||
        p.capability_hash !== capabilityHash(metamaskSmartAccountX402CapabilitySnapshot()))
        saFail("sa_gasless_capability");
    ...
    if (p.profile !== profile || p.profile_hash !== profileHash || p.drift.state !== "bound" || p.drift.reason !== "none" ||
        address === SA_ZERO_ADDRESS || p.account_binding_hash !== accountBindingHash(p.provider_id, address))
    ...
    if (expected !== undefined)
        assertSmartAccountGaslessBinding(expected, owner);
    return owner;
}
export function assertSmartAccountGaslessBinding(value, owner) {
    ...
    if (b.profileHash !== owner.profileHash || b.ownerAddress !== owner.address || b.accountBindingHash !== owner.accountBindingHash ||
        b.capabilityHash !== owner.capabilityHash || b.profileRevision !== owner.revision)
        saFail("sa_gasless_identity");
    return b;
}
