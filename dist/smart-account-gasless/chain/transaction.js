...
export async function verifySmartAccountOuterTransaction(raw, expectedHash, intent) {
    ...
    if (type > 4n || rpcHex(raw.hash, 32, 32) !== expectedHash ||
        rpcQuantity(raw.chainId) !== BigInt(intent.request.chainId))
    ...
    if (type === 0n) {
        ...
        if (type === 2n) {
            ...
            if (!Array.isArray(raw.blobVersionedHashes) || raw.blobVersionedHashes.length < 1 ||
                raw.blobVersionedHashes.length > 64)
                saFail("sa_gasless_evidence");
...
async function parseAuthorizations(value, intent) {
    if (!Array.isArray(value) || value.length < 1 || value.length > MAX_AUTHORIZATIONS)
        saFail("sa_gasless_evidence");
    const owners = [], authorizations = [];
    for (const item of value) {
        ...
        if (chainId !== intent.request.chainId || (yParity !== 0 && yParity !== 1))
            saFail("sa_gasless_evidence");
        assertSignatureParts(r, s);
        ...
        if (owner === intent.binding.ownerAddress || owner === intent.binding.sessionAddress)
            saFail("sa_gasless_evidence");
        owners.push(owner);
...
function rejectPresent(raw, keys) {
