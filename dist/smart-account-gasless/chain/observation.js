...
import { SaRpcBudgetError, SaRpcReorgError, parseReceiptLogs, quantity, receiptHash, recheckBlock, rpcAddress, rpcBlock, rpcHex, rpcQuantity, rpcRecord } from "./abi.js";
...
export async function observeSmartAccountGasless(context, input) {
    ...
    try {
        ...
        if (BigInt(safe.numberAtomic) < BigInt(input.intent.initialSnapshot.preparationBlock.numberAtomic)) {
            ...
            return observation(originalCursor, "pending", "sa_gasless_unknown", input.transactionHint, hashObject({ preparation: input.intent.initialSnapshot.preparationBlock, safe }), null, null, context.clock);
        }
    }
    catch (error) {
        return adverse(originalCursor, classify(error), context.clock);
    }
    let cursor;
    try {
        const scan = await advanceSmartAccountGaslessScan(context.call, input.intent, input.material.childDelegationHash, originalCursor, safe);
        cursor = scan.cursor;
        if (scan.partial)
            return observation(cursor, "pending", "sa_gasless_partial", null, scan.evidenceHash, null, null, context.clock);
    }
    catch (error) {
        return adverse(originalCursor, classify(error), context.clock);
    }
    if (cursor.candidateHashes.length > 1) {
        return observation(cursor, "pending", "sa_gasless_unknown", null, hashObject({ candidates: cursor.candidateHashes, ambiguous: true }), null, null, context.clock);
    }
    ...
    if (cursor.childScanComplete && cursor.transferScanComplete) {
        if (cursor.transferAnomalies.length === 0) {
            ...
            return observation(cursor, "pending", "sa_gasless_unknown", null, hashObject({ transferAnomalies: cursor.transferAnomalies }), null, null, context.clock);
        }
    }
...
async function inspectSafely(context, input, hash, safe, source) {
    try {
        return await inspectCandidate(context, input, hash, safe, source);
    }
    catch (error) {
...
async function inspectCandidate(context, input, transactionHash, safe, source) {
    ...
    if (new Set(transactionHashes).size !== transactionHashes.length || transactionIndex >= BigInt(transactionHashes.length) ||
        transactionHashes[Number(transactionIndex)] !== transactionHash)
        saFail("sa_gasless_evidence");
    if (blockNumber < BigInt(input.intent.initialSnapshot.preparationBlock.numberAtomic) ||
        ...
        BigInt(included.timestampAtomic) >= BigInt(input.intent.beforeUnix))
        saFail("sa_gasless_evidence");
    ...
    if (status !== 0n && status !== 1n)
        saFail("sa_gasless_evidence");
    if (status === 0n)
        return { kind: "pending", evidenceHash: hashObject({ transactionHash, included, reverted: true }) };
    const redemption = await verifySmartAccountRedemption(outer.input, input.operationId, input.fingerprint, input.intent, input.material, context.validator);
    ...
    const state = await readSmartAccountGaslessProofState(context.call, input.intent.binding, input.material.childDelegationHash, safe);
    if (state.childSpentAtomic !== input.intent.request.grossAtomic)
        saFail("sa_gasless_evidence");
...
async function unusedSafely(context, input, cursor, finalized) {
    const expiry = cursor.expiryBlock;
    if (expiry === null || BigInt(finalized.numberAtomic) < BigInt(expiry.numberAtomic) ||
        BigInt(finalized.timestampAtomic) < BigInt(input.intent.beforeUnix))
        return null;
    try {
        const state = await readSmartAccountGaslessProofState(context.call, input.intent.binding, input.material.childDelegationHash, finalized);
        if (state.childSpentAtomic !== "0")
            return null;
        await recheckBlock(context.call, cursor.startBlock, "sa_gasless_evidence", true);
        if (cursor.previousEndBlock !== null)
            await recheckBlock(context.call, cursor.previousEndBlock, "sa_gasless_evidence", true);
        ...
        const proof = { observedAt: instant(context.clock), startBlock: cursor.startBlock,
...
function success(cursor, settlement) {
...
function adverse(cursor, inspected, clock, candidate = null) {
    if (inspected.kind === "success")
        return success(cursor, inspected.settlement);
    const mapping = {
...
function observation(cursor, phase, reason, candidateTxHash, evidenceHash, settlement, unusedProof, clock, observedAt = instant(clock)) {
    return { cursor, observation: { observedAt, phase, reason, candidateTxHash, evidenceHash }, settlement, unusedProof };
}
function classify(error) {
    if (error instanceof SaRpcReorgError)
...
export function validateSmartAccountGaslessObserveInput(input) {
    ...
    saHex(material.childDelegationHash, 32);
    saIso(material.sealedAt);
    if (material.encodedRootHash !== input.intent.binding.encodedRootHash ||
        material.rootDelegationHash !== input.intent.binding.rootDelegationHash)
        saFail("sa_gasless_state_corrupt");
    if (input.transactionHint !== null)
        saHex(input.transactionHint, 32);
}
function assertHeads(start, safe, finalized) {
