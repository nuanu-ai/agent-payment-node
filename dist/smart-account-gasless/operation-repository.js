...
import { SA_FILE_LIMIT } from "./operation-model.js";
...
export class SmartAccountGaslessOperationRepository extends SecureStateStore {
    initialized;
    async ready() {
    ...
    async writeOperation(input) {
        ...
        if (Buffer.byteLength(canonicalJson(op), "utf8") + 1 > SA_FILE_LIMIT)
            saFail("sa_gasless_capacity");
        await this.ready();
    ...
    async repairReceipt(input) {
        ...
        if (stored === null || !saSame(stored, op))
            corrupt();
        const path = this.path(RECEIPTS, op.profileHash, op.operationId), previous = await this.readJson(path);
        if (previous !== null)
            validateSmartAccountGaslessReceipt(previous, op);
        const receipt = smartAccountGaslessReceipt(op);
        if (saSame(previous, receipt))
            return;
        if (Buffer.byteLength(canonicalJson(receipt), "utf8") + 1 > SA_FILE_LIMIT)
            saFail("sa_gasless_capacity");
    ...
    async loadReceipt(profileHash, operationId) {
        ...
        if (op === null)
            throw new ApnError("APN_OPERATION_NOT_FOUND", "The Smart Account gasless operation was not found.");
        const value = await this.readJson(this.path(RECEIPTS, profileHash, operationId));
        if (value !== null)
            validateSmartAccountGaslessReceipt(value, op);
        if (value === null || !saSame(value, smartAccountGaslessReceipt(op))) {
            throw new ApnError("APN_OPERATION_BLOCKED", "The saved Smart Account gasless receipt needs recovery.", { reason: "sa_gasless_unknown", operationId, nextActions: [`apn operation resume --operation ${operationId}`] });
        }
        return value;
    }
