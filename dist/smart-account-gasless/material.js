...
import { SA_MIN_REMAINING_MS } from "./model.js";
import { saFail } from "./reasons.js";
export class SmartAccountGaslessMaterialValidator {
    async validate(input) {
        const intent = directIntent(input.operationId, input.fingerprint, input.intent, input.rootContext);
        const validated = await validateErc7710Material(intent, input.paymentPayload);
        ...
        if (encodedRootHash !== input.intent.binding.encodedRootHash ||
            validated.rootDelegationHash !== input.intent.binding.rootDelegationHash)
            saFail("sa_gasless_state_corrupt");
        return { ...partial, materialHash: saMaterialHash(input.operationId, input.fingerprint, partial) };
    }
}
export class MetaMaskSmartAccountGaslessMaterial {
    permissions;
    materials;
    engine;
    validator;
    now;
    constructor(permissions, materials, engine = new OfficialErc7710Engine(), validator = new SmartAccountGaslessMaterialValidator(), now = () => new Date()) {
    ...
    async inspect(expected, nowUnix) {
    ...
    async seal(operation) {
        const existing = await this.materials.load(operation.operationId);
        if (existing !== null)
            return await this.recover(operation, existing);
        ...
        if (preSignClockMs >= record.granted_expires_at_unix * 1_000)
        ...
        let officialPayment;
        try {
            officialPayment = await this.engine.create(neutral, { sessionPrivateKey: record.session_private_key, rootContext });
        }
        catch {
    ...
    async recover(operation, record) {
        if (record.operation_id !== operation.operationId || record.profile_hash !== operation.profileHash ||
            ...
            record.root_grant_fingerprint !== operation.intent.binding.rootGrantFingerprint)
            saFail("sa_gasless_state_corrupt");
        const payload = JSON.parse(record.payment_payload_canonical_json);
        const hashes = await this.validator.validate({ operationId: operation.operationId, fingerprint: operation.fingerprint,
    ...
    async activeRecord(profileHash, nowUnix) {
...
export function directIntent(operationId, fingerprint, intent, rootContext) {
...
function bindingFrom(record, expected) {
    ...
    return { providerId: "metamask-smart-account", trustClass: "external_owner_delegated_local_session",
        profileHash: expected.profileHash, ownerAddress: record.owner_address.toLowerCase(),
        sessionAddress: record.session_address.toLowerCase(), accountBindingHash: expected.accountBindingHash,
        ...
        rootDelegationHash: hashDelegation(toDelegationStruct(root)).toLowerCase(),
        delegationManager: record.delegation_manager.toLowerCase(), rootCapAtomic: cap,
        rootStartsAtUnix: starts, rootExpiresAtUnix: record.granted_expires_at_unix,
        periodTerms: period.terms.toLowerCase(), rootNonceAtomic: BigInt(nonce.terms).toString() };
}
function canonicalRoot(context) {
    let roots;
    try {
        roots = decodeDelegations(context);
    }
    catch {
    ...
    if (roots.length !== 1 || roots[0] === undefined)
        saFail("sa_gasless_state_corrupt");
    return encodeDelegations([roots[0]]).toLowerCase();
}
function assertOperationAuthority(operation, record) {
    ...
    if (b.profileHash !== operation.profileHash || b.permissionRevision !== record.revision ||
        ...
        b.encodedRootHash !== saRootContextHash(canonicalRoot(record.grant_context)))
        saFail("sa_gasless_identity");
}
function materialHashes(record) {
...
function projection(record) {
    return { descriptor: descriptor(record), paymentPayload: JSON.parse(record.payment_payload_canonical_json),
        phase: record.phase };
}
...
function canonicalPaymentPayload(intent, permissionContext) {
...
function assertPreSignClock(operation, initialClockMs, currentClockMs) {
    if (currentClockMs < initialClockMs)
        saFail("sa_gasless_clock");
    for (const value of savedObservationTimes(operation)) {
        const observed = Date.parse(value);
        if (!Number.isSafeInteger(observed) || new Date(observed).toISOString() !== value)
            saFail("sa_gasless_state_corrupt");
        if (currentClockMs < observed)
            saFail("sa_gasless_clock");
    }
    ...
    if (!Number.isSafeInteger(afterMs) || !Number.isSafeInteger(beforeMs) || currentClockMs < afterMs ||
        beforeMs - currentClockMs < SA_MIN_REMAINING_MS)
        saFail("sa_gasless_expired");
}
function savedObservationTimes(operation) {
    const mutable = (value) => [
