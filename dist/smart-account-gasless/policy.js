...
export class SmartAccountGaslessClock {
    ...
    check(op, observations = []) {
        let now;
        try {
            now = this.clock.now().getTime();
        }
        catch {
            return saFail("sa_gasless_clock");
        }
        ...
        if (op !== undefined) {
            ...
            for (const value of [op.approval?.approvedAt, op.material?.sealedAt, op.exposureStartedAt, op.dispatchStartedAt,
                ...
                op.settlement?.observedAt, op.unusedProof?.observedAt])
                if (value !== undefined && value !== null)
                    times.push(value);
        }
        if (now < this.previous || times.some(value => Date.parse(saIso(value, "sa_gasless_clock")) > now))
            saFail("sa_gasless_clock");
        this.previous = now;
        return now;
    }
    live(op, minimumRemaining = 0) {
        const now = this.check(op), expiry = Date.parse(op.intent.expiresAt);
        if (now >= expiry || now + minimumRemaining > expiry)
            saFail("sa_gasless_expired");
        return now;
    }
    fresh(observedAt, op) {
        const now = this.check(op, [observedAt]);
        if (now - Date.parse(observedAt) > SA_TTL_MS)
            saFail("sa_gasless_expired");
        return now;
    }
    signing(op) { return this.live(op, SA_MIN_REMAINING_MS); }
    /** Forensic timestamp for refusal, never a new sample granting an effect. */
    failureAt(op) { return new Date(Math.max(this.previous, Date.parse(op.updatedAt))).toISOString(); }
}
export function smartAccountGaslessSnapshot(value, binding, rpc, op) {
    let snapshot;
    try {
        snapshot = saSnapshot(value, binding);
    }
    catch {
        return saFail("sa_gasless_evidence");
    }
    if (rpc.chainId !== 8453 || snapshot.chainId !== rpc.chainId || snapshot.endpointHash !== rpc.endpointHash ||
        snapshot.endpointOrigin !== rpc.endpointOrigin)
    ...
    if (op !== undefined) {
        ...
        if (BigInt(snapshot.safeState.usdcBalanceAtomic) < BigInt(op.intent.request.grossAtomic))
            saFail("sa_gasless_balance");
        if (BigInt(snapshot.safeState.availableAtomic) < BigInt(op.intent.request.grossAtomic))
...
export class TtySmartAccountGaslessApproval {
    options;
    constructor(options = {}) {
    ...
    async confirm(input) {
        ...
        const amount = (value) => `${BigInt(value) / 1000000n}.${(BigInt(value) % 1000000n).toString().padStart(6, "0")} USDC (${value} atomic)`;
        ...
        try {
            ...
            if (error instanceof ApnError && error.code === "APN_NATIVE_REJECTED") {
                ...
                if (error.details?.nativeCode === "APN_APPROVAL_EXPIRED")
                    throw saError("sa_gasless_expired");
            }
