...
export class MetaMaskGaslessClock {
    ...
    check(op, observations = []) {
        ...
        try {
            ...
            return mmFail("mm_gasless_clock");
        }
        ...
        if (op !== undefined) {
            ...
            if (op.approval !== null)
                times.push(op.approval.approvedAt);
            if (op.dispatchStartedAt !== null)
                times.push(op.dispatchStartedAt);
            if (op.providerObservation !== null)
                times.push(op.providerObservation.observedAt);
            if (op.observation !== null)
                times.push(op.observation.observedAt);
            if (op.settlement !== null)
                times.push(op.settlement.observedAt);
        }
        if (now < this.previous || times.some(at => Date.parse(mmIso(at)) > now))
            mmFail("mm_gasless_clock");
        this.previous = now;
        return now;
    }
    fresh(observedAt, op) {
        ...
        if (now - Date.parse(observedAt) > MM_OBSERVATION_MAX_AGE_MS)
