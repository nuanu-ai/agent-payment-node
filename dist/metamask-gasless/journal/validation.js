...
import { mmImmutable, mmMutable, MM_HISTORY_LIMIT, MM_OPERATION_KIND, MM_OPERATION_VERSION, MM_TERMINAL, mmRequestHash } from "../operation-model.js";
...
function step(previous, next) {
    if (!EDGES[previous.state].includes(next.state) || time(next.at) < time(previous.at))
        corrupt();
    if (previous.approval !== null && !mmSame(previous.approval, next.approval))
        corrupt();
    if (previous.submissionAttempts > next.submissionAttempts)
        corrupt();
    ...
    if (next.providerObservation?.status === "unavailable" &&
        next.providerObservation.txHash !== (previous.providerObservation?.txHash ?? null))
        corrupt();
    ...
    if (after < before || !mmSame(previous.cursor.startBlock, next.cursor.startBlock))
        corrupt();
    if (after === before && !mmSame(previous.cursor, next.cursor))
        corrupt();
    if (previous.cursor.previousEndBlock !== null && next.cursor.previousEndBlock !== null) {
        ...
        if (n < p || (n === p && !mmSame(previous.cursor.previousEndBlock, next.cursor.previousEndBlock)))
            corrupt();
    }
    if (previous.state === "failed_effects_pending" && next.state !== "failed_effects_pending" && next.state !== "completed")
        corrupt();
    if (previous.state !== "failed_effects_pending" && next.state === "failed_effects_pending" &&
        next.observation?.phase !== "reverted")
        corrupt();
    if (previous.state === "failed_effects_pending" && next.state === "failed_effects_pending" &&
...
function validate(value) {
    ...
    if (intent.profile.length === 0 || profileHash !== sha256(`profile\0${intent.profile}`) || createdAt !== intent.preparedAt)
        corrupt();
    ...
    if (fingerprint !== hashObject(mmImmutable(immutable)))
        corrupt();
    ...
    if (requestHash !== expectedRequestHash || !Array.isArray(r.transitions) || r.transitions.length < 1 ||
        r.transitions.length > MM_HISTORY_LIMIT)
        corrupt();
    const transitions = [];
    for (const raw of r.transitions) {
        ...
        if (prior === undefined) {
            ...
            if (current.previousHash !== prior.transitionHash)
                corrupt();
            step(prior, current);
        }
        if (current.approval !== null && time(current.approval.approvedAt) < time(createdAt))
            corrupt();
        if (current.dispatchStartedAt !== null) {
            for (const observedAt of [current.providerObservation?.observedAt, current.observation?.observedAt,
                current.settlement?.observedAt])
                if (observedAt !== undefined && time(observedAt) < time(current.dispatchStartedAt))
                    corrupt();
        }
    ...
    if (last.at !== updatedAt || !mmSame(mmMutable(last), currentMutable) ||
        r.terminal !== MM_TERMINAL.includes(currentMutable.state))
        corrupt();
    const operation = { ...immutable, fingerprint, updatedAt, terminal: r.terminal,
...
export function validateMetaMaskGaslessOperation(value) {
    try {
...
export function validateMetaMaskGaslessContinuity(previous, next) {
    ...
    if (before.terminal || before.fingerprint !== after.fingerprint ||
        ...
        !mmSame(before.transitions, after.transitions.slice(0, -1)))
        corrupt();
}
