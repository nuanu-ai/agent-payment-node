...
import { mmImmutable, mmMutable, MM_DISPATCH_RESERVE_BYTES, MM_DISPATCH_RESERVE_TRANSITIONS, MM_FILE_LIMIT, MM_HISTORY_LIMIT, MM_OPERATION_KIND, MM_OPERATION_VERSION, MM_TERMINAL } from "../operation-model.js";
...
function capacity() { return mmFail("mm_gasless_record_capacity"); }
function corrupt() { return mmFail("mm_gasless_state_corrupt"); }
function seal(value) {
    return { ...value, integrityHash: hashObject(value) };
}
function assertFileLimit(operation) {
    if (Buffer.byteLength(canonicalJson(operation), "utf8") + 1 > MM_FILE_LIMIT)
        capacity();
}
export function newMetaMaskGaslessOperation(identityInput, intent) {
    ...
    const mutable = { state: "awaiting_approval", approval: null, submissionAttempts: 0,
...
export function advanceMetaMaskGaslessOperation(operation, patch, atInput) {
    const current = validateMetaMaskGaslessOperation(operation), at = mmIso(atInput);
    if (Object.keys(patch).some(key => !PATCH_KEYS.includes(key)))
        corrupt();
    const nextMutable = { ...mmMutable(current), ...patch };
    if (mmSame(mmMutable(current), nextMutable))
        return current;
    if (current.terminal)
        corrupt();
...
export function metaMaskGaslessAtTransition(operation, index) {
    const current = validateMetaMaskGaslessOperation(operation), entry = current.transitions[index];
    if (entry === undefined || !Number.isSafeInteger(index) || index < 0)
        corrupt();
...
export function assertMetaMaskGaslessDispatchCapacity(operation) {
    ...
    if (MM_HISTORY_LIMIT - current.transitions.length < MM_DISPATCH_RESERVE_TRANSITIONS ||
        Buffer.byteLength(canonicalJson(current), "utf8") + 1 + MM_DISPATCH_RESERVE_BYTES > MM_FILE_LIMIT)
        capacity();
}
/** Conservative pre-read gate; the exact resulting transition is still checked before persistence. */
export function assertMetaMaskGaslessObservationCapacity(operation) {
    const current = validateMetaMaskGaslessOperation(operation);
    if (current.submissionAttempts !== 1 || current.dispatchStartedAt === null || current.terminal)
        corrupt();
    if (current.transitions.length >= MM_HISTORY_LIMIT ||
        Buffer.byteLength(canonicalJson(current), "utf8") + 1 + MM_DISPATCH_RESERVE_BYTES > MM_FILE_LIMIT)
        capacity();
