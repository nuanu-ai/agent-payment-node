...
import { HelperNetworkPolicy, networkContextEmpty, networkContextQuote, networkContextRequest } from "./network.js";
import { readPrivateState } from "./private-state.js";
import { helperRequest, MM_HELPER_VERSION } from "./protocol.js";
...
export async function executeHelperRequest(input, options = {}) {
    let request;
    try {
        request = helperRequest(input);
    }
    catch (error) {
        return { version: MM_HELPER_VERSION, ok: false, failure: mmClassify(error, "mm_gasless_input") };
    }
    ...
    try {
        ...
        if (request.mode === "buildUnsigned")
            policy = new HelperNetworkPolicy(networkContextEmpty("buildUnsigned"), options.exchange);
        else if (request.mode === "inspect")
            policy = new HelperNetworkPolicy(networkContextEmpty("inspect"), options.exchange);
        else {
            ...
            if (request.mode === "submit")
                policy.primeSubmitClock(firstObservedAt);
        ...
        if (fixedFailure)
            return { version: MM_HELPER_VERSION, ok: false, failure: mmFailure(fixedFailure) };
        ...
        restoreConsole(consoleMethods);
    }
}
async function inspect(home, expected, now) {
    ...
    if (hydrated.writes() !== 0 || hydrated.session.walletMode !== "server-wallet" ||
        !isPlainRecord(hydrated.walletState.selectedWallet))
...
async function quote(input, state, policy) {
    ...
    let cacheWrites = 0;
    let cache = null;
    const registry = new sdk.NetworkRegistry({ env: "prod", storage: { read: () => cache,
            write: (value) => { if (++cacheWrites !== 1)
                mmFail("mm_gasless_provider_unavailable"); cache = structuredClone(value); } },
        logger: logger(), getAuthTokenFn: () => state.token });
    const snapshot = await registry.getSnapshot();
    if (cacheWrites !== 1 || cache === null)
    ...
    if (rpcChain.rpcTarget !== input.rpcUrl)
        mmFail("mm_gasless_rpc_binding");
    const walletState = structuredClone(hydrated.walletState);
    walletState.customEvmChains = [...walletState.customEvmChains.filter((row) => !isPlainRecord(row) || row.chainId !== input.chainId), rpcChain];
    if (walletState.customEvmChains.filter((row) => isPlainRecord(row) && row.chainId === input.chainId).length !== 1)
        mmFail("mm_gasless_rpc_binding");
    let resolverCalls = 0;
    const service = sdk.createWalletServiceFromSession({ session: hydrated.session, walletState: walletState,
        ...
        logger: logger() });
    const relay = { host: PROXY_URL, env: "prod", getHeaders: () => ({ Authorization: `Bearer ${state.token}` }) };
    if (!await service.isGaslessRelaySupported({ chainId: input.chainId, relay }))
        mmFail("mm_gasless_capability_unavailable");
    const raw = await service.quoteGaslessTransfer({ state: walletState, chainId: input.chainId, recipient: input.recipient,
        amount: formatSix(input.netAtomic), token: input.token, gasToken: input.token, relay });
    if (resolverCalls !== 0 || hydrated.writes() !== 0)
        mmFail("mm_gasless_state_security");
...
async function buildUnsigned(input) {
    ...
    const result = { unsignedDelegation,
        delegationHash: controller.getDelegationHashOffchain(unsignedDelegation),
        signingDigest: hashTypedData(prepared.typedData), relayTo: mmAddress(prepared.relayTo), mode: prepared.modes[0] };
...
async function submit(intent, first, home, now, policy, firstObservedAt) {
    ...
    const { keyring } = await keyrings.createKeyringController({ mode: keyrings.KEYRING_KIND.SERVER,
        input: { baseUrl: SERVER_URL, authToken: first.token, projectId: first.projectId,
            wallets: firstHydrated.walletState.remoteWallets }, adapters: [(deps) => new evm.EvmServerAdapter(deps)], logLevel: "silent" });
    const adapter = evm.evmServerAdapter(keyring);
    const secondObservedAt = now();
    if (secondObservedAt.getTime() < firstObservedAt.getTime())
    ...
    const wireDelegation = evm.unsignedDelegationToWire(intent.unsignedDelegation);
    ...
    const status = await adapter.submitRelayTransaction({ from: intent.binding.address, chainId: intent.request.chainId,
        executions: wireExecutions, delegation: wireDelegation }, { chainId: intent.request.chainId, origin: "apn", requestId: intent.requestId });
...
async function observe(intent, state, policy, now) {
    ...
    const { keyring } = await keyrings.createKeyringController({ mode: keyrings.KEYRING_KIND.SERVER,
        input: { baseUrl: SERVER_URL, authToken: state.token, projectId: state.projectId,
            wallets: hydrated.walletState.remoteWallets }, adapters: [(deps) => new evm.EvmServerAdapter(deps)], logLevel: "silent" });
    try {
        ...
        if (policy.didObserveNotFound())
            return unavailable(intent, now());
        throw error;
    }
}
function validateSdkQuote(value, input, owner) {
    if (!isPlainRecord(value) || typeof value.amount !== "bigint" || value.amount !== mmUint(input.netAtomic, true) ||
        ...
        !Array.isArray(value.executions) || value.executions.length !== 2)
        mmFail("mm_gasless_quote_invalid");
    const feeRecipient = mmAddress(value.feeRecipient, "mm_gasless_quote_invalid");
    if (feeRecipient === owner || feeRecipient === input.recipient || feeRecipient === input.token)
        mmFail("mm_gasless_quote_invalid");
    const amounts = [value.amount, value.feeAmount], recipients = [input.recipient, feeRecipient];
    const executions = value.executions.map((raw, index) => {
        if (!isPlainRecord(raw) || typeof raw.value !== "bigint")
            mmFail("mm_gasless_quote_invalid");
        ...
        if (target !== input.token || raw.value !== 0n || callData !== expected)
            mmFail("mm_gasless_quote_invalid");
...
function providerObservation(value, intent, now, reason) {
...
function formatSix(value) {
    const n = mmUint(value, true), fraction = (n % 1000000n).toString().padStart(6, "0").replace(/0+$/u, "");
...
function fallback(mode) {
    return mode === "submit" ? "mm_gasless_submit_unknown" : mode === "buildUnsigned" ?
        "mm_gasless_internal" : "mm_gasless_provider_unavailable";
}
function logger() {
    return { debug: (_message) => undefined, info: (_message) => undefined,
        warn: (_message) => undefined, error: (_message) => undefined, reset: () => undefined,
        enable: (_methods) => undefined };
}
function saveConsole() {
    const entries = [];
    for (const key of ["log", "error", "warn", "info", "debug"]) {
        entries.push([key, console[key]]);
        console[key] = (() => undefined);
    }
    return entries;
}
function restoreConsole(entries) { for (const [key, value] of entries)
    console[key] = value; }
