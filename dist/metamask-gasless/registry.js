...
import { MM_CHAINS } from "./model.js";
...
export const MM_RPC_ENV = Object.freeze({
...
export const MM_SENTINEL_SLUG = Object.freeze({
...
if (MM_DEPLOYMENTS.length !== MM_CHAINS.length || MM_DEPLOYMENTS.some((entry, index) => entry.row.chainId !== MM_CHAINS[index] || entry.deploymentEvidenceHash !== hashObject({
...
freeze(MM_DEPLOYMENT_SOURCE_FILES);
freeze(MM_DEPLOYMENTS);
/** Static source-correlated input, never current account or mainnet payment acceptance. */
export function mmRegistry(chainId) {
