...
export function mmPrivateHash(kind, value, referenceKind) {
    if (typeof value !== "string" || value.length === 0 || value.length > 1024 ||
        (kind === "wallet-reference") !== (referenceKind !== undefined))
...
export function mmWalletIdentityHash(address) {
    return mmPrivateHash("wallet-id", mmAddress(address, "mm_gasless_identity"));
}
export function mmBinding(value, reason = "mm_gasless_identity") {
    ...
    if (b.providerId !== "metamask-agent-wallet" || b.namespace !== "eip155" || b.mode !== "server" || b.environment !== "prod" ||
        !Number.isSafeInteger(b.revision) || Number(b.revision) < 1 || mmCanonicalAddress(b.address, reason) === MM_ZERO_ADDRESS)
        mmFail(reason);
    for (const key of ["accountBindingHash", "capabilityHash", "projectHash", "walletReferenceHash", "walletIdHash"])
        mmHash(b[key], reason);
    if (b.walletIdHash !== mmWalletIdentityHash(b.address))
        mmFail(reason);
    return b;
}
export function mmAssertProfileBinding(binding, expected) {
    mmBinding(binding);
    if (binding.address !== expected.address || binding.accountBindingHash !== expected.accountBindingHash ||
        binding.capabilityHash !== expected.capabilityHash || binding.revision !== expected.revision)
        mmFail("mm_gasless_binding_changed");
}
export function mmAssertSameBinding(current, expected) {
    const binding = mmBinding(current);
    if (!mmSame(binding, expected))
        mmFail("mm_gasless_binding_changed");
    return binding;
}
