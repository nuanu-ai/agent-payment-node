...
function validateErc7710UnusedExpiryEvidence(value, operation) {
    const evidence = exactRecord(value, [
    ...
    if (evidence.schemaVersion !== "apn.x402.erc7710-unused-expiry-evidence.v1" || evidence.network !== CHAIN_CAIP2 ||
    ...
    uint(expiry.number);
    bytes32(expiry.hash);
    uint(expiry.timestamp);
    timestamp(expiry.observedAt);
    const finalized = exactRecord(evidence.finalizedHead, ["number", "hash", "timestamp", "observedAt"]);
    uint(finalized.number);
    bytes32(finalized.hash);
    uint(finalized.timestamp);
    timestamp(finalized.observedAt);
    if (BigInt(start.number) > BigInt(expiry.number) ||
        BigInt(expiry.number) > BigInt(finalized.number) ||
        BigInt(expiry.timestamp) < BigInt(evidence.effectiveExpiryUnix) ||
        BigInt(finalized.timestamp) < BigInt(evidence.effectiveExpiryUnix) ||
    ...
    if (scan.fromBlock !== start.number || scan.toBlock !== expiry.number || scan.matchingTransferCount !== "0")
    ...
    if (binding.method !== "erc7710")
        stateCorrupt("x402 ERC-7710 unused-expiry method binding is invalid.");
    hash(binding.operationBindingHash);
    hash(binding.offerHash);
    address(binding.delegationManager);
    address(binding.delegator);
    ...
    if (operation !== undefined) {
        ...
        const offer = record(operation.selectedOffer);
        if (evidence.effectiveExpiryUnix !== delegated.effectiveExpiryUnix || start.number !== prepared.number ||
