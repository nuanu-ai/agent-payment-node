...
export class HttpsBaseRpc {
    ...
    async assertX402Chain(chainId) {
        if (chainId !== this.x402ChainId)
            throw new ApnError("APN_CHAIN_MISMATCH", "x402 RPC binding belongs to another network.");
    ...
    async getX402AuthorizationUsedLogs(input) {
        ...
        for (const log of logs) {
            if (log.address.toLowerCase() !== x402Network(this.x402ChainId).token.toLowerCase() || log.topics.length !== 3 ||
    ...
    async getX402TransferLogs(input) {
        ...
        const to = parseAtomic(input.toBlock);
        if (from > to || to - from + 1n > 2048n)
            throw new ApnError("APN_RPC_PROTOCOL", "RPC Transfer range exceeds the fixed bound.");
...
export function parseRpcResultEnvelope(raw, id) {
...
export function parseRpcBatchResultEnvelope(raw, ids) {
    ...
    let parsed;
    try {
        parsed = parseJsonWithDuplicateRejection(raw);
    }
    catch {
...
export function parseRpcLogEnvelope(raw, id) {
    const message = strictRpcRecord(raw);
    ...
    if (exactKeys(message, ["jsonrpc", "id", "result"]))
...
function strictRpcRecord(raw) {
    try {
        return record(parseJsonWithDuplicateRejection(raw), "JSON-RPC response");
    }
    catch (error) {
