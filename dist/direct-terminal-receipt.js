...
export function validateEvmTransferEvidence(value) {
    ...
    if (!/^0x[0-9a-f]{64}$/u.test(evidence.blockHash) || evidence.blockHash === `0x${"0".repeat(64)}` ||
        typeof evidence.transactionVerified !== "boolean" || typeof evidence.tokenBalanceDeltasVerified !== "boolean")
        corrupt();
    for (const delta of [evidence.senderDeltaAtomic, evidence.recipientDeltaAtomic]) {
        if (delta !== undefined && (typeof delta !== "string" || delta.length > 79 || !/^(?:0|-?[1-9][0-9]*)$/u.test(delta)))
            corrupt();
    }
...
export function assertDirectTerminalReceiptAuthority(operation, receipt) {
    if (operation.evm === undefined)
        return assertProviderTerminalReceiptAuthority(operation, receipt);
    if (receipt === null || !receipt.terminal || !operation.terminal || receipt.operationId !== operation.operationId ||
        ...
        canonicalJson(receipt.evm) !== canonicalJson(operation.evm))
        corrupt();
    if (operation.state === "completed" || operation.state === "failed_confirmed_revert") {
        ...
        if (directEvmRequiresSafeHead(operation.chainId) ? !hasSafeEvmInclusion(evidence, receipt.blockNumberAtomic) : evidence.safeBlockNumberAtomic !== undefined)
            corrupt();
        if (operation.state === "failed_confirmed_revert") {
            ...
            if (operation.reason !== "confirmed_exact_native_transfer" || operation.proofClass !== "included_native_transaction_and_receipt")
                corrupt();
        }
            ...
            receipt.exactTransferLog !== true || evidence.tokenBalanceDeltasVerified !== true || evidence.senderDeltaAtomic !== operation.amountAtomic || evidence.recipientDeltaAtomic !== operation.amountAtomic)
            corrupt();
    }
    else if (receipt.blockNumberAtomic !== undefined || receipt.evmEvidence !== undefined ||
        !["failed_before_effect", "failed_proven_superseded"].includes(operation.state))
        corrupt();
}
function corrupt() { throw new ApnError("APN_STATE_CORRUPT", "Generic direct receipt does not prove its exact operation state."); }
//# sourceMappingURL=direct-terminal-receipt.js.map