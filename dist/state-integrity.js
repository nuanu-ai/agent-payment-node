...
export function validateOperation(value) {
    ...
    else if (operation.allowlist !== undefined || operation.allowlistLease !== undefined)
        stateCorrupt("Only an explicit EVM asset operation carries a direct allowlist binding.");
    if (operation.providerDirect === undefined)
        validateLocalDirect(operation);
    else
        validateProviderDirect(operation, operation.providerDirect);
    return operation;
}
...
function validateProviderDirect(operation, binding) {
    if (operation.transactionData !== undefined || operation.economics !== undefined ||
    ...
    else if (hasCoinbaseMetadata)
        stateCorrupt("Non-Coinbase operation carries Coinbase gasless metadata.");
    ...
    const allowed = {
        ...
        ambiguous_effect: ["provider_pending", "provider_acknowledged", "completed", "failed_provider_rejected", "failed_confirmed_revert", "abandoned_unknown",
            ...(binding.coinbaseGasless === undefined ? [] : ["ambiguous_effect"])],
        abandoned_unknown: [],
        completed: [],
    ...
    for (let index = 0; index < operation.transitions.length; index += 1) {
        ...
        if (current === undefined)
            stateCorrupt("Provider direct transition is missing.");
        if (index === 0) {
            if (current.state !== "awaiting_approval")
                stateCorrupt("Provider direct genesis state is invalid.");
            continue;
        }
...
function validateCoinbaseGasless(operation, value) {
    ...
    if (cursor.previousEndBlock === null) {
        if (next !== anchor + 1n)
            stateCorrupt("Coinbase gasless initial cursor does not follow its safe anchor.");
    }
    else {
        validateCoinbaseBlock(cursor.previousEndBlock);
        const previousEnd = BigInt(cursor.previousEndBlock.numberAtomic);
        if (previousEnd < anchor + 1n || next !== previousEnd + 1n)
            stateCorrupt("Coinbase gasless cursor is discontinuous.");
    }
    if (operation.coinbaseGaslessLocator !== undefined) {
        ...
        if (!isPlainRecord(locator) || !exactKeys(locator, ["schemaVersion", "hash", "provenance"]) ||
            ...
            !["awal_success_transaction_hash_field", "awal_error_text_hint"].includes(locator.provenance))
            stateCorrupt("Coinbase gasless locator is invalid.");
    }
    if (operation.coinbaseGaslessSettlement !== undefined) {
        ...
        if (!isPlainRecord(settlement) || !exactKeys(settlement, ["schemaVersion", "userOperationHash", "transactionHash", "nonceAtomic",
            ...
            operation.state !== "completed" || !operation.terminal)
            stateCorrupt("Coinbase gasless settlement is invalid.");
        parseAtomic(settlement.nonceAtomic);
        validateCoinbaseBlock(settlement.block);
        validateCoinbaseBlock(settlement.safeBlock);
        ...
        if (evidenceHash !== hashObject(evidenceBody))
            stateCorrupt("Coinbase gasless settlement evidence hash is invalid.");
    }
    else if (operation.state === "completed")
        stateCorrupt("Coinbase gasless completion lacks settlement evidence.");
}
function validateCoinbaseBlock(value) {
    if (!isPlainRecord(value) || !exactKeys(value, ["numberAtomic", "hash", "timestampAtomic"]) ||
        !/^0x[0-9a-f]{64}$/u.test(value.hash))
        stateCorrupt("Coinbase gasless block is invalid.");
    parseAtomic(value.numberAtomic);
    parseAtomic(value.timestampAtomic);
}
