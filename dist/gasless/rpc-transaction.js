...
export async function verifyGaslessOuterTransaction(raw, chainId, expectedHash) {
    ...
    if (type > 4n || rpcHex(raw.hash, 32, 32) !== expectedHash)
        fail("gasless_outer_chain_or_hash");
    ...
    if (type === 0n) {
        if (v < 35n)
            fail("gasless_outer_unprotected_legacy");
        y = Number((v - 35n) & 1n);
        const encodedChain = (v - 35n - BigInt(y)) / 2n;
        if (encodedChain !== BigInt(chainId))
            fail("gasless_outer_chain_or_hash");
        if (raw.chainId !== undefined && rpcQuantity(raw.chainId) !== BigInt(chainId))
            fail("gasless_outer_chain_or_hash");
    }
    else {
        if (raw.chainId === undefined || rpcQuantity(raw.chainId) !== BigInt(chainId))
        ...
        if (type === 2n)
            ...
            serializable = { ...feeFields, type: "eip7702", authorizationList: raw.authorizationList.map((item) => {
                    ...
                    if (parity !== 0 && parity !== 1)
                        fail("gasless_outer_authorization_parity");
    ...
    try {
        ...
        return fail("gasless_outer_signature_reconstruction");
    }
...
function signatureScalar(value) {
    ...
    if (typeof value === "string" && value.length === 66)
        return rpcHex(value, 32, 32);
    return `0x${rpcQuantity(value).toString(16).padStart(64, "0")}`;
}
function assertSignature(r, s) {
    if (BigInt(r) === 0n || BigInt(s) === 0n || BigInt(s) > HALF_CURVE_ORDER)
        fail("gasless_outer_signature");
}
function safeNumber(value) {
    if (value > BigInt(Number.MAX_SAFE_INTEGER))
        fail("gasless_outer_integer_bound");
    return Number(value);
}
function parseAccessList(value) {
    if (!Array.isArray(value) || value.length > 256)
        fail("gasless_outer_access_list");
    return value.map((item) => {
        const row = rpcRecord(item);
        if (!Array.isArray(row.storageKeys) || row.storageKeys.length > 256)
            fail("gasless_outer_storage_keys");
        return { address: rpcAddress(row.address), storageKeys: row.storageKeys.map((key) => rpcHex(key, 32, 32)) };
    });
}
function fail(reason) { return gaslessFailure("APN_RPC_PROTOCOL", reason); }
