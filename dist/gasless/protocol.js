...
import { GASLESS_ZERO_ADDRESS, gaslessAddress, gaslessChain, gaslessExact, gaslessFailure, gaslessHex, gaslessUint } from "./validation.js";
const TOPICS = {
...
export function gaslessAccounting(intent, userOperationHash, receipt) {
    validateReceipt(intent, userOperationHash, receipt);
    const userOperations = events(receipt.logs, intent.entryPoint, TOPICS.userOperation, GASLESS_ENTRYPOINT_ABI, "UserOperationEvent", 4, 128);
    ...
    if (ownerOperations.length !== 1 || hashOperations.length !== 1 || ownerOperations[0] !== hashOperations[0])
        receiptFailure();
    const operation = ownerOperations[0];
    if (operation.value.paymaster !== intent.paymaster ||
        operation.value.nonce.toString() !== intent.initialSnapshot.entryPointNonceAtomic)
        receiptFailure();
    const transfers = events(receipt.logs, intent.token, TOPICS.transfer, GASLESS_TOKEN_ABI, "Transfer", 3, 32);
    const approvals = events(receipt.logs, intent.token, TOPICS.approval, GASLESS_TOKEN_ABI, "Approval", 3, 32);
    ...
    const sponsored = events(receipt.logs, intent.paymaster, TOPICS.sponsored, GASLESS_PAYMASTER_ABI, "UserOperationSponsored", 3, 128);
    ...
    const postOp = events(receipt.logs, intent.entryPoint, TOPICS.postOpRevert, GASLESS_ENTRYPOINT_ABI, "PostOpRevertReason", 3);
    const prefundLow = events(receipt.logs, intent.entryPoint, TOPICS.prefundTooLow, GASLESS_ENTRYPOINT_ABI, "UserOperationPrefundTooLow", 3, 32);
    ...
    if (matchingSponsored.length === 1 && ownerSponsored.length === 1 && matchingSponsored[0] === ownerSponsored[0]) {
        ...
        if (event.token !== intent.token || event.nativeTokenPrice <= 0n || event.actualTokenNeeded <= 0n ||
            event.feeTokenAmount > event.actualTokenNeeded)
            receiptFailure();
        const refund = movements.refund?.value.value ?? 0n, fee = prefund - refund;
        if (refund > prefund || fee !== minimum(prefund, event.actualTokenNeeded))
            receiptFailure();
        if (operation.value.success) {
            ordered(approvalOrder.permit, movements.prefund, movements.delivery, approvalOrder.cleanup, ...optional(movements.refund), sponsoredEvent, operation);
        }
        else {
            ordered(approvalOrder.permit, movements.prefund, ...optional(movements.refund), sponsoredEvent, operation);
        }
        return accounting(intent, operation.value.success, "sponsored", prefund, refund, fee, operation.value.success ? BigInt(intent.recipientAtomic) : 0n, receipt.logs);
    }
    if (matchingSponsored.length !== 0 || ownerSponsored.length !== 0 || sponsored.some((event) => event.value.token === intent.token &&
        (event.value.sender === intent.owner.address || event.value.userOpHash === userOperationHash)))
        receiptFailure();
    if (operation.value.success || movements.refund !== undefined || movements.delivery !== undefined)
        receiptFailure();
...
function classifyFailure(postOp, prefundLow) {
    if (postOp.length === 1 && prefundLow.length === 0)
        return { kind: "post_op_reverted", events: postOp };
    if (postOp.length === 0 && prefundLow.length === 1)
        return { kind: "prefund_too_low", events: prefundLow };
...
function tokenMovements(intent, success, transfers) {
    ...
    if (prefunds.length !== 1 || refunds.length > 1 || unexpected.length !== 0 ||
        ...
        (success ? deliveries.length !== 1 || deliveries[0].value.value.toString() !== intent.recipientAtomic : deliveries.length !== 0))
        receiptFailure();
    return { prefund: prefunds[0], ...(refunds[0] === undefined ? {} : { refund: refunds[0] }),
        ...(deliveries[0] === undefined ? {} : { delivery: deliveries[0] }) };
}
function validateApprovals(intent, success, approvals) {
    ...
    if (relevant.length !== expected.length)
        receiptFailure();
    ...
    if (permits.length !== 1 || (success ? cleanup.length !== 1 : cleanup.length !== 0) ||
        expected.length !== permits.length + cleanup.length)
        receiptFailure();
    return { permit: permits[0], ...(cleanup[0] === undefined ? {} : { cleanup: cleanup[0] }) };
}
function matchingFrames(intent, hash, frames) {
    ...
    if (owner.length !== matching.length || owner.some((frame) => !matching.includes(frame)))
        receiptFailure();
    for (const frame of matching)
        if (frame.value.nonce.toString() !== intent.initialSnapshot.entryPointNonceAtomic)
            receiptFailure();
    return matching;
}
function validateReceipt(intent, userOperationHash, receipt) {
    const keys = ["chainId", "transactionHash", "block", "logs"];
    if (!isPlainRecord(receipt) || !exactKeys(receipt, keys) || gaslessChain(receipt.chainId, "APN_RPC_PROTOCOL") !== intent.request.chainId ||
        ...
        receipt.transactionHash === `0x${"0".repeat(64)}` || !Array.isArray(receipt.logs) || receipt.logs.length > 512)
        receiptFailure();
    ...
    if (gaslessHex(block.hash, 32, 32, "APN_RPC_PROTOCOL") !== block.hash || block.hash === `0x${"0".repeat(64)}`)
        receiptFailure();
    let previousIndex = -1n;
    for (const value of receipt.logs) {
        const log = gaslessExact(value, ["address", "topics", "data", "logIndexAtomic"], "APN_RPC_PROTOCOL");
        if (gaslessAddress(log.address, "APN_RPC_PROTOCOL") !== log.address || !Array.isArray(log.topics) || log.topics.length > 4)
            receiptFailure();
        for (const topic of log.topics)
            if (gaslessHex(topic, 32, 32, "APN_RPC_PROTOCOL") !== topic)
                receiptFailure();
        if (gaslessHex(log.data, 12 * 1024, undefined, "APN_RPC_PROTOCOL") !== log.data)
            receiptFailure();
    ...
    for (const address of [intent.owner.address, intent.token, intent.paymaster, intent.entryPoint, intent.delegate,
        ...
        if (address === GASLESS_ZERO_ADDRESS)
            receiptFailure();
}
function events(logs, address, topic, abi, eventName, topicCount, dataBytes) {
    const output = [];
    for (const log of logs) {
        if (log.address !== address || log.topics[0]?.toLowerCase() !== topic)
            continue;
        if (log.topics.length !== topicCount || (dataBytes !== undefined && log.data.length !== 2 + dataBytes * 2))
            receiptFailure();
        try {
            const decoded = decodeEventLog({ abi, eventName: eventName, data: log.data,
...
function ordered(...eventsToCheck) {
    for (let index = 1; index < eventsToCheck.length; index += 1) {
        if (eventsToCheck[index - 1].index >= eventsToCheck[index].index)
            receiptFailure();
    }
}
function optional(value) { return value === undefined ? [] : [value]; }
function accounting(intent, success, branch, prefund, refund, fee, delivered, logs) {
    const gross = BigInt(intent.request.grossAtomic);
    if (fee < 0n || fee > BigInt(intent.feeCapAtomic) || delivered + fee > gross)
        receiptFailure();
...
function receiptFailure() { return gaslessFailure("APN_RPC_PROTOCOL", "gasless_receipt_unresolved"); }
