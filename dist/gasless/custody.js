...
import { gaslessAuthorizationRequest, gaslessPermitTypedData, gaslessUserOperation, gaslessUserOperationTypedData, } from "./wire.js";
export class LocalGaslessCustody {
    state;
    now;
    wallets;
    effects;
    ...
    async seal(operation, role, owner, bootstrap, fees) {
        ...
        return await this.state.withLocks([`custody:${operation.profileHash}`], async () => {
            ...
            if (wallet === null)
                unavailable("gasless_encrypted_wallet_missing");
            try {
    ...
    async sealBootstrap(operation, account) {
    ...
    async sealUserOperation(operation, account, suppliedBootstrap, fees) {
        ...
        if (!gaslessSame(original, suppliedBootstrap))
            corrupt("gasless_original_bootstrap_mismatch");
...
function assertRemaining(operation, now) {
