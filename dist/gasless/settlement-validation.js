...
export function validateGaslessSettlement(i, userOperationHash, value) {
    if (!settlementSchema.safeParse(value).success)
        gaslessFailure("APN_STATE_CORRUPT", "gasless_settlement_shape");
    const p = value, a = p.accounting;
    if (p.chainId !== i.request.chainId || p.userOperationHash !== userOperationHash || p.outerSender === i.owner.address ||
        ...
        !consumed(i, p.effectAccount) || !consumed(i, p.safeAccount))
        gaslessFailure("APN_STATE_CORRUPT", "gasless_settlement_binding");
    ...
    if (prefund > BigInt(i.feeCapAtomic) || refund > prefund || fee !== prefund - refund ||
        ...
        (a.branch !== "sponsored" && (a.success || refund !== 0n)))
        gaslessFailure("APN_STATE_CORRUPT", "gasless_settlement_economics");
    return p;
}
function consumed(i, a) {
...
export function gaslessSettlementEffect(p) {
    const { safeBlock: _safe, safeAccount: _account, ...effect } = p;
    return effect;
}
export function assertGaslessSettlementContinuation(previous, next) {
...
export function gaslessSettlementHash(p) { return hashObject(p); }
//# sourceMappingURL=settlement-validation.js.map