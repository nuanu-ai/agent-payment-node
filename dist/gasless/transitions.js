...
import { GASLESS_TERMINAL, gaslessIntentBinding, gaslessSnapshot, newGaslessEffect, sealGaslessOperation } from "./operation-model.js";
...
export function newGaslessOperation(input) {
    const immutable = { schemaVersion: "apn.gasless-operation.v1", kind: "gasless_transfer", ...input };
    const fingerprint = hashObject(gaslessIntentBinding(immutable));
    const mutable = { state: "awaiting_approval", approval: null,
...
export function transitionGasless(op, patch, at) {
    validateGaslessOperation(op);
    const next = { ...op, ...patch }, mutable = gaslessSnapshot(next);
    if (gaslessSame(gaslessSnapshot(op), mutable))
        return op;
    ...
    if (Buffer.byteLength(canonicalJson(result), "utf8") + 1 > 1024 * 1024)
        gaslessFailure("APN_OPERATION_BLOCKED", "gasless_record_capacity");
    validateGaslessContinuity(op, result);
    return result;
}
export function gaslessAtTransition(op, index) {
