...
export function validateGaslessPermissionInvalidation(intent, bootstrapMaterialHash, value) {
    if (!permissionInvalidationSchema.safeParse(value).success)
        invalid();
    const p = value, initial = intent.initialSnapshot;
    const final = p.userOperationMaterialHash !== undefined;
    if (final !== (p.userOperationHash !== undefined))
        invalid();
    if ([p.safeBlock, p.headBlock].some((b) => /^0x0{64}$/u.test(b.hash)))
        invalid();
    if (p.chainId !== intent.request.chainId || p.intentHash !== hashObject(intent) ||
        ...
        (p.headBlock.numberAtomic === p.safeBlock.numberAtomic &&
            (!gaslessSame(p.headBlock, p.safeBlock) || !gaslessSame(p.headAccount, p.safeAccount))))
        invalid();
    for (const account of [p.safeAccount, p.headAccount])
        if (!invalidated(intent, account, final))
            invalid();
    if (BigInt(p.safeAccount.permitNonceAtomic) > BigInt(p.headAccount.permitNonceAtomic) ||
        ...
        p.safeAccount.pendingEoaNonceAtomic !== p.safeAccount.eoaNonceAtomic)
        invalid();
    return p;
}
export function assertGaslessPermissionClosure(op, s) {
    ...
    if (o?.status !== "permissions_invalidated" || o.permissionInvalidation === undefined ||
        ...
        o.evidenceHash !== hashObject(o.permissionInvalidation))
        invalid();
    ...
    if (p.userOperationMaterialHash === undefined) {
        if (u.signingAttempts !== 0 || u.disclosureAttempts !== 0 || u.submissionAttempts !== 0 ||
            ...
            o.reason !== GASLESS_PERMISSIONS_INVALIDATED)
            invalid();
    }
        ...
        BigInt(o.cursor.nextBlockAtomic) !== BigInt(p.safeBlock.numberAtomic) + 1n)
        invalid();
    return p;
}
function invalidated(intent, account, final) {
...
function invalid() { return gaslessFailure("APN_STATE_CORRUPT", "gasless_permission_invalidation_binding"); }
//# sourceMappingURL=permission-invalidation.js.map