...
export const GASLESS_DEPLOYMENTS = deepFreeze(GASLESS_REGISTRY_DATA.map((r) => {
    const token = getAddress(r.token), tokenDomain = { name: r.tokenName, version: "2",
    ...
    return { chainId: r.chainId, network: r.label, rpcEnv: `APN_${r.network}_RPC_URL`,
        ...
        code: [
            ...
            { kind: "storage", address: token, data: TOKEN_SLOT, expected: word(r.tokenImplementation) },
            { kind: "storage", address: PAYMASTER, data: PAYMASTER_SLOT, expected: word(r.paymasterImplementation) },
