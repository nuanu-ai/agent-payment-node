...
const word = (value) => `0x${value.replace(/^0x/u, "").toLowerCase().padStart(64, "0")}`;
const call = (address, signature, expected) => ({
    kind: "call", address, data: toFunctionSelector(signature), expected,
});
const storage = (address, slot, expected) => ({ kind: "storage", address, data: slot, expected });
function gaslessAssetRow(chainId, t) {
    const token = getAddress(t.address), paymaster = getAddress(t.paymaster.address);
    const domain = { name: t.name, version: t.permitDomainVersion, chainId,
...
export const GASLESS_DEPLOYMENTS = deepFreeze(GASLESS_REGISTRY_DATA.map((r) => {
    ...
    return { chainId, network: r.label, rpcEnv: `APN_${r.network}_RPC_URL`,
        ...
        code: [
            ...
            ...r.tokens.map((t) => storage(getAddress(t.paymaster.address), t.paymaster.implementationSlot, word(t.paymaster.implementation))),
        ...
        ], evidenceHash: "b05aca1551fafb5734cccc0fe2b51d493782471544d03e63ae267c4b58104972" };
}));
export function gaslessDeployment(chainId) {
    const row = GASLESS_DEPLOYMENTS.find((d) => d.chainId === gaslessChain(chainId));
    if (row === undefined)
        gaslessFailure("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "gasless_chain_unavailable");
    return row;
}
/** The admitted asset a chain/token pair names. No token outside its chain's row is ever addressable. */
export function gaslessAsset(chainId, token) {
    const asset = gaslessDeployment(chainId).assets.find((a) => a.token === token);
    if (asset === undefined)
        gaslessFailure("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "gasless_asset_unavailable");
    return asset;
}
/**
 ...
 */
export function gaslessIntentAsset(intent) {
...
export function gaslessProtocolHash(row) {
