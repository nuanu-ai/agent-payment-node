...
export class GaslessExecution {
    ...
    save;
    observation;
    constructor(state, rpc, custody, now, save) {
        this.state = state;
        this.rpc = rpc;
    ...
    async material(op, role, bootstrap) {
        ...
        if (effect.signingAttempts === 0) {
            ...
            try {
                await this.custody.seal(op, role, op.intent.owner, bootstrap);
            }
    ...
    async guard(op) { await guardGaslessOperation(this.state, this.rpc, op, this.now); }
    async halt(op, error) {
