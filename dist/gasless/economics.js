...
import { GASLESS_MAX_UINT, GASLESS_MAX_UINT120, gaslessAddress, gaslessChain, gaslessExact, gaslessFailure, gaslessHash, gaslessHex, gaslessUint } from "./validation.js";
const GAS_CEILINGS = {
...
export function gaslessGas(snapshot) {
    ...
    if (maximum > GASLESS_MAX_UINT120 || snapshot.maxFeePerGas !== maximum.toString())
        feeFailure();
...
export function gaslessFee(gas, config) {
    ...
    const withPostOp = checkedAdd(requiredPrefund, checkedMultiply(BigInt(configuration.additionalGasCharge), maximum));
...
export function validateGaslessGas(value) {
    if (!isPlainRecord(value) || !exactKeys(value, GAS_FIELDS))
        feeFailure();
    const gas = value;
    const quantities = gasQuantities(gas);
    for (let index = 0; index < quantities.length; index += 1) {
        const amount = quantities[index];
        const ceiling = GAS_CEILINGS[GAS_FIELDS[index]];
        if (amount === 0n || amount > GASLESS_MAX_UINT120 || (ceiling !== undefined && amount > ceiling))
            feeFailure();
    }
    ...
    if (maximum > GASLESS_MAX_UINT120 || priority > GASLESS_MAX_UINT120 || priority > maximum ||
        ...
        quantities[3] < MIN_POST_OP_GAS)
        feeFailure();
    return gas;
}
export function assertGaslessEstimate(intent, estimate) {
    const keys = ["verificationGasLimit", "callGasLimit", "paymasterVerificationGasLimit",
        "paymasterPostOpGasLimit", "preVerificationGas", "responseHash"];
    if (!isPlainRecord(estimate) || !exactKeys(estimate, keys))
        estimateFailure();
    gaslessHash(estimate.responseHash, "APN_PROVIDER_PROTOCOL");
    const offer = validateGaslessGas(intent.gas);
    for (const field of keys.slice(0, 5)) {
        const observed = gaslessUint(estimate[field], true, "APN_PROVIDER_PROTOCOL");
        if (observed > BigInt(offer[field]))
            estimateFailure();
    }
...
export function assertGaslessSnapshot(intent, current) {
    ...
    if (cap > userMaximum || cap > gross - minimum || delivered !== gross - cap ||
        ...
        gas.maxFeePerGas !== initial.maxFeePerGas || gas.maxPriorityFeePerGas !== initial.maxPriorityFeePerGas)
        feeFailure();
    if (initial.chainId !== intent.request.chainId || initial.token !== intent.token ||
        intent.tokenDomain.chainId !== intent.request.chainId || intent.tokenDomain.verifyingContract !== intent.token)
        protocolFailure();
    if (initial.owner !== intent.owner.address || observed.owner !== initial.owner || observed.delegation !== initial.delegation)
        identityFailure();
    if (initial.allowanceAtomic !== "0")
        gaslessFailure("APN_PERMISSION_ALLOWANCE_INSUFFICIENT", "gasless_allowance_drift");
    if (initial.pendingEoaNonceAtomic !== initial.eoaNonceAtomic)
        nonceFailure();
    if (BigInt(initial.balanceAtomic) < gross)
        gaslessFailure("APN_INSUFFICIENT_USDC", "gasless_fee_budget");
    for (const field of ["rpcOrigin", "rpcEndpointHash", "bundlerOrigin", "bundlerEndpointHash"]) {
        if (observed[field] !== initial[field])
            endpointFailure();
    }
    for (const field of ["chainId", "protocolHash", "token"])
        if (observed[field] !== initial[field])
            protocolFailure();
    for (const field of ["permitNonceAtomic", "entryPointNonceAtomic", "eoaNonceAtomic", "pendingEoaNonceAtomic"]) {
        if (observed[field] !== initial[field])
            nonceFailure();
    }
    if (observed.allowanceAtomic !== "0")
        gaslessFailure("APN_PERMISSION_ALLOWANCE_INSUFFICIENT", "gasless_allowance_drift");
...
function validateSnapshot(value) {
    ...
    if (gaslessAddress(value.owner, "APN_PROVIDER_PROTOCOL") !== value.owner ||
        gaslessAddress(value.token, "APN_PROVIDER_PROTOCOL") !== value.token)
        identityFailure();
    if (value.delegation !== "empty" && value.delegation !== "expected")
        identityFailure();
    ...
    if (gaslessHex(block.hash, 32, 32, "APN_PROVIDER_PROTOCOL") !== block.hash)
        protocolFailure();
    for (const field of ["balanceAtomic", "nativeBalanceWei", "allowanceAtomic", "permitNonceAtomic", "entryPointNonceAtomic",
        "eoaNonceAtomic", "pendingEoaNonceAtomic", "baseFeePerGas", "maxFeePerGas", "maxPriorityFeePerGas"]) {
...
function feeConfiguration(value) {
    const keys = ["additionalGasCharge", "feeSpread", "nativeTokenPrice"];
    if (!isPlainRecord(value) || !exactKeys(value, keys))
        feeFailure();
    for (const field of keys)
        gaslessUint(value[field], field === "nativeTokenPrice", "APN_FEE_BUDGET_EXCEEDED");
    return value;
}
function gasQuantities(gas) {
...
function feeFailure() { return gaslessFailure("APN_FEE_BUDGET_EXCEEDED", "gasless_fee_budget"); }
function identityFailure() { return gaslessFailure("APN_OPERATION_BLOCKED", "gasless_identity_drift"); }
function nonceFailure() { return gaslessFailure("APN_OPERATION_BLOCKED", "gasless_nonce_drift"); }
function endpointFailure() { return gaslessFailure("APN_RPC_CONFIG", "gasless_endpoint_identity"); }
function protocolFailure() { return gaslessFailure("APN_PROVIDER_PROTOCOL", "gasless_protocol_identity"); }
function estimateFailure() { return gaslessFailure("APN_FEE_BUDGET_EXCEEDED", "gasless_estimate_bounds"); }
