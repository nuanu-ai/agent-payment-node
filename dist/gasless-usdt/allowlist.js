...
export const USDT_GASLESS_ALLOWLIST_SCHEMA = "apn.gasless-usdt-allowlist.v1";
export function usdtGaslessUsageKey(operationId) { return `apn.gasless-usdt-usage:${operationId}`; }
/**
...
export class UsdtGaslessAllowlistGate {
    ...
    async reserve(subject, admission) {
        ...
        if (existing === null)
            evaluate(active, subject, (await this.ledger.usage(identity(subject), now)).amountAtomic);
        else if (existing.state !== "reserved")
            blocked("The gasless usage reservation already crossed a durable transition.");
    ...
    async follow(subject, target, outcomeDigest) {
        ...
        const move = async (state, withOutcome) => await this.ledger.transition({ ...identity(subject),
    ...
    async active(subject) {
        ...
        if (active === null)
            refuse("allowlist_policy_required", "Gasless USDT requires an owner-activated allowlist policy for this profile.");
...
function evaluate(active, subject, dailyUsageAtomic) {
    ...
    if (canonicalJson(row.mechanismPins?.gasless ?? null) !== canonicalJson(USDT_GASLESS.mechanism)) {
        refuse("allowlist_mechanism_mismatch", "The gasless admission must pin exactly this sponsor mechanism.", { provider: USDT_GASLESS.mechanism.provider, reference: USDT_GASLESS.mechanism.reference });
    }
