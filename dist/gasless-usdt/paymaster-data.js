...
import { USDT_GASLESS, USDT_PAYMASTER_MAX_VALIDITY_S, USDT_PAYMASTER_MIN_VALIDITY_S, usdtFailure, } from "./model.js";
...
export function decodeUsdtPaymasterData(value) {
    ...
    let cursor = 0;
    const take = (bytes) => { const part = hex.slice(cursor, cursor + bytes * 2); cursor += bytes * 2; return part; };
    const mode = Number.parseInt(take(1), 16), flags = Number.parseInt(take(1), 16);
    if (mode >> 1 !== ERC20_MODE)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_mode");
    if (flags !== 0)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_flags");
 ...
 */
export function validateUsdtPaymasterData(result, plan, nowSeconds) {
    ...
    if (gaslessAddress(result.paymaster) !== USDT_GASLESS.paymaster)
    ...
    if (payload.token !== USDT_GASLESS.token)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_token");
    if (payload.treasury !== USDT_GASLESS.treasury)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_treasury");
    if (payload.postOpGas !== plan.quote.postOpGas)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_post_op_gas");
    ...
    if (payload.exchangeRate === 0n)
        usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_paymaster_rate");
    ...
    if (signedBound > plan.feeCapAtomic) {
        usdtFailure("APN_FEE_BUDGET_EXCEEDED", "gasless_usdt_signed_rate_above_max_fee", `The sponsor's signed USDT rate prices the worst case at ${signedBound} atomic, above the fee budget ${plan.feeCapAtomic}.`);
    }
    return payload;
}
//# sourceMappingURL=paymaster-data.js.map