...
const X402_STATE_VERSION = "apn.x402.state.v1";
const TRANSITION_VERSION = "apn.x402.transition.v1";
...
function validateX402OperationUnsafe(value) {
    ...
    if (operation.schemaVersion !== X402_STATE_VERSION || operation.kind !== "x402_fetch")
        stateCorrupt("x402 operation discriminant is invalid.");
    for (const key of ["operationId", "idempotencyHash", "profileHash", "requestHash", "fingerprint", "integrityHash"])
        hash(operation[key]);
    if (typeof operation.profile !== "string" || operation.profile.length === 0)
    ...
    if (amount > cap)
        stateCorrupt("x402 amount exceeds the frozen cap.");
    ...
    if (operation.paymentIdentifier !== undefined) {
        ...
        if (paymentIdentifier.declarationHash !== domainHash("apn.x402.payment-identifier-declaration.v1", paymentIdentifier.declarationCanonicalJson) ||
    ...
    if (authorization.intentHash !== x402AuthorizationIntentHash(intent))
        stateCorrupt("x402 authorization intent hash is invalid.");
    const signatureKeys = ["signatureHash", "paymentPayloadHash", "paymentHeaderHash"];
    const signatureCount = signatureKeys.filter((key) => operation[key] !== undefined).length;
    if (signatureCount !== 0 && signatureCount !== signatureKeys.length)
        stateCorrupt("x402 payment material hashes are incomplete.");
    ...
    if (!Array.isArray(operation.transitions))
        stateCorrupt("x402 transitions are invalid.");
    ...
    if (operation.createdAt !== operation.transitions[0]?.at || operation.updatedAt !== last.at) {
    ...
    if (typed.integrityHash !== domainHash(X402_STATE_VERSION, canonicalJson(withoutIntegrity)))
        stateCorrupt("x402 operation integrity hash is invalid.");
    return typed;
}
function validateTransitions(values) {
    ...
    for (let index = 0; index < values.length; index += 1) {
        ...
        if (transition.sequence !== String(index + 1) || transition.previousHash !== previousHash)
            stateCorrupt("x402 transition chain is discontinuous.");
        validateStateTuple(transition.state, transition.terminal, transition.reason, transition.proofClass);
        if (index === 0 && transition.state !== "awaiting_approval")
            stateCorrupt("x402 transition genesis is invalid.");
        if (index > 0) {
            const prior = values[index - 1];
            if (prior === undefined || !legalNextState(prior.state, transition.state))
                stateCorrupt("x402 state transition is illegal.");
        }
        timestamp(transition.at);
        const transitionAt = Date.parse(transition.at);
        if (transitionAt < previousAt)
            stateCorrupt("x402 transition timestamps are not chronological.");
        previousAt = transitionAt;
        hash(transition.previousHash);
        hash(transition.hash);
        const { hash: _hash, ...body } = transition;
        if (transition.hash !== domainHash(TRANSITION_VERSION, canonicalJson(body)))
            stateCorrupt("x402 transition hash is invalid.");
        previousHash = transition.hash;
    }
    if (values.length === 0)
        stateCorrupt("x402 operation has no transitions.");
}
const STATE_SHAPE = {
...
function validateStateTuple(state, terminal, reason, proofClass) {
    if (typeof state !== "string" || !(state in STATE_SHAPE))
        stateCorrupt("x402 state is invalid.");
    const typedState = state;
    const shape = STATE_SHAPE[typedState];
    if (terminal !== shape.terminal || reason !== shape.reason || proofClass !== shape.proofClass)
        stateCorrupt("x402 state classification is invalid.");
    return typedState;
}
function validateStateSummary(operation, evidence) {
    ...
    if (operation.finalityClass !== shape.finalityClass)
        stateCorrupt("x402 finality class is invalid for state.");
    if (!Array.isArray(operation.nextActions) || operation.nextActions.some((item) => typeof item !== "string"))
    ...
    if (state === "seller_result_recovery_pending" && (evidence.signatureCount !== 3 || operation.paymentIdentifier === undefined || evidence.transactionHint === undefined ||
    ...
    if (state === "completed" && (evidence.signatureCount !== 3 || evidence.settlementResponseObservation === undefined || evidence.transactionHint === undefined ||
    ...
    if (evidence.transactionHint?.source === "payment_response") {
        if (evidence.settlementResponseObservation === undefined ||
            evidence.transactionHint.sourceBindingHash !== x402TransactionHintSourceBindingHash("payment_response", evidence.settlementResponseObservation.settlementResponseHash))
            stateCorrupt("x402 payment-response transaction hint binding is invalid.");
        const normalized = JSON.parse(evidence.settlementResponseObservation.normalizedCanonicalJson);
        if (evidence.transactionHint.transactionHash !== normalized.transaction)
            stateCorrupt("x402 payment-response transaction hint conflicts with its response.");
    }
    if (evidence.transactionHint?.source === "authorization_used_log") {
        const scan = evidence.authorizationUsedScan;
        const candidate = scan?.candidates[0];
        if (scan === undefined || scan.status !== "complete" || scan.candidates.length !== 1 || candidate === undefined ||
...
function validateAttempts(value, operation) {
...
function validateHttpObservation(value, attemptNumber, purpose, operation) {
...
function validateSettlementResponseObservation(value, operation, attempts) {
...
function validateNormalizedSettlement(value, classification, operation) {
...
function validateTransactionHint(value) {
...
function validateAuthorizationUsedScan(value, operation) {
...
function validateSettlementEvidence(value, operation) {
...
function validateUnusedExpiryEvidence(value, operation) {
...
function validateX402ResultUnsafe(value) {
    ...
    if (result.mediaType === "application/json") {
        try {
            ...
            stateCorrupt("x402 JSON result body is invalid.");
        }
    }
...
function validateX402ReceiptUnsafe(value) {
    ...
    if (!["completed", "failed_before_effect", "failed_expired_unused", "failed_settled_without_result"].includes(terminalState))
        stateCorrupt("x402 receipt terminal state is invalid.");
    const resource = exactRecord(receipt.resource, ["origin", "path", "urlHash"]);
    if (typeof resource.origin !== "string" || new URL(resource.origin).origin !== resource.origin || !resource.origin.startsWith("https://") || typeof resource.path !== "string" || !resource.path.startsWith("/"))
        stateCorrupt("x402 receipt resource is invalid.");
    hash(resource.urlHash);
    hash(receipt.fingerprint);
    hash(receipt.offerHash);
    ...
    if (receipt.result !== undefined) {
        ...
        mediaType(result.mediaType);
        uint(result.byteLength);
        hash(result.resultIntegrityHash);
    }
    ...
    if (terminalState === "failed_before_effect" && (receipt.settlementResponseHash !== undefined || settlementEvidence !== undefined || unusedExpiryEvidence !== undefined || receipt.result !== undefined))
        stateCorrupt("x402 failed-before-effect receipt is invalid.");
    hash(receipt.operationBindingHash);
    hash(receipt.previousLinkHash);
    timestamp(receipt.createdAt);
    hash(receipt.integrityHash);
    const { integrityHash: _hash, ...body } = receipt;
    if (receipt.integrityHash !== domainHash("apn.x402.receipt.v1", canonicalJson(body)))
        stateCorrupt("x402 receipt integrity hash is invalid.");
    return receipt;
}
function canonicalText(value, maxBytes = 48 * 1024) {
...
function allowedRecord(value, required, optional) {
    const output = record(value);
