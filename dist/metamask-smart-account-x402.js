    ...
    async create(input) {
        ...
        return {
            ...
            ...(paymentIdentifier === undefined ? {} : {
                extensions: { "payment-identifier": paymentIdentifier },
            }),
        };
    }
}
export class MetaMaskSmartAccountX402Adapter {
    permissions;
    materials;
    rpc;
    allowance;
    ...
    constructor(permissions, materials, rpc, allowance, engine = new OfficialSmartAccountX402Engine(), now = () => new Date(), approvedFacilitators = METAMASK_FACILITATOR_ADDRESSES) {
        ...
        this.engine = engine;
        this.now = now;
        this.approvedFacilitators = approvedFacilitators;
    }
    async prepare(input) {
        ...
        if (facilitators.length === 0)
            incompatible("Seller facilitators do not intersect the approved MetaMask facilitator set.");
        ...
        if (BigInt(effectiveExpiryUnix) <= BigInt(input.preparedAtUnix))
            inactive();
        return {
    ...
    async materialize(operation) {
        ...
        if (existing !== null)
            return this.recovered(operation, existing);
        const binding = delegatedBinding(operation);
        if (this.nowUnix() >= BigInt(binding.effectiveExpiryUnix))
            inactive();
        const record = await this.preflightRecord(operation.profileHash, operation.wallet, operation.amountAtomic, binding.rpcOriginHash);
        ...
        if (child === undefined)
            protocol("ERC-7710 child is missing after validated materialization.");
    ...
    async recover(operation) {
        ...
        if (record === null) {
            if (operation.state === "authorization_material_pending" && operation.attempts.length === 0 &&
    ...
    async preflightRecord(profileHash, wallet, amountAtomic, rpcOriginHash) {
        const record = await this.permissions.load(profileHash);
        if (record === null || !isGrantedPermissionRecord(record) || record.phase !== "active" ||
            this.nowUnix() >= BigInt(record.granted_expires_at_unix))
            inactive();
        if (record.owner_address.toLowerCase() !== wallet.toLowerCase())
            drift();
    ...
    nowUnix() { return BigInt(Math.floor(this.now().getTime() / 1000)); }
    instant() {
        const value = this.now();
        if (!Number.isFinite(value.getTime()))
            throw new ApnError("APN_INTERNAL", "Smart Account x402 clock is invalid.");
...
async function validateOfficialMaterial(payload, operation, record) {
    ...
    if (delegationPayload.delegationManager.toLowerCase() !== binding.delegationManager ||
        delegationPayload.delegator.toLowerCase() !== operation.wallet)
        protocol("ERC-7710 payload identity differs from the frozen operation.");
...
function validateChildCaveats(caveats, operation, binding) {
    ...
    const expectedArgs = ["0x00", "0x00", "0x", "0x00", "0x"];
    if (required.some((value) => value === undefined) || caveats.length !== required.length ||
        ...
        required.some((address, index) => !caveats.some((item) => item.enforcer.toLowerCase() === address?.toLowerCase() && item.args.toLowerCase() === expectedArgs[index]))) {
        protocol("ERC-7710 child caveat set is invalid.");
    }
    const caveat = (address) => caveats.find((item) => item.enforcer.toLowerCase() === address?.toLowerCase());
    try {
        const value = decodeValueLteTerms(caveat(required[0])?.terms);
        const scope = decodeERC20TransferAmountTerms(caveat(required[1])?.terms);
        const calldata = decodeAllowedCalldataTerms(caveat(required[2])?.terms);
        const timestamp = decodeTimestampTerms(caveat(required[3])?.terms);
        const redeemer = decodeRedeemerTerms(caveat(required[4])?.terms);
        ...
        if (value.maxValue !== 0n || getAddress(scope.tokenAddress).toLowerCase() !== BASE_USDC.toLowerCase() ||
...
function protectedRequirements(operation) {
    const value = JSON.parse(operation.selectedOffer.declaredCanonicalJson);
    if (!isPlainRecord(value))
        protocol("Frozen ERC-7710 requirements are invalid.");
    return value;
}
function erc7710Payload(payload) {
...
function delegatedBinding(operation) {
...
function assertAuthorityBinding(operation, record) {
    const binding = delegatedBinding(operation);
    if (binding.permissionRevision !== record.revision || binding.rootGrantFingerprint !== record.grant_fingerprint ||
        ...
        operation.wallet !== record.owner_address.toLowerCase())
        drift();
}
function materialProjection(record) {
    return {
...
function intersection(offered, approved) {
    ...
    return [...new Set(offered.map(lower).filter((address) => allowed.has(address)))].sort();
}
function canonicalAtomic(value) {
    if (!/^(?:0|[1-9][0-9]*)$/u.test(value))
        throw new ApnError("APN_PROVIDER_PROTOCOL", "Smart Account x402 atomic value is invalid.");
    return BigInt(value);
}
function lower(value) { return value.toLowerCase(); }
function inactive() { throw new ApnError("APN_PERMISSION_INACTIVE", "Smart Account permission is inactive for x402."); }
function drift() { throw new ApnError("APN_PROFILE_DRIFT", "Smart Account x402 authority differs from the frozen profile."); }
function incompatible(message) { throw new ApnError("APN_X402_UNSUPPORTED_OFFER", message); }
function protocol(message) { throw new ApnError("APN_PROVIDER_PROTOCOL", message); }
//# sourceMappingURL=metamask-smart-account-x402.js.map