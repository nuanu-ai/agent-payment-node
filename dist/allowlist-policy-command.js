...
import { allowlistAdmissions, allowlistDecisionCode, allowlistDecisionFingerprint, } from "./allowlist-policy-activation.js";
import { AllowlistPolicyStore, stagedRecordAccounts } from "./allowlist-policy-store.js";
import { parseAllowlistPolicyFile } from "./allowlist-policy-v2.js";
const MAX_POLICY_FILE_BYTES = 256 * 1024;
export async function executeAllowlistPolicyCommand(request, context) {
    ...
    switch (request.command) {
        ...
        case "allowlist.policy.stage": return outcome(await store.stage({ profile: request.profile, policy: await readPolicyFile(request.file),
            ...(request.expectedRevision === undefined ? {} : { expectedRevision: request.expectedRevision }), now: context.clock.now() }), "staged_unadmitted_allowlist_policy");
        ...
        case "allowlist.policy.revoke":
            return outcome(await decide(store, request, context), request.command === "allowlist.policy.activate" ? "owner_activated_allowlist_policy" : "owner_revoked_allowlist_policy");
    }
}
async function decide(store, request, context) {
    const approval = context.allowlistPolicyApproval;
    if (approval === undefined) {
        throw new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED", "Allowlist policy decisions require the foreground terminal approval.", { approval_boundary: "foreground_tty" });
    }
    ...
    if (record === undefined)
        throw new ApnError("APN_OPERATION_NOT_FOUND", "The staged allowlist revision does not exist.", { reason: "unknown_revision" });
    if (action === "activate") {
        if (active?.record.revision === record.revision)
            invalid("This revision is already the active allowlist policy.", "already_active");
...
async function status(store, profile, now) {
...
async function readPolicyFile(path) {
    ...
    try {
        for (const part of path.slice(component.length).split(sep).filter(Boolean)) {
            ...
            if ((await lstat(component)).isSymbolicLink())
                unsafe("The policy file path traverses a symbolic link; pass the real path.");
        }
    }
    catch (error) {
    ...
    try {
        ...
        try {
            ...
            return invalid("The policy file is not strict UTF-8 JSON.", "invalid_policy_file");
        }
...
function outcome(data, proofClass) {
    return { proofClass, data, operation: null, receipt: null, nextActions: [] };
}
function instant(value) {
    if (!(value instanceof Date) || !Number.isFinite(value.getTime()))
        invalid("The evaluation instant is invalid.", "invalid_time");
    return value.toISOString();
}
function invalid(message, reason) { throw new ApnError("APN_INVALID_INPUT", message, { reason }); }
function unsafe(message) { throw new ApnError("APN_STATE_SECURITY", message, { reason: "unsafe_policy_file" }); }
//# sourceMappingURL=allowlist-policy-command.js.map