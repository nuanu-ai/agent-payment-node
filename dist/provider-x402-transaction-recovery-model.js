...
export function validateProviderX402TransactionRecoveryBinding(binding, operationId) {
    ...
    if (!exactKeys(binding, [
        ...
        !["bound", "evidence_validated"].includes(binding.stage) ||
        (binding.stage === "bound" ? binding.evidenceDigest !== undefined : !hash(binding.evidenceDigest)))
        corrupt();
}
export function validateProviderX402TransactionRecoveryContinuity(previous, next) {
    ...
    if (Date.parse(next.updatedAt) < Date.parse(previous.updatedAt))
        corrupt();
    if (!["bound", "evidence_validated"].includes(next.stage))
        corrupt();
    if (next.stage === "bound" && canonicalJson(previous) !== canonicalJson(next))
        corrupt();
}
...
//# sourceMappingURL=provider-x402-transaction-recovery-model.js.map