...
import { ERC20_TRANSFER_TOPIC, PERMIT2_ADDRESS, PROXY_SETTLED_TOPIC, PROXY_SETTLED_WITH_PERMIT_TOPIC, X402_EXACT_PERMIT2_PROXY, } from "./registry.js";
 ...
 */
export function verifyPermit2SettlementReceipt(receipt, expected) {
    if (!isPlainRecord(receipt) || !Array.isArray(receipt.logs))
        fail("The settlement receipt is malformed.");
    ...
    if (transactionHash !== expected.transactionHash.toLowerCase())
        fail("The receipt is not the claimed settlement transaction.");
    if (receipt.status !== "0x1")
        fail("The settlement transaction did not succeed.");
    if (typeof receipt.blockNumber !== "string" || !QUANTITY.test(receipt.blockNumber))
        fail("The settlement block number is malformed.");
    ...
    for (const log of receipt.logs) {
        ...
        if (log.removed === true)
            fail("The settlement receipt contains a removed log.");
        if (log.transactionHash !== undefined && lowerHash(log.transactionHash) !== transactionHash)
            fail("A receipt log names another transaction.");
        const address = log.address.toLowerCase(), topic0 = typeof log.topics[0] === "string" ? log.topics[0].toLowerCase() : "";
        if (address === token && topic0 === ERC20_TRANSFER_TOPIC)
            transfers.push({ topics: log.topics, data: log.data, logIndex: log.logIndex });
        if (address === proxy && (topic0 === PROXY_SETTLED_TOPIC || topic0 === PROXY_SETTLED_WITH_PERMIT_TOPIC))
            settled.push(topic0);
    }
    ...
    if (matching.length !== 1 || transfers.length !== 1)
        fail("The receipt does not carry exactly the frozen token transfer.");
    const wanted = expected.withPermit ? PROXY_SETTLED_WITH_PERMIT_TOPIC : PROXY_SETTLED_TOPIC;
    if (settled.length !== 1 || settled[0] !== wanted)
        fail("The receipt does not carry exactly one matching proxy settlement.");
    const logIndex = matching[0].logIndex;
    if (typeof logIndex !== "string" || !QUANTITY.test(logIndex))
        fail("The transfer log index is malformed.");
    return { transactionHash: transactionHash, blockHash: blockHash, blockNumber: BigInt(receipt.blockNumber).toString(),
...
export function permit2NonceBitmapCall(owner, nonce) {
    const value = uint(nonce);
    if (!/^0x[0-9a-fA-F]{40}$/u.test(owner))
        throw new ApnError("APN_INVALID_INPUT", "The Permit2 owner is invalid.");
    return { to: PERMIT2_ADDRESS, data: `${NONCE_BITMAP_SELECTOR}${owner.slice(2).toLowerCase().padStart(64, "0")}${(value >> 8n).toString(16).padStart(64, "0")}` };
}
/** True when the frozen nonce is spent. A spent nonce without a matching receipt is ambiguous, never success. */
export function permit2NonceConsumed(bitmapWord, nonce) {
...
function lowerHash(value) {
    if (typeof value !== "string" || !HASH.test(value.toLowerCase()) || /^0x0{64}$/u.test(value))
        fail("A settlement hash is malformed.");
    return value.toLowerCase();
}
function topicAddress(value) {
    if (typeof value !== "string" || !/^0x0{24}[0-9a-fA-F]{40}$/u.test(value))
        return "";
...
function fail(message) { throw new ApnError("APN_X402_SETTLEMENT_INVALID", message); }
