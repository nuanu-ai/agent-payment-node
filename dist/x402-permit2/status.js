...
export async function readPermit2IntentStatus(root, requestedProfile, operationId) {
    ...
    if (intent === null)
        return { operationId, profile, state: "not_found",
            capability: "execution_blocked", blockerCodes: ["permit2_intent_not_found"] };
    if (intent.profileHash !== reader.profileHash(profile)) {
        throw new ApnError("APN_OPERATION_BLOCKED", "Permit2 intent does not belong to the requested profile.", { reason: "x402_permit2_profile_mismatch" });
    }
    return { operationId: intent.operationId, profile, chain: intent.chain, token: intent.token,
        owner: intent.owner, recipient: intent.recipient, state: "execution_blocked",
