...
const scalarTypes = new Set(["string", "base64", "profile", "provider_id", "provider_auth_method", "positive_integer", "https_url", "address", "decimal_usdc", "atomic_usdc", "wei", "operation_id", "transaction_hash", "provider_request_id", "idempotency_key", "integer_seconds"]);
const effectClasses = new Set(["none", "local_read", "network_read", "network_request", "local_write", "payment_prepare", "payment_submit", "recovery"]);
...
function validateCommandManifestShape(value, installedProductVersion) {
    ...
    for (const rawCommand of value.commands) {
        ...
        if (!["apn.cli.v1", "text"].includes(String(output.contract)) || output.success_exit !== 0 || output.failure_exit !== 1 || !nonempty(output.success) || !Array.isArray(output.failures) || !output.failures.every(nonempty))
            manifestInvalid();
...
//# sourceMappingURL=command-manifest-validation.js.map