...
export function validateX402OperationUnsafe(value) {
    ...
    if (BigInt(selectedOffer.index) >= BigInt(MAX_DECODED_X402_BYTES))
        stateCorrupt("x402 selected offer index exceeds the decoded seller wire budget.");
    ...
    if (operation.providerSigner !== undefined) {
        ...
        if (signer.schemaVersion !== "apn.x402.provider-signer.v1" ||
...
export function validateTransitions(values) {
...
export function legalNextState(from, to) { return LEGAL_NEXT[from].includes(to); }
export function validateStateTuple(state, terminal, reason, proofClass) {
...
export function validateStateSummary(operation, evidence) {
...
//# sourceMappingURL=x402-operation-validation.js.map