...
function validateJournal(value, op) {
    ...
    if (j.phase === "pending") {
        if (j.marker !== null || j.markedAt !== null || j.transactionHash !== null || j.observedAt !== null)
            corrupt("pending journal");
    }
    else {
...
export class RelayNativeSourceJournalRepository extends SecureStateStore {
    path(op) { return `relay-native-source-journals/${op.profileHash}/${op.operationId}.json`; }
    async load(op) {
    ...
    async advance(op, expected, phase, hash = null, now = new Date()) {
        ...
        return this.withLocks([`relay-native-journal:${op.operationId}`], async () => {
            ...
            if ((old?.integrityHash ?? null) !== expected)
                blocked("stale_journal");
...
export async function dispatchRelayNativeDepositOnce(op, journal, store, send, raw, now) {
    if (journal.phase === "submitting")
        return journal;
    if (journal.phase !== "sealed" || keccak256(raw) !== journal.transactionHash)
...
async function verifySigned(op, raw, expectedNonce) {
    ...
    if (tx.type !== "eip1559" || tx.chainId !== 56 || !same(signer, op.sourceAccount) ||
        ...
        tx.gas * tx.maxFeePerGas > BigInt(op.depositNetworkFeeCeilingWei))
...
export class RelayNativeSourceRuntime {
    ...
    journals;
    usage;
    constructor(state, wrapping, ports, clock = { now: () => new Date() }, origin) {
        this.state = state;
        this.ports = ports;
        this.clock = clock;
        this.origin = origin;
    ...
    async execute(operationId) {
        if (!HASH.test(operationId))
            throw new ApnError("APN_INVALID_INPUT", "Relay native execute requires an operation ID.");
    ...
    async owner(op) {
        ...
        for (const record of await this.permissions.listAll()) {
            if (isGrantedPermissionRecord(record) && same(record.owner_address, op.sourceAccount) &&
                record.profile_hash !== op.profileHash)
    ...
    async admission(op, now) {
        ...
        if (pin?.provider !== "relay" || pin.reference !== RELAY_BNB_POLYGON_ROUTE_REFERENCE)
            blocked("route_pin");
        return active;
    }
    ...
    async custody(op, signed) {
        return this.state.withLocks([walletCustodyLock(this.state, PROFILE)], async () => {
            ...
            try {
                ...
                if (signed !== undefined) {
                    if (old !== undefined)
                        blocked("signed_native_deposit_already_sealed");
                    if (await verifySigned(op, signed.raw) !== signed.hash)
                        corrupt("signed hash");
                    const entry = { payloadHash: custodyBinding(op), transactionHash: signed.hash,
                ...
                if (old.payloadHash !== custodyBinding(op) || old.transactionHash !== old.rawTransactionHash ||
                    keccak256(old.rawTransaction) !== old.transactionHash)
