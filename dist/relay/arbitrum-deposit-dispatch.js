...
function blocked(reason) { throw new ApnError("APN_OPERATION_BLOCKED", "Relay Arbitrum deposit dispatch is blocked.", { reason }); }
const same = (a, b) => a.toLowerCase() === b.toLowerCase();
/** Unlocks local custody only after a durable signing marker exists. */
export class LocalRelayArbitrumDepositSigner {
    state;
    wallets;
    ...
    async sign(op, nonce, journal) {
        ...
        try {
            ...
            const deposit = op.arbitrumDraft.rawQuote.steps[1].items[0].data;
            if (!same(deposit.to, ETHEREUM_DEPOSITORY) || deposit.value !== "0" ||
                ...
                blocked("deposit_envelope");
            return await account.signTransaction({ type: "eip1559", chainId: 42161, to: deposit.to,
                data: deposit.data, value: 0n, nonce: Number(BigInt(nonce)), gas: BigInt(deposit.gas),
...
export class RelayArbitrumDepositDispatchService {
    ...
    async reserve(op, active, journal) {
        const at = this.now();
        if (active.profile !== op.arbitrumDraft.profile || active.digest !== op.policyDigest || active.revision !== op.policyRevision ||
            !same(active.accounts.evm ?? "", op.sourceAccount))
        ...
        if (reservation.policyDigest !== op.policyDigest)
        ...
        if (reservation.state !== "reserved" && reservation.state !== "submitted")
            blocked("usage_lease_unavailable");
    ...
    async execute(profile, operationId) {
        ...
        if (!HASH.test(operationId))
            throw new ApnError("APN_INVALID_INPUT", "Relay Arbitrum deposit requires an operation ID.");
        ...
        return this.state.withLocks([`profile:${profileHash}`, `profile:${allowlistProfileHash(profile)}`,
            ...
            if (!fresh.readOnlyConditionsSatisfied ||
                ...
                blocked("deposit_preflight_changed");
            ...
            journal = await this.journals.transitionUnderLocks(profileHash, operationId, journal.integrityHash, { kind: "seal_signed", role: "deposit", rawTransaction: raw, nonce: fresh.confirmedNonce });
            ...
            await this.owner(op);
            journal = await this.journals.transitionUnderLocks(profileHash, operationId, journal.integrityHash, { kind: "mark_submitting", role: "deposit", at: this.now().toISOString() });
            ...
            journal = await this.journals.transitionUnderLocks(profileHash, operationId, journal.integrityHash, { kind: "record_send", role: "deposit", outcome });
            ...
            if (leaseState !== "unknown_finality")
                await this.usage.transition({ ...this.usageIdentity(op),
            ...
            return this.result(op, journal, outcome === "accepted" ? "deposit_submitted" : "observation_only", outcome === "accepted" ? "single_send_accepted" : "single_send_uncertain");
        });
    }
}
//# sourceMappingURL=arbitrum-deposit-dispatch.js.map