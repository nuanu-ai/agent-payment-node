...
import { preflightRelayArbitrumSourceDraft } from "./arbitrum-usdc-source-draft.js";
const blocked = (reason) => {
    throw new ApnError("APN_OPERATION_BLOCKED", "Relay Arbitrum deposit preflight is blocked.", { reason });
};
/** Three read-only batches total. The third binds nonce and block base fee to the funding block. */
export async function preflightRelayArbitrumDeposit(operation, active, publicAccount, dailyUsageAtomic, now, ports) {
    ...
    const funding = await preflightRelayArbitrumSourceDraft(draft, active, publicAccount, dailyUsageAtomic, now, ports);
    ...
    if (evmRpcQuantity(block.number).toString() !== funding.observationBlockNumber ||
        evmRpcHex(block.hash, 32) !== funding.observationBlockHash)
    ...
    const maxFee = BigInt(quote.deposit.maxFeePerGas);
    const priority = BigInt(quote.deposit.maxPriorityFeePerGas);
    ...
    const reasons = Object.freeze([
        ...
        ...(funding.approvalRequired ? ["allowance_below_principal"] : []),
        ...(nonceUncontended ? [] : ["pending_nonce_conflict"]),
        ...(feeWithinQuote ? [] : ["deposit_base_fee_exceeds_quote"]),
    ]);
    return Object.freeze({ kind: "relay_arbitrum_read_only_deposit_preflight",
        ...
        sourceChainId: 42161, sourceAccount: draft.owner,
        ...
        baseFeePerGas: baseFee.toString(), quotedMaxFeePerGas: quote.deposit.maxFeePerGas,
        quotedMaxPriorityFeePerGas: quote.deposit.maxPriorityFeePerGas,
...
export class RelayArbitrumDepositPreflightReader {
    url;
    state;
    guardFactory;
    holdAfterPost;
    ...
    async read(operation, active, publicAccount, dailyUsageAtomic, now, clock = () => new Date()) {
        const guard = this.guardFactory();
        return preflightRelayArbitrumDeposit(operation, active, publicAccount, dailyUsageAtomic, now, {
            now: clock,
            batch: async (calls) => {
                ...
                if (!Array.isArray(rows) || rows.length !== calls.length)
                    blocked("rpc_batch_shape");
                return rows;
            },
        });
    }
}
//# sourceMappingURL=arbitrum-deposit-preflight.js.map