...
function blocked() {
    throw new ApnError("APN_OPERATION_BLOCKED", "Saved Relay source route is not executable.", { reason: "saved_source_route_binding" });
}
export function relayExecutionRoute(op) {
    const quote = op.quote;
    if (op.sourceChainId !== 1 || quote === undefined ||
        ![56, 8453].includes(op.destinationChainId))
        blocked();
    if (op.destinationChainId === 56) {
        if (quote.routeReference !== undefined || quote.orderData.output.chainId !== "bnb")
            blocked();
    ...
    if (!/^(0|[1-9][0-9]*)$/u.test(payment?.minimumAmount ?? "") ||
        ...
        !same(quote.deposit.to, ETHEREUM_DEPOSITORY))
        blocked();
    return RELAY_BASE_ROUTE_REFERENCE;
}
//# sourceMappingURL=execution-route.js.map