...
import { FACILITATOR_FILE_LIMIT, FACILITATOR_HISTORY_LIMIT, FACILITATOR_INITIAL, FACILITATOR_KIND, FACILITATOR_OPERATION_VERSION, FACILITATOR_POLICY, FACILITATOR_TERMINAL, facilitatorBinding, facilitatorMutable, sealFacilitatorOperation } from "./operation-model.js";
...
export function facilitatorSame(left, right) { return canonicalJson(left) === canonicalJson(right); }
function corrupt() { return facilitatorFail("facilitator_gasless_state_corrupt"); }
function attempt(work) { try {
    return work();
}
catch {
    return corrupt();
} }
export function newFacilitatorOperation(input) {
...
export function transitionFacilitator(op, patch, at) {
    validateFacilitatorOperation(op);
    const mutable = facilitatorMutable({ ...facilitatorMutable(op), ...patch });
    if (facilitatorSame(facilitatorMutable(op), mutable))
        return op;
    ...
    if (op.transitions.length >= FACILITATOR_HISTORY_LIMIT - (terminal ? 0 : 2)) {
        ...
        if (!terminal && mutable.state === op.state)
            return op;
    ...
    if (Buffer.byteLength(canonicalJson(next), "utf8") + 1 > FACILITATOR_FILE_LIMIT)
        facilitatorFail("facilitator_gasless_capacity");
    validateFacilitatorOperation(next);
    validateFacilitatorContinuity(op, next);
    return next;
}
export function facilitatorAtTransition(op, index) {
...
export function validateFacilitatorOperation(value) {
    const parsed = facilitatorOperationSchema.safeParse(value);
    if (!parsed.success)
        corrupt();
    const op = parsed.data;
    const { integrityHash, ...body } = op;
    if (hashObject(body) !== integrityHash || hashObject(facilitatorBinding(op)) !== op.fingerprint)
        corrupt();
    validIntent(op.intent);
    if (op.createdAt !== op.intent.preparedAt || op.transitions[0].at !== op.intent.preparedAt ||
        op.updatedAt !== op.transitions.at(-1).at || op.terminal !== FACILITATOR_TERMINAL.includes(op.state))
        corrupt();
    let previous = null, link = op.fingerprint;
    for (const step of op.transitions) {
        const { at, previousHash, transitionHash, ...mutable } = step;
        if (previousHash !== link || hashObject({ ...mutable, at, previousHash }) !== transitionHash)
            corrupt();
        if (previous === null ? !facilitatorSame(mutable, FACILITATOR_INITIAL) : !validStep(previous, mutable))
            corrupt();
        validShape(op, mutable);
        previous = mutable;
        link = transitionHash;
    }
    if (!facilitatorSame(previous, facilitatorMutable(op)))
        corrupt();
    return op;
}
export function validateFacilitatorContinuity(previous, next) {
    if (!facilitatorSame(facilitatorBinding(previous), facilitatorBinding(next)) || previous.fingerprint !== next.fingerprint ||
        ...
        !facilitatorSame(previous.transitions, next.transitions.slice(0, previous.transitions.length)))
        corrupt();
}
function validIntent(i) {
...
function validShape(op, m) {
    ...
    if (!shape[m.state])
        corrupt();
    if (m.approval !== null && (m.approval.fingerprint !== op.fingerprint || m.approval.expiresAt !== op.intent.expiresAt))
        corrupt();
    if (signed !== null) {
        const a = signed.authorization;
        if (a.from !== op.intent.owner.address.toLowerCase() || a.to !== op.intent.requirement.payTo ||
            a.value !== op.intent.requirement.amount || signed.digest !== attempt(() => facilitatorAuthorizationDigest(a)))
            corrupt();
    }
    if (m.verify !== null && (m.verify.transactionHash !== null || m.verify.outcome === "pending"))
        corrupt();
    if (m.settle !== null && m.settle.transactionHash !== null && m.settle.outcome !== "accepted" && m.settle.outcome !== "pending")
        corrupt();
    if (m.settlement !== null && m.settlement.deliveredAtomic !== op.intent.request.grossAtomic)
        corrupt();
}
function validStep(p, n) {
    if (!NEXT[p.state].includes(n.state))
        return false;
    if (p.approval !== null && !facilitatorSame(p.approval, n.approval))
        return false;
    if (p.signed !== null && (n.signed === null || !facilitatorSame({ ...p.signed, signatureHash: null }, { ...n.signed, signatureHash: null }) ||
        (p.signed.signatureHash !== null && p.signed.signatureHash !== n.signed.signatureHash)))
        return false;
...
function keeps(p, n) {
    if (p === null)
        return true;
    if (n === null || p.startedAt !== n.startedAt)
        return false;
