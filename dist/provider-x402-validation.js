...
function validateRangeSettlement(evidence, operation) {
    ...
    if (evidence.schemaVersion !== "apn.provider-x402.settlement.v1" || evidence.evidenceHash !== hashObject(base) ||
...
function providerBinding(operation) {
    const { integrityHash: _integrityHash, state: _state, finalityClass: _finalityClass, terminal: _terminal, reason: _reason, proofClass: _proofClass, nextActions: _nextActions, updatedAt: _updatedAt, transitions: _transitions, ...stable } = operation;
    return hashObject({ domain: "apn.provider-x402.binding.v2", stable });
}
export function providerX402CompleteBindingHash(operation) {
    return providerBinding(operation);
}
function validTransfer(log, operation) {
...
//# sourceMappingURL=provider-x402-validation.js.map