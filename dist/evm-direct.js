...
export function evmTransaction(asset, from, recipient, amountAtomic) {
    evmUint(amountAtomic, true);
    const data = asset.kind === "native" ? "0x" : encodeFunctionData({
...
export function evmDirectFingerprint(operation) {
    return hashObject({
...
export function requireEvmFunding(balance, amountAtomic, quote, maximumFeeWei) {
    validateEvmFeeQuote(quote);
    if (balance.asset.chainId !== quote.chainId)
        throw new ApnError("APN_CHAIN_MISMATCH", "Funding and fee quotes belong to different chains.");
    if (evmUint(quote.totalQuoteWei, true) > evmUint(maximumFeeWei, true))
        throw new ApnError("APN_FEE_BUDGET_EXCEEDED", "The current total fee quote exceeds the caller's pre-submission budget.");
    if (evmUint(balance.assetAtomic) < evmUint(amountAtomic, true))
