...
function validatePaymentRequired(value) {
    ...
    if (paymentRequired.extensions !== undefined)
        validatePaymentRequiredExtensions(paymentRequired.extensions);
}
function validatePaymentPayload(value) {
    ...
    if (candidate.assetTransferMethod === "erc7710") {
        if (!isStrictErc7710Payload(payload))
            throw protocol("PAYMENT-SIGNATURE ERC-7710 payload shape is invalid.");
        if (paymentPayload.extensions !== undefined)
            validatePaymentPayloadExtensions(paymentPayload.extensions);
        return;
    }
...
function requiredKeys(value, keys) {
    if (keys.some((key) => !Object.hasOwn(value, key)))
        throw protocol("x402 required fields are missing.");
}
...
function validatePaymentRequiredExtensions(value) {
...
function validatePaymentIdentifierDeclaration(value, paymentPayload) {
