...
const REQUIRED_ENVELOPE_KEYS = ["status", "data"];
const KNOWN_ENVELOPE_KEYS = new Set(PROVIDER_X402_KNOWN_ENVELOPE_KEYS);
...
function validateEnvelopeKeys(value) {
    ...
    for (const key of Reflect.ownKeys(value)) {
        ...
        if (!KNOWN_ENVELOPE_KEYS.has(key)) {
            if (isConflictingProviderX402OuterKey(key))
                throw protocol();
            try {
                canonicalizeNormalizedProviderJson(descriptor.value);
            }
            catch {
                throw protocol();
            }
...
function paymentMetadata(value, expectedAmount) {
    const hasPaymentMade = Object.hasOwn(value, "paymentMade");
    const hasAmountPaid = Object.hasOwn(value, "amountPaid");
    if (hasPaymentMade !== hasAmountPaid)
        throw protocol();
    if (!hasPaymentMade)
        return {};
    if (value.paymentMade !== true || canonicalProviderAmount(value.amountPaid) !== expectedAmount)
        throw protocol();
