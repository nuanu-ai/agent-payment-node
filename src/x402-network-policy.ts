...
export async function assertCurrentNetworkPolicy(context: RuntimeContext, operation: X402OperationRecord, callerDeadlineMs?: number): Promise<void> {
  if (operation.chainId === "8453") return;
  ...
  if (payer.wallet !== operation.wallet || payer.transferMethod !== "eip3009") throw new ApnError("APN_WALLET_MISMATCH", "Current local payer differs from the frozen x402 wallet.");
  ...
  if (remainingMs !== undefined && remainingMs < 1) throw new ApnError("APN_STATE_BUSY", "Settlement wait expired before first-exposure policy checks.");
  ...
  if (BigInt(evidence.usdcAtomic) < BigInt(operation.amountAtomic)) throw new ApnError("APN_INSUFFICIENT_USDC", "Current selected-network USDC is insufficient.");
}
