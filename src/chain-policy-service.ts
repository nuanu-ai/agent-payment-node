...
import { assertChainPolicy, chainAsset, chainDecimal, chainUsage, sealChainPolicy, type ChainPolicy } from "./chain-policy.js";
...
export class ChainPolicyService {
  ...
  adapter(rail: DirectRailName, provider: ChainProvider): DirectRailPort {
    ...
    if (matches.length !== 1) throw new ApnError("APN_PROVIDER_UNAVAILABLE", "This direct rail and execution owner are unavailable.");
    return matches[0]!;
  }
  async assertProfileOwner(profile: string, provider: ChainProvider): Promise<void> {
    ...
    if (this.context.chainAccounts !== undefined) for (const rail of ["solana", "tron"] as const) {
  ...
  async account(profileInput: string, rail: DirectRailName): Promise<ChainAccount> {
    const profile = canonicalProfile(profileInput);
    if (this.context.chainAccounts === undefined) throw new ApnError("APN_PROVIDER_UNAVAILABLE", "Chain custody storage is unavailable.");
    const account = await this.context.chainAccounts.account(profile, rail);
    if (account === null) throw new ApnError("APN_OPERATION_BLOCKED", "The chain wallet is not initialized.");
    await this.assertProfileOwner(profile, account.provider);
    const adapter = this.adapter(rail, account.provider);
    if (adapter.canonicalAddress(account.address) !== account.address) throw new ApnError("APN_WALLET_MISMATCH", "The chain wallet identity is invalid.");
    return account;
  }
  async ensure(profileInput: string, rail: DirectRailName, provider: ChainProvider, acceptRisk: boolean): Promise<ChainAccount> {
    const profile = canonicalProfile(profileInput);
  ...
  async balance(profile: string, rail: DirectRailName, alias: ChainAssetAlias): Promise<unknown> {
    const account = await this.account(profile, rail); const adapter = this.adapter(rail, account.provider);
  ...
  async admit(input: {
    ...
    readonly maximumPerTransfer: string; readonly dailyLimit: string; readonly maximumFee: string;
  }): Promise<unknown> {
    ...
    if (BigInt(maximumPerTransferAtomic) > BigInt(dailyLimitAtomic)) throw new ApnError("APN_INVALID_INPUT", "The daily limit must cover the per-transfer limit.");
    const approval = this.context.chainPolicyApproval;
    if (approval === undefined) throw new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED", "Chain policy admission requires foreground human approval.");
    await this.context.ready();
    return await this.context.state.withLocks([`profile:${this.context.state.profileHash(profile)}`], async () => {
      ...
      if (canonicalJson(await this.account(profile, input.rail)) !== canonicalJson(account)) throw new ApnError("APN_PROFILE_DRIFT", "The chain account changed during policy admission.");
  ...
  async requiredPolicy(account: ChainAccount, alias: ChainAssetAlias): Promise<ChainPolicy> {
    const policy = await this.policies.load(account, chainAsset(account.rail, alias));
    if (policy === null) throw new ApnError("APN_WALLET_POLICY_REQUIRED", "This mainnet asset is denied until foreground policy admission.");
    return policy;
  }
  async authorize(account: ChainAccount, alias: ChainAssetAlias, amount: string, maximumFee: string, excluding?: string): Promise<ChainPolicy> {
    const policy = await this.requiredPolicy(account, alias);
    assertChainPolicy(policy, account, chainAsset(account.rail, alias), amount, maximumFee,
      await this.records.listOperations(account.profileHash), this.context.clock.now(), excluding);
    return policy;
  }
}
