...
import { normalizeX402HttpRequest, validateFrozenX402HttpRequest, type X402HttpRequestV1 } from "./x402-http-request.js";

export function providerHttpRequest(endpoint: URL, httpRequest?: X402HttpRequestV1): ProviderX402OperationRecord["request"] {
...
export function validateProviderHttpRequest(request: ProviderX402OperationRecord["request"]): void {
  try {
    ...
    if (canonicalJson(request) === canonicalJson(expected)) return;
  } catch {}
  throw new ApnError("APN_STATE_CORRUPT", "Provider HTTP request binding is invalid.");
}

export function assertProviderHttpRequest(port: X402ExecutionPort, request: X402HttpRequestV1 | undefined): void {
...
export function assertAwalHttpRequest(request: X402HttpRequestV1): void {
