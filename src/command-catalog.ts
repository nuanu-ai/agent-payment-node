...
const BASE_COMMANDS: readonly CommandDefinition[] = [
  ...
  command(["operation", "status"], "apn operation status --operation <operation-id>", "Read one durable operation without network access.", [operationRequired], "local_read", "Reads public operation state and never resumes an effect.", "none", "Never.", allOperationStates, [
    ...
    { command_path: ["operation", "abandon"], when: "Only for a provider-atomic ambiguous effect with no transaction hash or provider recovery reference, or a local Solana/TRON transfer whose validity window passed with no RPC history, after accepting the unresolved financial risk." },
  ...
  command(["operation", "abandon"], "apn operation abandon --operation <operation-id>",
    "Administratively terminalize one strictly eligible unknown provider or expired local rail effect.", [operationRequired], "local_write",
    "After exact owner confirmation, appends an abandonment transition and authoritative receipt; expired local rail eligibility uses read-only RPC checks and never contacts a wallet, signer or provider.",
