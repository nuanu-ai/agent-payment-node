...
const BASE_COMMANDS: readonly CommandDefinition[] = [
  command(["relay", "arbitrum", "prepare"],
    ...
    [profileRequired, option("--owner", "address", true, noDefault, ["canonical_evm_address"], "public"),
      ...
      option("--idempotency-key", "idempotency_key", true, noDefault, ["8_to_128_safe_ascii_characters"], "operator_input")],
    "payment_prepare", "Reads a local quote file and active Arbitrum owner policy; writes an immutable unsigned operation. Source signing, approval, deposit, dispatch and destination credit are unavailable.",
    "none", "Never.", { terminal: [], non_terminal: ["prepared"] },
    [{ command_path: ["operation", "status"], when: "Inspect the saved unsigned operation." }],
    ["apn relay arbitrum prepare --profile default --owner <account> --amount-atomic 500000 --min-output-atomic 94065 --max-provider-fee-atomic 401482 --max-approval-network-fee-wei 2000000000000 --max-deposit-network-fee-wei 2000000000000 --quote-file /absolute/quote.json --idempotency-key relay-arb-eth-0001"]),
  command(["relay", "arbitrum", "observe"],
    "apn relay arbitrum observe --operation <operation-id> --rpc-url <arbitrum-rpc>",
    "Read canonical safe Arbitrum source receipts for a saved Relay attempt.",
    [operationRequired, rpcRequired], "network_read",
    "Reads only Arbitrum, with at most five physical RPC POSTs under persisted 750 ms pacing and no 429 retry. A matching safe receipt may advance only the saved source effect journal. No signer, broadcaster, destination credit or paid acceptance.",
    "prior_operation_authorization", "A hash-bound submitting marker in the saved source effect journal is required.",
    { terminal: [], non_terminal: ["source_effect_not_recorded", "observation_only", "source_proof_pending", "approval_source_confirmed", "approval_skipped", "deposit_source_confirmed"] },
    [], ["apn relay arbitrum observe --operation <operation-id> --rpc-url https://arbitrum-rpc.example"]),
  command(["relay", "arbitrum", "approval-check"],
    "apn relay arbitrum approval-check --profile default --operation <operation-id> --rpc-url <arbitrum-rpc>",
    "Check the saved Arbitrum Relay allowance and persist an approval skip only with fresh canonical proof.",
    [profileRequired, operationRequired, rpcRequired], "payment_prepare",
    "At most six read-only Arbitrum RPC POSTs under one persisted 24-POST guard and 750 ms pacing. An insufficient or changed allowance writes nothing. No signing, send, deposit dispatch, destination credit or paid acceptance.",
    "prior_operation_authorization", "An exact saved unsigned operation and active owner policy are required.",
    { terminal: [], non_terminal: ["approval_required", "preflight_blocked", "approval_skipped"] },
    [], ["apn relay arbitrum approval-check --profile default --operation <operation-id> --rpc-url https://arbitrum-rpc.example"]),
  command(["relay", "arbitrum", "approval-execute"], "apn relay arbitrum approval-execute --profile default --operation <operation-id> --rpc-url <arbitrum-rpc>",
    "Foreground-confirm and submit one exact saved Arbitrum USDC approval.",
    [profileRequired, operationRequired, rpcRequired], "payment_submit",
    "Requires the pinned active owner policy, fresh canonical allowance, balance, nonce and fee checks, an encrypted local wallet, and a durable single-send marker. A started approval is observation-only on replay. Never dispatches the deposit.",
    "foreground_tty", "After saved operation validation and before signing or submission.",
    { terminal: [], non_terminal: ["approval_not_needed", "approval_submitted", "observation_only"] },
    [{ command_path: ["relay", "arbitrum", "observe"], when: "Observe the saved approval transaction at safe finality." }],
    ["apn relay arbitrum approval-execute --profile default --operation <operation-id> --rpc-url https://arbitrum-rpc.example"]),
  command(["relay", "arbitrum", "deposit-dispatch"], "apn relay arbitrum deposit-dispatch --profile default --operation <operation-id> --rpc-url <arbitrum-rpc>",
    "Foreground-confirm and submit one exact saved Arbitrum USDC Relay deposit.",
    [profileRequired, operationRequired, rpcRequired], "payment_submit",
    "Requires a confirmed approval or verified approval skip, pinned active owner policy, fresh canonical allowance, balance, nonce and fee checks, encrypted local wallet and durable single-send marker. A started deposit is observation-only on replay. At most 24 physical RPC POSTs under persisted 750 ms pacing; 429 ends the invocation. Ethereum delivery and paid acceptance remain unproven.",
    "foreground_tty", "After saved operation validation and before deposit signing or submission.",
    { terminal: [], non_terminal: ["deposit_submitted", "observation_only"] },
    [{ command_path: ["relay", "arbitrum", "observe"], when: "Observe the saved deposit transaction at safe finality." }],
    ["apn relay arbitrum deposit-dispatch --profile default --operation <operation-id> --rpc-url https://arbitrum-rpc.example"]),
