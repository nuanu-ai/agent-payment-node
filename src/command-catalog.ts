...
export { renderHelp, renderReadmeCommandReference };export { assertCompatibleManifestEvolution, validateCommandManifest };
...
export interface CommandOption {
  ...
  readonly default: { readonly kind: "none" } | { readonly kind: "literal"; readonly value: string };
...
export interface CommandDefinition {
  ...
  readonly effect: { readonly class: EffectClass; readonly summary: string };
  readonly approval: { readonly class: ApprovalClass; readonly when: string };
  readonly output: {
  ...
  readonly states: { readonly terminal: readonly string[]; readonly non_terminal: readonly string[] };
...
const defaultZero = { kind: "literal", value: "0" } as const;
...
const allOperationStates = {
  terminal: [...directStates.terminal, ...x402States.terminal.filter((state) => !directStates.terminal.includes(state as never))],
  non_terminal: [...directStates.non_terminal, ...x402States.non_terminal.filter((state) => !directStates.non_terminal.includes(state as never))],
} as const;
...
const BASE_COMMANDS: readonly CommandDefinition[] = [
  ...
  command(["stargate", "token", "prepare"], "apn stargate token prepare --profile <profile> --amount-atomic <usdc> [--native-drop-atomic <wei>] --min-output-atomic <usdc> --max-native-debit-atomic <wei> --idempotency-key <key>", "Freeze the exact Optimism USDC to Polygon USDC plus native-drop operation.", [profileRequired, option("--amount-atomic", "atomic_usdc", true, noDefault, ["positive_integer"], "operator_input"), option("--native-drop-atomic", "string", false, defaultZero, ["canonical_uint128", "default_zero", "executor_cap_when_nonzero"], "operator_input"), option("--min-output-atomic", "atomic_usdc", true, noDefault, ["positive_integer"], "operator_input"), option("--max-native-debit-atomic", "wei", true, noDefault, ["positive_integer"], "operator_input"), option("--idempotency-key", "idempotency_key", true, noDefault, ["8_to_128_safe_ascii_characters"], "operator_input")], "payment_prepare", "Reads both chains, the Executor cap, policy and allowance; writes exact unsigned approval/send envelopes.", "none", "Never.", { terminal: [], non_terminal: ["prepared"] }, [{ command_path: ["stargate", "token", "execute"], when: "After reviewing the frozen operation." }], ["apn stargate token prepare --profile owner --amount-atomic 100000 --native-drop-atomic 50000000000000000 --min-output-atomic 99000 --max-native-debit-atomic 20000000000000000 --idempotency-key op-polygon-usdc-0001"]),
  command(["stargate", "token", "execute"], "apn stargate token execute --operation <operation-id>", "Foreground-confirm and execute the exact durable approval and bridge effects.", [operationRequired], "payment_submit", "May sign each frozen effect once after fresh quote, cap, policy, allowance and simulation checks.", "foreground_tty", "Before any signature.", { terminal: ["observed"], non_terminal: ["approved", "allowance_submission_started", "allowance_unknown_finality", "allowance_submitted", "allowance_observed", "submission_started", "submitted", "unknown_finality"] }, [{ command_path: ["stargate", "token", "status"], when: "Inspect local durable state." }], ["apn stargate token execute --operation <operation-id>"]),
  command(["stargate", "token", "cleanup"], "apn stargate token cleanup --operation <operation-id>", "Explicitly revoke a stranded exact Stargate token allowance.", [operationRequired], "payment_submit", "Freshly freezes, foreground-confirms, marks and submits one approve-zero transaction; ambiguous results are never resent.", "foreground_tty", "Immediately before signing the revoke transaction.", { terminal: ["cleaned"], non_terminal: ["cleanup_required", "cleanup_submission_started", "cleanup_submitted", "cleanup_unknown_finality"] }, [{ command_path: ["stargate", "token", "observe"], when: "Observe an attempted cleanup without signing." }], ["apn stargate token cleanup --operation <operation-id>"]),
  command(["stargate", "token", "observe"], "apn stargate token observe --operation <operation-id>", "Observe safe approval, source and destination evidence without resending.", [operationRequired], "network_read", "Reads both chains and only advances an attempted operation.", "prior_operation_authorization", "A durable attempt marker and transaction hash already exist.", { terminal: ["observed"], non_terminal: ["allowance_unknown_finality", "allowance_submitted", "submitted", "unknown_finality"] }, [], ["apn stargate token observe --operation <operation-id>"]),
  command(["stargate", "token", "status"], "apn stargate token status --operation <operation-id>", "Read the local Stargate token journal.", [operationRequired], "local_read", "Reads one owner-only journal record and performs no RPC.", "none", "Never.", { terminal: ["observed"], non_terminal: ["prepared", "approved", "allowance_submission_started", "allowance_unknown_finality", "allowance_submitted", "allowance_observed", "submission_started", "submitted", "unknown_finality"] }, [], ["apn stargate token status --operation <operation-id>"]),
  command(["stargate", "token", "receipt"], "apn stargate token receipt --operation <operation-id>", "Read the local canonical receipt after exact destination proof.", [operationRequired], "local_read", "Reads one observed journal record and performs no RPC.", "none", "Never.", { terminal: ["observed"], non_terminal: [] }, [], ["apn stargate token receipt --operation <operation-id>"]),
  ...EVM_COMMANDS,
  ...
  command(
    ...
    "local_write",
    "Runs foreground provider authentication and may persist an encrypted local session plus exact provider grant and safe public binding facts.",
    "foreground_tty",
    "Authentication stays foreground: terminal-native providers use the CLI terminal and Smart Account permission consent opens the browser.",
    completedStates,
    [],
    [
  ...
  command(
    ...
    "Read the locally persisted bounded provider permission without contacting the provider.",
    [profileRequired],
    "local_read",
    "Reads only safe permission identity, bounds, lifecycle, revision and freshness metadata.",
    "none",
    "Never.",
    permissionStates,
    [
  ...
  command(
    ...
    "Foreground-sync the exact persisted permission against MetaMask granted permissions.",
    [profileRequired, option("--expected-revision", "positive_integer", true, noDefault, ["must_equal_current_permission_revision"], "public")],
    "local_write",
    "Opens bounded foreground provider consent, then records only confirmed presence, absence, drift or unverified freshness.",
    "foreground_tty",
    "The human selects the bound account in the foreground MetaMask browser; MCP returns a CLI handoff.",
    permissionStates,
    [],
    ["apn wallet permission sync --profile smart-account --expected-revision 1"],
  ),
  command(
    ["wallet", "permission", "disable"],
    "apn wallet permission disable --profile <profile> --expected-revision <positive-integer>",
    "Disable one local provider permission binding without claiming provider-side revocation.",
    [profileRequired, option("--expected-revision", "positive_integer", true, noDefault, ["must_equal_current_permission_revision"], "public")],
    "local_write",
    "Revision-guards and durably disables future APN effects while retaining safe audit metadata.",
    "none",
    "Never; this is local disable, not provider revoke.",
    permissionStates,
    [],
  ...
  command(
    ...
    "Delete the local session, permission material and profile binding.",
    [profileRequired, option("--expected-revision", "positive_integer", true, noDefault, ["must_equal_current_permission_revision"], "public")],
    "local_write",
    "Deletes only local protected state and warns that MetaMask-side authority may remain.",
    "none",
    "Caller must intentionally name the current revision; no provider revoke is implied.",
    { terminal: ["forgotten", "classified_failure"], non_terminal: [] },
    [],
  ...
  command(
    ...
    "Create, lower or raise owner-approved balance and x402 limits.",
    [
      profileRequired,
    ...
    "local_write",
    "Writes an encrypted wallet-bound profile policy.",
    "foreground_tty",
    "Required when a policy is created or any limit increases; a pure decrease is non-interactive.",
    completedStates,
    [],
  ...
  command(
    ["x402", "fetch", "prepare"],
    "apn x402 fetch prepare --profile <profile> --url <https-url> --idempotency-key <key> --rpc-url <https-url> [--max-amount-atomic <atomic>]" + httpSynopsis,
    "Freeze a policy-bounded standard x402 purchase.",
    [
      profileRequired,
      ...httpOptions,
      option("--url", "https_url", true, noDefault, ["credential_free_https_without_fragment", "maximum_2048_utf8_bytes", "canonical_whatwg_serialization", "public_target_required_at_runtime"], "public"),
    ...
    "payment_prepare",
    "Reads seller and Base evidence, then durably freezes one purchase intent without signing.",
    "prior_profile_policy",
    "An owner-approved profile policy must already bound the purchase.",
    x402States,
    [{ command_path: ["x402", "fetch", "approve"], when: "After a successful prepare returns an operation ID." }],
    ["apn x402 fetch prepare --profile default --url https://seller.example/resource --idempotency-key <idempotency-key> --rpc-url <https-base-rpc-url>"],
  ),
  command(
    ["x402", "fetch", "approve"],
  ...
  command(
    ...
    "Freeze one exact direct Base-USDC transfer.",
    [
      profileRequired,
      option("--idempotency-key", "idempotency_key", true, noDefault, ["8_to_200_safe_ascii_characters"], "operator_input"),
    ...
    "payment_prepare",
    "Reads Base evidence and provider authority and balances where applicable, then durably freezes one exact intent without child creation or signing.",
    "none",
    "Prepare does not authorize submission.",
    directStates,
    [{ command_path: ["pay", "transfer", "approve"], when: "After a successful prepare returns an operation ID." }],
    ["apn pay transfer prepare --profile default --idempotency-key <idempotency-key> --to <recipient-address> --amount-usdc 0.01 --rpc-url <https-base-rpc-url>"],
  ),
  command(
    ["pay", "transfer", "approve"],
    "apn pay transfer approve --operation <operation-id> --rpc-url <https-url>",
    "Approve, sign and submit one frozen direct transfer.",
    [operationRequired, rpcRequired],
    "payment_submit",
    "After an exact foreground approval, provider-atomic profiles invoke their bound sender; Smart Account profiles derive one exact child authority, seal one DelegationManager transaction and submit it.",
    "foreground_tty",
    "Always requires the exact operation-bound phrase in foreground stdin/stderr TTYs. It never reopens MetaMask permission consent or widens the root grant.",
    directStates,
    [
      { command_path: ["operation", "status"], when: "To inspect any returned non-terminal state." },
      { command_path: ["operation", "resume"], when: "When documented durable recovery is permitted." },
  ...
  command(["operation", "abandon"], "apn operation abandon --operation <operation-id>",
    "Administratively terminalize one strictly eligible unknown provider, expired local rail or expired gasless effect.", [operationRequired], "local_write",
    "After exact owner confirmation, appends an abandonment transition and authoritative receipt; expired local rail eligibility uses read-only RPC checks, gasless eligibility first retries read-only observation, and it never signs, submits or relays.",
    "foreground_tty", "Requires the exact operation-bound phrase and acknowledgement that the financial outcome remains unknown.",
  ...
  command(
    ...
    "prior_operation_authorization",
    "Uses the authorization already bound to the durable operation; it cannot widen the frozen effect.",
    allOperationStates,
    [
