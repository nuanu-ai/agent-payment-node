...
export function evmAddressLock(address: string): string {
  if (!ADDRESS.test(address)) throw new ApnError("APN_INVALID_INPUT", "EVM address is invalid.");
...
export async function assertExclusiveEvmOwner(state: StateStore, address: string, ownProfileHash: string): Promise<void> {
  ...
  const check = (ownerHash: string, candidate: string): void => {
...
export async function assertExclusiveRelayExecutionOwner(
  state: StateStore,
  permissions: Pick<EncryptedSmartAccountPermissionStore, "listAll">,
  address: string,
  ownProfileHash: string,
): Promise<void> {
  if (!ADDRESS.test(address) || !PROFILE_HASH.test(ownProfileHash)) {
...
export async function assertExclusiveEvmOwnerIncludingGrants(
  state: StateStore,
  permissions: Pick<EncryptedSmartAccountPermissionStore, "listAll">,
  address: string,
  ownProfileHash: string,
