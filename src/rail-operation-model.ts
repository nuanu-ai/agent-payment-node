...
const RECORD_KEYS = ["schemaVersion", "kind", "operationId", "profile", "profileHash", "idempotencyHash", "requestHash",
  ...
  "rawPayloadHash", "evidence", "createdAt", "updatedAt", "transitions", "integrityHash"] as const;
...
export function validateRailContinuity(previous: RailOperationRecord, next: RailOperationRecord): void {
  ...
  if (previous.send !== undefined && canonicalJson(previous.send) !== canonicalJson(next.send)) corrupt();
}
