...
export function newRailOperation(intent: RailIntent): RailOperationRecord {
  const fingerprint = hashObject(intent);
  const entry = {
    state: "awaiting_approval" as const, at: intent.prepared.preparedAt,
...
export function validateRailContinuity(previous: RailOperationRecord, next: RailOperationRecord): void {
  validateRailOperation(previous); validateRailOperation(next);
  ...
  if (previous.terminal || previous.fingerprint !== next.fingerprint || next.transitions.length !== previous.transitions.length + 1 ||
    canonicalJson(next.transitions.slice(0, -1)) !== canonicalJson(previous.transitions)) corrupt();
}

export function validateRailOperation(value: unknown): RailOperationRecord {
  if (!isPlainRecord(value) || !exactKeys(value, ["schemaVersion", "kind", "operationId", "profile", "profileHash", "idempotencyHash", "requestHash", "account", "prepared", "policyHash", "fingerprint", "state", "terminal", "reason", "proofClass", "transactionId", "rawPayloadHash", "evidence", "createdAt", "updatedAt", "transitions", "integrityHash"])) corrupt();
  ...
  const operation = value as unknown as RailOperationRecord;
  if (hashObject(intentOf(operation)) !== value.fingerprint) corrupt();
  isoDate(value.createdAt); isoDate(value.updatedAt);
  if (value.createdAt !== prepared.preparedAt || !Array.isArray(value.transitions) || value.transitions.length < 1 || value.transitions.length > 32) corrupt();
  let previous: RailTransition | undefined;
  for (const raw of value.transitions) {
    if (!isPlainRecord(raw) || !exactKeys(raw, ["state", "at", "reason", "proofClass", "transactionId", "rawPayloadHash", "evidence", "previousHash", "transitionHash"])) corrupt();
    if (typeof raw.state !== "string" || !Object.hasOwn(EDGES, raw.state)) corrupt();
    for (const key of ["reason", "proofClass"]) if (typeof raw[key] !== "string" || !/^[a-z][a-z0-9_]{0,95}$/u.test(raw[key])) corrupt();
    isoDate(raw.at);
    const entry = raw as unknown as RailTransition;
    const { transitionHash, ...body } = entry;
    if (hashObject(body) !== transitionHash || entry.previousHash !== (previous?.transitionHash ?? value.fingerprint)) corrupt();
    if (previous === undefined) {
      if (entry.state !== "awaiting_approval" || entry.at !== value.createdAt || entry.transactionId !== null || entry.rawPayloadHash !== null || entry.evidence !== null) corrupt();
    } else {
      if (!EDGES[previous.state].includes(entry.state) || entry.at < previous.at) corrupt();
      if (previous.transactionId !== null && entry.transactionId !== previous.transactionId) corrupt();
      if (previous.rawPayloadHash !== null && entry.rawPayloadHash !== previous.rawPayloadHash) corrupt();
    }
    ...
    if (["completed", "failed_confirmed_revert"].includes(entry.state)) {
      if (entry.evidence === null || entry.transactionId === null) corrupt();
      validateRailEvidence(entry.evidence, prepared, entry.transactionId, entry.state === "completed");
      if (entry.evidence.observedAt > entry.at) corrupt();
    } else if (entry.evidence !== null) corrupt();
    previous = entry;
  }
  if (previous === undefined || canonicalJson(latest(previous)) !== canonicalJson({ state: value.state, reason: value.reason, proofClass: value.proofClass, transactionId: value.transactionId, rawPayloadHash: value.rawPayloadHash, evidence: value.evidence, updatedAt: value.updatedAt })) corrupt();
  if (value.terminal !== TERMINAL.includes(previous.state)) corrupt();
...
export function validateRailPrepared(value: unknown, account: ChainAccount): RailPreparedTransfer {
  ...
  if (value.expiresAt <= value.preparedAt || Date.parse(value.expiresAt) - Date.parse(value.preparedAt) > 300_000) corrupt();
  if (value.lastValidBlockHeight !== null) atomic(value.lastValidBlockHeight);
  if (account.provider === "local") {
    if (typeof value.unsignedPayload !== "string" || value.unsignedPayload.length < 1 || value.unsignedPayload.length > 16_384) corrupt();
  } else if (value.unsignedPayload !== null) corrupt();
...
export function validateRailEvidence(value: unknown, prepared: RailPreparedTransfer, transactionId: string, success: boolean): asserts value is RailFinalEvidence {
  ...
  if (atomic(value.actualNetworkFeeAtomic) > atomic(prepared.economics.networkFeeMaximumAtomic) || atomic(value.actualRecipientRentAtomic) > atomic(prepared.economics.recipientRentAtomic)) corrupt();
...
export function publicRailOperation(operation: RailOperationRecord) {
  validateRailOperation(operation);
...
export function railNextActions(operation: RailOperationRecord): readonly string[] {
  if (operation.terminal) return [];
  if (operation.state === "awaiting_approval") return [`apn pay transfer approve --operation ${operation.operationId}`];
  if (operation.state === "unknown_finality" && operation.transactionId === null) return ["Manual provider reconciliation is required; do not repeat the send."];
  return [`apn operation resume --operation ${operation.operationId}`];
}
export function railReceipt(operation: RailOperationRecord) {
  const body = { ...publicRailOperation(operation), schema_version: "apn.rail-receipt.v1" as const, operation_binding_hash: operation.integrityHash };
...
function seal(body: Omit<RailOperationRecord, "integrityHash">): RailOperationRecord { return validateRailOperation({ ...body, integrityHash: hashObject(body) }); }
function addressShape(value: unknown, rail: ChainAccount["rail"]): void {
  if (typeof value !== "string" || !(rail === "solana" ? /^[1-9A-HJ-NP-Za-km-z]{32,44}$/u : /^T[1-9A-HJ-NP-Za-km-z]{33}$/u).test(value)) corrupt();
}
function corrupt(): never { throw new ApnError("APN_STATE_CORRUPT", "The direct-rail operation or evidence binding is invalid."); }
