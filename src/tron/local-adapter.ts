...
import type { TronResourceSnapshot } from "./model.js";
import { tronBandwidthBytes } from "./resource-model.js";
import { readTronWindow, revalidateTronWindow } from "./resources.js";
import { assertTronNetwork, tronBlock, type TronRpcPort } from "./rpc.js";
import { buildTronTransaction, signTronTransaction, unsignedTronPrepared, validateTronEffect } from "./transaction.js";

export class TronLocalAdapter implements DirectRailPort {
  readonly rail = "tron" as const;
  readonly provider = "local" as const;
  readonly execution = "local_signed" as const;
  constructor(private readonly storage: ChainWalletStoragePort, readonly rpc: TronRpcPort, private readonly now: () => Date = () => new Date()) {}
  asset(alias: ChainAssetAlias): ChainAsset { return chainAsset("tron", alias); }
  canonicalAddress(input: string): string { return tronAddress(input); }
  async assertNetwork(): Promise<string> { return await assertTronNetwork(this.rpc); }
  async account(profile: string): Promise<ChainAccount | null> {
    const account = await this.storage.account(profile, "tron");
    if (account !== null && (account.provider !== "local" || tronAddress(account.address) !== account.address)) mismatch();
    return account;
  }
  async ensureAccount(profile: string): Promise<ChainAccount> {
    return await this.storage.ensureLocal({ profile, rail: "tron", create: async () => {
      const seed = randomBytes(32); const bytes = [...seed];
      try { return { seed, address: tronAddress(utils.crypto.getBase58CheckAddress(utils.crypto.getAddressFromPriKey(bytes))) }; }
      catch { seed.fill(0); throw new ApnError("APN_NATIVE_REJECTED", "The separate TRON key could not be generated."); }
      finally { bytes.fill(0); }
    } });
  }
  async balance(account: ChainAccount, asset: ChainAsset): Promise<ChainBalance> {
    await this.currentAccount(account); this.requireAsset(asset); await this.assertNetwork();
    if (asset.kind === "token") await requireTronUsdt(this.rpc);
  ...
  async prepare(input: { readonly account: ChainAccount; readonly asset: ChainAsset; readonly recipient: string; readonly amountAtomic: string; readonly maximumFeeAtomic: string; readonly now: Date }): Promise<RailPreparedTransfer> {
    ...
    await this.currentAccount(account); this.requireAsset(asset); tronAddress(input.recipient);
    if (input.recipient === account.address) throw new ApnError("APN_INVALID_INPUT", "The TRON recipient must differ from the sender.");
    atomic(input.amountAtomic, true); const cap = atomic(input.maximumFeeAtomic, true); await this.assertNetwork();
    const window = await readTronWindow(this.rpc, this.now); const p = window.parameters;
    const owner = await readTronAccount(this.rpc, account.address); requireTronOwner(owner, account.address);
    ...
    if (solid.number > window.header.number || asset.kind === "native" && !recipient.normal) throw new ApnError("APN_OPERATION_BLOCKED", "The TRON recipient or reference state is not admitted by this transfer profile.");
    let tokenBalance = 0n; let estimate = 0n;
    if (asset.kind === "token") {
      await requireTronUsdt(this.rpc); tokenBalance = await tronUsdtBalance(this.rpc, account.address);
      if (tokenBalance < atomic(input.amountAtomic)) insufficientAsset();
    ...
    if (asset.kind === "token") {
      ...
      if (estimate > energyLimit / atomic(p.energyPriceAtomic)) feeExceeded();
    }
    const transaction = buildTronTransaction({ ...intent, energyFeeLimitAtomic: energyLimit.toString() });
    const rawSize = BigInt(transaction.raw_data_hex.length / 2); const bytes = tronBandwidthBytes(rawSize); const bandwidth = bytes * atomic(p.bandwidthPriceAtomic);
    ...
    if (total > cap) feeExceeded();
    requireFunds(owner.balance, tokenBalance, atomic(input.amountAtomic), total, asset.kind === "native");
    if (BigInt(this.now().getTime()) + 10_000n >= window.expiration) tronReprepare();
    const resources: TronResourceSnapshot = { schemaVersion: "apn.tron-resources.v1", protocolVersion: "4.8.2.1", parameters: p, parameterHash: hashObject(p),
  ...
  async revalidate(account: ChainAccount, prepared: RailPreparedTransfer): Promise<void> {
    await this.currentAccount(account); validateRailPrepared(prepared, account); unsignedTronPrepared(prepared); await this.assertNetwork();
    const snapshot = prepared.resources; if (snapshot === undefined) mismatch();
    ...
    if (prepared.asset.kind === "native") {
      ...
      if (!recipient.normal || snapshot.recipientActivatedAtSolidHead && !recipient.exists) tronReprepare();
    } else {
      await requireTronUsdt(this.rpc); token = await tronUsdtBalance(this.rpc, account.address);
      const estimate = await tronEnergyEstimate(this.rpc, account.address, prepared.recipient, prepared.amountAtomic);
      if (estimate > atomic(snapshot.energyFeeLimitAtomic) / atomic(snapshot.parameters.energyPriceAtomic)) tronReprepare();
    }
    ...
    if (BigInt(this.now().getTime()) + 10_000n >= atomic(snapshot.expirationMsAtomic)) tronReprepare();
  }
  async sign(binding: RailEffectBinding): Promise<RailSignedEffect> {
    const previous = await this.recoverEffect(binding); if (previous !== null) return previous;
    ...
    const effect = await this.storage.withSeed(binding.account, async (seed) => {
      const transaction = signTronTransaction(binding.prepared, seed); const rawPayload = canonicalJson(transaction);
    ...
    validateTronEffect(binding.prepared, effect); await this.storage.saveEffect(binding.account, effect); return effect;
  }
  async recoverEffect(binding: RailEffectBinding): Promise<RailSignedEffect | null> {
    ...
    if (effect !== null) validateTronEffect(binding.prepared, effect); return effect;
  }
  async submit(binding: RailEffectBinding, effect: RailSignedEffect | null): Promise<{ readonly transactionId: string }> {
    ...
    const stored = await this.recoverEffect(binding); if (stored === null || canonicalJson(stored) !== canonicalJson(effect)) mismatch();
    const transaction = validateTronEffect(binding.prepared, effect); await this.revalidate(binding.account, binding.prepared);
    const result = tronRecord(await this.rpc.call("wallet/broadcasttransaction", transaction as unknown as Readonly<Record<string, unknown>>));
    if (result.result !== true || result.txid !== effect.transactionId) throw new ApnError("APN_RPC_AMBIGUOUS", "TRON did not acknowledge the exact frozen transaction identity.");
    return { transactionId: effect.transactionId };
  }
  async inspect(account: ChainAccount, prepared: RailPreparedTransfer, transactionId: string, expectedRawPayloadHash?: string): Promise<RailInspection> {
...
function requireFunds(native: bigint, token: bigint, amount: bigint, maximumFee: bigint, isNative: boolean): void {
  if (!isNative && token < amount) insufficientAsset();
  if (native < maximumFee + (isNative ? amount : 0n)) throw new ApnError("APN_INSUFFICIENT_GAS", "The TRON balance does not cover the full frozen native debit bound.");
}
