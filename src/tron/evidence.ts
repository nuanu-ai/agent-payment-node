...
import type { TronFinalResources } from "./model.js";
...
export async function inspectTron(rpc: TronRpcPort, account: ChainAccount, prepared: RailPreparedTransfer, transactionId: string, expectedRawPayloadHash: string, now: Date): Promise<RailInspection> {
  try { return await inspect(rpc, account, prepared, transactionId, expectedRawPayloadHash, now); }
  catch { return { status: "unproven", reason: "solidified_transaction_effect_not_proven" }; }
}
async function inspect(rpc: TronRpcPort, account: ChainAccount, prepared: RailPreparedTransfer, transactionId: string, expectedRawPayloadHash: string, now: Date): Promise<RailInspection> {
  await assertTronNetwork(rpc); tronHash(transactionId);
  if (account.address !== prepared.sender || unsignedTronPrepared(prepared).txID !== transactionId) tronProtocolFailure();
  ...
  if (Object.keys(body).length === 0 || Object.keys(info).length === 0) return { status: "pending", reason: "solidified_history_absent" };
  const signed = validateTronChainTransaction(body, prepared);
  if (sha256(canonicalJson(signed)) !== expectedRawPayloadHash) tronProtocolFailure();
  if (info.id !== transactionId) tronProtocolFailure();
  ...
  if (block.number !== blockNumber || tronAtomic(info.blockTimeStamp) !== block.timestamp) tronProtocolFailure();
  const snapshot = prepared.resources; if (snapshot === undefined || blockNumber <= atomic(snapshot.referenceBlockNumberAtomic)) tronProtocolFailure();
  ...
  if (parent.number + 1n !== blockNumber || parent.id !== parentHash || parent.timestamp + 3000n > block.timestamp ||
    ...
    parent.number === atomic(snapshot.referenceBlockNumberAtomic) && parent.id !== snapshot.referenceBlockId) tronProtocolFailure();
  const members = tronArray(block.body.transactions, 4000).map(tronRecord).filter((item) => item.txID === transactionId);
  if (members.length !== 1 || canonicalJson(validateTronChainTransaction(members[0], prepared)) !== canonicalJson(signed)) tronProtocolFailure();
  const ret = resultRow(body); const blockRet = resultRow(members[0]);
  if (canonicalJson(ret) !== canonicalJson(blockRet)) tronProtocolFailure();
  const receipt = tronRecord(info.receipt);
  const overall = zeroEnum(info.result, "SUCESS"); const transactionOverall = zeroEnum(ret.ret, "SUCESS");
  const contractResult = ret.contractRet; const vmResult = zeroEnum(receipt.result, "DEFAULT");
  ...
  if (!success && !reverted || success && info.resMessage !== undefined && info.resMessage !== "") tronProtocolFailure();
  if (prepared.asset.kind === "token") {
    await requireTronUsdt(rpc);
    if (info.contract_address !== undefined && info.contract_address !== "" && info.contract_address !== TRON_USDT_HEX) tronProtocolFailure();
    const logs = info.log === undefined ? [] : tronArray(info.log, 32);
    if (success) {
      if (logs.length !== 1) tronProtocolFailure();
      const log = tronRecord(logs[0]); const topics = tronArray(log.topics, 3);
      if (log.address !== TRON_USDT_HEX.slice(2) || topics.length !== 3 || topics[0] !== TRON_TRANSFER_TOPIC || topics[1] !== tronWord(prepared.sender) || topics[2] !== tronWord(prepared.recipient) || log.data !== atomic(prepared.amountAtomic).toString(16).padStart(64, "0")) tronProtocolFailure();
    } else if (logs.length !== 0) tronProtocolFailure();
  } else if (info.log !== undefined && tronArray(info.log).length !== 0) tronProtocolFailure();
  const head = tronBlock(await rpc.call("walletsolidity/getnowblock", {}));
  if (head.number < blockNumber || head.timestamp < block.timestamp) tronProtocolFailure();
  ...
  const total = tronAtomic(info.fee, true); const net = tronAtomic(receipt.net_fee, true); const energy = tronAtomic(receipt.energy_fee, true);
  if (total < net + energy) tronProtocolFailure();
  const resources: TronFinalResources = { schemaVersion: "apn.tron-resource-evidence.v1", parentBlockId: parent.id, parentBlockNumberAtomic: parent.number.toString(),
  ...
  const evidence: RailFinalEvidence = { networkIdentity: TRON_GENESIS, transactionId, blockNumberAtomic: block.number.toString(), blockId: block.id, finality: "solidified",
...
function resultRow(value: unknown): Record<string, unknown> {
  const rows = tronArray(tronRecord(value).ret, 1); if (rows.length !== 1) tronProtocolFailure(); const row = tronRecord(rows[0]);
  if (Object.keys(row).some((key) => !["contractRet", "ret", "fee"].includes(key))) tronProtocolFailure();
  return { contractRet: row.contractRet, ret: zeroEnum(row.ret, "SUCESS"), fee: tronAtomic(row.fee, true).toString() };
}
function zeroEnum(value: unknown, zero: string): unknown { return value === undefined || value === 0 || value === 0n ? zero : value; }
