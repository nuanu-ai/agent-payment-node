...
export const tronHttpsFetch: typeof fetch = async (input, init) => {
  ...
  return await new Promise<Response>((resolve, reject) => {
    ...
    const finish = (response?: Response): void => {
      ...
      if (response === undefined) reject(new ApnError("APN_RPC_PROTOCOL", "The bounded TRON HTTPS transport did not return valid evidence."));
...
async function resolveBounded(endpoint: URL, signal: AbortSignal): Promise<readonly PinnedAddress[]> {
  ...
  try {
    return await Promise.race([resolvePublicAddresses(endpoint, "APN_RPC_CONFIG", "TRON RPC endpoint"), new Promise<never>((_resolve, reject) => {
      abort = () => reject(new ApnError("APN_RPC_PROTOCOL", "TRON RPC address validation exceeded its deadline."));
