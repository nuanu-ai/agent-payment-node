...
export class TronRpc implements TronRpcPort {
  ...
  async call(method: TronMethod, body: Readonly<Record<string, unknown>>): Promise<unknown> {
    ...
    const controller = new AbortController(); const deadline = setTimeout(() => controller.abort(), 10_000); deadline.unref();
    ...
    try {
      ...
      while (true) {
        ...
        length += next.value.byteLength; if (length > 2_097_152) tronProtocolFailure(); chunks.push(next.value);
      }
      ...
      if ("Error" in value || "error" in value) tronProtocolFailure();
      return value;
    } catch (error) {
      if (error instanceof ApnError) {
        if (error.code !== "APN_RPC_PROTOCOL") throw error;
        throw new ApnError(error.code, error.message, { ...error.details, rpcMethod: method });
      }
      throw new ApnError(method === "wallet/broadcasttransaction" ? "APN_RPC_AMBIGUOUS" : "APN_RPC_PROTOCOL",
...
export function tronBlock(value: unknown): TronBlock {
  const body = tronRecord(value); const id = tronHash(body.blockID);
  const raw = tronRecord(tronRecord(body.block_header).raw_data);
  const number = tronAtomic(raw.number, true); const timestamp = tronAtomic(raw.timestamp, true);
  if (number > 0xffff_ffff_ffff_ffffn || id.slice(0, 16) !== number.toString(16).padStart(16, "0")) tronProtocolFailure();
  return { id, number, timestamp, body };
}
export async function assertTronNetwork(rpc: TronRpcPort): Promise<string> {
  const block = tronBlock(await rpc.call("wallet/getblockbynum", { num: 0 }));
  if (block.number !== 0n || block.id !== TRON_GENESIS) throw new ApnError("APN_CHAIN_MISMATCH", "The RPC does not attest the full TRON mainnet genesis.");
  return TRON_GENESIS;
}
