...
function boundedX402ReadPort(rpc: RpcPort, timeoutMs: number): X402RpcPort | null {
...
async function assertWaitRpcProvenance(
  rpc: X402RpcPort,
  operation: X402OperationRecord,
): Promise<void> {
  const chain = await rpc.assertBaseChain();
...
function isPostExposureWaitState(operation: X402OperationRecord): boolean {
