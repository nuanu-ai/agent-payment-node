  ...
  try {
    ...
    if (bound.request.command === "operation.abandon") {
      ...
      return failureEnvelope(bound.request.command, randomUUID(), new ApnError(
        ...
        "Review the unresolved financial risk and confirm abandonment in the foreground CLI.",
    ...
    if (bound.request.command === "allowlist.policy.activate" || bound.request.command === "allowlist.policy.revoke") {
      ...
      return failureEnvelope(bound.request.command, randomUUID(), new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED",
        `Review every admission, cap and pin and ${action} this allowlist revision in the foreground CLI.`,
        { ...cliHandoffDetails(handoff), foreground_auth: true, approval_boundary: "foreground_tty" }));
    }
    if (bound.request.command === "bridge.approve") {
      ...
      return failureEnvelope(bound.request.command, randomUUID(), new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED",
        "Review and approve this bridge intent in the foreground CLI.", { ...cliHandoffDetails(handoff), foreground_auth: true }));
    }
    ...
    if (bound.request.command === "swap.jupiter.approve" || bound.request.command === "swap.jupiter.execute") {
      const action = bound.request.command.endsWith(".approve") ? "approve" : "execute";
    ...
    if (bound.request.command === "swap.orca.approve" || bound.request.command === "swap.orca.execute") {
      ...
      return failureEnvelope(bound.request.command, randomUUID(), new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED",
        "Guarded swap approval and execution must continue in the foreground CLI.", { ...cliHandoffDetails(handoff), foreground_auth: true }));
    }
    if (bound.request.command === "oneclick.source.submit") {
      return failureEnvelope(bound.request.command, randomUUID(), new ApnError("APN_FOREGROUND_APPROVAL_REQUIRED",
        "Review and approve this exact 1Click deposit in the foreground CLI.", { ...cliHandoffDetails(oneClickSubmitHandoff(bound.request)), foreground_auth: true }));
    }
    ...
    const policyApproval = bound.request.command === "wallet.policy.set"
      ...
      : undefined;
    if (bound.request.command === "policy.admit-solana" || bound.request.command === "policy.admit-tron") chainPolicyHandoff(bound.request);
