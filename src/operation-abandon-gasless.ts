...
import { FACILITATOR_EXPOSED, type FacilitatorOperationRecord } from "./facilitator-gasless/operation-model.js";
...
export async function abandonFacilitatorGasless(d: Base & { readonly facilitatorGasless: FacilitatorGaslessService },
  operationId: string): Promise<unknown> {
  const initial = await facilitatorRecord(d, operationId);
  if (initial.state === "abandoned_unknown" && initial.terminal) return publicFacilitatorOperation(initial);
  ...
  return await d.context.state.withLocks([`profile:${initial.profileHash}`, `operation:${operationId}`], async () => {
    ...
    if (op.terminal) return publicFacilitatorOperation(op);
    ...
    const next = transitionFacilitator(op, { state: "abandoned_unknown", failure: "facilitator_gasless_owner_abandoned" },
      d.context.clock.now().toISOString());
...
async function facilitatorRecord(d: Base, operationId: string): Promise<FacilitatorOperationRecord> {
...
function assertAuthorizationEnded(d: Base, op: FacilitatorOperationRecord): void {
  ...
  if (op.terminal || !FACILITATOR_EXPOSED.includes(op.state) || op.settlement !== null || validBefore === null ||
    ...
    throw new ApnError("APN_OPERATION_BLOCKED",
      "Only an exposed Avalanche facilitator transfer with an unproven outcome can be abandoned, and only after its authorization has expired.");
  }
}
