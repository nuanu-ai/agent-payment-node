...
import type { publicFacilitatorOperation } from "./receipt.js";
import { AVALANCHE_FACILITATOR as R } from "./registry.js";

export class TtyFacilitatorApproval implements GaslessApprovalPort {
  ...
  async confirm(input: Parameters<GaslessApprovalPort["confirm"]>[0]): Promise<boolean> {
    const s = input.summary as ReturnType<typeof publicFacilitatorOperation>, t = s.transfer;
    const usdc = (atomic: string) => `${formatUnits(BigInt(atomic), R.decimals)} USDC`;
