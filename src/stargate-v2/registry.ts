...
import { NATIVE_ASSET_ADDRESS, type EvmAssetSelection } from "../evm-asset.js";
...
const row = (chainId: DirectEvmChainId, eid: number, asset: StargateV2Asset, token: string, pool: string,
  kind: StargateV2Deployment["kind"]): StargateV2Deployment =>
  Object.freeze({ chainId, eid, asset, token: getAddress(token), pool: getAddress(pool), kind,
...
export const STARGATE_V2_DEPLOYMENTS = Object.freeze([
  row(1, 30101, "ETH", NATIVE_ASSET_ADDRESS, "0x77b2043768d28E9C9aB44E1aBfC95944bcE57931", "oft"),
  ...
  row(8453, 30184, "ETH", NATIVE_ASSET_ADDRESS, "0xdc181Bd607330aeeBEF6ea62e03e5e1Fb4B6F7C7", "oft"),
  row(8453, 30184, "USDC", "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", "0x27a16dc786820B16E5c9028b75B99F6f604b5d26", "pool"),
  row(42161, 30110, "ETH", NATIVE_ASSET_ADDRESS, "0xA45B5130f36CDcA45667738e2a258AB09f4A5f7F", "oft"),
  row(42161, 30110, "USDC", "0xaf88d065e77c8cC2239327C5EDb3A432268e5831", "0xe8CDF27AcD73a434D661C84887215F7598e7d0d3", "pool"),
  row(10, 30111, "ETH", NATIVE_ASSET_ADDRESS, "0xe8CDF27AcD73a434D661C84887215F7598e7d0d3", "oft"),
  ...
  row(130, 30320, "ETH", NATIVE_ASSET_ADDRESS, "0xe9aBA835f813ca05E50A6C0ce65D0D74390F7dE7", "oft"),
] as const);
...
export function stargateV2Deployment(chainId: unknown, token: unknown): StargateV2Deployment {
  if (typeof chainId !== "number" || !Number.isSafeInteger(chainId)) return fail("unknown chain");
  let address: Address;
  try { address = getAddress(token as string); } catch { return fail("unknown token"); }
...
export function stargateV2Route(source: EvmAssetSelection, destination: EvmAssetSelection) {
  ...
  if (from.chainId === to.chainId) return fail("source and destination chain are identical");
  if (from.asset !== to.asset) return fail("source and destination asset identities differ");
