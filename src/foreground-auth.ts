...
export class TtyForegroundAuthentication implements ForegroundAuthenticationPort {
  private readonly openTerminal: () => Promise<AuthTerminal>;
  ...
  async readChallengeResponse(): Promise<string> {
  ...
  private async prompt(label: string, validate: (value: string) => string, secret = false): Promise<string> {
    let terminal: AuthTerminal;
    try {
    ...
    try {
      if (!this.isTerminal(terminal.fd)) throw foregroundFailure();
      await terminal.write(label);
      bytes = secret ? await terminal.readSecretLine() : await terminal.readLine();
      if (secret) await terminal.write("\n");
      if (bytes.length === 0 || bytes.length > MAX_AUTH_INPUT_BYTES) invalidAuth();
...
async function openAuthTerminal(): Promise<AuthTerminal> {
...
export async function readSecretLine(
  ...
  try {
    ...
    return await new Promise<Buffer>((resolve, reject) => {
      ...
      const onData = (chunk: Buffer | string): void => {
        ...
        try {
          for (const byte of incoming) {
            if (byte === 3) return fail("foreground terminal interrupted");
            if (byte === 10 || byte === 13) return finish();
            if (byte === 8 || byte === 127) {
              if (bytes.length > 0) bytes.pop();
              continue;
            }
            if (bytes.length >= MAX_AUTH_INPUT_BYTES) return fail("foreground terminal read failed");
            bytes.push(byte);
          }
        } finally {
          incoming.fill(0);
      ...
      const onError = (): void => fail("foreground terminal read failed");
      const onSigint = (): void => fail("foreground terminal interrupted");
...
async function readLine(input: NodeJS.ReadStream): Promise<Buffer> {
  return await new Promise<Buffer>((resolve, reject) => {
    const bytes: number[] = [];
    const cleanup = (): void => {
      input.pause();
    ...
    const onData = (chunk: Buffer | string): void => {
      ...
      try {
        for (const byte of incoming) {
          if (byte === 10) return finish();
          if (byte === 13) continue;
          if (bytes.length >= MAX_AUTH_INPUT_BYTES) return onError();
          bytes.push(byte);
        }
      } finally {
        incoming.fill(0);
    ...
    const onEnd = (): void => finish();
    const onError = (): void => { bytes.fill(0); cleanup(); reject(new Error("foreground terminal read failed")); };
    const onSigint = (): void => { bytes.fill(0); cleanup(); reject(new Error("foreground terminal interrupted")); };
    input.on("data", onData);
    input.once("end", onEnd);
    input.once("error", onError);
...
function validateIdentity(value: string): string {
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(value) || Buffer.byteLength(value, "utf8") > 320) invalidAuth();
  return value;
}

function validateChallenge(value: string): string {
  if (!/^[0-9]{4,12}$/u.test(value)) invalidAuth();
  return value;
}

function foregroundFailure(): ApnError {
