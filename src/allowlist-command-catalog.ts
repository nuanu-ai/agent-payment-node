import type { CommandDefinition, CommandGroup, CommandOption, ScalarType } from "./command-catalog.js";
...
const option = (name: `--${string}`, required: boolean, constraints: readonly string[], type: ScalarType = "string"): CommandOption => ({
...
const envelope = (success: string): CommandDefinition["output"] => ({ contract: "apn.cli.v1", success_exit: 0, failure_exit: 1,
...
export function bindAllowlistCommand(path: string, options: Readonly<Record<string, string>>): CommandRequest {
  switch (path) {
...
function kind(options: Readonly<Record<string, string>>): "native" | "token" {
...
function expectedRevision(options: Readonly<Record<string, string>>): { readonly expectedRevision?: number } {
...
function revision(selected: string): number {
  if (!/^[1-9][0-9]*$/u.test(selected) || !Number.isSafeInteger(Number(selected))) {
