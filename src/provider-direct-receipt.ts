...
const DURABLE_PRE_EFFECT_REASONS = new Set([
  ...
  "delegated_preflight_unavailable",
  "delegated_preflight_failed",
  "delegated_apn_insufficient_usdc",
  "delegated_apn_insufficient_gas",
  "delegated_apn_permission_inactive",
  "delegated_apn_permission_allowance_insufficient",
  "delegated_apn_profile_drift",
  "delegated_apn_provider_unavailable",
  "delegated_apn_provider_protocol",
  "delegated_apn_rpc_protocol",
...
export function providerDirectReceipt(
  ...
  if (transition === undefined) stateCorrupt("Provider direct operation has no receipt transition.");
  return sealReceipt({
...
export function recoverProviderTerminalOperation(
  operation: OperationRecord,
  receipt: ReceiptRecord | null,
): OperationRecord | null {
  if (receipt === null || receipt.operationIntegrityHash === operation.integrityHash) return null;
...
export function assertProviderTerminalReceiptAuthority(
  operation: OperationRecord,
  receipt: ReceiptRecord | null,
): void {
  if (
    receipt === null || !operation.terminal || !receipt.terminal ||
...
function assertTerminalReceipt(receipt: ReceiptRecord, operation: OperationRecord): void {
  ...
  if (receipt.state === "failed_confirmed_revert") {
    if (
      receipt.reason !== "confirmed_receipt_revert" || receipt.proofClass !== "confirmed_receipt" ||
  ...
  if (!valid) stateCorrupt("Provider direct pre-effect receipt classification is invalid.");
}
