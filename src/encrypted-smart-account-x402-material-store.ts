...
const RECORD_VERSION = "apn.metamask-smart-account-x402-material.v1" as const;
const ENVELOPE_VERSION = "apn.metamask-smart-account-x402-material-envelope.v1" as const;
...
export type UnsealedSmartAccountX402Material = Omit<
  SmartAccountX402MaterialRecord,
  "material_identity_hash" | "integrity_hash"
>;
...
export class EncryptedSmartAccountX402MaterialStore implements SmartAccountX402MaterialStorePort {
  private readonly files: MaterialEnvelopeState;
  ...
  async load(operationId: string): Promise<SmartAccountX402MaterialRecord | null> {
    ...
    if (wrapping === null) corrupt("The Smart Account x402 wrapping secret is missing.");
    try { return decrypt(envelope, wrapping); }
    finally { wrapping.fill(0); }
  }

  async seal(record: UnsealedSmartAccountX402Material): Promise<SmartAccountX402MaterialRecord> {
  ...
  async markExposed(operationId: string, updatedAt: string): Promise<SmartAccountX402MaterialRecord> {
    const current = await this.load(operationId);
    if (current === null) corrupt("The sealed Smart Account x402 material is missing.");
  ...
  private async write(record: SmartAccountX402MaterialRecord): Promise<void> {
...
function decrypt(envelope: MaterialEnvelope, wrapping: Buffer): SmartAccountX402MaterialRecord {
  ...
  try {
    ...
    if (canonicalJson(value) !== text) corrupt("Smart Account x402 material plaintext is not canonical.");
...
function validateRecord(value: unknown): SmartAccountX402MaterialRecord {
  ...
  const record = value as unknown as SmartAccountX402MaterialRecord;
  const { integrity_hash: _integrity, ...body } = record;
  if (
    record.schema_version !== RECORD_VERSION || !hashes(record.operation_id, record.profile_hash, record.fingerprint,
      record.request_hash, record.offer_hash, record.root_grant_fingerprint, record.child_hash,
      record.permission_context_hash, record.payment_payload_hash, record.payment_header_hash,
      record.material_identity_hash, record.integrity_hash) ||
  ...
  let payload: unknown;
  try { payload = JSON.parse(record.payment_payload_canonical_json) as unknown; }
  catch { corrupt("Smart Account x402 payment payload is not JSON."); }
  ...
  let child;
  try { child = decodeDelegations(record.child_permission_context)[0]; }
  catch { corrupt("Smart Account x402 child permission context is invalid."); }
  if (child === undefined || record.child_hash !== domainHash(
    "apn.x402.erc7710.child.v1", encodeDelegations([child]),
  )) corrupt("Smart Account x402 child hash is invalid.");
  return record;
}

function materialIdentity(value: Omit<SmartAccountX402MaterialRecord, "material_identity_hash" | "integrity_hash"> | SmartAccountX402MaterialRecord): string {
  return hashObject({
...
function parseEnvelope(value: unknown, operationId: string): MaterialEnvelope {
  ...
  return value as unknown as MaterialEnvelope;
}
...
function decodeBase64(value: string, length?: number): Buffer {
  ...
  if (decoded.toString("base64") !== value || decoded.length === 0 || (length !== undefined && decoded.length !== length)) {
    decoded.fill(0); corrupt("Smart Account x402 material envelope encoding is invalid.");
  }
  return decoded;
}

function hashes(...values: readonly string[]): boolean { return values.every((value) => /^[a-f0-9]{64}$/u.test(value)); }
function addresses(...values: readonly string[]): boolean { return values.every((value) => /^0x[0-9a-fA-F]{40}$/u.test(value)); }
