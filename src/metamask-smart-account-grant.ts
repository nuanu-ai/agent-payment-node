...
export const BASE_CHAIN_HEX = "0x2105" as const;
export const UINT256_MAX_HEX = `0x${"f".repeat(64)}` as Hex;

export function smartAccountEnvironment() {
  return getSmartAccountsEnvironment(CHAIN_ID);
}

export function validateSmartAccountObservation(
  value: unknown,
  request: SmartAccountPermissionRequestShape,
): ValidatedSmartAccountGrant {
  const observation = parseObservation(value);
  assertPreflight(observation);
  if (observation.permission_responses.length !== 1) mismatch("MetaMask returned an unexpected permission count.");
  ...
  if (
    response.chainId.toLowerCase() !== BASE_CHAIN_HEX ||
  ...
  if (
    permission.type !== "erc20-token-allowance" || permission.isAdjustmentAllowed !== true ||
  ...
  if (
    delegation.delegate.toLowerCase() !== request.sessionAddress.toLowerCase() ||
...
export function assertSmartAccountPreflight(value: unknown): SmartAccountProviderObservation {
  const observation = parseObservation(value);
  assertPreflight(observation);
  return observation;
}

function assertPreflight(observation: SmartAccountProviderObservation): void {
  if (observation.chain_id.toLowerCase() !== BASE_CHAIN_HEX) mismatch("MetaMask is not connected to Base chain 8453.");
  const owner = canonicalAddress(observation.owner_address, "owner");
  ...
  if (!isPlainRecord(supported)) mismatch("MetaMask did not return supported execution permissions.");
...
function validateCaveats(
  ...
  if (config === undefined) mismatch("The pinned MetaMask package has no ERC-20 allowance decoder.");
  const checksummed = caveats.map((item) => ({ ...item, enforcer: getAddress(item.enforcer) }));
  let decoded: Record<string, unknown>;
  try { decoded = config.validateAndDecodeData(checksummed, config.contractAddresses); }
  catch { return mismatch("The signed allowance caveats do not match the pinned Gator contract."); }
  if (
    canonicalAddress(decoded.tokenAddress, "decoded token").toLowerCase() !== BASE_USDC.toLowerCase() ||
  ...
  if (nonce?.terms.length !== 66) mismatch("The Gator nonce caveat is malformed.");
  const required = new Map(Object.entries(config.requiredEnforcers).map(([address, count]) => [address as Hex, count]));
  let decodedRules: unknown[];
  try {
  ...
  } catch { return mismatch("The Gator permission rules are malformed."); }
...
function parseObservation(value: unknown): SmartAccountProviderObservation {
  if (!isPlainRecord(value) || !exactKeys(value, [
    ...
    !isHex(value.account_code) || !Array.isArray(value.permission_responses)) mismatch("MetaMask returned an unsupported bridge response.");
  return value as unknown as SmartAccountProviderObservation;
}

function parsePermissionResponse(value: unknown) {
  if (!isPlainRecord(value) || !exactKeys(value, [
  ...
  return {
    ...
    from: value.from,
    to: value.to,
    permission: value.permission as { type: unknown; isAdjustmentAllowed: unknown; data: Record<string, unknown> },
    rules: value.rules,
...
function decodeRootDelegation(context: Hex) {
  try {
    const delegations = decodeDelegations(context);
    if (delegations.length !== 1 || delegations[0] === undefined) mismatch("Permission context is not one signed root delegation.");
...
function responseExpiry(rules: readonly unknown[]): number {
...
function permissionContracts(enforcers: Record<string, Hex>): EnforcerAddressesByName {
  return {
...
function canonicalAddress(value: unknown, label: string): Address {
  if (typeof value !== "string" || !isAddress(value)) mismatch(`MetaMask ${label} is not a canonical address.`);
  return getAddress(value) as Address;
}

function requiredAddress(value: Hex | undefined): Hex {
  if (value === undefined || !isAddress(value)) mismatch("The pinned MetaMask environment is incomplete.");
  return getAddress(value);
}

function hexQuantity(value: unknown): value is Hex {
