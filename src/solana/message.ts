...
export async function solanaMessage(input: Pick<RailPreparedTransfer, "sender" | "recipient" | "asset" | "amountAtomic" | "blockReference" | "lastValidBlockHeight" | "createsRecipientAccount" | "sourceTokenAccount" | "destinationTokenAccount">) {
  ...
  if (input.lastValidBlockHeight === null) invalid();
  const message = appendTransactionMessageInstructions(instructions,
    setTransactionMessageLifetimeUsingBlockhash({ blockhash: blockhash(input.blockReference), lastValidBlockHeight: BigInt(input.lastValidBlockHeight) },
...
export async function validateSolanaMessage(prepared: RailPreparedTransfer) {
  const expected = await solanaMessage(prepared);
  if (expected.unsignedPayload !== prepared.unsignedPayload) invalid();
  return expected;
}
export async function validateSolanaEffect(prepared: RailPreparedTransfer, effect: RailSignedEffect): Promise<void> {
