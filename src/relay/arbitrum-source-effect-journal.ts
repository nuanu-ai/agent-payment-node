...
export type ArbitrumEffectEvent =
  | { readonly kind: "begin_signing"; readonly role: ArbitrumEffectRole; readonly marker: string; readonly at: string }
  | { readonly kind: "seal_signed"; readonly role: ArbitrumEffectRole; readonly rawTransaction: string; readonly nonce: string }
  | { readonly kind: "mark_submitting"; readonly role: ArbitrumEffectRole; readonly at: string }
  | { readonly kind: "record_send"; readonly role: ArbitrumEffectRole; readonly outcome: "accepted" | "uncertain" }
  | { readonly kind: "record_verified_observation"; readonly role: ArbitrumEffectRole;
      readonly outcome: "confirmed" | "failed"; readonly proofDigest: string };
...
function blocked(reason: string): never { throw new ApnError("APN_OPERATION_BLOCKED", "Relay Arbitrum effect transition refused.", { reason }); }
function corrupt(reason: string): never { throw new ApnError("APN_STATE_CORRUPT", "Relay Arbitrum source effect journal is invalid.", { reason }); }
function fields(journal: ArbitrumSourceEffectJournal): Omit<ArbitrumSourceEffectJournal, "integrityHash"> {
  const { integrityHash: _, ...rest } = journal; return rest;
}
function operationDraft(op: RelayUnsignedOperation) {
  validateRelayUnsignedOperation(op);
  if (op.sourceChainId !== 42161 || op.destinationChainId !== 1 || op.arbitrumDraft === undefined ||
    op.arbitrumDraft.executionAdmitted !== false) blocked("exact_unsigned_arbitrum_operation_required");
  return op.arbitrumDraft;
}
function effectEnvelope(op: RelayUnsignedOperation, role: ArbitrumEffectRole) {
...
async function verifySigned(op: RelayUnsignedOperation, role: ArbitrumEffectRole,
  ...
  if (!TX.test(raw) || raw.length > 16_386 || !UINT.test(nonce)) corrupt("signed_encoding_or_nonce");
  let tx: ReturnType<typeof parseTransaction>, signer: string;
  try {
    tx = parseTransaction(raw as Hex);
    signer = await recoverTransactionAddress({ serializedTransaction: raw as Parameters<typeof recoverTransactionAddress>[0]["serializedTransaction"] });
  } catch { corrupt("signed_decode"); }
  const envelope = effectEnvelope(op, role) as { to: string; data: string; value: string; gas: string;
  ...
  if (tx.type !== "eip1559" || tx.chainId !== 42161 || !same(signer, op.sourceAccount) ||
    ...
    tx.gas * tx.maxFeePerGas > BigInt(ceiling)) corrupt("signed_envelope_drift");
  return keccak256(raw as Hex);
}
function shape(j: ArbitrumSourceEffectJournal, op: RelayUnsignedOperation): void {
  ...
  for (const [index, role] of ["approval", "deposit"].entries()) {
    ...
    if (a === null || !exact(a, ["marker", "markedAt", "rawTransaction", "transactionHash", "nonce",
      ...
      Date.parse(a.markedAt) < Date.parse(j.createdAt)) corrupt("attempt_shape");
    if (effect.phase === "signing_started") {
      if (a.rawTransaction !== null || a.transactionHash !== null || a.nonce !== null ||
        a.submittingAt !== null || a.observationDigest !== null) corrupt("signing_marker");
      continue;
    }
    if (typeof a.rawTransaction !== "string" || !TX.test(a.rawTransaction) || a.rawTransaction.length > 16_386 ||
      typeof a.transactionHash !== "string" || a.transactionHash !== keccak256(a.rawTransaction as Hex) ||
      typeof a.nonce !== "string" || !UINT.test(a.nonce)) corrupt("signed_material");
    if (effect.phase === "sealed" ? a.submittingAt !== null : !iso(a.submittingAt) ||
      Date.parse(a.submittingAt) < Date.parse(a.markedAt)) corrupt("submitting_marker");
    if (["confirmed", "failed"].includes(effect.phase) ? !HASH.test(a.observationDigest ?? "") :
      a.observationDigest !== null) corrupt("observation_binding");
  }
  if (j.effects[1]!.phase !== "pending" && !["confirmed", "approval_skipped"].includes(j.effects[0]!.phase)) corrupt("deposit_order");
  ...
  if (approvalNonce !== null && approvalNonce !== undefined && depositNonce !== null && depositNonce !== undefined &&
    BigInt(depositNonce) <= BigInt(approvalNonce)) corrupt("nonce_order");
}
export async function validateArbitrumSourceEffectJournal(value: unknown,
  op: RelayUnsignedOperation): Promise<ArbitrumSourceEffectJournal> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) corrupt("journal_shape");
  const j = value as ArbitrumSourceEffectJournal;
  try {
    shape(j, op);
...
export async function createArbitrumSourceEffectJournal(op: RelayUnsignedOperation,
  createdAt: string): Promise<ArbitrumSourceEffectJournal> {
  operationDraft(op);
  if (!iso(createdAt)) blocked("creation_time");
  const pending = (role: ArbitrumEffectRole): ArbitrumSourceEffect => ({ role, phase: "pending", attempt: null });
  const data = { schemaVersion: "apn.relay-arbitrum-source-effect-journal.v1" as const,
    ...
    effects: [pending("approval"), pending("deposit")] as const, createdAt };
  return validateArbitrumSourceEffectJournal({ ...data, integrityHash: hashObject(data) }, op);
}
export async function advanceArbitrumSourceEffectJournal(j: ArbitrumSourceEffectJournal,
  op: RelayUnsignedOperation, event: ArbitrumEffectEvent): Promise<ArbitrumSourceEffectJournal> {
  ...
  if (event.role === "deposit" && !["confirmed", "approval_skipped"].includes(j.effects[0]!.phase)) blocked("approval_not_confirmed_or_verified_skipped");
  let next: ArbitrumSourceEffect;
  switch (event.kind) {
    case "begin_signing":
      if (current.phase !== "pending" || !HASH.test(event.marker) || !iso(event.at) ||
        Date.parse(event.at) < Date.parse(j.createdAt)) blocked("duplicate_or_invalid_signing_marker");
      ...
      break;
    case "seal_signed": {
      if (current.phase !== "signing_started") blocked("signed_material_requires_marker");
    ...
    case "mark_submitting":
      if (current.phase !== "sealed" || !iso(event.at) ||
        Date.parse(event.at) < Date.parse(current.attempt!.markedAt)) blocked("submitting_requires_seal");
  ...
  const effects = [...j.effects] as [ArbitrumSourceEffect, ArbitrumSourceEffect]; effects[index] = next;
...
export class ArbitrumSourceEffectJournalRepository extends SecureStateStore {
  constructor(root: string, private readonly verifiedObservation?: (input: {
  ...
  private async operation(profileHash: string, operationId: string): Promise<RelayUnsignedOperation> {
    ...
    if (op === null) throw new ApnError("APN_OPERATION_NOT_FOUND", "Relay Arbitrum prepared operation was not found.");
    operationDraft(op); return op;
  }
  async load(profileHash: string, operationId: string): Promise<ArbitrumSourceEffectJournal | null> {
  ...
  async create(profileHash: string, operationId: string, createdAt: string): Promise<ArbitrumSourceEffectJournal> {
    await this.initialize();
    return this.withLocks([`profile:${profileHash}`, `operation:${operationId}`, `relay-arbitrum-effect:${operationId}`],
      async () => this.createUnderLocks(profileHash, operationId, createdAt));
  }
  /** Caller holds profile, operation and effect locks in the shared canonical order. */
  async createUnderLocks(profileHash: string, operationId: string, createdAt: string): Promise<ArbitrumSourceEffectJournal> {
    await this.initialize();
    ...
    if (await this.readJson(path) !== null) blocked("journal_already_exists");
    ...
    await this.writeJson(path, j, true); return j;
  }
  async transition(profileHash: string, operationId: string, expectedIntegrityHash: string,
    event: ArbitrumEffectEvent): Promise<ArbitrumSourceEffectJournal> {
    await this.initialize();
    return this.withLocks([`profile:${profileHash}`, `operation:${operationId}`, `relay-arbitrum-effect:${operationId}`],
      async () => this.transitionUnderLocks(profileHash, operationId, expectedIntegrityHash, event));
  }
  /** Caller holds profile, operation and effect locks in the shared canonical order. */
  async transitionUnderLocks(profileHash: string, operationId: string, expectedIntegrityHash: string,
    event: ArbitrumEffectEvent): Promise<ArbitrumSourceEffectJournal> {
    await this.initialize();
    const op = await this.operation(profileHash, operationId), path = this.path(profileHash, operationId);
    if (await new RelayRetirementRepository(this.root).load(op) !== null) blocked("operation_retired");
  ...
  async beginSigning(profileHash: string, operationId: string, expectedIntegrityHash: string,
  ...
  /** Caller holds profile, operation and effect locks in the shared canonical order. */
  async beginSigningUnderLocks(profileHash: string, operationId: string, expectedIntegrityHash: string,
    role: ArbitrumEffectRole, at: string): Promise<ArbitrumSourceEffectJournal> {
    return this.transitionUnderLocks(profileHash, operationId, expectedIntegrityHash,
      { kind: "begin_signing", role, marker: randomBytes(32).toString("hex"), at });
  }
