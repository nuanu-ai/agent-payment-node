...
export class RelayReadOnlyPreflightService {
  constructor(private readonly state: StateStore, private readonly clock: ClockPort,
    private readonly ports: RelayPreflightPorts) {}

  async preflight(input: { readonly profile: string; readonly operationId: string }) {
    ...
    if (!Number.isFinite(now.getTime())) throw new ApnError("APN_INVALID_INPUT", "Relay preflight clock is invalid.");
    if (now.getTime() + 60_000 >= Date.parse(operation.deadline)) blocked("relay_quote_expired_or_near_deadline");
    ...
    if (active.registry.expiresAt !== undefined && now.toISOString() >= active.registry.expiresAt) blocked("relay_policy_expired");
    ...
    if (publicAccount?.toLowerCase() !== operation.sourceAccount.toLowerCase()) blocked("relay_public_profile_owner_mismatch");
    ...
    if (quote.paymentDetails.depository.toLowerCase() !== ETHEREUM_DEPOSITORY ||
      ...
      quote.deposit.from !== operation.sourceAccount.toLowerCase()) blocked("relay_quote_envelope_changed");
    ...
    if (quote.approval.data.toLowerCase() !== expectedApprovalData) blocked("relay_approval_spender_or_amount_changed");
