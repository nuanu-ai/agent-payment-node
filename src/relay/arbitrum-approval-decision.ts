...
export class RelayArbitrumApprovalDecisionService {
  constructor(private readonly state: StateStore,
    ...
    private readonly ports: RelayArbitrumApprovalDecisionPorts = {}) {}

  async decide(profile: string, operationId: string) {
    ...
    if (operation === null) throw new ApnError("APN_OPERATION_NOT_FOUND", "Relay Arbitrum operation was not found in the requested profile.");
    ...
    if (await new RelayRetirementRepository(this.state.root).load(operation) !== null) blocked("operation_retired");
    ...
    const snapshot = async (lockedPolicy?: ActiveAssetPolicy) => {
    ...
    const output = (state: "approval_required" | "approval_skipped" | "preflight_blocked",
      result: typeof first, journalIntegrityHash: string | null) => ({ operationId, profile,
      ...
      journalIntegrityHash, proofClass: state === "approval_skipped" ? "canonical_allowance_observation" as const :
        "read_only_rpc_observation" as const, executionAdmitted: false as const, nextActions: [] as const });
    ...
    const journal = await new ArbitrumSourceEffectJournalRepository(this.state.root, undefined, undefined,
      ...
      async input => {
        ...
        if (verified.value.approvalRequired) return null;
        if (!verified.value.readOnlyConditionsSatisfied) blocked("read_only_conditions_changed");
      ...
      [`relay-arbitrum-approval-execute:${operationId}`, evmAddressLock(operation.sourceAccount)]);
    if (journal === null) {
