/** Offline composition of independently observed safe receipts. No execution or Relay status claim. */
import { getOrderId } from "@relay-protocol/settlement-sdk";
import { recoverMessageAddress } from "viem";
import type { Hex } from "viem";
import { hashObject } from "../canonical.js";
import { RELAY_ARBITRUM_USDC, validateRelayArbitrumUsdcEthereumUsdcQuote } from "./arbitrum-usdc-ethereum-quote.js";
...
import { ETHEREUM_DEPOSITORY, ETHEREUM_USDC, RELAY_SOLVER } from "./quote.js";
...
export type RelayOrderReferenceResult = Readonly<{
  orderReferencedRecipientCreditProven: boolean;
  ...
  proof: null | Readonly<{
    ...
    proofClass: "safe_onchain_order_reference_and_recipient_credit";
  }>;
}>;

function refuse(reason: string): RelayOrderReferenceResult {
  return { orderReferencedRecipientCreditProven: false, relayOrderFulfillmentProven: false,
    cryptographicCausalityProven: false, paidAcceptance: false, reason, proof: null };
}
...
/** Caller supplies fresh receipt/transaction/block reads bound to the two safe observer results. */
export async function proveRelayOrderReferencedRecipientCredit(e: RelayOrderReferenceEvidence): Promise<RelayOrderReferenceResult> {
  const q = e.quote;
  try {
    const { quoteDigest, ...body } = q;
    if (hashObject(body) !== quoteDigest || q.schemaVersion !== "apn.relay-arbitrum-usdc-ethereum-usdc-quote.v1" ||
    ...
    const derived = getOrderId(q.orderData as Parameters<typeof getOrderId>[0], CHAINS);
    const signer = await recoverMessageAddress({ message: { raw: derived as Hex }, signature: q.orderSignature as Hex });
    if (!same(derived, q.orderId) || !same(signer, RELAY_SOLVER)) return refuse("signed_order_binding");
  ...
  return { orderReferencedRecipientCreditProven: true, relayOrderFulfillmentProven: false,
    cryptographicCausalityProven: false, paidAcceptance: false, reason: null, proof: {
