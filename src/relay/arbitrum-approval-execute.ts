...
function blocked(reason: string): never { throw new ApnError("APN_OPERATION_BLOCKED", "Relay Arbitrum approval execution is blocked.", { reason }); }
...
export interface RelayArbitrumApprovalSummary {
  ...
  readonly owner: string; readonly token: string; readonly spender: string; readonly approvalValueAtomic: string;
  readonly quoteDigest: string; readonly deadline: string; readonly approvalNetworkFeeCeilingWei: string;
}
export interface RelayArbitrumApprovalSigner { sign(op: RelayUnsignedOperation, nonce: string, journal: ArbitrumSourceEffectJournal): Promise<Hex> }
...
export class LocalRelayArbitrumApprovalSigner implements RelayArbitrumApprovalSigner {
  ...
  async sign(op: RelayUnsignedOperation, nonce: string, journal: ArbitrumSourceEffectJournal): Promise<Hex> {
    ...
    try {
      if (wallet.identity.profile !== op.arbitrumDraft!.profile || !same(wallet.identity.address, op.sourceAccount)) blocked("encrypted_wallet_owner");
      ...
      const approval = (op.arbitrumDraft!.rawQuote as { steps: Array<{ items: Array<{ data: {
        to: string; data: string; value: string; gas: string; maxFeePerGas: string; maxPriorityFeePerGas: string } }> }> }).steps[0]!.items[0]!.data;
      if (!same(approval.to, RELAY_ARBITRUM_USDC) || approval.value !== "0" ||
        BigInt(approval.gas) * BigInt(approval.maxFeePerGas) > BigInt(op.approvalNetworkFeeCeilingWei!)) blocked("approval_envelope");
      return await account.signTransaction({ type: "eip1559", chainId: 42161, to: approval.to as Hex,
        data: approval.data as Hex, value: 0n, nonce: Number(BigInt(nonce)), gas: BigInt(approval.gas),
...
export class RelayArbitrumApprovalExecuteService {
  ...
  constructor(private readonly state: StateStore,
    private readonly reader: Pick<RelayArbitrumApprovalPreflightReader, "read">,
    private readonly ports: RelayArbitrumApprovalExecutePorts,
  ...
  private async reserve(op: RelayUnsignedOperation, active: ActiveAssetPolicy): Promise<void> {
    ...
    if (reservation.state !== "reserved" || reservation.policyDigest !== op.policyDigest) blocked("usage_lease_unavailable");
  }
  ...
  private async lockedPolicy(profile: string): Promise<ActiveAssetPolicy | null> {
    const at = this.now();
    return this.ports.activePolicyUnderLock === undefined ? activeAssetPolicyFromState(
      await new AllowlistPolicyStore(this.state.root).readUnderProfileLock(profile), at) :
  ...
  async execute(profile: string, operationId: string) {
    ...
    return this.state.withLocks([`profile:${profileHash}`, `profile:${allowlistProfileHash(profile)}`,
      ...
      return this.result(op, journal, outcome === "accepted" ? "approval_submitted" : "observation_only",
