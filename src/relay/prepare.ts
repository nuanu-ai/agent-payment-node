...
export class RelayUnsignedPrepareService {
  constructor(private readonly state: StateStore, private readonly clock: ClockPort,
    private readonly operations = new OperationService(state), private readonly ports: RelayPreparePorts = {}) {}

  async prepare(input: RelayPrepareInput, route: "bnb" | "base" = "bnb") {
    if (input.profile !== "default") refuse("relay_default_profile_only");
    ...
    if (!Number.isFinite(now.getTime())) throw new ApnError("APN_INVALID_INPUT", "Relay prepare clock is invalid.");
    ...
    if (active === null || active.profile !== input.profile || active.accounts.evm === undefined ||
      !ADDRESS.test(active.accounts.evm)) refuse("relay_active_owner_policy_required");
    ...
    const intent: RelayQuoteIntent = { payer, recipient: input.recipient.toLowerCase(), amountAtomic: input.amountAtomic,
      ...
      ...(route === "base" ? { destinationChainId: 8453 as const } : {}) };
  ...
  async prepareBase(input: RelayPrepareInput) { return this.prepare(input, "base"); }

  /** A local quote file is validated before create-only persistence; no network or wallet is used. */
  async prepareArbitrum(input: RelayArbitrumPrepareInput) {
    ...
    const now = this.clock.now();
    if (!Number.isFinite(now.getTime())) throw new ApnError("APN_INVALID_INPUT", "Relay Arbitrum prepare clock is invalid.");
    const active = await (this.ports.activePolicy?.(input.profile) ?? loadActiveAssetPolicyRegistry(
      { state: this.state, clock: this.clock }, input.profile));
    if (active === null || active.accounts.evm?.toLowerCase() !== owner) refuse("relay_arbitrum_active_owner_policy_required");
    ...
    const rawQuote = await (this.ports.arbitrumQuoteFile?.(input.quoteFile) ?? (async () => {
      ...
      try {
        ...
        if (bytes.length > 65_536) throw new ApnError("APN_INVALID_INPUT", "Relay Arbitrum quote file is too large.");
  ...
  async prepareNative(input: RelayNativePrepareInput) {
    ...
    await checkOwner();
    await this.operations.assertProfileAvailable(profileHash);
    const intent: RelayNativeQuoteIntent = { payer, recipient: input.recipient.toLowerCase(), amountAtomic: input.amountAtomic,
      minimumOutputWei: input.minOutputAtomic, nowSeconds: Math.floor(now.getTime() / 1000) };
