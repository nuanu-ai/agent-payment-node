...
function hashes(value: unknown): readonly string[] {
  ...
  if (!Array.isArray(value) || value.length > 32 || !value.every(item => typeof item === "string" && TX_HASH.test(item))) invalid();
  return value.map((item: string) => item.toLowerCase());
}

function reason(value: unknown): string | null {
  if (value === undefined || value === null) return null;
  if (typeof value !== "string" || !REASON.test(value)) invalid();
  return value;
}

async function boundedJson(response: Response): Promise<unknown> {
  const length = response.headers.get("content-length");
  if (length !== null && (!/^\d+$/u.test(length) || Number(length) > MAX_RESPONSE_BYTES)) invalid();
  ...
  try {
    for (;;) {
      ...
      if (size > MAX_RESPONSE_BYTES) invalid();
      chunks.push(value);
    }
...
export class RelayKeylessStatusService {
  ...
  async status(operationId: string) {
    ...
    if (response.redirected || !response.ok || response.status !== 200) invalid();
    ...
    if (typeof row.status !== "string" || !STATUSES.has(row.status) ||
      row.originChainId !== operation.sourceChainId || row.destinationChainId !== operation.destinationChainId) invalid();
