...
const fail = (reason: string): never => { throw new Error(`Relay Arbitrum quote rejected: ${reason}`); };
function object(value: unknown, reason: string): Record<string, unknown> {
  return value !== null && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : fail(reason);
}
function array(value: unknown, reason: string): unknown[] { return Array.isArray(value) ? value : fail(reason); }
function one(value: unknown, reason: string): Record<string, unknown> {
  const rows = array(value, reason); return rows.length === 1 ? object(rows[0], reason) : fail(reason);
}
function string(value: unknown, reason: string): string { return typeof value === "string" ? value : fail(reason); }
function address(value: unknown, reason: string): string {
  const result = string(value, reason); return ADDRESS.test(result) ? result.toLowerCase() : fail(reason);
}
function atomic(value: unknown, reason: string): bigint {
  const result = string(value, reason); return UINT.test(result) ? BigInt(result) : fail(reason);
}
function keys(row: Record<string, unknown>, allowed: readonly string[], reason: string): void {
  if (Object.keys(row).some(key => !allowed.includes(key))) fail(reason);
}
...
export function relayArbitrumUsdcEthereumUsdcQuoteRequest(intent: RelayArbitrumQuoteIntent): Record<string, unknown> {
  const payer = address(intent.payer, "payer");
  if (atomic(intent.amountAtomic, "amount") <= 0n || atomic(intent.minimumOutputAtomic, "minimum") <= 0n) fail("amount");
...
function transaction(value: unknown, id: "approve" | "deposit", payer: string) {
  ...
  if (item.status !== "incomplete") fail("step status");
  ...
  if (!same(address(tx.from, "transaction from"), payer) || tx.chainId !== 42161 ||
    atomic(tx.value, "transaction value") !== 0n || gas === 0n || gas > 500_000n ||
    maxFee === 0n || maxFee > 100_000_000_000n || priority > maxFee || priority > 10_000_000_000n) fail("transaction envelope");
  const data = string(tx.data, "transaction data");
  if (!/^0x(?:[0-9a-fA-F]{2})+$/u.test(data)) fail("transaction data");
  return { from: payer, to: address(tx.to, "transaction target"), data: data.toLowerCase(),
    value: "0" as const, chainId: 42161 as const, gas: gas.toString(), maxFeePerGas: maxFee.toString(),
...
export async function validateRelayArbitrumUsdcEthereumUsdcQuote(value: unknown, intent: RelayArbitrumQuoteIntent) {
  ...
  const steps = array(quote.steps, "steps"); if (steps.length !== 2) fail("steps");
  ...
  for (const stepValue of steps) {
    ...
    if (step.requestId !== requestId) fail("step request id");
  }
  const approvalStep = object(steps[0], "approval step"), depositStep = object(steps[1], "deposit step");
  if (one(approvalStep.items, "approval items").check !== undefined) fail("approval check");
  const check = object(one(depositStep.items, "deposit items").check, "deposit check");
  keys(check, ["endpoint", "method"], "check extension");
  if (check.method !== "GET" || relayStatusLocator(requestId, check.endpoint).endpoint !== locator.endpoint) fail("deposit check");
  ...
  if (order.version !== "v1" || order.solverChainId !== "base" ||
    ...
    deadline <= intent.nowSeconds + 60 || deadline > intent.nowSeconds + 7 * 86400) fail("order identity");
  const minimum = atomic(payee.minimumAmount, "output minimum");
  const expected = atomic(payee.expectedAmount, "expected output");
  if (minimum < atomic(intent.minimumOutputAtomic, "required minimum") || expected < minimum) fail("output bound");
  const normalizedRefunds = refunds.map((value, index) => {
    ...
    if (refund.chainId !== chainId || !same(address(refund.recipient, "refund recipient"), expectedRecipient) ||
      ...
      refund.deadline !== deadline || refund.extraData !== ROUTER_DATA) fail("refund identity");
  ...
  if (!SIGNATURE.test(signature)) fail("order signature");
  let signer: string;
  ...
  if (details.chainId !== "arbitrum" || !same(address(details.depository, "depository"), ETHEREUM_DEPOSITORY) ||
    ...
    atomic(details.amount, "payment amount") !== atomic(intent.amountAtomic, "intent amount")) fail("payment details");
  ...
  if (!same(address(quoteDetails.sender, "sender"), payer) || !same(address(quoteDetails.recipient, "recipient"), recipient) ||
    ...
    atomic(currencyOut.minimumAmount, "details minimum") !== minimum) fail("quote details");
  const approval = transaction(steps[0], "approve", payer), deposit = transaction(steps[1], "deposit", payer);
  if (!same(approval.to, RELAY_ARBITRUM_USDC) || !same(deposit.to, ETHEREUM_DEPOSITORY)) fail("transaction target");
  ...
  const feeAmounts = Object.fromEntries(["gas", "relayer", "relayerGas", "relayerService", "app", "subsidized"].map(name => {
    ...
    if (currency.chainId !== 42161 || !same(address(currency.address, `${name} currency`), token)) fail("fee currency");
    const amount = atomic(fee.amount, `${name} amount`), minimumFee = atomic(fee.minimumAmount, `${name} minimum`);
    if (minimumFee !== amount) fail("fee minimum");
    return [name, amount];
  ...
  if (feeAmounts.gas !== BigInt(deposit.maximumNetworkFeeWei) ||
    ...
    atomic(intent.amountAtomic, "amount") - feeAmounts.relayer! !== expected) fail("fee accounting");
  try {
    const decodedApproval = decodeFunctionData({ abi: APPROVE, data: approval.data as Hex });
    if (decodedApproval.functionName !== "approve" || !same(decodedApproval.args[0], ETHEREUM_DEPOSITORY) ||
      ...
      !same(encodeFunctionData({ abi: APPROVE, functionName: "approve", args: [...decodedApproval.args] }), approval.data)) fail("approval calldata");
    const decodedDeposit = decodeFunctionData({ abi: DEPOSIT, data: deposit.data as Hex });
    if (decodedDeposit.functionName !== "depositErc20" || !same(decodedDeposit.args[0], payer) ||
      ...
      !same(encodeFunctionData({ abi: DEPOSIT, functionName: "depositErc20", args: [...decodedDeposit.args] }), deposit.data)) fail("deposit calldata");
  } catch { return fail("call data"); }
  const projection = { schemaVersion: "apn.relay-arbitrum-usdc-ethereum-usdc-quote.v1" as const,
