import { encodeFunctionData, getAddress, keccak256, parseTransaction, recoverTransactionAddress, serializeTransaction, type TransactionSerialized } from "viem";
...
const APPROVE_ABI = [{ type: "function", name: "approve", stateMutability: "nonpayable",
  inputs: [{ name: "spender", type: "address" }, { name: "value", type: "uint256" }], outputs: [{ type: "bool" }] }] as const;
const APPROVAL_TOPIC = keccak256(Buffer.from("Approval(address,address,uint256)"));
export function approvalData(spender: Address, amount: string): Hex {
  return encodeFunctionData({ abi: APPROVE_ABI, functionName: "approve", args: [spender, BigInt(amount)] });
}
export function approvalIncluded(receipt: BridgeProtocolReceipt, token: Address, owner: Address, spender: Address, amount: string): void {
  const topic = (a: Address) => `0x${a.slice(2).toLowerCase().padStart(64, "0")}`;
  ...
  if (logs.length !== 1 || logs[0]!.topics.length !== 3 || logs[0]!.topics[2] !== topic(spender) ||
    logs[0]!.data !== `0x${BigInt(amount).toString(16).padStart(64, "0")}`) bridgeFailure("APN_RPC_PROTOCOL", "approval_event_identity");
}
/** Only validation is exported; custody owns the one operation-bound signing entry. */
export async function verifyBridgeSigned(raw: Hex, expectedHash: Hex, e: BridgeEnvelope): Promise<void> {
  ...
  } catch { bridgeFailure("APN_PROVIDER_EFFECT_UNAVAILABLE", "signed_bridge_binding"); }
}
