...
function projectBridgeOperation(op: BridgeOperationRecord, legacy: boolean) {
  ...
  return {
    ...
    ...(op.failure?.observationRpc === undefined ? {} : { observation_rpc_failure: {
      ...
      ...(op.failure.observationRpc.endpointRole === undefined ? {} : { endpoint_role: op.failure.observationRpc.endpointRole }),
    } }),
    observation_rpc_telemetry: (op.observationTelemetry ?? []).map((row) => ({ schema_version: row.schemaVersion, stage: row.stage,
      effect_role: row.effectRole, outcome: row.outcome, physical_requests: row.physicalRequests, http_attempts: row.httpAttempts,
      logical_rpc_items: row.logicalRpcItems, batch_count: row.batchCount, max_batch_size: row.maxBatchSize,
      budget_rejected_before_transport: row.budgetRejectedBeforeTransport, attempts_by_endpoint_role: row.attemptsByEndpointRole,
      attempts_by_method_class: row.attemptsByMethodClass })),
...
function receiptWithoutObservationTelemetry(receipt: Record<string, unknown>): Record<string, unknown> {
  const body = structuredClone(receipt); delete body.receipt_hash; delete body.observation_rpc_telemetry;
  return { ...body, receipt_hash: hashObject(body) };
}
