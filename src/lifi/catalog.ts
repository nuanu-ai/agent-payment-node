...
export function bridgeCapabilities(profile?: string) {
  return { schema_version: "apn.bridge-capabilities.v1", ...(profile === undefined ? {} : { profile: canonicalProfile(profile), profile_binding_inspected: false }),
    chains: BRIDGE_CHAINS.map((id) => {
      ...
      return { chain: row.caip2, name: row.name,
        native_coin: { symbol: row.nativeCoin.symbol, coin_key: row.nativeCoin.coinKey, decimals: row.nativeCoin.decimals,
          ...
          role: "gas_messaging_fee_and_native_principal", token: "native",
          tools: id === 1 || id === 8453 ? ["across", "stargateV2"] : ["across"],
        ...
        tokens: row.tokens.map((asset) => ({ token: asset.address, symbol: asset.symbol, coin_key: asset.coinKey,
          ...
          tools: asset.stargate === null ? ["across"] : ["across", "stargateV2"], approval: asset.approval,
    ...
    profiles: [
      { provider: "local", custody: "local_software", execution_owner: "apn", retry_owner: "apn_observation_only_after_first_send",
        ...
        prerequisites: ["existing_local_wallet", "exact_chain_RPCs", "source_asset_and_native_funding", "foreground_bridge_consent", "zero_or_exact_existing_allowance"] },
    ...
    inventory_only: [{ tool: "polymer", reason: "protocol_unproved_unique_source_destination_correlation" },
      { asset: "native_principal_via_stargateV2_except_ethereum_to_base", reason: "stargate_native_pair_not_reviewed" },
      ...
      { asset: "stargate_pool_asset_other_than_1_or_ethereum_base_native_13", reason: "pool_not_reviewed" },
...
export function bridgeInventory(responses: Readonly<Record<"chains" | "tokens" | "tools" | "connections", LifiResponse>>) {
  const projected = Object.fromEntries(Object.entries(responses).map(([kind, response]) => {
    if (response.status !== 200) bridgeFailure("APN_PROVIDER_UNAVAILABLE", `inventory_${kind}_status`);
...
function inventoryValue(value: unknown, depth: number): unknown {
  ...
  if (Array.isArray(value)) {
    if (value.length > 20_000) bridgeFailure("APN_PROVIDER_PROTOCOL", "inventory_count");
    return value.map((entry) => inventoryValue(entry, depth + 1));
  }
  const r = bridgeRecord(value);
  const admitted = new Set<string>(BRIDGE_CHAINS.map(String));
