 ...
 */
let verifiedRegistry: Readonly<Record<number, BridgeChainRow>> | null = null;

export function assertBridgeRegistryListed(registry: Readonly<Record<number, BridgeChainRow>>): void {
  if (verifiedRegistry === registry) return;
  ...
  for (const row of Object.values(registry)) {
    ...
    if (native.length !== 1 || native[0]!.symbol !== row.nativeCoin.symbol || native[0]!.decimals !== row.nativeCoin.decimals ||
      row.nativeCoin.listing !== "frozen_list") bridgeFailure("APN_INTERNAL", "bridge_registry_native_list_drift");
    for (const token of row.tokens) {
      if (token.listing === "legacy_pinned") continue;
      ...
      if (token.listing !== "frozen_list" || listed.length !== 1 || listed[0]!.symbol !== token.symbol ||
        listed[0]!.decimals !== token.decimals) bridgeFailure("APN_INTERNAL", "bridge_registry_token_list_drift");
    }
  }
...
export function bridgeTokenListed(caip2: string, token: Address): boolean {
