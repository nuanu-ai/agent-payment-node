...
function fail(reason: string): never { return bridgeFailure("APN_RPC_PROTOCOL", `mayan_source_${reason}`); }
function wordAddress(word: Hex, reason: string): Address {
  if (!/^0x0{24}[0-9a-f]{40}$/u.test(word)) fail(reason);
  return getAddress(`0x${word.slice(26)}`);
}
function bytes(value: Hex, start: number, length: number): Hex { return `0x${value.slice(2 + start * 2, 2 + (start + length) * 2)}` as Hex; }
function integer(value: Hex): bigint { return BigInt(value); }
function oneLog(logs: readonly BridgeLog[], address: Address, topic: Hex, reason: string): BridgeLog {
  const matches = logs.filter(log => log.address.toLowerCase() === address.toLowerCase() && log.topics[0]?.toLowerCase() === topic.toLowerCase());
  if (matches.length !== 1) fail(`${reason}_count`);
  return matches[0]!;
}

/** No RPC, provider, custody or execution dependency. Inputs must be obtained and authenticated by the caller. */
export function decodeMayanBaseSolanaSourceReceiptOffline(
  quoteValue: unknown, binding: MayanOfflineBinding, transactionValue: unknown, receiptValue: unknown,
): MayanSourceReceiptProof {
  ...
  if (hash === BRIDGE_ZERO_WORD || tx.chainId !== 8453 || receipt.chainId !== 8453 ||
    ...
    tx.value !== "0x0") fail("transaction_binding");
  ...
  if (blockHash === BRIDGE_ZERO_WORD || !Array.isArray(receipt.logs) || receipt.logs.length > 512) fail("receipt_shape");
  ...
  let bridge: Record<string, unknown>, receiver: Hex, transactionId: Hex, destinationChainId: bigint;
  try {
    bridge = decodeEventLog({ abi: bridgeEventsAbi, eventName: "LiFiTransferStarted", data: lifi.data, topics: lifi.topics as [Hex, ...Hex[]], strict: true }).args.bridgeData as unknown as Record<string, unknown>;
    const decoded = decodeEventLog({ abi: NON_EVM_ABI, eventName: "BridgeToNonEVMChainBytes32", data: nonEvm.data, topics: nonEvm.topics as [Hex, ...Hex[]], strict: true });
    ({ receiver, transactionId, destinationChainId } = decoded.args);
  } catch { return fail("facet_event_decode"); }
  ...
  if (bridge.transactionId !== frozen.transactionId || bridge.bridge !== "mayanMCTP" || bridge.integrator !== "lifi-api" ||
    ...
    transactionId !== frozen.transactionId || destinationChainId !== 1151111081099710n || receiver.toLowerCase() !== recipientBytes) fail("facet_event_binding");
  const cctp = oneLog(logs, BASE_CCTP_V1_MESSAGE_TRANSMITTER, MESSAGE_TOPIC, "cctp_message");
  if (cctp.topics.length !== 1) fail("cctp_topics");
  let message: Hex;
  try { message = decodeEventLog({ abi: CCTP_ABI, eventName: "MessageSent", data: cctp.data, topics: cctp.topics as [Hex, ...Hex[]], strict: true }).args.message; }
  catch { return fail("cctp_decode"); }
  message = bridgeHex(message, 12 * 1024, undefined, "APN_RPC_PROTOCOL");
  if (encodeAbiParameters([{ type: "bytes" }], [message]).toLowerCase() !== cctp.data.toLowerCase()) fail("cctp_noncanonical_event");
  if (message.length !== 2 + 248 * 2 || integer(bytes(message, 0, 4)) !== 0n || integer(bytes(message, 4, 4)) !== 6n ||
    integer(bytes(message, 8, 4)) !== 5n || integer(bytes(message, 116, 4)) !== 0n) fail("cctp_v1_format");
  ...
  if (bytes(message, 52, 32) !== destinationProgram || sender !== BASE_CCTP_V1_TOKEN_MESSENGER ||
    ...
    amount === 0n || amount > BigInt(frozen.bridgeAmountAtomic) || mintRecipient === BRIDGE_ZERO_WORD) fail("cctp_burn_binding");
