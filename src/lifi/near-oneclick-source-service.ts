...
export class OneClickSourceService {
  constructor(private readonly state: StateStore, private readonly wrapping: WrappingSecretPort,
    ...
    private readonly https: BridgeHttps = new BridgeHttps(), private readonly now: () => number = Date.now,
    private readonly approve: (record: OneClickSourceRecord) => Promise<void> = approveTty) {}
  ...
  async submit(input: OneClickSubmitRequest): Promise<unknown> {
    ...
    try {
      ...
      if (bridgeHex(check.hash, 32, 32) !== blockHash || this.now() > Date.parse(actualRequest.deadline) - 45_000) fail("block_or_quote_expired");
      ...
      if (this.now() > Date.parse(record.quoteRequestDeadline) - 30_000) fail("quote_expired_after_approval");
      ...
      if (this.now() > Date.parse(record.quoteRequestDeadline) - 15_000) fail("quote_expired_before_send");
  ...
  async status(operationId: string): Promise<unknown> {
    ...
    if (response.status === 200) {
      try {
        ...
        if (hashObject(body.quoteResponse) !== record.quoteHash) fail("status_quote_binding");
  ...
  private async observeBase(url: string, rpc: CircleBaseJsonRpc, record: OneClickSourceRecord): Promise<unknown> {
    ...
    if (bridgeHex(included.hash, 32, 32) !== bridgeHex(r.blockHash, 32, 32)) fail("receipt_block");
...
async function approveTty(record: OneClickSourceRecord): Promise<void> {
