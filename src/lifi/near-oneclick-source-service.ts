...
import { OneClickSourceJournal, type OneClickSourceRecord } from "./near-oneclick-source-journal.js";
...
function tron(value: string): string { try { return tronAddress(value); } catch { return fail("tron_recipient"); } }
...
export function inspectOneClickSourceQuote(response: unknown, request: Record<string, unknown>, minOutput: bigint, maxLoss: bigint, now: number) {
  ...
  if (amountIn !== requestedAmount || amountIn < bridgeUint(quote.minAmountIn) || minimum > amountOut || minimum < minOutput ||
    ...
    deadline - now < 45_000 || deadline - now > 180_000) fail("quote_amount_or_deadline");
  ...
  if (deposit === getAddress("0x0000000000000000000000000000000000000000") || deposit === USDC) fail("deposit_address");
...
export interface OneClickSubmitRequest {
  readonly profile: string; readonly expectedPayer: string; readonly recipient: string; readonly amountAtomic: string;
...
export class OneClickSourceService {
  ...
  async submit(input: OneClickSubmitRequest): Promise<unknown> {
    ...
    if (amount > 10_000_000_000_000n || maxPriority > maxFee || input.idempotencyKey.length < 8 ||
    ...
    const rpcUrl = this.environment.APN_BASE_RPC_URL; if (rpcUrl === undefined) fail("base_rpc_missing");
    ...
    try {
      ...
      const readBase = async () => {
        if (quantity(await rpc.call("eth_chainId", [])) !== 8453n) fail("chain");
        ...
        if (gas > maxGas || fee > maxFee || tip > maxPriority) fail("gas_fee");
        ...
        if (nativeDebit > maxNative || quantity(nativeBalance) < nativeDebit) fail("native_balance_or_cap");
        const check = bridgeRecord(await rpc.call("eth_getBlockByNumber", [hex(blockNumber), false]));
        if (bridgeHex(check.hash, 32, 32) !== blockHash || now() > Date.parse(q.effectiveDeadline) - 45_000) fail("block_or_quote_expired");
      ...
      if (now() > Date.parse(record.effectiveDeadline) - 15_000) fail("quote_expired_before_sign");
  ...
  async status(operationId: string): Promise<unknown> {
    ...
    if (response.status === 200) {
      try {
        ...
        if (!oneClickStatusQuoteMatchesRecord(body.quoteResponse, record)) fail("status_quote_binding");
        providerStatus = body.status;
      } catch { fail("status_json_or_binding"); }
    }
    return { operationId, sourceTransactionHash: record.transactionHash, sourceState: record.phase,
      sourceReceipt: source, oneClickProviderStatus: providerStatus, tronDestinationClaimed:
        providerStatus === "SUCCESS", providerStatusProvenance: "oneclick_https_untrusted",
      tronDestinationFinalized: false };
  }
  private async observeBase(url: string, rpc: CircleBaseJsonRpc, https: BridgeHttps, record: OneClickSourceRecord): Promise<unknown> {
    ...
    if (response.status !== 200) fail("receipt_http");
    let receipt: unknown;
    try { const body = bridgeRecord(JSON.parse(response.body)); if (body.jsonrpc !== "2.0" || body.id !== 1 || Object.hasOwn(body, "error")) fail("receipt_rpc"); receipt = body.result; } catch { return fail("receipt_json"); }
...
export class TtyOneClickSourceApproval {
  async approve(record: OneClickSourceRecord): Promise<void> {
  if (!stdin.isTTY || !stderr.isTTY) fail("foreground_tty_required");
  ...
  try { if ((await rl.question(`Type ${record.operationId} to sign and submit once: `)).trim() !== record.operationId) fail("approval_denied"); }
  finally { rl.close(); }
}
}
