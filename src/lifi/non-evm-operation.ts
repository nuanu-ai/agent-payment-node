...
const solanaRecipient = z.string().refine((v) => { try { return solanaAddress(v) === v; } catch { return false; } });
const tronRecipient = z.string().refine((v) => { try { return tronAddress(v) === v; } catch { return false; } });
...
export function validateNonEvmBridgeOperation(value: unknown): NonEvmBridgeOperation {
  ...
  if (op.sourceCall.dataSha256 !== sha256(Buffer.from(op.sourceCall.data.slice(2), "hex"))) corrupt();
  if (op.route === "base_usdc_to_solana_usdc_circle_cctp_v2") {
    ...
    !op.sourceCall.data.startsWith("0x3110c7b9") || op.sourceCall.data.length < 10 + 64 * 3) corrupt();
  return op;
}
