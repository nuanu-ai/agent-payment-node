...
const solanaAddress = z.string().regex(/^[1-9A-HJ-NP-Za-km-z]{32,44}$/u);
const tronAddress = z.string().regex(/^T[1-9A-HJ-NP-Za-km-z]{33}$/u);
...
const sourceCall = z.strictObject({ chainId: z.literal(8453), from: addressSchema,
  to: addressSchema, valueAtomic: uintSchema, data: hexSchema });
const common = {
  schemaVersion: z.literal("apn.non-evm-bridge-operation.v1"), kind: z.literal("non_evm_bridge_intent"),
  executionAdmitted: z.literal(false), state: z.literal("frozen"),
...
const circle = z.strictObject({ ...common, route: z.literal("base_usdc_to_solana_usdc_circle_cctp_v2"),
  destination: z.strictObject({ chain: z.literal("solana-mainnet"),
    token: z.literal(BASE_SOLANA_CIRCLE_V2_CANDIDATE.toToken), recipient: solanaAddress,
...
const near = z.strictObject({ ...common, route: z.literal("base_usdc_to_tron_usdt_lifi_near_intents"),
  destination: z.strictObject({ chainId: z.literal(BASE_TRON_USDT_CANDIDATE.toChainId),
    token: z.literal(BASE_TRON_USDT_CANDIDATE.toToken), recipient: tronAddress,
    minimumReceivedAtomic: positive }),
...
export type NonEvmBridgeOperationInput = NonEvmBridgeOperation extends infer T ? T extends NonEvmBridgeOperation
  ? Omit<T, "integrityHash"> : never : never;

function corrupt(): never { return bridgeFailure("APN_STATE_CORRUPT", "non_evm_bridge_contract"); }
export function validateNonEvmBridgeOperation(value: unknown): NonEvmBridgeOperation {
  ...
  if (integrityHash !== hashObject(body) || Date.parse(op.expiresAt) <= Date.parse(op.createdAt) ||
    ...
    BigInt(op.sourceCall.valueAtomic) > BigInt(op.maxSourceNativeDebitWei)) corrupt();
  if (op.sourceCall.data === "0x") corrupt();
  if (op.route === "base_usdc_to_solana_usdc_circle_cctp_v2") {
    if (op.sourceCall.to !== BASE_CCTP_V2_TOKEN_MESSENGER_WITH_FEES || op.sourceCall.valueAtomic !== "0" ||
      !op.sourceCall.data.startsWith("0xc62fa55e") ||
      BigInt(op.destination.minimumReceivedAtomic) > BigInt(op.source.amountAtomic)) corrupt();
  } else if (op.sourceCall.to !== BRIDGE_DIAMOND || op.sourceCall.valueAtomic !== "0" ||
    !op.sourceCall.data.startsWith("0x3110c7b9")) corrupt();
  return op;
}
export function freezeNonEvmBridgeOperation(input: NonEvmBridgeOperationInput): NonEvmBridgeOperation {
...
export class NonEvmBridgeOperationRepository extends SecureStateStore {
  async save(op: NonEvmBridgeOperation): Promise<void> {
    validateNonEvmBridgeOperation(op);
    const previous = await this.load(op.profileHash, op.operationId);
    if (previous !== null) {
      if (previous.integrityHash !== op.integrityHash) corrupt();
      return;
    }
  ...
  async load(profileHash: string, operationId: string): Promise<NonEvmBridgeOperation | null> {
