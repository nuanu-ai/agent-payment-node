...
export async function inspectCircleV2UpfrontOffline(input: CircleV2UpfrontInput): Promise<CircleV2UpfrontEvidence> {
  ...
  if (amount > 10_000_000_000_000n || request.amount !== amount.toString() ||
      ...
      request.requests.length !== 1) fail("quote_request");
  if (bridgeRecord(request.requests[0]).type !== "FORWARD") fail("fee_types");
  ...
  if (!Array.isArray(response.items) || (response.items.length !== 1 && response.items.length !== 2)) fail("fee_items");
  let sum = 0n;
  for (const [index, raw] of response.items.entries()) {
    const item = bridgeRecord(raw);
    if (item.type !== (index === 0 ? "FORWARD" : "PROTOCOL") || !Array.isArray(item.args) || item.args.some(arg =>
      typeof arg !== "string" || !/^(?:0x[0-9a-fA-F]*|(?:0|[1-9][0-9]*))$/u.test(arg) || arg.length > 1026)) fail("fee_item");
    if (index === 0 && (item.args.length !== 5 || bridgeAddress(item.args[0]) !== WITH_FEES ||
        ...
        bridgeHex(item.args[4], 1024) !== bridgeRecord(bridgeRecord(request.requests[0]).params).hookData)) fail("forward_args");
