...
type MayanData = Readonly<{ nonEVMReceiver: Hex; mayanProtocol: Address; protocolData: Hex; swapProtocol: Address; swapData: Hex; middleToken: Address; minMiddleAmount: bigint; refundRecipient: Address; mayanAmountIn: bigint }>;
...
function fail(reason: string): never { return bridgeFailure("APN_PROVIDER_PROTOCOL", `mayan_offline_${reason}`); }
function field(record: Record<string, unknown>, name: string): unknown { return record[name]; }
function equalAddress(value: unknown, expected: Address): boolean { return bridgeAddress(value) === expected; }
function tokenIdentity(value: unknown, chainId: number, address: string): boolean {
...
function canonical(abi: typeof MAYAN_ABI | typeof CIRCLE_ABI | typeof feeForwarderAbi, name: string, args: readonly unknown[], original: Hex): boolean {
  return encodeFunctionData({ abi, functionName: name as never, args: args as never }).toLowerCase() === original.toLowerCase();
}
function base58Bytes(value: string): Uint8Array {
  if (typeof value !== "string" || value.length < 32 || value.length > 44) fail("solana_address_shape");
  let number = 0n;
  for (const character of value) {
    const digit = ALPHABET.indexOf(character);
    if (digit < 0) fail("solana_address_base58");
    number = number * 58n + BigInt(digit);
  }
  const bytes = new Uint8Array(32);
  for (let index = 31; index >= 0; index--) { bytes[index] = Number(number & 255n); number >>= 8n; }
  if (number !== 0n) fail("solana_address_width");
  let encoded = "", remaining = BigInt(`0x${Buffer.from(bytes).toString("hex")}`);
  while (remaining > 0n) { encoded = ALPHABET[Number(remaining % 58n)] + encoded; remaining /= 58n; }
  for (const byte of bytes) { if (byte !== 0) break; encoded = `1${encoded}`; }
  if (encoded !== value) fail("solana_address_noncanonical");
  return bytes;
}

export function decodeMayanBaseSolanaQuoteOffline(quoteValue: unknown, binding: MayanOfflineBinding): MayanOfflineDecode {
  ...
  if (quote.tool !== "mayanMCTP" || quote.type !== "lifi" || quote.integrator !== "lifi-api" ||
    ...
    minimumOutput > quotedOutput || quotedOutput > sourceAmount) fail("quote_binding");
  const gasLimit = BigInt(tx.gasLimit as string);
  if (gasLimit <= 0n || gasLimit > 5_000_000n) fail("gas_bound");
  ...
  let bridge: BridgeData, swaps: readonly SwapData[], mayan: MayanData;
  try {
    const decoded = decodeFunctionData({ abi: MAYAN_ABI, data });
    if (decoded.functionName !== "swapAndStartBridgeTokensViaMayan" || !canonical(MAYAN_ABI, decoded.functionName, decoded.args, data)) fail("source_noncanonical");
    [bridge, swaps, mayan] = decoded.args as unknown as readonly [BridgeData, readonly SwapData[], MayanData];
  } catch (error) {
  ...
  if (bridge.transactionId === `0x${"0".repeat(64)}` || quote.transactionId !== bridge.transactionId ||
    ...
    bridge.minAmount <= 0n || bridge.minAmount >= sourceAmount || swaps.length !== 1) fail("bridge_data");
  const swap = swaps[0]!;
  if (swap.callTo !== FEE_FORWARDER || swap.approveTo !== FEE_FORWARDER || swap.sendingAssetId !== SOURCE_TOKEN ||
    ...
    swap.callData.slice(0, 10) !== FEE_FORWARDER_SELECTOR) fail("fee_step");
  ...
  try {
    ...
    const [token, distributions] = decoded.args as unknown as readonly [Address, readonly { recipient: Address; amount: bigint }[]];
    if (token !== SOURCE_TOKEN || distributions.length !== 1 || distributions[0]!.recipient !== FEE_RECIPIENT || distributions[0]!.amount !== fee) fail("fee_distribution");
  } catch (error) {
    if (error instanceof Error && error.name === "ApnError") throw error;
    return fail("fee_abi");
  }
  const quotedFees = field(estimate, "feeCosts");
  if (!Array.isArray(quotedFees) || quotedFees.length !== 1) fail("fee_rows");
  const feeRow = bridgeRecord(quotedFees[0]);
  const feeSplit = bridgeRecord(feeRow.feeSplit);
  if (!Array.isArray(feeSplit.recipients) || feeSplit.recipients.length !== 1) fail("fee_rows");
  ...
  if (feeRow.name !== "LIFI Fixed Fee" || feeRow.included !== true || feeRow.amount !== fee.toString() ||
    ...
    feeRecipient.name !== "lifi" || feeRecipient.type !== "FIXED" || feeRecipient.fee !== fee.toString()) fail("fee_rows");
  ...
  if (feeStep.type !== "protocol" || feeStep.tool !== "feeCollection" || bridgeStep.type !== "cross" || bridgeStep.tool !== "mayanMCTP" ||
    ...
    !Array.isArray(bridgeEstimate.feeCosts) || bridgeEstimate.feeCosts.length !== 0) fail("included_steps");
  return {
