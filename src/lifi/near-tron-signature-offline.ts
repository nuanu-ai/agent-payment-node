...
function fail(reason: string): never { return bridgeFailure("APN_PROVIDER_PROTOCOL", `near_tron_signature_${reason}`); }

/** Proves that the saved calldata signature recovers a caller-supplied signer for this exact domain and payload. */
export async function verifyNearBaseTronSignatureOffline(value: unknown, binding: NearTronOfflineBinding, context: NearTronSignatureContext): Promise<NearTronSignatureProof> {
  ...
  if (expected === "0x0000000000000000000000000000000000000000" || diamond !== BRIDGE_DIAMOND || context.chainId !== 8453) fail("context");
  ...
  const typed = {
    ...
    types: TYPES,
    primaryType: "NEARIntentsPayload" as const,
    message: {
  ...
  try { recovered = getAddress(await recoverTypedDataAddress({ ...typed, signature: near.signature })); }
  catch { return fail("recovery"); }
  if (recovered !== expected) fail("backend_signer");
