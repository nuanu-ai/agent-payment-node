...
const indexedKey = (keys: readonly string[], raw: unknown): string => {
  if (!Number.isSafeInteger(raw) || (raw as number) < 0 || (raw as number) >= keys.length) fail();
  return keys[raw as number]!;
};
...
export async function inspectCircleV2SolanaDestinationOffline(input: CircleV2SolanaDestinationInput) {
  ...
  for (const item of instructions) {
    ...
    if (program !== CIRCLE_V2_MESSAGE_TRANSMITTER) continue;
...
export async function inspectCircleV2SolanaMintEventOffline(input: CircleV2SolanaDestinationInput) {
  ...
  const receiveIndexes = outer.flatMap((item, index) => {
    ...
    if (program !== CIRCLE_V2_MESSAGE_TRANSMITTER || typeof ix.data !== "string") return [];
  ...
  const parsed = inner.map(item => {
    const ix = bridgeRecord(item);
    if (typeof ix.data !== "string") fail();
    const accounts = rpcArray(ix.accounts, 64).map(index => indexedKey(keys, index));
    return { program: indexedKey(keys, ix.programIdIndex), accounts, data: keyBytes(ix.data as string) };
  });
