...
function decodeStargate(m: BridgeMaterialization, bridge: BridgeData, swaps: readonly SwapData[], stargate: StargateData, data: Hex, sourceAmount: bigint, minimum: bigint, value: bigint): DecodedBridgeCall {
  ...
  if (stargate.assetId !== 1 || p.dstEid !== expectedEid || p.to.toLowerCase() !== addressWord(m.request.recipient) ||
    ...
    stargate.fee.nativeFee !== value || value === 0n || stargate.fee.lzTokenFee !== 0n || stargate.refundAddress !== m.sender) fail("stargate_taxi_semantics");
...
function validateFeeRows(m: BridgeMaterialization, forwardedFee: bigint, value: bigint, tool: "across" | "stargateV2"): void {
  ...
  if (fixed !== 1 || included + BigInt(m.quotedOutputAtomic) > BigInt(m.request.amountAtomic) || (tool === "across" ? native !== 0 : native !== 1)) fail("fee_reconciliation");
}
