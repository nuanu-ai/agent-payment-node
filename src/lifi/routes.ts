...
import { BRIDGE_DIAMOND, BRIDGE_MAX_GAS, BRIDGE_USDC, BRIDGE_ZERO_ADDRESS, bridgeAddress, bridgeChain, bridgeFailure,
...
export interface BridgeRouteChoice {
  readonly routeId: string; readonly stepId: string; readonly tool: string; readonly quotedOutputAtomic: string;
  readonly minimumOutputAtomic: string; readonly routeHash: string; readonly stepHash: string;
  readonly stepIdentityHash: string; readonly preparable: boolean; readonly unavailableReason: string | null;
}
export interface ParsedBridgeRoute { readonly choice: BridgeRouteChoice; readonly route: Record<string, unknown>; readonly step: Record<string, unknown> }
export function parseBridgeRoutes(response: LifiResponse, request: BridgeRouteRequest, sender: Address): readonly ParsedBridgeRoute[] {
  if (response.status !== 200) bridgeFailure("APN_PROVIDER_UNAVAILABLE", "route_discovery_status");
  const body = bridgeRecord(bridgeJson(response.body, LIFI_ROUTE_RESPONSE_BYTES));
  const routes = list(body.routes, 20, "route_count");
  const results = routes.map((value): ParsedBridgeRoute => {
    const route = bridgeRecord(value), routeId = bridgeOpaque(route.id);
    assertTuple(route, request, sender, false);
    if (route.containsSwitchChain !== false || list(route.steps, 1, "top_level_step_count").length !== 1) bridgeFailure("APN_PROVIDER_PROTOCOL", "single_source_effect_required");
    const step = bridgeRecord((route.steps as unknown[])[0]);
    ...
    if (route.toAmount !== output || route.toAmountMin !== minimum) bridgeFailure("APN_PROVIDER_PROTOCOL", "route_step_output_identity");
  ...
  if (new Set(results.map((r) => r.choice.routeId)).size !== results.length || new Set(results.map((r) => r.choice.stepId)).size !== results.length) bridgeFailure("APN_PROVIDER_PROTOCOL", "duplicate_route_identity");
  return results;
}
export function materializeBridgeRoute(selected: ParsedBridgeRoute, response: LifiResponse, request: BridgeRouteRequest, sender: Address): {
  ...
  if (Object.keys(tx).some((k) => !allowed.includes(k)) || (tx.type !== undefined && tx.type !== 2 && tx.type !== "0x2") ||
    ...
    ((tx.maxFeePerGas === undefined) !== (tx.maxPriorityFeePerGas === undefined))) bridgeFailure("APN_PROVIDER_PROTOCOL", "transaction_envelope_fields");
  for (const k of ["gasPrice", "maxFeePerGas", "maxPriorityFeePerGas"]) if (tx[k] !== undefined) providerQuantity(tx[k]);
  const gas = providerQuantity(tx.gasLimit);
  if (gas < 1n || gas > BRIDGE_MAX_GAS || bridgeChain(tx.chainId) !== request.fromChainId ||
    bridgeAddress(tx.from) !== sender || bridgeAddress(tx.to) !== BRIDGE_DIAMOND) bridgeFailure("APN_PROVIDER_PROTOCOL", "materialized_transaction_identity");
  const materialization: BridgeMaterialization = {
    routeId: selected.choice.routeId, stepId: selected.choice.stepId, tool: selected.choice.tool as BridgeTool, request, sender,
...
export function validateRouteEconomics(m: BridgeMaterialization): string {
  ...
  for (const fee of m.feeCosts) {
    if (fee.included) {
      if ((fee.chainId !== r.fromChainId && fee.chainId !== r.toChainId) || fee.asset !== BRIDGE_USDC[fee.chainId]) bridgeFailure("APN_PROVIDER_PROTOCOL", "included_fee_asset");
...
function assertStep(step: Record<string, unknown>, request: BridgeRouteRequest, sender: Address): void {
  ...
  if (step.type !== "lifi" || bridgeRecord(step.toolDetails).key !== step.tool ||
    (step.integrator !== undefined && step.integrator !== "lifi-api") || (step.fee !== undefined && step.fee !== 0)) bridgeFailure("APN_PROVIDER_PROTOCOL", "step_identity");
  rejectExecutionExtensions(step);
  const action = bridgeRecord(step.action); assertTuple(action, request, sender, true);
  const estimate = bridgeRecord(step.estimate);
  if (estimate.tool !== step.tool || estimate.fromAmount !== request.amountAtomic ||
    bridgeAddress(estimate.approvalAddress) !== BRIDGE_DIAMOND || estimate.approvalReset === true || estimate.skipApproval === true) bridgeFailure("APN_PROVIDER_PROTOCOL", "approval_semantics");
  for (const key of ["approvalReset", "skipApproval", "skipPermit"]) if (estimate[key] !== undefined && typeof estimate[key] !== "boolean") bridgeFailure("APN_PROVIDER_PROTOCOL", "approval_flag");
  bridgeUint(estimate.toAmount, true); bridgeUint(estimate.toAmountMin, true);
  ...
  for (const value of list(estimate.gasCosts, 8, "gas_cost_count")) {
    ...
    if (bridgeChain(token.chainId) !== request.fromChainId || bridgeAddress(token.address) !== BRIDGE_ZERO_ADDRESS ||
      token.decimals !== 18 || typeof gas.type !== "string" || !["SEND", "APPROVE", "FEE"].includes(gas.type)) bridgeFailure("APN_PROVIDER_PROTOCOL", "gas_cost_asset");
...
function assertTuple(value: Record<string, unknown>, request: BridgeRouteRequest, sender: Address, action: boolean): void {
  assertToken(value.fromToken, request.fromChainId); assertToken(value.toToken, request.toChainId);
...
function assertIncludedAction(a: Record<string, unknown>, request: BridgeRouteRequest, amount: string, collection: boolean): void {
  actionKeys(a); assertToken(a.fromToken, request.fromChainId); assertToken(a.toToken, collection ? request.fromChainId : request.toChainId);
...
function assertToken(value: unknown, chainId: BridgeRouteRequest["fromChainId"]): void {
  const token = bridgeRecord(value);
  if (token.chainId !== chainId || token.decimals !== 6 || bridgeAddress(token.address) !== BRIDGE_USDC[chainId]) bridgeFailure("APN_PROVIDER_PROTOCOL", "canonical_USDC_metadata");
}
function parseFees(value: unknown, request: BridgeRouteRequest): readonly BridgeFee[] {
  return list(value, 16, "fee_count").map((value) => {
    ...
    if (fee.included ? token.decimals !== 6 || address !== BRIDGE_USDC[chainId] || ![request.fromChainId, request.toChainId].includes(chainId)
      : token.decimals !== 18 || address !== BRIDGE_ZERO_ADDRESS || chainId !== request.fromChainId) bridgeFailure("APN_PROVIDER_PROTOCOL", "fee_asset");
...
function rejectExecutionExtensions(step: Record<string, unknown>): void {
  ...
  if ((step.executionType !== undefined && step.executionType !== "transaction") || (step.gasless !== undefined && step.gasless !== false)) bridgeFailure("APN_PROVIDER_PROTOCOL", "execution_mode");
}
function list(value: unknown, maximum: number, reason: string): unknown[] {
  if (!Array.isArray(value) || value.length > maximum) bridgeFailure("APN_PROVIDER_PROTOCOL", reason);
  return value;
}
function providerQuantity(value: unknown): bigint {
  if (typeof value === "string" && /^0x(?:0|[1-9a-f][0-9a-f]{0,63})$/u.test(value)) return BigInt(value);
  return bridgeUint(value);
}
export function bridgeRouteProjection(choice: BridgeRouteChoice) {
