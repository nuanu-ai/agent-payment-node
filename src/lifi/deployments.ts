...
import { bridgeAssetTool, bridgePeerToken, bridgeTokenRow, type BridgeTokenAsset } from "./asset-registry.js";
...
export function bridgeDeployment(chainId: EvmChainId, peerChainId: EvmChainId, tool: BridgeTool, token: Address): BridgeDeploymentContract {
  ...
  const asset = bridgeTokenRow(chainId, token, "APN_PROVIDER_CAPABILITY_UNAVAILABLE");
  ...
  const commonCode = [code(BRIDGE_DIAMOND, DIAMOND_HASH[chainId]),
    code(FEE_FORWARDER, "0x7ee455a6853068874bfd201f93d6383ed6d88934a922316db5575b057e2ebe74"), ...assetCode(asset)];
  const reads = [
    ...
    call(asset.address, "decimals", [], wordUint(asset.decimals)),
    ...assetReads(asset),
  ];
  if (pool === null) {
    ...
    return {
      ...
      token: asset.address, protocolEmitter: a.spoke.address, endpointId: null,
...
function assetCode(asset: BridgeTokenAsset): readonly Code[] {
  const c = asset.code, proxy = code(asset.address, c.codeHash);
  if (c.upgradeability === "immutable") return [proxy];
  if (c.upgradeability === "legacy_proxy") return [proxy, code(c.implementation, c.implementationCodeHash)];
  return [proxy, code(c.beacon, c.beaconCodeHash), code(c.implementation, c.implementationCodeHash)];
}
function assetReads(asset: BridgeTokenAsset): readonly BridgeDeploymentContract["reads"][number][] {
  const c = asset.code;
  if (c.upgradeability === "immutable") return [];
  if (c.upgradeability === "legacy_proxy") {
    return [storage(asset.address, LEGACY_IMPLEMENTATION, wordAddress(c.implementation)),
      storage(asset.address, LEGACY_ADMIN, wordAddress(c.admin))];
  }
  return [storage(asset.address, EIP1967_BEACON, wordAddress(c.beacon)),
    call(c.beacon, "implementation", [], wordAddress(c.implementation))];
}
