...
const SIX_WORD = `0x${"0".repeat(63)}6` as Hex;
...
type Common = Readonly<{ diamondHash: Hex; tokenImpl: Address; tokenImplHash: Hex; tokenAdmin: Address }>;
type Across = Readonly<{ facet: Code; spoke: Code; implementation: Code; wrapped: Hex }>;
type Stargate = Readonly<{
  facet: Code; messaging: Code; router: Code; endpoint: Code; localEid: number;
  addressConfig: readonly [Address, Address, Address, Address, Address, Address];
}>;

const COMMON: Readonly<Record<EvmChainId, Common>> = {
...
const TOKEN_PROXY_HASH: Readonly<Record<EvmChainId, Hex>> = {
...
export function bridgeDeployment(chainId: EvmChainId, peerChainId: EvmChainId, tool: BridgeTool): BridgeDeploymentContract {
...
export function bridgeProtocolEmitter(chainId: EvmChainId, tool: BridgeTool): Address {
...
function stargate(facet: string, facetHash: Hex, messaging: string, messagingHash: Hex, router: string, routerHash: Hex, endpointHash: Hex, localEid: number, config: readonly string[]): Stargate {
  return { facet: code(facet, facetHash), messaging: code(messaging, messagingHash), router: code(router, routerHash), endpoint: code(LAYER_ZERO_ENDPOINT, endpointHash), localEid, addressConfig: config.map((address) => getAddress(address)) as unknown as Stargate["addressConfig"] };
}
