...
function acrossDestination(source: BridgeSourceProof, decoded: DecodedBridgeCall, receipt: BridgeProtocolReceipt): BridgeDestinationProof {
  ...
  if (decoded.destinationToken === BRIDGE_ZERO_ADDRESS) {
    ...
    return destinationResult(source, decoded, receipt, info.updatedOutputAmount.toString(), info.fillType as 0 | 1 | 2, relayerCredit, repaymentChainIdAtomic, balance);
  }
...
function destinationResult(source: BridgeSourceProof, decoded: DecodedBridgeCall, receipt: BridgeProtocolReceipt, amountAtomic: string, fillType: 0 | 1 | 2 | null, relayerCredit: Hex | null, repaymentChainIdAtomic: string | null, nativeBalance: BridgeDestinationProof["nativeBalance"]): BridgeDestinationProof {
  return {
