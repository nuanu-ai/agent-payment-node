...
import { BRIDGE_CHAIN_IDS, BRIDGE_EXECUTION_CHAIN_IDS, BRIDGE_EXECUTION_SOURCE_CHAIN_IDS,
  type BridgeChainId, type BridgeExecutionChainId, type BridgeExecutionSourceChainId } from "./chains.js";
...
const native = (chainId: BridgeChainId, symbol: "ETH" | "BNB" | "MON", peers: readonly BridgeChainId[], wrapped: BridgeWrappedNative): BridgeNativeCoin =>
  ({ kind: "native", chainId, symbol, coinKey: symbol, decimals: 18, pairKey: symbol.toLowerCase() as BridgeNativeCoin["pairKey"], acrossSupported: true,
...
const WRAPPED_NATIVE: Readonly<Record<BridgeChainId, BridgeWrappedNative>> = {
...
const NATIVE_STARGATE_ETHEREUM: BridgeStargatePool = { assetId: 13,
...
const NATIVE_STARGATE_BASE: BridgeStargatePool = { assetId: 13,
...
export const BRIDGE_ASSET_REGISTRY: Readonly<Record<BridgeChainId, BridgeChainRow>> = {
...
export function bridgeChain(value: unknown, code: ErrorCode = "APN_PROVIDER_PROTOCOL"): BridgeChainId {
  if (typeof value !== "number" || !BRIDGE_CHAINS.includes(value as BridgeChainId)) bridgeFailure(code, "chain_identity");
  return value as BridgeChainId;
}
export function bridgeCaip2(value: unknown): BridgeChainId {
...
export function bridgeDestinationCaip2(value: unknown): BridgeChainId {
  ...
  return Number(quoted[0]) as BridgeChainId;
}
...
export function bridgeDestinationChain(value: unknown, code: ErrorCode = "APN_PROVIDER_PROTOCOL"): BridgeChainId {
  if (typeof value === "number" && BRIDGE_CHAINS.includes(value as BridgeChainId)) return value as BridgeChainId;
  bridgeQuoteDestination(value, code);
  return value as BridgeChainId;
}
export function bridgeExecutionDestination(value: unknown): value is BridgeExecutionChainId {
  return typeof value === "number" && BRIDGE_EXECUTION_CHAIN_IDS.includes(value as BridgeExecutionChainId);
}
/** A source needs the reviewed LI.FI Diamond and FeeForwarder path; destination-only proof support is insufficient. */
export function bridgeExecutionSource(value: unknown): value is BridgeExecutionSourceChainId {
  return typeof value === "number" && BRIDGE_EXECUTION_SOURCE_CHAIN_IDS.includes(value as BridgeExecutionSourceChainId);
}
export function bridgeExecutionSourceCaip2(value: unknown): boolean {
...
export function bridgeAssetRow(chainId: unknown, address: unknown, code: ErrorCode = "APN_PROVIDER_PROTOCOL"): BridgeAsset {
  if (typeof chainId !== "number" || !BRIDGE_CHAINS.includes(chainId as BridgeChainId)) {
    ...
    return { kind: "erc20", chainId: chainId as BridgeChainId, address: row.token, symbol: "USDC", coinKey: "USDC",
...
export function bridgeCrossNativeConversion(request: Pick<BridgeRouteRequest, "fromChainId" | "toChainId" | "fromToken" | "toToken">): boolean {
...
export function bridgeNativeDenominationConversion(request: Pick<BridgeRouteRequest, "fromChainId" | "toChainId" | "fromToken" | "toToken">): boolean {
...
export function bridgeProviderBoundNativeDestination(request: Pick<BridgeRouteRequest, "fromChainId" | "toChainId" | "fromToken" | "toToken">): boolean {
...
export function bridgePeerToken(asset: BridgeTokenAsset, peerChainId: BridgeChainId,
...
export function validateBridgeRequest(value: unknown, code: ErrorCode = "APN_INVALID_INPUT"): BridgeRouteRequest {
  ...
  if ((!bridgeNativeDenominationConversion(r as unknown as BridgeRouteRequest) && minimum > amount) || typeof r.slippageBps !== "number" || !Number.isInteger(r.slippageBps) ||
