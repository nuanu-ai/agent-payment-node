...
import { SA_MIN_REMAINING_MS, SA_TTL_MS, type SmartAccountGaslessIntent, type SmartAccountGaslessRequest } from "./model.js";
import { saRequestHash, type SmartAccountGaslessOperationRecord } from "./operation-model.js";
...
import type { SmartAccountGaslessMaterialPort, SmartAccountGaslessProviderPort,
...
export interface SmartAccountGaslessPreparationOptions {
  readonly state: StateStore; readonly records: SmartAccountGaslessRepositoryPort; readonly operations: OperationService;
  readonly material: SmartAccountGaslessMaterialPort; readonly provider: SmartAccountGaslessProviderPort;
  readonly rpcFor: SmartAccountGaslessRpcFactory; readonly clock: ClockPort;
}
export class SmartAccountGaslessPreparation {
  constructor(private readonly o: SmartAccountGaslessPreparationOptions) {}
  async prepare(input: { readonly profile: string; readonly request: SmartAccountGaslessRequest;
    readonly idempotencyKey: string }): Promise<SmartAccountGaslessOperationRecord> {
    ...
    return await state.withLocks([`profile:${profileHash}`, `operation:${operationId}`, `operation:idempotency:${idempotencyHash}`], async () => {
      ...
      if (BigInt(binding.rootCapAtomic) < BigInt(request.grossAtomic)) saFail("sa_gasless_allowance");
      const provider = saProviderBinding(await this.o.provider.supported()); clock.fresh(provider.observedAt);
      const rpc = this.o.rpcFor(request.chainId);
      if (rpc.chainId !== request.chainId) saFail("sa_gasless_rpc_binding");
      const snapshot = smartAccountGaslessSnapshot(await rpc.snapshot(binding), binding, rpc); clock.fresh(snapshot.observedAt);
      if (BigInt(snapshot.safeState.usdcBalanceAtomic) < BigInt(request.grossAtomic)) saFail("sa_gasless_balance");
      if (BigInt(snapshot.safeState.availableAtomic) < BigInt(request.grossAtomic)) saFail("sa_gasless_allowance");
      ...
      if (beforeUnix * 1000 - prepared < SA_MIN_REMAINING_MS || afterUnix < binding.rootStartsAtUnix) saFail("sa_gasless_expired");
      const intent: SmartAccountGaslessIntent = { profile, request, binding, token: registry.token.address, decimals: 6,
      ...
      clock.live(op); await this.o.records.persist(op); return op;
    });
  }
}
