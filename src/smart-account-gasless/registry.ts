...
const row = {
  ...
  ownerDesignationCode: `0xef0100${SA_BASE_PROTOCOL.delegate.address.slice(2)}` as Hex,
  ownerDesignationCodeHash: keccak256(`0xef0100${SA_BASE_PROTOCOL.delegate.address.slice(2)}` as Hex),
...
if (Object.keys(row.protocol).length !== SA_PROTOCOL_NAMES.length ||
  ...
  hashDomain({ domain: { name: row.token.name, version: row.token.version, chainId: BigInt(SA_CHAIN_ID),
    ...
    { name: "verifyingContract", type: "address" }] } }) !== row.token.domainSeparator) saFail("sa_gasless_state_corrupt");
...
export type SmartAccountGaslessDeployment = typeof deployment;
/** Inventory elsewhere does not admit any executable chain or replace its own grant. */
export function saRegistry(chainId: number): SmartAccountGaslessDeployment {
  if (chainId !== SA_CHAIN_ID) saFail("sa_gasless_capability");
  return deployment;
}
