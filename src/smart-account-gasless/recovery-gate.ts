...
interface ArchiveManifest {
  ...
  readonly allArchiveFilesMatchGitSourceAndInstalledBytes: true;
  readonly files: readonly { readonly path: string; readonly sha256: string; readonly bytes: number }[];
}
const MAX_FILE_BYTES = 64 * 1024 * 1024;
function incompatible(): never { throw saError("sa_gasless_archive_incompatible"); }

/** Read-only selection preflight. It never imports or invokes a target executable, initializes state, or repairs receipts. */
export async function selectSmartAccountGaslessRecoveryArchive(input: SmartAccountGaslessRecoveryTarget) {
  try {
    if (!isPlainRecord(input) || Object.keys(input).sort().join(",") !==
      "archive,manifest,manifestSha256,packageRoot,stateRoot" || !digest(input.manifestSha256)) incompatible();
    await directory(input.stateRoot); await directory(input.packageRoot);
    ...
    if (sha256(bytes) !== input.manifestSha256) incompatible();
    ...
    if (manifest.archive !== input.archive || manifest.packageRoot !== input.packageRoot ||
      sha256(await regular(input.archive, MAX_FILE_BYTES)) !== manifest.archiveSha256) incompatible();
    const paths = new Map(manifest.files.map(file => [file.path, file]));
    const installed = await inventory(input.packageRoot);
    if (canonicalJson([...paths.keys()].sort()) !== canonicalJson(installed)) incompatible();
    for (const file of manifest.files) {
      const data = await regular(join(input.packageRoot, file.path), MAX_FILE_BYTES);
      if (data.length !== file.bytes || sha256(data) !== file.sha256) incompatible();
    }
    ...
    for (const path of SA_RECOVERY_REQUIRED_ARTIFACTS) {
      ...
      if (file === undefined) { supportsFamily = false; continue; }
      ...
      if (file.sha256 !== requiredHashes[path]) supportsFamily = false;
    }
    if (before.operations.length > 0 && !supportsFamily) incompatible();
    ...
    return { proof_class: "verified_archive_recovery_selection", target_execution_performed: false,
      ...
      guards_held: after.operations.filter(operation => !operation.terminal).length } as const;
...
class RecoveryJournalReader extends SmartAccountGaslessOperationRepository {
  async snapshot() {
    ...
    for (const profile of await this.readDirectory(receipts)) {
      if (!profile.isDirectory() || profile.isSymbolicLink() || !digest(profile.name)) incompatible();
      for (const file of await this.readDirectory(`${receipts}/${profile.name}`)) {
        if (!file.isFile() || file.isSymbolicLink() || !identities.has(`${profile.name}/${file.name}`)) incompatible();
      }
    }
    const bindings = [];
...
function parseManifest(value: unknown): ArchiveManifest {
  if (!isPlainRecord(value) || typeof value.archive !== "string" || !isAbsolute(value.archive) ||
    ...
    !Array.isArray(value.files) || value.files.length < 1 || value.files.length > 8192 || value.fileCount !== value.files.length) incompatible();
  for (const file of value.files) {
    if (!isPlainRecord(file) || Object.keys(file).sort().join(",") !== "bytes,path,sha256" ||
      ...
      !digest(file.sha256) || !Number.isSafeInteger(file.bytes) || Number(file.bytes) < 0 || Number(file.bytes) > MAX_FILE_BYTES) incompatible();
  }
  if (new Set(value.files.map(file => file.path)).size !== value.files.length) incompatible();
...
async function directory(path: string): Promise<void> {
  if (typeof path !== "string" || !isAbsolute(path) || resolve(path) !== path || await realpath(path) !== path) incompatible();
  const entry = await lstat(path); if (!entry.isDirectory() || entry.isSymbolicLink()) incompatible();
}
async function regular(path: string, maximum: number): Promise<Buffer> {
  ...
  try {
    ...
    if (!before.isFile() || before.nlink !== 1 || before.size > maximum) incompatible();
    ...
    if (bytes.length !== before.size || after.size !== before.size || after.mtimeMs !== before.mtimeMs ||
      current.isSymbolicLink() || current.ino !== before.ino || current.dev !== before.dev || await realpath(path) !== path) incompatible();
    return bytes;
...
async function inventory(root: string): Promise<string[]> {
  ...
  async function visit(prefix: string): Promise<void> {
    for (const entry of await readdir(join(root, prefix), { withFileTypes: true })) {
      if (prefix === "" && entry.name === "node_modules") continue; // Exact dependency closure belongs to independent archive verification.
      const path = prefix ? `${prefix}/${entry.name}` : entry.name;
      if (entry.isSymbolicLink()) incompatible();
      if (entry.isDirectory()) await visit(path);
      else if (entry.isFile()) result.push(path);
      else incompatible();
      if (result.length > 8192) incompatible();
    }
  }
  ...
  try {
    ...
    for (let index = 0; index < args.length; index += 2) {
      ...
      if (key === undefined || value === undefined || options[key] !== undefined ||
        !["--state-root", "--archive", "--manifest", "--manifest-sha256", "--package-root"].includes(key)) incompatible();
      options[key] = value;
    }
    if (Object.keys(options).length !== 5) incompatible();
