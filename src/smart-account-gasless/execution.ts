...
import type { SmartAccountGaslessMutable, SmartAccountGaslessRpcObservation, SmartAccountGaslessSealedMaterial } from "./model.js";
...
import { SmartAccountGaslessClock, smartAccountGaslessApprovalPhrase, smartAccountGaslessApprovalSummary,
  smartAccountGaslessSnapshot } from "./policy.js";
import type { SmartAccountGaslessApprovalPort, SmartAccountGaslessMaterialPort,
  SmartAccountGaslessProviderPort, SmartAccountGaslessRpcFactory, SmartAccountGaslessRpcPort } from "./ports.js";
import { saClassify, saError, saFail, saFailure, type SmartAccountGaslessFailure, type SmartAccountGaslessReason } from "./reasons.js";
...
export type SmartAccountGaslessSave = (op: SmartAccountGaslessOperationRecord, patch: Partial<SmartAccountGaslessMutable>,
  at: string) => Promise<SmartAccountGaslessOperationRecord>;
...
export class SmartAccountGaslessExecution {
  private readonly clock: SmartAccountGaslessClock;
  ...
  async approve(op: SmartAccountGaslessOperationRecord, approval: SmartAccountGaslessApprovalPort): Promise<SmartAccountGaslessStep> {
    ...
    let now: number;
    try { now = this.clock.live(op); } catch (error) { return await this.halt(op, error); }
    ...
    try {
      ...
      try { this.clock.live(op); } catch (clockError) { return await this.halt(op, clockError); }
      if (saClassify(error, "sa_gasless_approval").reason === "sa_gasless_expired") return await this.halt(op, error);
      throw saError("sa_gasless_approval"); // Missing foreground input preserves awaiting_approval.
    }
    try { now = this.clock.live(op); } catch (error) { return await this.halt(op, error); }
    if (accepted !== true) return { operation: await this.save(op, { state: "failed_before_effect",
  ...
  async run(input: SmartAccountGaslessOperationRecord): Promise<SmartAccountGaslessStep> {
    let op = input;
    if (op.terminal || op.state === "awaiting_approval") return { operation: op };
    // Durable exposure is the sole recovery boundary. No custody or provider read is needed afterward.
    if (op.exposureAttempts === 1) return await this.observe(op);
    if (!["execution_pending", "material_pending", "material_sealed"].includes(op.state) || op.approval === null)
      saFail("sa_gasless_state_corrupt");
    let writing = false;
    const persist = async (patch: Partial<SmartAccountGaslessMutable>, at = new Date(this.clock.check(op)).toISOString()) => {
      writing = true;
    ...
    try {
      this.clock.check(op);
      let sealed: SmartAccountGaslessSealedMaterial;
      if (op.signingAttempts === 1) {
        const recovered = await this.material.load(op); this.clock.check(op);
        if (recovered === null) {
          if (op.material !== null) saFail("sa_gasless_state_corrupt");
      ...
      if (op.material === null) await persist({ state: "material_sealed", material: sealed.descriptor });
      this.clock.live(op);
      ...
      const exposed = await this.material.markExposed(op, sealed); this.clock.live(op);
      if (exposed.phase !== "exposed" || !saSame(exposed.descriptor, sealed.descriptor) ||
        !saSame(exposed.paymentPayload, sealed.paymentPayload)) saFail("sa_gasless_state_corrupt");
      ...
      if (writing || failure.reason === "sa_gasless_state_corrupt") throw saError(failure.reason);
      if (op.exposureAttempts === 0) return await this.halt(op, error);
      if (failure.reason === "sa_gasless_clock" || failure.reason === "sa_gasless_capacity") return { operation: op, warning: failure };
      return await this.observe(op, failure.reason);
    }
    return await this.observe(op);
  }

  private checkSeal(op: SmartAccountGaslessOperationRecord, material: SmartAccountGaslessSealedMaterial): SmartAccountGaslessSealedMaterial {
    saExact(material, ["descriptor", "paymentPayload", "phase"]);
    if (material.phase !== "sealed" || (op.material !== null && !saSame(op.material, material.descriptor))) saFail("sa_gasless_state_corrupt");
    // The material adapter authenticates the payload; this independent journal check binds every public hash to this operation.
    if (op.material === null) advanceSmartAccountGaslessOperation(op, { state: "material_sealed", material: material.descriptor },
      material.descriptor.sealedAt);
    return material;
  }
  private rpc(op: SmartAccountGaslessOperationRecord): SmartAccountGaslessRpcPort {
    ...
    if (rpc.chainId !== op.intent.request.chainId || rpc.endpointHash !== op.intent.initialSnapshot.endpointHash ||
      rpc.endpointOrigin !== op.intent.initialSnapshot.endpointOrigin) saFail("sa_gasless_rpc_binding");
    return rpc;
  }
  private async guard(op: SmartAccountGaslessOperationRecord, signing: boolean,
    material?: SmartAccountGaslessSealedMaterial): Promise<void> {
    this.clock.live(op);
    const owner = await smartAccountGaslessOwner(this.state, op.intent.profile, op.intent.binding); this.clock.live(op);
    ...
    this.clock.live(op);
    if (!saSame(binding, op.intent.binding) || saPolicyHash(binding, op.intent.request) !== op.intent.policyHash) saFail("sa_gasless_identity");
    ...
    this.clock.fresh(snapshot.observedAt, op); this.clock.live(op);
    if (material !== undefined) {
      await rpc.assertUnspent({ binding, material: material.descriptor, safeBlock: snapshot.safeBlock }); this.clock.live(op);
    }
  ...
  private async observe(op: SmartAccountGaslessOperationRecord, priorFailure?: SmartAccountGaslessReason): Promise<SmartAccountGaslessStep> {
    try {
      ...
      let limited = false;
      try { assertSmartAccountGaslessCapacity(op); } catch (error) {
        if (saClassify(error, "sa_gasless_internal").reason !== "sa_gasless_capacity") throw error;
        limited = true;
      }
      ...
      try {
        ...
        if (failure.reason === "sa_gasless_clock") throw error;
      ...
      if (limited && patch.state !== "completed" && patch.state !== "expired_unused") return { operation: op, warning: saFailure("sa_gasless_capacity") };
      ...
      try { advanceSmartAccountGaslessOperation(op, patch, at); }
      catch (error) {
        if (saClassify(error, "sa_gasless_evidence").reason === "sa_gasless_capacity") throw error;
  ...
  private decide(chain: SmartAccountGaslessRpcObservation, prior?: SmartAccountGaslessReason): Partial<SmartAccountGaslessMutable> {
  ...
  private async halt(op: SmartAccountGaslessOperationRecord, error: unknown): Promise<SmartAccountGaslessStep> {
    const failure = saClassify(error, "sa_gasless_internal");
    if (op.exposureAttempts !== 0 || failure.reason === "sa_gasless_state_corrupt") throw saError("sa_gasless_state_corrupt");
