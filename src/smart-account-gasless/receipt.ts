...
import { SA_FILE_LIMIT, SA_RECEIPT_VERSION, type SmartAccountGaslessOperationRecord,
  type SmartAccountGaslessPublicOperation, type SmartAccountGaslessReceipt } from "./operation-model.js";
...
export function publicSmartAccountGaslessOperation(operation: SmartAccountGaslessOperationRecord): SmartAccountGaslessPublicOperation {
  const op = validateSmartAccountGaslessOperation(operation), i = op.intent, s = op.settlement;
  const zero = op.terminal && s === null ? "0" : null;
  const status: SmartAccountGaslessPublicOperation["permission"]["status"] = op.state === "completed" ? "consumed" :
...
export function smartAccountGaslessReceipt(operation: SmartAccountGaslessOperationRecord): SmartAccountGaslessReceipt {
...
export function validateSmartAccountGaslessReceipt(value: unknown, operation: SmartAccountGaslessOperationRecord): SmartAccountGaslessReceipt {
  try {
    const op = validateSmartAccountGaslessOperation(operation);
    if (Buffer.byteLength(canonicalJson(value), "utf8") + 1 > SA_FILE_LIMIT) saFail("sa_gasless_state_corrupt");
    ...
    if (index < 0) saFail("sa_gasless_state_corrupt");
    const historical = smartAccountGaslessAtTransition(op, index);
    if (historical.integrityHash !== binding || !saSame(receipt, smartAccountGaslessReceipt(historical))) saFail("sa_gasless_state_corrupt");
    return receipt as unknown as SmartAccountGaslessReceipt;
