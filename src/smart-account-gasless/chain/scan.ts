...
import { SA_INCREASED_SPENT_TOPIC, SA_TRANSFER_TOPIC, SaRpcBudgetError, SaRpcRangeError, SaRpcReorgError,
  SaScanLimitError, addressWord, quantity, recheckBlock, rpcBlock, rpcRecord, sameBlock,
...
export function initialSmartAccountGaslessCursor(intent: SmartAccountGaslessIntent): SmartAccountGaslessCursor {
...
export function validateSmartAccountGaslessCursor(intent: SmartAccountGaslessIntent,
  cursor: SmartAccountGaslessCursor): SmartAccountGaslessCursor {
  const start = saBlock(cursor.startBlock), expected = intent.initialSnapshot.preparationBlock;
  if (!sameBlock(start, expected)) saFail("sa_gasless_state_corrupt");
  ...
  if (next < startNumber || !Array.isArray(cursor.candidateHashes) || !Array.isArray(cursor.transferAnomalies) ||
    ...
    new Set(cursor.transferAnomalies).size !== cursor.transferAnomalies.length) saFail("sa_gasless_state_corrupt");
  for (const hash of [...cursor.candidateHashes, ...cursor.transferAnomalies]) {
    if (!/^0x[0-9a-f]{64}$/u.test(hash)) saFail("sa_gasless_state_corrupt");
  }
  if (cursor.previousEndBlock === null) {
    if (next !== startNumber) saFail("sa_gasless_state_corrupt");
  } else {
...
export async function advanceSmartAccountGaslessScan(call: SaRpcCall, intent: SmartAccountGaslessIntent,
  childHash: Hex, cursorInput: SmartAccountGaslessCursor,
  safeHead: SmartAccountGaslessBlock): Promise<SmartAccountScanResult> {
  ...
  try {
    ...
    if (cursor.previousEndBlock !== null) await recheckBlock(call, cursor.previousEndBlock, "sa_gasless_evidence", true);
    if (cursor.previousEndBlock !== null &&
      BigInt(safeHead.numberAtomic) < BigInt(cursor.previousEndBlock.numberAtomic)) throw new SaRpcReorgError();
    let expiry = cursor.expiryBlock;
    if (expiry !== null) await recheckBlock(call, expiry, "sa_gasless_evidence", true);
    if (BigInt(safeHead.numberAtomic) < BigInt(cursor.startBlock.numberAtomic)) saFail("sa_gasless_evidence");
    ...
    while (span >= 1n) {
      ...
      try {
        ...
        if (!(error instanceof SaRpcRangeError)) throw error;
        if (span === 1n) throw new SaScanLimitError();
...
async function scanRange(call: SaRpcCall, intent: SmartAccountGaslessIntent, childHash: Hex,
  cursor: SmartAccountGaslessCursor,
  expiry: SmartAccountGaslessBlock | null, safeHead: SmartAccountGaslessBlock, start: bigint,
  end: bigint): Promise<SmartAccountGaslessCursor> {
  ...
  if (!Array.isArray(childRaw) || !Array.isArray(transferRaw) || childRaw.length > SA_SCAN_LOG_LIMIT ||
    transferRaw.length > SA_SCAN_LOG_LIMIT) throw new SaScanLimitError();
  const additions: Hex[] = [], transfers: Hex[] = [];
  ...
  for (const value of canonicalScanRecords(transferRaw)) {
    ...
    if (hash !== null) transfers.push(hash);
  }
...
function canonicalScanRecords(values: readonly unknown[]): readonly Record<string, unknown>[] {
  ...
  for (const value of values) {
    const record = rpcRecord(value), position = [rpcQuantity(record.blockNumber),
      rpcQuantity(record.transactionIndex), rpcQuantity(record.logIndex)] as const;
    if (previous !== null && comparePosition(position, previous) <= 0) saFail("sa_gasless_evidence");
    previous = position; records.push(record);
  }
  return records;
}

function comparePosition(left: readonly bigint[], right: readonly bigint[]): number {
...
async function findExpiryBlock(call: SaRpcCall, start: SmartAccountGaslessBlock, head: SmartAccountGaslessBlock,
  threshold: bigint): Promise<SmartAccountGaslessBlock> {
  let low = BigInt(start.numberAtomic), high = BigInt(head.numberAtomic), answer = head;
  if (BigInt(start.timestampAtomic) >= threshold) return start;
  while (low <= high) {
    const middle = (low + high) / 2n, block = (await rpcBlock(call, quantity(middle))).block;
    if (BigInt(block.timestampAtomic) >= threshold) { answer = block; high = middle - 1n; }
    else low = middle + 1n;
  }
  await recheckBlock(call, answer, "sa_gasless_evidence", true);
  if (BigInt(answer.timestampAtomic) < threshold) saFail("sa_gasless_evidence");
  if (BigInt(answer.numberAtomic) > BigInt(start.numberAtomic)) {
    ...
    if (BigInt(prior.timestampAtomic) >= threshold) saFail("sa_gasless_evidence");
    await recheckBlock(call, prior, "sa_gasless_evidence", true);
  }
  return answer;
}

function append(existing: readonly Hex[], values: readonly Hex[]): readonly Hex[] {
