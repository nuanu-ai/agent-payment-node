...
import { SA_DELEGATION_TUPLE_PARAMETER, SA_INCREASED_SPENT_TOPIC, SA_REDEEMED_TOPIC, SA_TRANSFER_TOPIC,
  rpcAddress, rpcHex, rpcQuantity, topicAddress, twoWords, type SaReceiptLog } from "./abi.js";
...
export function verifySmartAccountReceipt(intent: SmartAccountGaslessIntent, outerSender: Address,
  redemption: VerifiedSmartAccountRedemption, logs: readonly SaReceiptLog[]): SmartAccountReceiptAccounting {
  const registry = saRegistry(8453), gross = BigInt(intent.request.grossAtomic);
  const exactTransfers: SaReceiptLog[] = [], ownerOutflows: SaReceiptLog[] = [];
  ...
  if (managerEvents.length !== 2) saFail("sa_gasless_evidence");
  const expectedData = [redemption.child, redemption.root].map(delegation =>
    encodeAbiParameters([SA_DELEGATION_TUPLE_PARAMETER], [delegation]));
  const observedData = managerEvents.map(log => {
    if (log.topics.length !== 3 || topicAddress(log.topics[1]!) !== intent.binding.ownerAddress ||
      topicAddress(log.topics[2]!) !== outerSender) saFail("sa_gasless_evidence");
    return log.data;
  });
  if (expectedData.some(value => observedData.filter(observed => observed === value).length !== 1) ||
    observedData.some(value => !expectedData.includes(value))) saFail("sa_gasless_evidence");

  const spentEvents = logs.filter(log => log.address === registry.protocol.amount.address &&
    log.topics[0] === SA_INCREASED_SPENT_TOPIC);
  if (spentEvents.length !== 1) saFail("sa_gasless_evidence");
  ...
  if (limit !== gross || amount !== gross) saFail("sa_gasless_evidence");
...
export function verifyChildScanLog(value: Record<string, unknown>, intent: SmartAccountGaslessIntent,
  childHash: Hex, from: bigint, to: bigint): Hex {
  ...
  if (rpcAddress(value.address) !== registry.protocol.amount.address || value.removed !== false ||
    ...
    rpcHex(topics[3], 32, 32) !== childHash) saFail("sa_gasless_evidence");
  twoWords(rpcHex(value.data, 64, 64)); validateScanIdentity(value);
  return rpcHex(value.transactionHash, 32, 32);
}

export function verifyTransferScanLog(value: Record<string, unknown>, intent: SmartAccountGaslessIntent,
  from: bigint, to: bigint): Hex | null {
  ...
  if (rpcAddress(value.address) !== registry.token.address || value.removed !== false || number < from || number > to ||
    ...
    rpcHex(topics[2], 32, 32) !== addressTopic(intent.request.recipient)) saFail("sa_gasless_evidence");
  const amount = BigInt(rpcHex(value.data, 32, 32)); validateScanIdentity(value);
  return amount === BigInt(intent.request.grossAtomic) ? rpcHex(value.transactionHash, 32, 32) : null;
}

function validateScanIdentity(value: Record<string, unknown>): void {
  rpcHex(value.blockHash, 32, 32); rpcQuantity(value.transactionIndex); rpcQuantity(value.logIndex);
}
function addressTopic(address: Address): Hex { return `0x${address.slice(2).padStart(64, "0")}`; }
