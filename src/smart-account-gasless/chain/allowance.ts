...
import { SA_MAX_UINT, type SmartAccountGaslessBinding, type SmartAccountGaslessBlock } from "../model.js";
import { saFail } from "../reasons.js";
import { saRegistry } from "../registry.js";
import { SA_CURRENT_NONCE_SELECTOR, SA_GET_AVAILABLE_SELECTOR, SA_SPENT_MAP_SELECTOR,
  quantity, rpcHex, type SaRpcCall } from "./abi.js";
...
export function validatePeriodTerms(binding: SmartAccountGaslessBinding): void {
  ...
  if (terms.slice(0, 42) !== registry.token.address || amount.toString() !== binding.rootCapAtomic ||
    duration !== SA_MAX_UINT || start !== BigInt(binding.rootStartsAtUnix)) saFail("sa_gasless_permission");
}

/** Exact 96-byte return validation intentionally does not use a permissive ABI decoder. */
export async function readCurrentAllowance(call: SaRpcCall, binding: SmartAccountGaslessBinding,
  block: SmartAccountGaslessBlock): Promise<SmartAccountAllowanceRead> {
  ...
  if (!data.startsWith(SA_GET_AVAILABLE_SELECTOR)) saFail("sa_gasless_internal");
  ...
  if (boolWord !== "0".repeat(64) && boolWord !== `${"0".repeat(63)}1`) saFail("sa_gasless_evidence");
  if (available > BigInt(binding.rootCapAtomic)) saFail("sa_gasless_evidence");
  return { availableAtomic: available.toString(), isNewPeriod: boolWord.endsWith("1"),
    currentPeriodAtomic: period.toString() };
}

export async function readCurrentNonce(call: SaRpcCall, binding: SmartAccountGaslessBinding,
  block: SmartAccountGaslessBlock): Promise<string> {
  const registry = saRegistry(8453);
  const data = encodeFunctionData({ abi: NONCE_ABI, functionName: "currentNonce",
    args: [binding.delegationManager, binding.ownerAddress] });
  if (!data.startsWith(SA_CURRENT_NONCE_SELECTOR)) saFail("sa_gasless_internal");
  ...
  if (current !== binding.rootNonceAtomic) saFail("sa_gasless_permission");
  return current;
}

export async function readChildSpent(call: SaRpcCall, manager: Address, childHash: Hex,
  block: SmartAccountGaslessBlock): Promise<string> {
  const registry = saRegistry(8453);
  const data = encodeFunctionData({ abi: SPENT_ABI, functionName: "spentMap", args: [manager, childHash] });
  if (!data.startsWith(SA_SPENT_MAP_SELECTOR)) saFail("sa_gasless_internal");
