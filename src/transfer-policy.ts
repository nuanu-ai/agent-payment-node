...
export function validateEconomics(nonceAtomic: string, fees: FeeEstimate): Economics {
  ...
  if (parseAtomic(priority) > parseAtomic(maxFee)) throw new ApnError("APN_RPC_PROTOCOL", "Priority fee exceeds maximum fee.");
  return {
    nonceAtomic: nonce,
    gasLimitAtomic: gas,
...
export function requireFunding(snapshot: BalanceSnapshot, amountAtomic: string, gasAtomic: string): void {
...
export function parseEffect(value: unknown): NativeEffect {
  ...
  for (const key of ["transactionHash", "rawTransaction", "rawTransactionHash"] as const) {
  ...
  const transactionHash = value.transactionHash as string;
  const rawTransaction = value.rawTransaction as string;
  const rawTransactionHash = value.rawTransactionHash as string;
  ...
  return {
    ...
    rawTransactionHash: rawTransactionHash as Hex,
  };
}

export async function verifyEffect(effect: NativeEffect, operation: OperationRecord): Promise<void> {
  ...
  try {
    const serialized = effect.rawTransaction as TransactionSerialized;
    transaction = parseTransaction(serialized);
    recovered = getAddress(await recoverTransactionAddress({ serializedTransaction: serialized })) as Address;
  } catch {
...
export function hasExactTransfer(receipt: RpcReceipt, operation: OperationRecord): boolean {
...
export function publicOperation(operation: OperationRecord): unknown {
  return {
...
function operationNextActions(operation: OperationRecord): readonly string[] {
  ...
  if (operation.state === "failed_before_effect" && operation.reason === "provider_amount_encoding_incompatible") return [
  ...
  if (operation.state === "failed_before_effect") return [
  ...
  if (operation.state === "awaiting_approval") return [
