export type NormalizedProviderJson =
  | null
  | boolean
  | number
  | string
  | readonly NormalizedProviderJson[]
...
export function canonicalizeNormalizedProviderJson(value: unknown): string {
  const state = { nodes: 0, bytes: 0, output: [] as string[], path: new WeakSet<object>() };
  appendValue(value, 0, state);
  return state.output.join("");
}

export function isSafeNormalizedProviderJson(value: unknown): value is NormalizedProviderJson {
  try {
...
function appendValue(value: unknown, depth: number, state: CanonicalState): void {
  if (depth > NORMALIZED_PROVIDER_JSON_LIMITS.maxDepth) invalid();
  state.nodes += 1;
  if (state.nodes > NORMALIZED_PROVIDER_JSON_LIMITS.maxNodes) invalid();

  if (value === null) return append("null", state);
  if (typeof value === "boolean") return append(value ? "true" : "false", state);
  ...
  if (state.path.has(value)) invalid();
  state.path.add(value);
  try {
    if (Array.isArray(value)) appendArray(value, depth, state);
    else appendObject(value, depth, state);
  } finally {
    state.path.delete(value);
  }
}

function appendArray(value: unknown[], depth: number, state: CanonicalState): void {
  if (value.length > NORMALIZED_PROVIDER_JSON_LIMITS.maxArrayItems) invalid();
...
function appendObject(value: object, depth: number, state: CanonicalState): void {
...
function append(value: string | undefined, state: CanonicalState): void {
  ...
  if (state.bytes > NORMALIZED_PROVIDER_JSON_LIMITS.maxCanonicalBytes) invalid();
