...
export function decodeX402RequestBody(value: unknown): Buffer | undefined {
...
export function normalizeX402HttpRequest(value: unknown): X402HttpRequestV1 {
  ...
  if (endpoint.toString() !== value.url) throw invalid("HTTP request URL must be canonical.");
  const method = value.method.toUpperCase();
  if (method === "CONNECT" || method === "TRACE") throw invalid("HTTP tunneling and trace methods are forbidden.");
  if (!isPlainRecord(value.headers) || Object.keys(value.headers).length > 32) throw invalid("HTTP headers are invalid.");
  ...
  for (const [originalName, headerValue] of Object.entries(value.headers)) {
    ...
    if (headerBytes > 8192) throw invalid("HTTP headers exceed 8 KiB.");
  ...
  return Object.freeze({
    ...
    bodyBase64: value.bodyBase64 as string | null,
  });
}

export function optionalX402HttpRequest(url: string, value: unknown): X402HttpRequestV1 | undefined {
  if (value === undefined) return undefined;
  const request = normalizeX402HttpRequest(value);
  if (request.url !== url) throw invalid("HTTP request does not match the seller URL.");
  return request;
}

export function bindX402HttpRequest(options: Readonly<Record<string, string>>): { readonly httpRequest?: X402HttpRequestV1 } {
  if (options["--method"] === undefined && options["--headers-json"] === undefined && options["--body-base64"] === undefined) return {};
  let headers: unknown = {};
  const encodedHeaders = options["--headers-json"];
  if (encodedHeaders !== undefined) {
    if (Buffer.byteLength(encodedHeaders, "utf8") > 16 * 1024) throw invalid("HTTP headers JSON is oversized.");
    try { headers = parseJsonWithDuplicateRejection(encodedHeaders); }
    catch { throw invalid("HTTP headers must be a JSON object without duplicate names."); }
  }
...
export function x402HttpRequestBinding(request: X402HttpRequestV1 | undefined): { readonly httpRequestHash?: string } {
  return request === undefined ? {} : { httpRequestHash: domainHash("apn.http-request.v1", canonicalJson(request)) };
}

export function validateFrozenX402HttpRequest(url: string, value: unknown): void {
  if (value === undefined) return;
  try {
    if (canonicalJson(optionalX402HttpRequest(url, value)) === canonicalJson(value)) return;
  } catch {}
...
export function validateX402Resource(value: unknown): X402OperationRecord["resource"] {
  try {
    ...
    if (endpoint.toString() !== value.canonicalUrl || value.origin !== endpoint.origin ||
      value.path !== endpoint.pathname || value.urlHash !== sha256(value.canonicalUrl)) throw invalid("Invalid resource binding.");
    validateFrozenX402HttpRequest(value.canonicalUrl, value.httpRequest);
    return value as unknown as X402OperationRecord["resource"];
  } catch {
    throw new ApnError("APN_STATE_CORRUPT", "Frozen x402 resource is invalid.");
  }
}

export function isX402ResultStatus(resource: unknown, status: unknown): boolean {
