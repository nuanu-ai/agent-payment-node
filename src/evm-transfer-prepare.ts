...
export async function prepareEvmTransfer(
  ...
  return await state.withLocks([`profile:${profileHash}`, `operation:${operationId}`, `operation:idempotency:${idempotencyHash}`], async () => {
    ...
    if (selection.chainId === 1 || selection.chainId === 56 || selection.chainId === 8453 || selection.chainId === 42161) rpcPort.armEvmDirectRpcGuard?.();
    ...
    if ((request.batchRpcReads || ethereumNative || selection.chainId === 56) && grouped === undefined) throw new ApnError("APN_RPC_CONFIG", "Selected RPC does not support batched prepare reads.");
