...
import { effectiveX402Cap, type ProfilePolicyRecord } from "./profile-policy.js";
import type { ProviderAdapterBundle, ProviderBalanceObservation } from "./provider-ports.js";
import { capabilityHash, LOCAL_PROVIDER_ID, markProviderProfileDrift, type ProviderProfileRecord } from "./provider-profile.js";
...
export async function requireProviderX402Profile(context: RuntimeContext, profileHash: string): Promise<ProviderProfileRecord> {
  const profile = await context.requireProfileRepository().load(profileHash);
  if (
    profile === null || profile.drift.state !== "bound" || profile.provider_id === LOCAL_PROVIDER_ID ||
...
export function requireProviderX402Adapter(context: RuntimeContext, profile: ProviderProfileRecord): ExecutableProviderX402Adapter {
  const adapter = context.requireProviderRegistry().resolve(profile.provider_id);
  if (
    adapter.x402?.mode !== "provider_atomic_paid_fetch" || adapter.x402.execute === undefined ||
  ...
  return adapter as ExecutableProviderX402Adapter;
}

export async function observeProviderX402Balance(
  ...
  if (
    observed.chain !== "base" || observed.asset !== "USDC" || observed.decimals !== 6 ||
  ...
  if (
    observed.address.toLowerCase() !== profile.public_address.toLowerCase() ||
...
export function assertFrozenProviderX402Policy(
...
export function assertProviderX402RpcBinding(context: RuntimeContext, operation: ProviderX402OperationRecord): void {
...
export function canonicalProviderX402RpcUrl(value: string): string {
