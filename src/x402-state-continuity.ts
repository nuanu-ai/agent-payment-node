...
export function validateX402AppendOnly(previous: X402OperationRecord, next: X402OperationRecord): void {
  const immutableKeys = [
...
export function validateX402ScanContinuity(previous: X402OperationRecord, next: X402OperationRecord): void {
  const priorScan = previous.authorizationUsedScan;
  const nextScan = next.authorizationUsedScan;
...
export function sameOptionalCanonical(left: unknown, right: unknown): boolean {
