...
import type {
  ProviderAuthorizationBinding,
  ProviderAuthorizationRecord,
  ProviderAuthorizationStorePort,
...
import {
  verifyAndConstructX402PaymentMaterial,
  type VerifiedX402PaymentMaterial,
  type X402NativeAuthorizationMaterial,
} from "./x402-native.js";
import type { X402OperationRecord } from "./x402-state-integrity.js";

export type ProviderAuthorizationOutcome =
  | { readonly disposition: "signed"; readonly material: VerifiedX402PaymentMaterial }
...
export class ProviderX402AuthorizationService {
  constructor(
    ...
    private readonly store: ProviderAuthorizationStorePort,
  ) {}

  async authorize(
    operation: X402OperationRecord,
    kind: "create" | "get",
  ): Promise<ProviderAuthorizationOutcome> {
    ...
    if (existing !== null) {
      if (existing.requestHash !== requestHash) corrupt("Provider authorization request binding changed.");
      return await this.continueExisting(operation, binding, existing, signer);
    }
    if (kind === "get") corrupt("Provider authorization recovery marker is missing.");
    ...
    let outcome: X402SigningResult;
    try { outcome = await signer.request(intent); }
    catch { return { disposition: "ambiguous" }; }
  ...
  private async continueExisting(
    ...
    existing: ProviderAuthorizationRecord,
    signer: X402SigningPort,
  ): Promise<ProviderAuthorizationOutcome> {
    if (existing.phase === "signed") return {
      disposition: "signed",
      material: await this.verified(operation, existing.signature, existing.signatureHash),
    };
    if (existing.phase === "rejected") return { disposition: "rejected" };
    if (existing.phase === "invocation_started") return { disposition: "ambiguous" };
    let outcome: X402SigningResult;
    try {
    ...
    if (outcome.disposition === "ambiguous") return { disposition: "pending" };
  ...
  private async acceptOutcome(
    ...
    if (outcome.disposition === "ambiguous") return { disposition: "ambiguous" };
  ...
  private async verified(
    ...
    const native: X402NativeAuthorizationMaterial = {
      authorization: {
  ...
  private async requiredSigner(operation: X402OperationRecord): Promise<X402SigningPort> {
    const binding = operation.providerSigner;
    if (binding === undefined) corrupt("Provider x402 signer binding is missing.");
    ...
    if (
      adapter.provider_id !== binding.providerId || adapter.capabilities.x402.mode !== binding.executionMode ||
...
export function providerAuthorizationBinding(operation: X402OperationRecord): ProviderAuthorizationBinding {
  const signer = operation.providerSigner;
  if (signer === undefined) corrupt("Provider x402 signer binding is missing.");
  return {
...
export function signingIntent(operation: X402OperationRecord): X402SigningIntent {
  if (operation.chainId !== "8453") corrupt("Provider signing is declared only on Base.");
...
export function providerAuthorizationRequestHash(
  binding: ProviderAuthorizationBinding,
  intent: X402SigningIntent,
): string {
