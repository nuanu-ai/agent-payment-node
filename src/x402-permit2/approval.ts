...
export interface Permit2ApprovalSummary {
  ...
  readonly caps: { readonly maximumPerTransferAtomic: string; readonly dailyLimitAtomic: string };
...
export function permit2ApprovalScreen(summary: Permit2ApprovalSummary): Permit2ApprovalScreen {
  const { plan } = summary, asset = plan.selection.listAsset, auth = plan.authorization;
  const units = (atomic: string) => `${formatAtomic(atomic, asset.decimals)} ${asset.symbol} (${atomic} atomic)`;
...
export class TtyPermit2Approval {
  ...
  async confirm(screen: Permit2ApprovalScreen, approveBefore: string): Promise<boolean> {
    try {
