...
import { assetUsageReservationId, validateAssetUsageReservation, type AssetUsageIdentity,
...
export class Permit2ExposureLifecycle extends SecureStateStore {
  ...
  async load(operationId: string): Promise<Permit2ExposureRecord | null> {
    ...
    if (!isPlainRecord(value) || !exactKeys(value, ["schemaVersion", "operationId", "reservationId", "profileHash",
      ...
      "reference", "proofDigest", "integrityHash"])) corrupt();
    ...
    if (value.schemaVersion !== SCHEMA || value.operationId !== operationId ||
        ...
        (value.usageReservationId !== null &&
          value.usageReservationId !== permit2UsageReservationId(value as unknown as Permit2EffectBinding))) corrupt();
   ...
   */
  async resume(operationId: string, prepared: Permit2PreparedMaterial, ports: Permit2EffectPorts): Promise<Permit2ExposureRecord> {
    ...
    let lease: AssetUsageReservation | null = null;
    if (first.state === "reserving") {
      ...
      if (lease !== null) assertLease(first.binding, lease);
    }
    return this.withLocks([this.lock(operationId)], async () => {
      const intent = await this.intents.load(operationId);
      if (intent === null || canonicalJson(bound(intent)) !== canonicalJson(first.binding)) corrupt();
      assertPreparedBinding(prepared, first.binding);
      let current = await this.load(operationId);
      if (current === null || canonicalJson(bound(current)) !== canonicalJson(first.binding)) corrupt();
      if (current.state === "reserving") {
        if (first.state !== "reserving") corrupt();
      ...
      if (current.state === "reserved") {
        if (BigInt(first.binding.deadline) <= BigInt(clock(ports))) return current;
...
export function permit2UsageIdentity(value: Permit2EffectBinding): AssetUsageIdentity {
  return { account: value.owner, chain: value.chain, asset: { kind: "token", identifier: value.token } };
}
export function permit2UsageKey(value: Permit2EffectBinding): string {
  return `permit2-exposure:${value.reservationId}`;
}
export function permit2UsageReservationId(value: Permit2EffectBinding): string {
  return assetUsageReservationId(permit2UsageIdentity(value), permit2UsageKey(value));
}
function assertLease(binding: Permit2EffectBinding, value: unknown): asserts value is AssetUsageReservation {
  ...
  if (lease.reservationId !== permit2UsageReservationId(binding) ||
      ...
      lease.amountAtomic !== binding.amountAtomic || lease.state !== "reserved") corrupt();
}
function clock(ports: Permit2EffectPorts): number {
