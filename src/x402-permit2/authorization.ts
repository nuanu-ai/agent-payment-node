...
export async function assemblePermit2Payment(plan: Permit2SigningPlan, signer: Permit2SignerPort): Promise<unknown> {
  const payer = plan.authorization.from;
  const signature = await signer.signPermit2Material(payer, plan.permit2);
  await requireSigner(plan.permit2, signature, payer);
  let extensions: Record<string, unknown> | undefined;
  if (plan.eip2612 !== null) {
    const permitSignature = await signer.signPermit2Material(payer, plan.eip2612.typedData);
    await requireSigner(plan.eip2612.typedData, permitSignature, payer);
    extensions = { [EIP2612_GAS_SPONSORING]: { info: { ...plan.eip2612.info, signature: permitSignature } } };
  }
  return {
    x402Version: 2,
    accepted: structuredClone(plan.selection.requirement),
    payload: { signature, permit2Authorization: structuredClone(plan.authorization) },
    ...(extensions === undefined ? {} : { extensions }),
  };
}

async function requireSigner(typedData: Permit2TypedData, signature: Hex, payer: Address): Promise<void> {
  if (typeof signature !== "string" || !/^0x[0-9a-fA-F]{130}$/u.test(signature)) {
    throw new ApnError("APN_NATIVE_PROTOCOL", "The local signer returned a malformed signature.");
  }
  let recovered: string;
  try {
    recovered = await recoverTypedDataAddress({ ...typedData, signature } as Parameters<typeof recoverTypedDataAddress>[0]);
  } catch { throw new ApnError("APN_NATIVE_PROTOCOL", "The local signature does not recover."); }
