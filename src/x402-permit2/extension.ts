...
const FIELDS = ["from", "asset", "spender", "amount", "nonce", "deadline", "signature", "version"] as const;

/** Recognize the version-one server declaration from @x402/extensions, without altering its contents. */
export function isEip2612GasSponsoringDeclaration(value: unknown): boolean {
  if (!isPlainRecord(value) || !isPlainRecord(value.info) || !isPlainRecord(value.schema)) return false;
  ...
  if (typeof info.description !== "string" || info.description.length === 0 || info.description.length > 1024 ||
      ...
      schema.required.length !== FIELDS.length || !schema.required.every((field: unknown) => typeof field === "string") ||
      !exactKeys(Object.fromEntries(schema.required.map((field: string) => [field, true])), FIELDS)) return false;
