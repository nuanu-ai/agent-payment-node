...
function bindParsedCatalog(parsed: ParsedCatalogCommand): BoundCommand {
  ...
  if (parsed.command.path.join(" ") === "relay arbitrum prepare") return { request: { command: "relay.arbitrum.prepare", profile: value(options, "--profile"), owner: value(options, "--owner"), amountAtomic: value(options, "--amount-atomic"), minOutputAtomic: value(options, "--min-output-atomic"), maxProviderFeeAtomic: value(options, "--max-provider-fee-atomic"), maxApprovalNetworkFeeWei: value(options, "--max-approval-network-fee-wei"), maxDepositNetworkFeeWei: value(options, "--max-deposit-network-fee-wei"), quoteFile: value(options, "--quote-file"), idempotencyKey: value(options, "--idempotency-key") } };
  if (parsed.command.path.join(" ") === "relay arbitrum observe") return { request: { command: "relay.arbitrum.observe", operationId: value(options, "--operation") }, rpcUrl: value(options, "--rpc-url") };
  if (parsed.command.path.join(" ") === "relay arbitrum approval-check") return { request: { command: "relay.arbitrum.approval-check", profile: value(options, "--profile"), operationId: value(options, "--operation") }, rpcUrl: value(options, "--rpc-url") };
  if (parsed.command.path.join(" ") === "relay arbitrum approval-execute") return { request: { command: "relay.arbitrum.approval-execute", profile: value(options, "--profile"), operationId: value(options, "--operation") }, rpcUrl: value(options, "--rpc-url") };
  if (parsed.command.path.join(" ") === "relay arbitrum deposit-dispatch") return { request: { command: "relay.arbitrum.deposit-dispatch", profile: value(options, "--profile"), operationId: value(options, "--operation") }, rpcUrl: value(options, "--rpc-url") };
  ...
  if (parsed.command.path.join(" ") === "relay native execute") return { request: { command: "relay.native.execute", operationId: value(options, "--operation") }, rpcUrl: value(options, "--rpc-url") };
  ...
  if (parsed.command.path[0] === "swap") {
    ...
    if (path.startsWith("swap solana orca ")) return { request: bindOrcaCommand(path, options) };
...
function solanaAsset(options: Readonly<Record<string, string>>): "sol" | "usdc" | "usdt" {
  const asset = value(options, "--asset");
  if (asset !== "sol" && asset !== "usdc" && asset !== "usdt") throw new ApnError("APN_INVALID_INPUT", "Select the explicit sol, usdc or usdt asset alias.");
  return asset;
}
