...
const asset = option("--asset", "string", ["sol_usdc_or_usdt", "exact_mainnet_asset_identity"]);
...
const output = { contract: "apn.cli.v1", success_exit: 0, failure_exit: 1,
  success: "Exact Solana mainnet identity with bounded public data and classified recovery.", failures: ["Classified APN error; unavailable providers never fall back to another wallet."] } as const;
const completed = { terminal: ["completed", "classified_failure"], non_terminal: [] };
export const CHAIN_COMMANDS: readonly CommandDefinition[] = [
  ...
  { path: ["wallet", "balance-solana"], synopsis: "apn wallet balance-solana --profile <profile> --asset <sol-usdc-or-usdt>",
    summary: "Read SOL, canonical USDC or pinned USDT and the separate SOL fee balance through APN_SOLANA_RPC_URL.", options: [profile, asset],
  ...
  { path: ["policy", "admit-solana"], synopsis: "apn policy admit-solana --profile <profile> --asset <sol-usdc-or-usdt> --max-per-transfer <decimal> --daily-limit <decimal> --max-fee-sol <decimal>",
  ...
  { path: ["pay", "transfer", "prepare-solana"], synopsis: "apn pay transfer prepare-solana --profile <profile> --asset <sol-usdc-or-usdt> --to <solana-address> --amount <decimal> --max-fee-sol <decimal> --idempotency-key <key>",
...
export function includeRailRecovery(commands: readonly CommandDefinition[]): readonly CommandDefinition[] {
  return commands.map((command) => {
    ...
    if (!["pay transfer approve", "operation resume", "operation status", "receipt get"].includes(path)) return command;
