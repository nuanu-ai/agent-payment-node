...
import {
  USDT_EXCHANGE_RATE_MAX, USDT_GASLESS, USDT_GASLESS_GAS, USDT_POST_OP_GAS_MAX, usdtFailure,
  type UsdtGasPrice, type UsdtGaslessGas, type UsdtTokenQuote, type UsdtTransferPlan, type UsdtTransferRequest,
} from "./model.js";
...
export function validateUsdtTokenQuote(result: unknown): UsdtTokenQuote {
  ...
  const quote: unknown = result.quotes[0];
  if (!isPlainRecord(quote) || !exactKeys(quote, QUOTE_KEYS)) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_quote_shape");
  if (gaslessAddress(quote.paymaster) !== USDT_GASLESS.paymaster) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_quote_paymaster");
  if (gaslessAddress(quote.token) !== USDT_GASLESS.token) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_quote_token");
  ...
  if (postOpGas === 0n || postOpGas > USDT_POST_OP_GAS_MAX) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_quote_post_op_gas");
  if (exchangeRate === 0n || exchangeRate > USDT_EXCHANGE_RATE_MAX || exchangeRateNativeToUsd === 0n ||
    exchangeRateNativeToUsd > USDT_EXCHANGE_RATE_MAX) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_quote_rate");
...
export function validateUsdtGasPrice(result: unknown): { readonly fast: UsdtGasPrice; readonly slow: UsdtGasPrice } {
  if (!isPlainRecord(result) || !exactKeys(result, ["slow", "standard", "fast"])) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_price_shape");
  const tier = (value: unknown): UsdtGasPrice => {
    ...
    if (maxFeePerGas === 0n || maxPriorityFeePerGas > maxFeePerGas) usdtFailure("APN_PROVIDER_PROTOCOL", "gasless_usdt_price_bounds");
 ...
 */
export function usdtFeeBound(gas: UsdtGaslessGas, maxFeePerGas: bigint, postOpGas: bigint, exchangeRate: bigint): bigint {
 ...
 */
export function planUsdtTransfer(request: UsdtTransferRequest, quote: UsdtTokenQuote, price: UsdtGasPrice): UsdtTransferPlan {
  const { grossAtomic: gross, maxFeeAtomic: maxFee, minReceivedAtomic: minimum } = request;
  if (gross <= 0n || minimum <= 0n || minimum > gross || maxFee < 0n) usdtFailure("APN_INVALID_INPUT", "gasless_amount_bounds");
  if (request.recipient === request.sender) usdtFailure("APN_INVALID_INPUT", "gasless_usdt_self_transfer");
  ...
  if (feeCapAtomic === 0n) {
    usdtFailure("APN_FEE_BUDGET_EXCEEDED", "gasless_usdt_fee_budget_zero",
      "The sponsor charges its fee in USDT; --max-fee and --min-received must leave a positive fee budget.");
  }
  ...
  if (quotedFeeAtomic > feeCapAtomic) {
    usdtFailure("APN_FEE_BUDGET_EXCEEDED", "gasless_usdt_quote_above_max_fee",
      `The current USDT fee quote (${quotedFeeAtomic} atomic) exceeds the fee budget (${feeCapAtomic} atomic).`);
  }
