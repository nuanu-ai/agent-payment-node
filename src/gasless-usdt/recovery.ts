...
import { USDT_GASLESS, USDT_GASLESS_GAS, type UsdtTransferPlan } from "./model.js";
import { verifyUsdtReceipt, verifyUsdtRevert, type UsdtChainReceipt } from "./receipt.js";
...
export class UsdtRecoveryService {
  constructor(private readonly journal: UsdtExecutionJournal, private readonly port: UsdtRecoveryPort,
    private readonly now: () => Date) {}

  async observe(value: UsdtBoundOperation): Promise<UsdtExecutionRecord> {
    ...
    if (current.state === "finalized" || current.state === "failed_confirmed_revert") return current;
    ...
    const hash = current.userOperationHash as Hex;
    let outcome: { state: "finalized" | "failed_confirmed_revert"; digest: string;
      settlement: import("./receipt.js").UsdtSettlement | null } | null = null;
    try {
      ...
      if (locator !== null && locator.userOpHash.toLowerCase() === hash &&
        ...
        if (receipt !== null && receipt.transactionHash.toLowerCase() === locator.transactionHash.toLowerCase()) {
          if (locator.success) {
            ...
            verifyUsdtRevert(current.sender as `0x${string}`, hash, receipt);
    ...
    if (outcome === null) return await this.journal.markUnknownFinality(bound, this.now());
