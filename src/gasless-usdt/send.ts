...
export class GuardedUsdtSendService {
  ...
  async send(value: UsdtBoundOperation, identity: UsdtSigningIdentity): Promise<UsdtExecutionRecord> {
    ...
    return await this.journal.withEffectLock(bound.operationId, async abortUnsent => {
      const prior = await this.journal.load(bound.operationId);
      if (prior !== null) {
        if (["planned", "reserved", "failed_before_effect"].includes(prior.state)) {
          await abortUnsent(bound, this.port.now());
        }
        throw new ApnError("APN_OPERATION_BLOCKED", "Gasless USDT send has an existing execution record.", { rail: "gasless_usdt" });
      }
      try {
        ...
        // The fsynced submitting transition is the last action before the one permitted network call.
        ...
        catch { return await this.journal.markUnknownFinality(bound, this.port.now()); }
        if (response !== signed.userOperationHash) return await this.journal.markUnknownFinality(bound, this.port.now());
        return await this.journal.markSubmitted(bound, response, this.port.now());
      } catch (error) {
        // Before the durable hash-bound marker no network send can have begun. Preserve the marker on every later failure.
        await abortUnsent(bound, this.port.now());
        throw error;
      }
    });
