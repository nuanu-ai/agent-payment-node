...
export class ProviderWalletService {
  ...
  async connect(request: Extract<CommandRequest, { readonly command: "wallet.connect" }>): Promise<unknown> {
    ...
    return await this.context.state.withLocks([`profile:${profileHash}`], async () => {
      ...
      if (existing !== null) existing = await upgradeProviderProfile(adapter, existing, repository);
  ...
  async status(profileInput: string): Promise<unknown | null> {
    ...
    if (initial?.provider_id === "local") return null;
    if (initial === null) {
      if (this.context.providerRegistry === undefined || await this.context.state.loadWallet(profileHash) !== null) return null;
      await assertWalletLifecycleAvailable(this.context, profileHash);
    ...
    return await this.context.state.withLocks([`profile:${profileHash}`], async () => {
      ...
      if (current?.provider_id === "local") return null;
      if (current === null && await this.context.state.loadWallet(profileHash) !== null) return null;
      if (current === null && this.context.providerRegistry === undefined) return null;
      await assertWalletLifecycleAvailable(this.context, profileHash);
      if (current === null) {
        if (this.context.providerRegistry === undefined) return null;
        const pending = await findPendingProviderPermission(this.context.requireProviderRegistry(), profileHash);
  ...
  async assertPaymentAvailable(profileInput: string, kind: "direct" | "x402", idempotencyKey: string): Promise<void> {
    ...
    await this.context.state.withLocks([`profile:${profileHash}`], async () => {
      ...
      const operations = new OperationService(this.context.state, this.context.providerX402Repository,
...
function alignPermissionProfile(
  ...
  const observed = permissionObservation(adapter, permission);
  if (permission.revision < profile.revision && sameBinding(profile, observed)) return profile;
