...
export const uintSchema = z.string().refine((v) => { try { return gaslessUint(v) >= 0n; } catch { return false; } });
export const addressSchema = z.string().refine((v) => { try { return gaslessAddress(v) === v; } catch { return false; } });
export const isoSchema = z.string().refine((v) => { try { return gaslessIso(v) === v; } catch { return false; } });
...
export const originSchema = z.string().max(256).refine((v) => {
  try { const u = new URL(v); return u.protocol === "https:" && u.origin === v && !u.username && !u.password; }
  catch { return false; }
});
...
export const intentSchema = z.strictObject({ wireVersion: z.enum(["apn.gasless-wire.v2", "apn.gasless-wire.v3", "apn.gasless-wire.v4"]).optional(),
  ...
  allowlist: z.strictObject({ policyDigest: hashSchema, policyRevision: z.number().int().positive().safe(),
    activationDigest: hashSchema, chain: z.enum(["eip155:8453", "eip155:1"]), token: addressSchema,
