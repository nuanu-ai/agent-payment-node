...
export function gaslessObservationRpcEnv(value: unknown): string {
...
export function gaslessObservationSource(intent: GaslessIntent, environmentName: string,
  rpc: Pick<GaslessObservationPort, "rpcOrigin" | "rpcEndpointHash">): GaslessObservationSource {
  const source: GaslessObservationSource = { policy: "apn.gasless.observation-rpc.v1",
...
export function assertGaslessObservationSource(intent: GaslessIntent, source: GaslessObservationSource): void {
  ...
  } catch { gaslessFailure("APN_STATE_CORRUPT", "gasless_observation_source_binding"); }
}
