...
import { addressWord, rpcBool, rpcHex, rpcQuantity, rpcWord, type GaslessRpcCall } from "./rpc-codec.js";
import { GASLESS_MAX_UINT120, gaslessAddress, gaslessFailure, gaslessUint } from "./validation.js";

const callData = (signature: string, words: readonly Hex[] = []): Hex =>
  `${toFunctionSelector(signature)}${words.map((word) => word.slice(2)).join("")}` as Hex;
const ZERO_WORD = `0x${"0".repeat(64)}` as Hex;

export async function verifyProtocolAt(call: GaslessRpcCall, deployment: GaslessDeployment,
  block: GaslessBlock): Promise<void> {
  ...
  if (codes.length !== deployment.code.length) gaslessFailure("APN_RPC_PROTOCOL", "gasless_protocol_identity");
  await Promise.all(deployment.reads.map(async (row) => {
    ...
    if (rpcHex(raw, 64 * 1024) !== row.expected) gaslessFailure("APN_RPC_PROTOCOL", "gasless_protocol_identity");
  }));
}

export async function readAccountAt(call: GaslessRpcCall, deployment: GaslessDeployment, ownerInput: Address,
  block: GaslessBlock, readPending: boolean): Promise<GaslessAccountState> {
  const owner = gaslessAddress(ownerInput);
  if (owner !== ownerInput) gaslessFailure("APN_RPC_PROTOCOL", "gasless_owner_identity");
  ...
  let delegation: "empty" | "expected";
  if (code === "0x") delegation = "empty";
  else if (code === expectedDelegation) delegation = "expected";
  else gaslessFailure("APN_RPC_PROTOCOL", "gasless_delegation_identity");
...
export async function readFeeConfigurationAt(call: GaslessRpcCall, deployment: GaslessDeployment,
  owner: Address, block: GaslessBlock): Promise<GaslessFeeConfiguration> {
  const pinned = { blockHash: block.hash, requireCanonical: true };
  ...
  if (paused) gaslessFailure("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "gasless_paymaster_paused");
  if (denylisted) gaslessFailure("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "gasless_owner_denylisted");
...
export function gasPrices(rawBaseFee: unknown, rawPriority: unknown): {
  readonly baseFeePerGas: string; readonly maxFeePerGas: string; readonly maxPriorityFeePerGas: string;
} {
