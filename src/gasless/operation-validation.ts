...
import { GASLESS_TERMINAL, gaslessIntentBinding, gaslessSnapshot, newGaslessEffect,
  type GaslessEffect, type GaslessEffectPhase, type GaslessOperationRecord, type GaslessState,
  type GaslessTransition } from "./operation-model.js";
...
export function gaslessCorrupt(): never { return gaslessFailure("APN_STATE_CORRUPT", "gasless_durable_binding"); }
export function validateGaslessOperation(value: unknown): GaslessOperationRecord {
  if (!operationSchema.safeParse(value).success) gaslessCorrupt();
  const op = value as GaslessOperationRecord, { integrityHash, ...body } = op;
  if (integrityHash !== hashObject(body) || op.fingerprint !== hashObject(gaslessIntentBinding(op))) gaslessCorrupt();
  try { validateGaslessIntent(op.intent); } catch { gaslessCorrupt(); }
  ...
  for (const entry of op.transitions) {
    ...
    if (hashObject(entryBody) !== transitionHash || entry.previousHash !== (previous?.transitionHash ?? op.fingerprint)) gaslessCorrupt();
    if (previous === undefined) {
      if (entry.state !== "awaiting_approval" || entry.at !== op.createdAt || entry.approval !== null ||
        ...
        entry.observation !== null || entry.settlement !== null || entry.failure !== null || !gaslessSame(entry.cursor,
          { startBlock: op.intent.initialSnapshot.block, nextBlockAtomic: op.intent.initialSnapshot.block.numberAtomic, previousEndBlock: null })) gaslessCorrupt();
    } else validateTransition(previous, entry);
    try { validateGaslessMutable(op, entry, entry.at); } catch { gaslessCorrupt(); }
    previous = entry;
  }
  if (previous === undefined || previous.at !== op.updatedAt || op.terminal !== GASLESS_TERMINAL.includes(op.state)) gaslessCorrupt();
  const { at: _at, previousHash: _previous, transitionHash: _transition, ...last } = previous;
  if (!gaslessSame(last, gaslessSnapshot(op))) gaslessCorrupt();
  return op;
}
function validateTransition(p: GaslessTransition, n: GaslessTransition): void {
  if (GASLESS_TERMINAL.includes(p.state) || (p.state !== n.state && !EDGES[p.state].includes(n.state)) || n.at < p.at) gaslessCorrupt();
  if (p.approval !== null && !gaslessSame(p.approval, n.approval)) gaslessCorrupt();
  validateEffectStep(p.bootstrap, n.bootstrap); validateEffectStep(p.userOperation, n.userOperation);
  if (p.settlement !== null && !gaslessSame(p.settlement, n.settlement)) gaslessCorrupt();
  const proven = p.observation?.settlement;
  if (proven !== null && proven !== undefined) {
    const next = n.observation?.settlement;
    if (next === null || next === undefined) gaslessCorrupt();
    assertGaslessSettlementContinuation(proven, next);
  }
}
function validateEffectStep(p: GaslessEffect, n: GaslessEffect): void {
  if (p.role !== n.role || (p.phase !== n.phase && !PHASE_EDGES[p.phase].includes(n.phase)) ||
    p.signingAttempts > n.signingAttempts || p.disclosureAttempts > n.disclosureAttempts || p.submissionAttempts > n.submissionAttempts) gaslessCorrupt();
  for (const key of ["materialHash", "userOperationHash", "signingStartedAt", "sealedAt", "disclosedAt", "submittedAt", "estimate"] as const) {
    if (p[key] !== null && !gaslessSame(p[key], n[key])) gaslessCorrupt();
  }
  if (p.materialHash === null && n.materialHash !== null && n.phase !== "sealed") gaslessCorrupt();
  if (p.phase === "unknown_finality" && n.phase === "sealed" && (p.disclosureAttempts !== 0 || p.submissionAttempts !== 0)) gaslessCorrupt();
}
export function validateGaslessContinuity(previous: GaslessOperationRecord, next: GaslessOperationRecord): void {
  validateGaslessOperation(previous); validateGaslessOperation(next);
  if (gaslessSame(previous, next)) return;
  if (previous.terminal || previous.fingerprint !== next.fingerprint || next.transitions.length !== previous.transitions.length + 1 ||
    !gaslessSame(previous.transitions, next.transitions.slice(0, -1))) gaslessCorrupt();
}
