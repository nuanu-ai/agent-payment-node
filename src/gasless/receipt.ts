...
export function publicGaslessOperation(op: GaslessOperationRecord) {
  ...
  return {
    kind: "gasless_transfer" as const, schema_version: op.schemaVersion, operation_id: op.operationId,
...
export function gaslessReceipt(op: GaslessOperationRecord) {
  const body = { ...publicGaslessOperation(op), schema_version: "apn.gasless-receipt.v1" as const,
