...
import { addressWord, MM_APPROVAL_TOPIC, MM_INCREASED_COUNT_TOPIC, MM_TRANSFER_TOPIC,
  rpcAddress, rpcHex, rpcQuantity, topicAddress, twoWords, type MmReceiptLog } from "./abi.js";
...
export function verifyMetaMaskReceiptAccounting(intent: MetaMaskGaslessIntent, outerSender: Address,
  logs: readonly MmReceiptLog[]): MetaMaskGaslessAccounting {
  ...
  if (tokenLogs.length !== 2 || countLogs.length !== 1) mmFail("mm_gasless_evidence_invalid");
  ...
  for (const log of logs) {
    ...
    if (log.address === row.token && log.topics[0] === MM_APPROVAL_TOPIC && log.topics.length >= 2 &&
      topicAddress(log.topics[1]!) === intent.binding.address) mmFail("mm_gasless_evidence_invalid");
  }
  const expectedRecipients = [intent.request.recipient, intent.quote.feeRecipient] as const;
  const expectedAmounts = [intent.quote.netAtomic, intent.quote.feeAtomic] as const;
  ...
  for (let index = 0; index < tokenLogs.length; index += 1) {
    ...
    if (log.topics.length !== 3 || log.topics[0] !== MM_TRANSFER_TOPIC || log.data.length !== 66 ||
      ...
      BigInt(log.data).toString() !== expectedAmounts[index]) mmFail("mm_gasless_evidence_invalid");
...
export function verifyScanLog(value: Record<string, unknown>, intent: MetaMaskGaslessIntent,
  from: bigint, to: bigint): Hex {
  const row = mmRegistry(intent.request.chainId).row;
  ...
  if (rpcAddress(value.address) !== row.protocol.limitedCalls.address || value.removed !== false ||
    ...
    rpcHex(topics[3], 32, 32) !== intent.delegationHash) mmFail("mm_gasless_evidence_invalid");
  topicAddress(rpcHex(topics[2], 32, 32));
  const data = rpcHex(value.data, 64, 64), [limit, count] = twoWords(data);
  if (limit !== 1n || count !== 1n) mmFail("mm_gasless_evidence_invalid");
  rpcHex(value.blockHash, 32, 32); rpcQuantity(value.logIndex);
  return rpcHex(value.transactionHash, 32, 32);
}

function addressTopic(address: Address): Hex { return addressWord(address); }
