...
import { MM_MIN_REMAINING_MS, type MetaMaskGaslessBinding, type MetaMaskGaslessDispatch,
  type MetaMaskGaslessQuote, type MetaMaskGaslessSnapshot, type MetaMaskGaslessUnsignedResult } from "./model.js";
...
import type { MetaMaskGaslessApprovalPort, MetaMaskGaslessProviderPort, MetaMaskGaslessRpcFactory,
  MetaMaskGaslessRpcPort } from "./ports.js";
import { metaMaskGaslessQuote } from "./quotation.js";
import { mmClassify, mmFail, mmFailure, type MetaMaskGaslessFailureReason } from "./reasons.js";
...
const MM_TRANSIENT_REASONS = new Set<MetaMaskGaslessFailureReason>(["mm_gasless_provider_unavailable",
  "mm_gasless_rpc_unavailable", "mm_gasless_state_busy", "mm_gasless_quote_unstable"]);
/** What the guard proved, and the repriced material to dispatch when the provider's price moved. */
interface MetaMaskGaslessGuarded {
  readonly observedAt: string;
  readonly dispatch: MetaMaskGaslessDispatch | null;
}

export class MetaMaskGaslessExecution {
  private readonly clock: MetaMaskGaslessClock;
  private readonly observation: MetaMaskGaslessObservationService;
  constructor(private readonly state: StateStore, private readonly rpcFor: MetaMaskGaslessRpcFactory,
    private readonly provider: MetaMaskGaslessProviderPort, clock: ClockPort,
    private readonly save: MetaMaskGaslessSave, private readonly wait: WaitPort) {
  ...
  async observeWith(op: MetaMaskGaslessOperationRecord, observer: MetaMaskGaslessObserver): Promise<MetaMaskGaslessStep> {
    return await new MetaMaskGaslessObservationService(this.state, this.rpcFor, this.provider, this.clock,
      this.save, observer).run(op);
  }
  ...
  async approve(op: MetaMaskGaslessOperationRecord, approval: MetaMaskGaslessApprovalPort): Promise<MetaMaskGaslessStep> {
    ...
    let now: number;
    try { now = this.clock.beforeApproval(op); } catch (error) { return await this.halt(op, error); }
    ...
    try {
      ...
      try { this.clock.check(op); } catch (clockError) { return await this.halt(op, clockError); }
      if (mmClassify(error, "mm_gasless_approval").reason === "mm_gasless_expired") return await this.halt(op, error);
      throw error;
    }
    try {
      now = this.clock.check(op);
      if (accepted) now = this.clock.beforeApproval(op);
    } catch (error) { return await this.halt(op, error); }
    if (!accepted) return { operation: await this.save(op, { state: "failed_before_effect",
  ...
  async run(op: MetaMaskGaslessOperationRecord): Promise<MetaMaskGaslessStep> {
    if (op.terminal || op.state === "awaiting_approval") return { operation: op };
    if (op.submissionAttempts === 1) return await this.observation.run(op);
    if (op.state !== "execution_pending" || op.approval === null || op.approval.fingerprint !== op.fingerprint) {
      mmFail("mm_gasless_state_corrupt");
    }
    let submitted: MetaMaskGaslessProviderRead;
    ...
    try {
      const result = await this.state.withLocks(["provider-session:metamask-agent-wallet"], async () => {
        ...
        try {
          ...
          const value = await this.provider.submit(mmDispatchIntent(op)); this.clock.check(op);
          const hint = metaMaskGaslessProviderObservation(value, op);
          this.clock.check(op, [hint.observedAt]);
          return { hint, failure: null } satisfies MetaMaskGaslessProviderRead;
        } catch (error) {
          if (mmClassify(error, "mm_gasless_submit_unknown").reason === "mm_gasless_clock") throw error;
          this.clock.check(op);
          return { hint: null, failure: "mm_gasless_submit_unknown" } satisfies MetaMaskGaslessProviderRead;
        }
      });
      this.clock.check(op);
      submitted = result;
    } catch (error) {
      if (op.submissionAttempts === 0 && !markerWriteStarted) return await this.halt(op, error);
      if (op.submissionAttempts === 0) throw error;
   ...
   */
  private async guard(op: MetaMaskGaslessOperationRecord): Promise<MetaMaskGaslessGuarded> {
    this.clock.beforeApproval(op);
    const owner = await metaMaskGaslessOwner(this.state, op.intent.profile, op.intent.binding); this.clock.check(op);
    const binding = await this.provider.inspect(owner); this.clock.check(op);
    mmAssertSameBinding(binding, op.intent.binding);
    if (op.intent.policyHash !== mmPolicyHash(op.profileHash, binding, op.intent.request)) mmFail("mm_gasless_binding_changed");
  ...
  private async redelegate(op: MetaMaskGaslessOperationRecord, binding: MetaMaskGaslessBinding,
    quote: MetaMaskGaslessQuote): Promise<MetaMaskGaslessUnsignedResult> {
    const input = { owner: binding.address, chainId: op.intent.request.chainId, executions: quote.executions };
    const unsigned = mmValidateUnsigned(await this.provider.buildUnsigned(input), input); this.clock.check(op);
    return unsigned;
  }
  ...
  private async chainState(op: MetaMaskGaslessOperationRecord, rpc: MetaMaskGaslessRpcPort,
    ...
    for (const key of ["safeState", "headState"] as const) {
      if (current[key].ownerCodeHash !== op.intent.initialSnapshot[key].ownerCodeHash ||
        current[key].designation !== op.intent.initialSnapshot[key].designation) mmFail("mm_gasless_rpc_binding");
    }
    return current;
  }
   ...
   */
  private async steady<T>(op: MetaMaskGaslessOperationRecord, act: () => Promise<T>): Promise<T> {
    for (let attempt = 1; ; attempt += 1) {
      try { return await act(); } catch (error) {
        if (attempt >= MM_GUARD_ATTEMPTS || !transient(error) ||
          ...
          await this.wait.wait(MM_GUARD_RETRY_MS) === "interrupted") throw error;
      }
    }
  }

  private async halt(op: MetaMaskGaslessOperationRecord, error: unknown): Promise<MetaMaskGaslessStep> {
    ...
    if (["mm_gasless_state_corrupt", "mm_gasless_state_security", "mm_gasless_state_busy"].includes(failure.reason)) throw error;
    try {
      ...
      if (saveFailure.reason === "mm_gasless_record_capacity") return { operation: op, warning: saveFailure };
      throw saveError;
    }
  }
}
