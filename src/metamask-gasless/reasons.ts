...
export interface MetaMaskGaslessFailure { readonly code: ErrorCode; readonly reason: MetaMaskGaslessFailureReason }

export function mmError(reason: MetaMaskGaslessFailureReason): ApnError {
  return new ApnError(MM_REASON_CODES[reason], "MetaMask gasless operation could not advance safely.", { reason });
}
export function mmFail(reason: MetaMaskGaslessFailureReason): never { throw mmError(reason); }
export function mmFailure(reason: MetaMaskGaslessFailureReason): MetaMaskGaslessFailure {
...
export function mmClassify(error: unknown, fallback: MetaMaskGaslessFailureReason): MetaMaskGaslessFailure {
  if (!(error instanceof ApnError)) return mmFailure(fallback);
  const reason = error.details?.reason;
  if (typeof reason === "string" && Object.hasOwn(MM_REASON_CODES, reason) && reason !== "mm_gasless_success" &&
    MM_REASON_CODES[reason as MetaMaskGaslessFailureReason] === error.code) return mmFailure(reason as MetaMaskGaslessFailureReason);
  const retained: Partial<Record<ErrorCode, MetaMaskGaslessFailureReason>> = {
