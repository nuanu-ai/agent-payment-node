...
export function installGaslessDependencyBoundary(): void {
  registerHooks({ resolve(specifier, context, nextResolve) {
    if (blocked.has(specifier) || blockedPath.test(specifier)) throw new Error("gasless dependency boundary rejected Solana decoder");
    const resolved = nextResolve(specifier, context);
    if (blockedPath.test(resolved.url)) throw new Error("gasless dependency boundary rejected Solana decoder");
    return resolved;
  } });
}
