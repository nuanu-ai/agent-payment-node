...
const CLOSE_ON_EXEC = (constants as unknown as Record<string, number>).O_CLOEXEC ?? 0;
...
export async function readPrivateState(homeDirectory: string, expected: Expected, now: Date): Promise<PrivateState> {
  if (typeof homeDirectory !== "string" || homeDirectory.length < 1 || homeDirectory.length > 4096 ||
    ...
    !Number.isSafeInteger(now.getTime())) mmFail("mm_gasless_state_security");
  ...
  if (await assertPrivateDirectories(homeDirectory, directory) !== directoryGeneration) mmFail("mm_gasless_state_busy");
  if (firstSession.hash !== secondSession.hash || firstWallet.hash !== secondWallet.hash) mmFail("mm_gasless_state_busy");
  ...
  const projectId = session.projectId as string, token = session.cliToken as string;
  const selected = walletState.selectedWallet as Record<string, unknown>;
  const ref = selected.ref as Record<string, unknown>;
  const referenceKind = Object.keys(ref)[0] as "id" | "name" | "address";
  if (referenceKind === "id") mmFail("mm_gasless_identity");
  const reference = ref[referenceKind] as string;
  const remote = (walletState.remoteWallets as Record<string, unknown>[]).filter((row) => row.namespace === "evm");
  ...
  if (matches.length !== 1) mmFail("mm_gasless_identity");
  const address = mmAddress(matches[0]!.address, "mm_gasless_identity");
  if (address !== expected.address) mmFail("mm_gasless_binding_changed");
  ...
  if ("providerId" in expected) mmAssertSameBinding(binding, expected);
...
function validateSession(envelope: Record<string, unknown>, now: Date): Record<string, unknown> {
  const root = mmExact(envelope, ["schemaVersion", "data"], "mm_gasless_state_corrupt");
  if (root.schemaVersion !== "1.0.0" || !isPlainRecord(root.data)) mmFail("mm_gasless_state_corrupt");
  const data = root.data;
  for (const key of ["cliToken", "cliRefreshToken", "projectId", "chain", "walletMode", "tradingMode", "authMethod", "loginMethod", "consent"]) {
    if (!Object.hasOwn(data, key)) mmFail("mm_gasless_state_corrupt");
  }
  if (!boundedString(data.cliToken) || !boundedString(data.projectId) || !nullableString(data.cliRefreshToken) ||
    ...
    ![null, "guard", "beast"].includes(data.tradingMode as never) || ![null, "qr", "social"].includes(data.authMethod as never) ||
    ![null, "google", "email", "qr", "other"].includes(data.loginMethod as never) ||
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(projectId)) mmFail("mm_gasless_session_unavailable");
  validateJwt(data.cliToken as string, projectId, now.getTime());
  return data;
}

function validateWallet(envelope: Record<string, unknown>): Record<string, unknown> {
  const root = mmExact(envelope, ["schemaVersion", "data"], "mm_gasless_state_corrupt");
  if (root.schemaVersion !== "0.0.1" || !isPlainRecord(root.data)) mmFail("mm_gasless_state_corrupt");
  const data = root.data;
  ...
  if (Object.keys(data).some((key) => !allowed.has(key)) || !Array.isArray(data.byokWallets) || data.byokWallets.length > 128 ||
    ...
    !isPlainRecord(data.selectedWallet)) mmFail("mm_gasless_state_corrupt");
  for (const item of data.remoteWallets) validateRemote(item);
  const selected = mmExact(data.selectedWallet, ["mode", "namespace", "ref"], "mm_gasless_state_corrupt");
  if (selected.mode !== "server" || selected.namespace !== "evm" || !isPlainRecord(selected.ref)) mmFail("mm_gasless_identity");
  ...
  if (refKeys[0] === "address") mmAddress(selected.ref.address, "mm_gasless_identity");
  return data;
}

function validateRemote(value: unknown): void {
  if (!isPlainRecord(value)) mmFail("mm_gasless_state_corrupt");
  ...
  if (value.namespace !== "solana") mmAddress(value.address, "mm_gasless_state_corrupt");
}

function validateJwt(token: string, projectId: string, nowMs: number): void {
  ...
  } catch { return mmFail("mm_gasless_session_unavailable"); }
  if (!isPlainRecord(payload) || payload.sub !== projectId || !Number.isSafeInteger(payload.exp) || Number(payload.exp) < 0 ||
    Number(payload.exp) * 1000 <= nowMs) mmFail("mm_gasless_session_unavailable");
...
async function secureRead(path: string, maximum: number): Promise<{ bytes: Buffer; hash: string }> {
  try {
    const before = await lstat(path, { bigint: true });
    assertMetadata(before, maximum);
    const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW | CLOSE_ON_EXEC);
    try {
      const opened = await handle.stat({ bigint: true }); assertMetadata(opened, maximum); assertSame(before, opened);
      const size = Number(opened.size), bytes = Buffer.alloc(size); let offset = 0;
      while (offset < size) { const read = await handle.read(bytes, offset, size - offset, offset); if (read.bytesRead < 1) mmFail("mm_gasless_state_busy"); offset += read.bytesRead; }
      const probe = Buffer.alloc(1); if ((await handle.read(probe, 0, 1, size)).bytesRead !== 0) mmFail("mm_gasless_state_busy");
      const after = await handle.stat({ bigint: true }); assertSame(opened, after);
      const final = await lstat(path, { bigint: true }); assertSame(after, final);
      return { bytes, hash: sha(bytes) };
    } finally { await handle.close(); }
  } catch (error) {
    if (isReason(error)) throw error;
    if (isFsCode(error, "ENOENT")) mmFail("mm_gasless_session_unavailable");
    return mmFail("mm_gasless_state_security");
  }
}
function assertMetadata(stat: BigIntStats, maximum: number): void {
  const uid = process.getuid?.();
  if (!stat.isFile() || uid === undefined || stat.uid !== BigInt(uid) || (stat.mode & 0o077n) !== 0n ||
    stat.size < 1n || stat.size > BigInt(maximum)) mmFail("mm_gasless_state_security");
}
async function assertPrivateDirectories(home: string, directory: string): Promise<string> {
  try {
    ...
    if (uid === undefined || !homeStat.isDirectory() || !privateStat.isDirectory() || homeStat.uid !== BigInt(uid) ||
      privateStat.uid !== BigInt(uid) || (homeStat.mode & 0o022n) !== 0n || (privateStat.mode & 0o077n) !== 0n) {
...
async function assertNoSymlinkAncestors(path: string): Promise<void> {
  ...
  for (const component of path.slice(root.length).split(sep).filter(Boolean)) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory()) mmFail("mm_gasless_state_security");
  }
}
function assertSame(a: BigIntStats, b: BigIntStats): void {
...
function parseObject(bytes: Buffer): Record<string, unknown> {
  try { const parsed: unknown = JSON.parse(decoder.decode(bytes)); if (isPlainRecord(parsed)) return parsed; }
...
function boundedString(value: unknown): value is string { return typeof value === "string" && value.length > 0 && value.length <= 8192; }
function nullableString(value: unknown): boolean { return value === null || boundedString(value); }
function isFsCode(error: unknown, code: string): boolean {
  return typeof error === "object" && error !== null && "code" in error && (error as { code?: unknown }).code === code;
}
function isReason(error: unknown): boolean { return error instanceof ApnError; }
