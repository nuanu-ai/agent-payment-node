 ...
 */
const MULTICALL3_CODE = "0xd5c15df687b16f2ff992fc8d767b4216323184a2bbc6ee2f9c398c318e770891" as const;
const MULTICALL3_CODE_ALTERNATE_METADATA = "0x8ec8aa5d07a37e47597b3a254f5cbf39fa2080526ae34db3a564fffed82b99ad" as const;
...
export type PortfolioEndpoint =
  | { readonly source: "default_public"; readonly env: string; readonly url: URL; readonly display: string }
  | { readonly source: "env"; readonly env: string; readonly url: URL }
  | { readonly source: "invalid_env"; readonly env: string }
  | { readonly source: "not_configured"; readonly env: string | null };

export function portfolioNetworkRpc(chain: string): PortfolioNetworkRpc | undefined {
...
export function portfolioEndpoint(chain: string, environment: Readonly<Record<string, string | undefined>>): PortfolioEndpoint {
  const row = portfolioNetworkRpc(chain);
  if (row === undefined) return { source: "not_configured", env: null };
  ...
  try { return { source: "env", env: row.env, url: portfolioUrl(row.family, value) }; }
  catch { return { source: "invalid_env", env: row.env }; }
}

function portfolioUrl(family: PortfolioFamily, value: string): URL {
