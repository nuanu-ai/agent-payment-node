...
import { assertTronNetwork, tronBlock, type TronMethod, type TronRpcPort } from "../tron/rpc.js";
 ...
 */
export class TronPortfolioPort implements FamilyBalanceBatchPort {
  ...
  async read(request: BatchBalanceRequest): Promise<BatchBalanceResult> {
    ...
    try {
      if (request.chain !== `tron:${TRON_GENESIS}`) throw new PortfolioReadFailure("protocol");
      ...
      const balances: BatchBalanceRow[] = [];
...
class PortfolioTronRpc implements TronRpcPort {
  readonly originHash: string;
  constructor(private readonly base: URL, private readonly counter: CountingPortfolioHttp) { this.originHash = sha256(base.origin); }
  ...
  async call(method: TronMethod, body: Readonly<Record<string, unknown>>): Promise<unknown> {
    ...
    try { value = tronRecord(parseJsonWithBigInts(raw)); } catch { throw new PortfolioReadFailure("protocol"); }
    if ("Error" in value || "error" in value) throw new PortfolioReadFailure("rpc_error");
    return value;
  }
}

async function trc20Balance(rpc: TronRpcPort, contract: string, owner: string): Promise<bigint> {
  ...
  if (tronRecord(value.result).result !== true) tronProtocolFailure();
