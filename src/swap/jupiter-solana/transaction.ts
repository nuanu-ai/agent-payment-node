...
export interface SolanaAccountBinding extends SolanaAccountDescriptor { readonly signer: boolean; readonly writable: boolean; readonly source: "static" | "lookup" }
export interface SolanaInstructionEnvelope { readonly programId: string; readonly accounts: readonly SolanaAccountBinding[]; readonly data: Uint8Array }
...
export async function parseJupiterV0Envelope(transactionBase64: string, resolver: SolanaAccountResolverPort): Promise<JupiterV0Envelope> {
  const bytes = canonicalBase64(transactionBase64); let wire: ReturnType<ReturnType<typeof getTransactionDecoder>["decode"]>;
  try { wire = getTransactionDecoder().decode(bytes); } catch { return invalid("Jupiter transaction cannot be decoded."); }
  let message: ReturnType<ReturnType<typeof getCompiledTransactionMessageDecoder>["decode"]>;
  try { message = getCompiledTransactionMessageDecoder().decode(wire.messageBytes); } catch { return invalid("Jupiter transaction message cannot be decoded."); }
  if (message.version !== 0) invalid("Jupiter transaction must use message version 0.");
  ...
  const staticAddresses = message.staticAccounts.map(String); staticAddresses.forEach(canonicalAddress);
  if (staticAddresses.length === 0 || signerKeys[0] !== staticAddresses[0]) invalid("Jupiter fee payer and signer binding is invalid.");
  ...
  for (const lookup of lookups) {
    const table = byTable.get(String(lookup.lookupTableAddress)); if (table === undefined) invalid("Jupiter address table identity changed during resolution.");
    const writable = resolveIndexes(table, lookup.writableIndexes); const readonly = resolveIndexes(table, lookup.readonlyIndexes);
    if (new Set([...lookup.writableIndexes, ...lookup.readonlyIndexes]).size !== lookup.writableIndexes.length + lookup.readonlyIndexes.length) invalid("Jupiter address table index is duplicated.");
    loadedWritable.push(...writable); loadedReadonly.push(...readonly);
  ...
  const accounts = allAddresses.map((accountAddress, index): SolanaAccountBinding => {
    const descriptor = descriptorMap.get(accountAddress); if (descriptor === undefined) invalid("Jupiter account resolution changed an account identity.");
    ...
    if (writable && descriptor.executable) invalid("Jupiter transaction makes an executable account writable.");
    return Object.freeze({ ...descriptor, signer, writable, source: index < staticCount ? "static" : "lookup" });
  });
  const instructions = message.instructions.map((instruction): SolanaInstructionEnvelope => {
    const program = accounts[instruction.programAddressIndex]; if (program === undefined || !program.executable || program.writable) invalid("Jupiter instruction program account is invalid.");
    const instructionAccounts = (instruction.accountIndices ?? []).map((index) => { const account = accounts[index]; if (account === undefined) invalid("Jupiter instruction account index is invalid."); return account; });
...
function resolveIndexes(table: SolanaAddressTableDescriptor, indexes: readonly number[]): string[] {
  if (indexes.length > 64) invalid("Jupiter address table loads too many accounts.");
  return indexes.map((index) => { if (!Number.isSafeInteger(index) || index < 0 || index >= table.addresses.length) invalid("Jupiter address table index is invalid."); return table.addresses[index]!; });
}
