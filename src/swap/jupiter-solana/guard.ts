...
export interface JupiterGuardPolicy {
  readonly taker: string; readonly recipient: string; readonly recipientTokenAccount: string; readonly requestId: string; readonly blockhash: string;
  readonly now: string;
 ...
 */
export async function guardJupiterTransaction(envelope: JupiterV0Envelope, quote: JupiterQuoteResponse, build: JupiterBuildResponse,
  policy: JupiterGuardPolicy, snapshot: JupiterProgramSnapshot): Promise<JupiterGuardRefusal> {
  ...
  if (envelope.feePayer !== policy.taker || envelope.signerCount !== 1) invalid("Jupiter taker, fee payer, or signer count changed.");
  if (envelope.blockhash !== policy.blockhash) invalid("Jupiter recent blockhash changed.");
  ...
  for (const instruction of envelope.instructions) {
    if (!allowed.has(instruction.programId)) invalid("Jupiter transaction invokes an unpinned program.");
    if (instruction.programId === COMPUTE_BUDGET_PROGRAM) {
      const decoded = computeBudget(instruction); if (decoded.kind === "limit") { if (unitLimit !== null) invalid("Jupiter compute unit limit is duplicated."); unitLimit = decoded.value; }
      else { if (unitPrice !== null) invalid("Jupiter compute unit price is duplicated."); unitPrice = decoded.value; }
    } else if (instruction.programId === SYSTEM_PROGRAM) tip += systemTip(instruction, policy);
    else if (instruction.programId === ASSOCIATED_TOKEN_PROGRAM) validateAtaCreate(instruction, policy);
    else if (instruction.programId === TOKEN_PROGRAM) validateTokenCleanup(instruction, policy);
    else if (instruction.programId === JUPITER_V6_PROGRAM) sawJupiter = true;
    else if (routes.has(instruction.programId)) invalid("Top-level route-program instructions are not covered by the pinned JUP6 decoder.");
  }
  if (!sawJupiter) invalid("Jupiter V6 instruction is absent.");
  if (unitLimit === null || unitLimit > BigInt(policy.maximumComputeUnits)) invalid("Jupiter compute unit limit exceeds policy.");
  const price = unitPrice ?? 0n; if (price > atomic(policy.maximumComputeUnitPriceMicroLamports, true)) invalid("Jupiter compute unit price exceeds policy.");
  const priority = (unitLimit * price + 999_999n) / 1_000_000n;
  if (priority > atomic(policy.maximumPriorityFeeLamports, true) || tip > atomic(policy.maximumTipLamports, true)) invalid("Jupiter priority fee or tip exceeds policy.");
  if (atomic(build.prioritizationFeeLamports, true) !== priority || atomic(build.tipLamports, true) !== tip ||
      ...
      atomic(build.referralFeeAtomic, true) > atomic(policy.maximumReferralFeeAtomic, true)) invalid("Jupiter build fee fields do not match the transaction or policy caps.");
...
function computeBudget(instruction: SolanaInstructionEnvelope): { kind: "limit" | "price"; value: bigint } {
  if (instruction.accounts.length !== 0) invalid("Jupiter compute budget instruction has accounts."); const data = Buffer.from(instruction.data);
  if (data.length === 5 && data[0] === 2) return { kind: "limit", value: BigInt(data.readUInt32LE(1)) };
  if (data.length === 9 && data[0] === 3) return { kind: "price", value: data.readBigUInt64LE(1) };
  return invalid("Jupiter compute budget instruction is unknown.");
}
function systemTip(instruction: SolanaInstructionEnvelope, policy: JupiterGuardPolicy): bigint {
  ...
  if (data.length !== 12 || data.readUInt32LE(0) !== 2 || instruction.accounts.length !== 2 ||
      ...
      !policy.allowedTipAccounts.includes(instruction.accounts[1]!.address) || !instruction.accounts[1]?.writable) invalid("Jupiter system instruction is not an allowed exact tip.");
...
function validateTokenCleanup(instruction: SolanaInstructionEnvelope, policy: JupiterGuardPolicy): void {
  ...
  if (data.length !== 1 || opcode !== 9 && opcode !== 17) invalid("Jupiter top-level token instruction is unknown.");
  if (opcode === 9 && (instruction.accounts.length !== 3 || instruction.accounts[1]?.address !== policy.taker || instruction.accounts[2]?.address !== policy.taker)) invalid("Jupiter token close destination or authority changed.");
  if (opcode === 17 && instruction.accounts.length !== 1) invalid("Jupiter sync-native instruction is invalid.");
}
