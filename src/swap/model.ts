...
export type NewSwapOperation = Omit<SwapOperationRecord, "schemaVersion" | "ownerProfileHash" | "state" | "revision" | "createdAt" | "updatedAt" |
  "usageLease" | "submissionMarker" | "receiptProof" | "failureProofHash" | "previousIntegrityHash" | "integrityHash"> & { readonly now: Date };

const HASH = /^[a-f0-9]{64}$/u, VERSION = /^[a-z0-9][a-z0-9._-]{0,63}$/u;
const MAX_UINT256 = (1n << 256n) - 1n;

export function newSwapOperation(input: NewSwapOperation): SwapOperationRecord {
  const at = date(input.now, "input"), quote = validateSwapQuote(input.quote, "input");
  if (at < quote.effectiveAt || at >= quote.expiresAt) invalid("Swap quote is not effective at operation creation.");
...
export function validateSwapOperation(value: unknown): SwapOperationRecord {
  ...
  if (lease !== null && (lease.account !== quote.account || lease.chain !== quote.sourceAsset.chain || lease.rail !== "swap" ||
      ...
      lease.policyDigest !== value.policyDigest)) corrupt("Swap usage lease binding is invalid.");
  validateStateBindings(value as unknown as SwapOperationRecord);
  const { integrityHash, ...body } = value;
  if (typeof integrityHash !== "string" || integrityHash !== digest(body)) corrupt("Swap operation integrity validation failed.");
...
function approval(value: unknown, input: string, mode: "input" | "stored"): string {
  try { if (typeof value !== "string") throw new Error(); const amount = parseAtomic(value); const maximum = BigInt(input);
    if (amount > maximum || amount === MAX_UINT256) throw new Error(); return amount.toString(); }
  catch { return failure(mode, "Swap approval cap must be canonical, bounded by exact input, and never unlimited."); }
}
function digest(value: unknown): string { return domainHash(SWAP_OPERATION_SCHEMA, canonicalJson(value)); }
function hash(value: unknown, mode: "input" | "stored"): string { if (typeof value !== "string" || !HASH.test(value)) failure(mode, "Swap hash binding is invalid."); return value as string; }
function version(value: unknown, mode: "input" | "stored"): string { if (typeof value !== "string" || !VERSION.test(value)) failure(mode, "Swap version binding is invalid."); return value as string; }
function date(value: unknown, mode: "input" | "stored"): string { if (typeof value !== "string" && !(value instanceof Date)) failure(mode, "Swap operation instant is invalid.");
  const raw = value instanceof Date ? value.toISOString() : value; if (!Number.isFinite(Date.parse(raw)) || new Date(raw).toISOString() !== raw) failure(mode, "Swap operation instant is invalid."); return raw; }
function failure(mode: "input" | "stored", message: string): never { throw new ApnError(mode === "input" ? "APN_INVALID_INPUT" : "APN_STATE_CORRUPT", message); }
