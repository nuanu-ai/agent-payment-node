...
type Prepare = Extract<CommandRequest, { readonly command: "swap.uniswap-token.prepare" }>;
...
export class InstalledUniswapTokenRuntime implements UniswapTokenCommandRuntime {
  ...
  constructor(private readonly builder: UniswapTokenQuoteBuilder, private readonly materials: SavedUniswapTokenMaterialStore,
    private readonly journal: UniswapTokenJournal, private readonly ports: UniswapTokenRuntimePorts) { this.execution = new UniswapTokenExecution(journal, ports); }
  inventory() { return this.builder.inventory(); }
  async quote(request: Extract<CommandRequest, { readonly command: "swap.uniswap-token.quote" }>) { return await this.builder.quote(request); }
  async prepare(request: Prepare) { const material = await this.materials.load(request.quoteHash);
    if (material === null) throw new ApnError("APN_OPERATION_NOT_FOUND", "Prepared Uniswap token quote was not found.");
    if (material.profile !== request.profile || material.mechanismDigest !== swapMechanismDigest(UNISWAP_TOKEN_MECHANISM_PIN)) blocked("Token quote binding changed.", "uniswap_token_quote_binding");
    if (this.ports.now().getTime() >= material.route.deadline * 1000) blocked("Token quote expired.", "uniswap_token_quote_expired");
    ...
    const prior = await this.journal.load(operationId); if (prior !== null) return prior;
