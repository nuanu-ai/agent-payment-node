...
export function validateUniswapTokenMaterial(value: unknown, mode: "input" | "stored" = "stored"): UniswapTokenMaterial {
  ...
  if (!isPlainRecord(value) || !exactKeys(value, ["schemaVersion", "quoteHash", "profile", "account", "route", "expectedOutputAtomic",
    ...
    "mechanismDigest", "blockNumber", "blockHash", "createdAt"]) || value.schemaVersion !== UNISWAP_TOKEN_MATERIAL_SCHEMA) fail("Uniswap token material schema is invalid.");
  ...
  if (!/^[a-f0-9]{64}$/u.test(quoteHash) || quoteHash !== hashObject(body) || material.approvalCapAtomic !== route.amountIn ||
      ...
      !/^0x[a-f0-9]{64}$/u.test(material.blockHash) || !instant(material.createdAt)) fail("Uniswap token material binding is invalid.");
  const maximum = gas(material.approvalGas) + gas(material.swapGas) + gas(material.cleanupGas);
  if (uint(material.maximumNativeDebitWei) < maximum) fail("Uniswap token native debit budget does not cover all effects.");
  void uint(material.blockNumber); void canonicalJson(material);
  return material;
}
...
export class SavedUniswapTokenMaterialStore extends SecureStateStore {
  async save(value: UniswapTokenMaterial): Promise<UniswapTokenMaterial> {
    const material = validateUniswapTokenMaterial(value, "input"); await this.initialize(); await this.ensureDirectory("uniswap-token-quotes");
    return await this.withLocks([`uniswap-token-quote:${material.quoteHash}`], async () => {
      ...
      if (prior !== null && canonicalJson(validateUniswapTokenMaterial(prior)) !== canonicalJson(material)) fail("A different token quote owns this hash.");
      if (prior === null) await this.writeJson(this.path(material.quoteHash), material, true); return material;
    });
  }
  async load(hash: string): Promise<UniswapTokenMaterial | null> { stateIdentifier(hash, "Uniswap token quote"); await this.initialize();
    const value = await this.readJson(this.path(hash)); if (value === null) return null; const material = validateUniswapTokenMaterial(value);
    if (material.quoteHash !== hash) fail("Uniswap token quote path binding is invalid."); return material; }
  private path(hash: string) { return `uniswap-token-quotes/${hash}.json`; }
}
function gas(value: TokenGasEnvelope) { if (!isPlainRecord(value) || !exactKeys(value, ["gasLimit", "maxFeePerGas", "maxPriorityFeePerGas"]) || uint(value.maxPriorityFeePerGas) > uint(value.maxFeePerGas)) fail("Uniswap token gas envelope is invalid."); return uint(value.gasLimit) * uint(value.maxFeePerGas); }
function uint(value: unknown) { try { if (typeof value !== "string") throw new Error(); return parseAtomic(value); } catch { return fail("Uniswap token integer is invalid."); } }
