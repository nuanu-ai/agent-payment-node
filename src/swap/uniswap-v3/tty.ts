...
export class TtyUniswapSwapApproval implements GuardedSwapForegroundApprovalPort {
  constructor(private readonly quotes: Pick<SavedUniswapQuoteStore, "load">, private readonly clock: ClockPort,
...
export function uniswapApprovalCode(intent: GuardedSwapApprovalIntent, lines: readonly string[]): string {
  return approvalCode("swap", intent.operationId, domainHash("apn.uniswap-approval-screen.v1", canonicalJson({ intent, lines })));
}

export async function uniswapApprovalScreen(quotes: Pick<SavedUniswapQuoteStore, "load">, intent: GuardedSwapApprovalIntent): Promise<readonly string[]> {
