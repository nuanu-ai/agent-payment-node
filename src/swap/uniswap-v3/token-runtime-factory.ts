...
export function createUniswapTokenRuntime(input: { readonly state: StateStore; readonly wrapping: WrappingSecretPort;
  ...
  const { state, wrapping, clock, call } = input, materials = new SavedUniswapTokenMaterialStore(state.root), custody = new UniswapTokenCustody(state, wrapping, call, () => clock.now()),
    observer = new UniswapTokenObserver(call), revalidator = new UniswapTokenRevalidator(call);
  ...
  const admit = async (request: Parameters<UniswapTokenQuoteBuilder["quote"]>[0], now: Date) => {
    const active = await loadActiveAssetPolicyRegistry({ state, clock }, request.profile);
    if (active === null) blocked("An active owner allowlist is required.", "swap_owner_admission_required");
    for (const [identifier, amountAtomic] of [[request.sourceToken, request.amountAtomic], [request.outputToken, request.minimumOutputAtomic]] as const) {
