...
import { createUniswapTokenRoute, encodeUniswapTokenApproval, UNISWAP_TOKEN_MECHANISM_PIN,
  UNISWAP_V3_SWAP_ROUTER, verifyUniswapTokenRoutePins } from "./token-route.js";
...
export type UniswapTokenQuoteRequest = Extract<CommandRequest, { readonly command: "swap.uniswap-token.quote" }>;
...
export class UniswapTokenQuoteBuilder {
  constructor(private readonly call: EvmRpcCall, private readonly store: SavedUniswapTokenMaterialStore,
    private readonly admit: UniswapTokenPolicyAdmission, private readonly now: () => Date) {}
  inventory() { return { chain: "eip155:1", router: UNISWAP_V3_SWAP_ROUTER, pair: "USDC/USDT", fee: 100,
  ...
  async quote(input: UniswapTokenQuoteRequest): Promise<unknown> {
    ...
    if (request.deadline <= seconds || request.deadline - seconds > 1_800) invalid("Uniswap token deadline must be in the next 30 minutes.");
    if (evmRpcQuantity(await this.call("eth_chainId", [])) !== 1n) throw new ApnError("APN_CHAIN_MISMATCH", "Uniswap token route requires Ethereum chain 1.");
    ...
    if (allowance !== 0n && allowance !== request.amount) blocked("Allowance must start at zero or the exact input amount.", "uniswap_allowance_mismatch");
    ...
    if (expected < request.minimum) blocked("Quoted output is below the explicit minimum.", "uniswap_output_floor");
    ...
    const approval = encodeUniswapTokenApproval(route.inputToken, route.amountIn), approvalTx = { from: request.account, to: approval.to, data: approval.data, value: "0x0" },
      swapTx = { from: request.account, to: route.router, data: route.calldata, value: "0x0" };
    if (allowance === 0n) await safeApprovalCall(this.call, approvalTx, block.tag);
    else await simulated(this.call, swapTx, block.tag, request.maxSwapGas);
    ...
    if (required > request.maxNativeDebit || balance < required) blocked("Native balance or debit cap does not cover approval, swap, and cleanup.", "uniswap_native_debit");
...
async function readAllowance(call: EvmRpcCall, token: string, owner: string, tag: Hex) { const data = encodeFunctionData({ abi: ERC20, functionName: "allowance", args: [owner as Hex, UNISWAP_V3_SWAP_ROUTER] });
  return decodeFunctionResult({ abi: ERC20, functionName: "allowance", data: evmRpcHex(await call("eth_call", [{ to: token, data }, tag]), 32) }); }
async function simulated(call: EvmRpcCall, tx: object, tag: Hex, cap: bigint) { await call("eth_call", [tx, tag]); const estimate = evmRpcQuantity(await call("eth_estimateGas", [tx, tag]));
  if (estimate > cap) blocked("Exact transaction gas estimate exceeds the owner cap.", "uniswap_gas_cap"); }
async function safeApprovalCall(call: EvmRpcCall, tx: object, tag: Hex) { const result = evmRpcHex(await call("eth_call", [tx, tag]));
  if (result !== "0x" && result !== `0x${"0".repeat(63)}1`) blocked("Token approval simulation returned an unsafe result.", "uniswap_approval_simulation"); }
function valid(input: UniswapTokenQuoteRequest) { const account = address(input.account), recipient = address(input.recipient), inputToken = address(input.sourceToken), outputToken = address(input.outputToken);
  const amount = uint(input.amountAtomic), minimum = uint(input.minimumOutputAtomic), cap = uint(input.approvalCapAtomic), maxApprovalGas = uint(input.maxApprovalGasLimit),
    maxSwapGas = uint(input.maxSwapGasLimit), maxCleanupGas = uint(input.maxCleanupGasLimit), maxFee = uint(input.maxFeePerGas), maxPriority = uint(input.maxPriorityFeePerGas, false), maxNativeDebit = uint(input.maxNativeDebitWei);
  if (cap !== amount || maxPriority > maxFee || maxApprovalGas > 30_000_000n || maxSwapGas > 30_000_000n || maxCleanupGas > 30_000_000n) invalid("Uniswap token caps are inconsistent.");
...
function address(value: string) { try { const result = getAddress(value); if (result !== value) throw new Error(); return result as Hex; } catch { return invalid("Uniswap token address is invalid."); } }
function uint(value: string, positive = true) { try { return parseAtomic(value, { positive }); } catch { return invalid("Uniswap token integer is invalid."); } }
