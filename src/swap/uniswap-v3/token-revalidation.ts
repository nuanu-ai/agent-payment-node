...
export class UniswapTokenRevalidator {
  ...
  async revalidate(op: UniswapTokenOperation): Promise<void> {
    if (evmRpcQuantity(await this.call("eth_chainId", [])) !== 1n) throw new ApnError("APN_CHAIN_MISMATCH", "Uniswap token execution requires Ethereum chain 1.");
    ...
    if (evmRpcQuantity(await this.call("eth_estimateGas", [tx, block.tag])) > BigInt(op.swapGas.gasLimit)) blocked("Fresh swap gas exceeds the approved cap.", "uniswap_token_gas_cap");
