...
import { tokenBatch, tokenChain, tokenHex, type TokenRpcCall } from "./token-rpc.js";
...
export async function verifyUniswapTokenRoutePins(call: EvmRpcCall, tag: Hex): Promise<void> {
  ...
  if (rpc.primaryPoolEnabled?.() === true) {
    ...
    code(first[0], UNISWAP_V3_SWAP_ROUTER_CODE_HASH); code(first[1], UNISWAP_V3_USDC_USDT_100_CODE_HASH);
    ...
    const safety = await tokenBatch(rpc, "archive", ["deprecated", "basisPointsRate", "maximumFee"].map((functionName) => ({
      method: "eth_call", params: [{ to: ETHEREUM_USDT,
        data: encodeFunctionData({ abi: USDT_SAFETY, functionName: functionName as "deprecated" | "basisPointsRate" | "maximumFee" }) }, tag],
      cachePolicy: "immutable" as const, decoder: usdtDecoder(functionName as "deprecated" | "basisPointsRate" | "maximumFee") })));
    assertUsdtSafety("deprecated", safety[0]); assertUsdtSafety("basisPointsRate", safety[1]); assertUsdtSafety("maximumFee", safety[2]);
  ...
  const safety = await tokenBatch(rpc, "archive", [
    ...
    ...["deprecated", "basisPointsRate"].map((functionName) => ({ method: "eth_call", params: [{ to: ETHEREUM_USDT,
      ...
      decoder: usdtDecoder(functionName as "deprecated" | "basisPointsRate") })),
  ]);
...
function usdtDecoder(functionName: "deprecated" | "basisPointsRate" | "maximumFee") { return (value: unknown) => {
  decodeFunctionResult({ abi: USDT_SAFETY, functionName, data: evmRpcHex(value, 32) }); return value; }; }
