import { decodeFunctionData, encodeFunctionData, getAddress, keccak256, parseAbi, type Hex } from "viem";
...
export interface UniswapTokenRoute { readonly schemaVersion: typeof UNISWAP_TOKEN_ROUTE_SCHEMA; readonly inputToken: string;
  readonly outputToken: string; readonly recipient: string; readonly amountIn: string; readonly amountOutMinimum: string;
  readonly deadline: number; readonly router: typeof UNISWAP_V3_SWAP_ROUTER; readonly pool: typeof UNISWAP_V3_USDC_USDT_100;
  readonly fee: 100; readonly calldata: Hex; readonly routeHash: string }

export function createUniswapTokenRoute(input: { readonly inputToken: string; readonly outputToken: string; readonly recipient: string;
  readonly amountIn: string; readonly amountOutMinimum: string; readonly deadline: number }): UniswapTokenRoute {
  const tokenIn = token(input.inputToken), tokenOut = token(input.outputToken), recipient = address(input.recipient), amountIn = uint(input.amountIn),
    amountOutMinimum = uint(input.amountOutMinimum);
  if (tokenIn === tokenOut || !Number.isSafeInteger(input.deadline) || input.deadline <= 0) invalid("Uniswap token route is invalid.");
...
export function validateUniswapTokenRoute(value: unknown): UniswapTokenRoute {
  if (!isPlainRecord(value) || !exactKeys(value, ["schemaVersion", "inputToken", "outputToken", "recipient", "amountIn",
    "amountOutMinimum", "deadline", "router", "pool", "fee", "calldata", "routeHash"])) invalid("Uniswap token route is invalid."); const route = value as unknown as UniswapTokenRoute;
  const expected = createDecoded(route.calldata);
  const { routeHash, ...body } = route;
  if (canonicalJson(expected) !== canonicalJson(body) || routeHash !== domainHash(UNISWAP_TOKEN_ROUTE_SCHEMA, canonicalJson(body))) invalid("Uniswap token route binding changed.");
  return route;
}
function createDecoded(data: Hex): Omit<UniswapTokenRoute, "routeHash"> {
  try { const decoded = decodeFunctionData({ abi: ROUTER, data }); if (decoded.functionName !== "exactInputSingle") throw new Error(); const p = decoded.args[0];
    if (p.fee !== 100 || p.sqrtPriceLimitX96 !== 0n || p.deadline > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error();
    return { schemaVersion: UNISWAP_TOKEN_ROUTE_SCHEMA, inputToken: token(p.tokenIn), outputToken: token(p.tokenOut), recipient: address(p.recipient),
      ...
      router: UNISWAP_V3_SWAP_ROUTER, pool: UNISWAP_V3_USDC_USDT_100, fee: 100, calldata: data }; } catch { return invalid("Uniswap exactInputSingle calldata is invalid."); }
}
export function encodeUniswapTokenApproval(tokenAddress: string, amount: string): { readonly to: string; readonly spender: string; readonly data: Hex; readonly amount: string } {
  const to = token(tokenAddress), value = uint(amount).toString(); return { to, spender: UNISWAP_V3_SWAP_ROUTER,
...
export async function verifyUniswapTokenRoutePins(call: EvmRpcCall, tag: Hex): Promise<void> {
  for (const [addressValue, hash] of [[UNISWAP_V3_SWAP_ROUTER, UNISWAP_V3_SWAP_ROUTER_CODE_HASH],
    ...
    if (keccak256(evmRpcHex(await call("eth_getCode", [addressValue, tag]))) !== hash) blocked("Uniswap token route code pin changed.", "uniswap_code_pin_drift");
  }
  if (UNISWAP_V3_FACTORY !== "0x1F98431c8aD98523631AE4a59f267346ea31F984") throw new ApnError("APN_STATE_CORRUPT", "Uniswap factory pin changed.");
}
function token(value: string) { const v = address(value); if (v !== UNISWAP_USDC && v !== ETHEREUM_USDT) invalid("Only canonical Ethereum USDC and USDT are admitted."); return v; }
function address(value: string) { try { const v = getAddress(value); if (v !== value || /^0x0{40}$/u.test(v)) throw new Error(); return v; } catch { return invalid("Uniswap address is invalid."); } }
function uint(value: string) { try { return parseAtomic(value, { positive: true }); } catch { return invalid("Uniswap amount is invalid."); } }
