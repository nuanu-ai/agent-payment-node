...
export class UniswapTokenExecution {
    ...
    private async required(id: string) { const op = await this.journal.load(id); if (op === null)
...
function debit(op: UniswapTokenOperation, added: string) { const total = BigInt(op.accumulatedNativeDebitWei) + BigInt(added); if (total > BigInt(op.maximumNativeDebitWei))
