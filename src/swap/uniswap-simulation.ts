...
export interface UniswapSimulationProof {
  readonly requestHash: string; readonly resultHash: string; readonly success: true; readonly blockNumber: string;
  readonly blockHash: `0x${string}`; readonly headBlockNumber: string; readonly maxHeadDrift: number; readonly gasEstimate: string;
}
export class UniswapEvmSimulator {
  constructor(private readonly call: EvmRpcCall, private readonly maxHeadDrift = 64) {
    if (!Number.isSafeInteger(maxHeadDrift) || maxHeadDrift < 0 || maxHeadDrift > 256) throw new ApnError("APN_RPC_CONFIG", "Uniswap head drift bound is invalid.");
  }
  async simulate(envelope: UniswapTransactionEnvelope): Promise<UniswapSimulationProof> {
    ...
    let callResult: `0x${string}`, gas: bigint;
    ...
    catch { throw new ApnError("APN_OPERATION_BLOCKED", "Exact Uniswap simulation reverted or was unavailable.", { reason: "uniswap_simulation" }); }
    const rechecked = await evmRpcBlock(this.call, block.tag); if (rechecked.hash !== block.hash) throw new ApnError("APN_OPERATION_BLOCKED", "Simulation block changed during proof.", { reason: "uniswap_block_changed" });
    const head = await evmRpcBlock(this.call, "latest"), drift = BigInt(head.number) - BigInt(block.number);
    if (drift < 0n || drift > BigInt(this.maxHeadDrift)) throw new ApnError("APN_OPERATION_BLOCKED", "Simulation reference is outside the allowed head drift.", { reason: "uniswap_head_drift" });
    if (evmRpcQuantity(await this.call("eth_chainId", [])) !== 1n) throw new ApnError("APN_CHAIN_MISMATCH", "Ethereum RPC chain changed during simulation.");
