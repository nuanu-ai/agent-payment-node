...
function response(value: unknown, field: "energy_used" | "energy_required"): bigint {
  if (!isPlainRecord(value) || !isPlainRecord(value.result) || value.result.result !== true || value.result.message !== undefined) revert();
  const energy = integer(value[field]); if (energy <= 0n) revert(); return energy;
}
function integer(value: unknown): bigint {
  if (typeof value === "number" && !Number.isSafeInteger(value)) revert();
  if ((typeof value !== "string" && typeof value !== "number" && typeof value !== "bigint") || !/^[0-9]+$/u.test(String(value))) revert();
  return BigInt(value);
}
