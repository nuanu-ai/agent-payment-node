...
import { SUNSWAP_EXECUTION_MATERIAL_SCHEMA, sunSwapGasOrEnergy, type SunSwapPreparedMaterial, type SunSwapPreparedMaterialPort } from "./prepared.js";
import { createSunSwapQuoteSnapshot } from "./quote.js";
import { simulateSunSwapTransaction } from "./simulation.js";
import { buildSunSwapUnsignedTransaction, sunSwapUnsignedPayloadHash, type SunSwapUnsignedIntent } from "./transaction.js";
...
export interface SunSwapGuardedPreparedMaterial {
  readonly quote: SwapQuoteSnapshot; readonly approvalCapAtomic: string;
  readonly gasOrEnergy: Readonly<Record<string, string>>; readonly execution: unknown;
}
...
export type SunSwapKeylessQuoteRequest = Extract<CommandRequest, { readonly command: "swap.sunswap.quote" }> & {
  readonly feeLimitSun: string; readonly deadline: number;
};

const PROFILE = /^[a-z0-9][a-z0-9._-]{0,63}$/u;
const MAX_WINDOW_MS = 600_000n;
 ...
 */
export class SunSwapKeylessQuoteBuilder implements SunSwapGuardedReadOnlyBuilder<SunSwapKeylessQuoteRequest> {
  constructor(private readonly rpc: TronRpcPort, private readonly store: SunSwapPreparedMaterialPort) {}
  ...
  async quote(input: SunSwapKeylessQuoteRequest & { readonly now: Date }): Promise<SunSwapPreparedMaterial> {
    ...
    const intent: SunSwapUnsignedIntent = { owner: request.account, recipient: request.recipient, inputAmountAtomic: request.amountAtomic,
      ...
      expirationMs: (BigInt(request.deadline) * 1_000n).toString(), feeLimitSun: request.feeLimitSun,
    ...
    await assertOwnerFunding(this.rpc, request.account, BigInt(request.amountAtomic) + BigInt(request.feeLimitSun));
    ...
    return await this.store.save({ quote, approvalCapAtomic: "0", gasOrEnergy: sunSwapGasOrEnergy(intent, simulation, transaction),
      execution: { schemaVersion: SUNSWAP_EXECUTION_MATERIAL_SCHEMA, intent, transaction, simulation, market, pricing } });
  }
...
function validateRequest(input: SunSwapKeylessQuoteRequest & { readonly now: Date }) {
  if (!isPlainRecord(input) || !exactKeys(input, ["command", "profile", "account", "recipient", "amountAtomic", "slippageBps",
    "ownerSlippageCapBps", "feeLimitSun", "deadline", "now"]) || input.command !== "swap.sunswap.quote" ||
  ...
  if (typeof input.amountAtomic !== "string" || !/^[1-9][0-9]{0,15}$/u.test(input.amountAtomic) ||
      BigInt(input.amountAtomic) > BigInt(Number.MAX_SAFE_INTEGER)) invalid("SunSwap amount must be a positive safe SUN integer.");
  if (!Number.isSafeInteger(input.slippageBps) || !Number.isSafeInteger(input.ownerSlippageCapBps) || input.slippageBps < 0 ||
      input.ownerSlippageCapBps > 10_000 || input.slippageBps > input.ownerSlippageCapBps) invalid("SunSwap slippage exceeds the owner cap.");
  ...
  const window = BigInt(input.deadline) * 1_000n - BigInt(input.now.getTime());
  if (window <= 0n || window > MAX_WINDOW_MS) invalid("SunSwap deadline must be within the next 10 minutes (TRON expiration bound).");
  return input;
}

async function chainParameters(rpc: TronRpcPort): Promise<{ readonly energyPriceSun: bigint; readonly maximumFeeLimitSun: bigint }> {
  let value: unknown;
  ...
  const read = (key: string): bigint => {
    const rows = (value as { chainParameter: unknown[] }).chainParameter.filter((row) => isPlainRecord(row) && row.key === key);
    if (rows.length !== 1) protocol();
    const amount = tronAtomic((rows[0] as Record<string, unknown>).value); if (amount <= 0n) protocol(); return amount;
  };
  return { energyPriceSun: read("getEnergyFee"), maximumFeeLimitSun: read("getMaxFeeLimit") };
}

/** Economic guard: the owner must hold the exact call value plus the full owner fee_limit before any signing exists. */
async function assertOwnerFunding(rpc: TronRpcPort, owner: string, requiredSun: bigint): Promise<void> {
  ...
  if (tronAtomic(value.balance, true) < requiredSun) {
    throw new ApnError("APN_INSUFFICIENT_ASSET", "Owner TRX cannot cover the exact call value plus the owner fee_limit.",
...
function protocol(): never { throw new ApnError("APN_RPC_PROTOCOL", "TRON returned SunSwap chain evidence with an invalid shape."); }
