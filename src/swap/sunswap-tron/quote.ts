...
import { priceSunSwapV2Market, validateSunSwapV2Market, type SunSwapV2Market } from "./market.js";

export interface SunSwapQuoteSnapshotInput {
  readonly profile: string; readonly account: string; readonly recipient: string;
  readonly slippageBps: number; readonly ownerSlippageCapBps: number; readonly effectiveAt: string; readonly expiresAt: string;
  readonly unsignedTransactionPayloadHash: string; readonly market: SunSwapV2Market; readonly simulation: SwapSimulationProof;
}
...
export function sunSwapOnChainEvidenceHash(market: SunSwapV2Market): string {
