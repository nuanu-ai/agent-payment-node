...
import { rpcAtomic, solanaAddress, type SolanaRpcPort } from "../../solana/rpc.js";
import { associatedTokenAddress, decodeTickArray, decodeWhirlpool, readRawAccounts, TICK_ARRAY_ACCOUNT_BYTES,
  whirlpoolOracleAddress, whirlpoolTickArrayAddress, type RawSolanaAccount, type TickArrayState, type WhirlpoolState } from "./accounts.js";
import { quoteWhirlpoolExactInAToB, tickArrayStart, type WhirlpoolSwapQuote } from "./math.js";
import {
  ORCA_POOL_FEE_RATE, ORCA_POOL_TICK_SPACING, ORCA_SOL_USDC_POOL, ORCA_SOL_VAULT, ORCA_USDC_VAULT, sha256Hex, TICK_ARRAY_ACCOUNT_DISCRIMINATOR,
  TOKEN_PROGRAM, USDC_MINT, WHIRLPOOL_ACCOUNT_DISCRIMINATOR, WHIRLPOOL_PROGRAM, WHIRLPOOLS_CONFIG, WSOL_MINT,
} from "./pins.js";
import { orcaOwnerBalances, type OrcaOwnerBalances } from "./simulation.js";

export interface OrcaMarketRead {
  readonly slot: bigint;
  readonly pool: WhirlpoolState;
  readonly poolView: { readonly address: string; readonly tickSpacing: number; readonly feeRate: number; readonly protocolFeeRate: number;
 ...
 */
export async function readOrcaMarket(rpc: SolanaRpcPort, owner: string, amountIn: bigint): Promise<OrcaMarketRead> {
  solanaAddress(owner);
  ...
  const read = await readRawAccounts(rpc, [ORCA_SOL_USDC_POOL, ...arrays, oracle, ORCA_SOL_VAULT, ORCA_USDC_VAULT, owner, wsolAccount, usdcAccount],
    TICK_ARRAY_ACCOUNT_BYTES);
  ...
  const tickArrays = [a0, a1, a2].map((account, index) => decodeTickArray(arrays[index]!, account ?? null, WHIRLPOOL_PROGRAM,
    TICK_ARRAY_ACCOUNT_DISCRIMINATOR));
  if (oracleAccount !== null) blocked("The pool has an adaptive-fee oracle; its variable fee is outside this pinned quote.", "orca_adaptive_fee");
  const vaultSol = vaultAmount(solVault ?? null, WSOL_MINT), vaultUsdc = vaultAmount(usdcVault ?? null, USDC_MINT);
  if (wsolState !== null) blocked("The owner's wSOL account already exists; unwrap or close it before this swap.", "orca_wsol_account_present");
  ...
  if (rent === 0n) blocked("Token account rent could not be priced.", "orca_rent");
  ...
  [[ORCA_SOL_USDC_POOL, poolAccount], [arrays[0], a0], [arrays[1], a1], [arrays[2], a2], [ORCA_SOL_VAULT, solVault], [ORCA_USDC_VAULT, usdcVault],
    ...
    if (account !== null && account !== undefined && typeof key === "string") hashes[key] = sha256Hex((account as RawSolanaAccount).data);
  });
...
async function tickArrayAddresses(pool: WhirlpoolState): Promise<readonly string[]> {
...
function pinnedPool(account: RawSolanaAccount | null): WhirlpoolState {
  ...
  if (pool.feeRate !== ORCA_POOL_FEE_RATE) blocked("The Whirlpool fee rate changed from the pinned 0.04%.", "orca_pool_fee_drift");
  return pool;
}
function vaultAmount(account: RawSolanaAccount | null, mint: string): bigint {
  if (account === null || account.owner !== TOKEN_PROGRAM || account.data.length !== 165) blocked("A pinned pool vault is missing.", "orca_pool_pin_drift");
  const token = getTokenDecoder().decode(account.data);
  if (token.mint !== mint || token.owner !== ORCA_SOL_USDC_POOL || token.state !== 1) blocked("A pinned pool vault changed mint or authority.", "orca_pool_pin_drift");
  return token.amount;
}
