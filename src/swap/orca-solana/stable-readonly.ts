...
import { assertSolanaNetwork, type SolanaRpcPort } from "../../solana/rpc.js";
import { decodeTickArray, decodeWhirlpool, readRawAccounts, TICK_ARRAY_ACCOUNT_BYTES, whirlpoolOracleAddress,
  whirlpoolTickArrayAddress, type RawSolanaAccount, type WhirlpoolState } from "./accounts.js";
...
export function orcaStableInventory() {
  return { chain: ORCA_SOLANA_CHAIN, mode: "read_only", admitted: false, signable: false, executable: false,
    ...
    tickSpacing: 1, feeRate: ORCA_STABLE_FEE_RATE, direction: "USDC_to_USDT_exact_input" } as const;
}
...
export async function quoteOrcaStableReadOnly(rpc: SolanaRpcPort, request: OrcaStableQuoteRequest) {
  ...
  const arrays = await Promise.all([0, 1, 2].map((index) =>
    whirlpoolTickArrayAddress(WHIRLPOOL_PROGRAM, ORCA_STABLE_POOL, start - index * 88)));
  const oracle = await whirlpoolOracleAddress(WHIRLPOOL_PROGRAM, ORCA_STABLE_POOL);
  const read = await readRawAccounts(rpc, [ORCA_STABLE_POOL, ...arrays, oracle, ORCA_STABLE_VAULT_A, ORCA_STABLE_VAULT_B],
    TICK_ARRAY_ACCOUNT_BYTES);
  if (read.slot < initial.slot) blocked("RPC returned a stale market snapshot.", "orca_slot_regressed");
  const [poolAccount, a0, a1, a2, oracleAccount, vaultA, vaultB] = read.accounts;
  const pool = pinnedPool(poolAccount ?? null);
  if (tickArrayStart(pool.tickCurrentIndex, 1) !== start) blocked("Pool moved across a tick array while reading.", "orca_tick_array_moved");
  if (oracleAccount !== null) blocked("Adaptive fee state is outside this pinned quote.", "orca_adaptive_fee");
  const ticks = [a0, a1, a2].map((account, index) =>
    decodeTickArray(arrays[index]!, account ?? null, WHIRLPOOL_PROGRAM, TICK_ARRAY_ACCOUNT_DISCRIMINATOR));
  ...
  if (swap.priceImpactBps > request.maximumPriceImpactBps) blocked("Pool impact exceeds the owner cap.", "orca_price_impact");
  if (BigInt(swap.amountOutAtomic) > destinationReserve) blocked("Quote exceeds the destination vault reserve.", "orca_pool_state");
  const minimum = (BigInt(swap.amountOutAtomic) * BigInt(10_000 - request.slippageBps) + 9_999n) / 10_000n;
  if (minimum <= 0n) blocked("Minimum output is zero.", "orca_output_floor");
  return { ...orcaStableInventory(), slot: read.slot.toString(), sourceReserveAtomic: sourceReserve.toString(),
    ...
    spotUsdtPerUsdcAtomic: spotOutput(1_000_000n, pool.sqrtPrice).toString(),
    tickCurrentIndex: pool.tickCurrentIndex, tickArrayStarts: ticks.map((row) => row.startTickIndex),
    initializedTicksCrossed: swap.initializedTicksCrossed, signed: false, broadcast: false } as const;
}
...
function pinnedPool(account: RawSolanaAccount | null): WhirlpoolState {
  ...
  if (pool.config !== WHIRLPOOLS_CONFIG || pool.mintA !== USDC_MINT || pool.mintB !== SOLANA_USDT ||
      ...
      pool.feeTierIndexSeed !== 1) blocked("Stable Whirlpool identity changed.", "orca_pool_pin_drift");
  if (pool.feeRate !== ORCA_STABLE_FEE_RATE) blocked("Stable Whirlpool fee changed.", "orca_pool_fee_drift");
  return pool;
}
