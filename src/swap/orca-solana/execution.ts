...
import { assertSolanaNetwork, rpcAtomic, type SolanaRpcPort } from "../../solana/rpc.js";
...
import { createOrcaExecutionBinding, type OrcaExecutionBinding, type OrcaExecutionBindingStore, type OrcaExecutionFreshness,
  type OrcaLocalSigner, type OrcaSingleSender } from "./effects.js";
...
export class OrcaExecutionGuard {
  constructor(private readonly rpc: SolanaRpcPort, private readonly clock: ClockPort, private readonly verifyPins: OrcaProgramPinVerifier) {}

  async inspect(operationValue: SwapOperationRecord, materialValue: OrcaKeylessMaterial): Promise<OrcaExecutionFreshness> {
    ...
    await assertSolanaNetwork(this.rpc); await this.verifyPins(this.rpc);
    ...
    if (rent > BigInt(execution.usdcAccountRentLamports)) blocked("The USDC account rent is no longer covered by the approval.", "orca_rent_drift");
    ...
    if (fee > BigInt(execution.networkFeeLamports)) blocked("The network fee exceeds the approved fee.", "orca_fee_drift");
    ...
    if (height > BigInt(lifetime.lastValidBlockHeight)) blocked("The fresh blockhash expired before signing.", "orca_lifetime_expired");
 ...
 */
export class OrcaSolanaExecutionDriver implements GuardedSwapExecutionDriver {
  constructor(private readonly d: OrcaExecutionDriverDependencies) {}
  ...
  async execute(input: GuardedSwapExecutionInput): Promise<SwapOperationRecord> {
    ...
    if (operation.state !== "reserved" || operation.submissionMarker !== null) blocked("Orca execution requires the exact approved reservation.", "orca_execution_state");
    const material = validateOrcaKeylessMaterial(input.material);
    let admission, freshness: OrcaExecutionFreshness;
    try {
      ...
      if (freshness.checkedAt >= operation.quote.expiresAt) blocked("The approval window closed before signing.", "orca_deadline");
    ...
    let binding: OrcaExecutionBinding;
    try { binding = createOrcaExecutionBinding({ operation, material, freshness, admission }); await this.d.bindings.save(operation, binding, material); }
    catch { return await this.d.core.recordPossibleSend(operation, "unknown_finality", this.d.clock.now()); }
    let signature: string, sent: "submitted" | "possible_send";
    try {
  ...
  async observe(input: GuardedSwapObservationInput): Promise<SwapOperationRecord> {
    let operation = validateSwapOperation(input.operation);
    if (operation.submissionMarker === null) blocked("Orca observation is available only after the submission marker.", "orca_resume_before_marker");
    const material = validateOrcaKeylessMaterial(input.material);
    ...
    if (binding === null || effect === null) return operation;
    return await this.observeExact(operation, binding, material, effect.transactionId);
  }

  private async observeExact(operation: SwapOperationRecord, binding: OrcaExecutionBinding, material: OrcaKeylessMaterial,
    signature: string): Promise<SwapOperationRecord> {
    let outcome;
    try { outcome = await this.d.observer.observeOutcome(operation, binding, material, signature); }
