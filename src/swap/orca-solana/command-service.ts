...
import {
  ...
  WHIRLPOOL_PROGRAM, WHIRLPOOLS_CONFIG, WSOL_MINT,
} from "./pins.js";

type Request = Extract<CommandRequest, { readonly command: `swap.orca.${string}` }>;

/** Inventory is the owner's source for the keyless pin and digest; nothing here admits or signs anything. */
export function orcaInventory(runtimeInstalled: boolean): unknown {
...
export async function executeOrcaCommand(request: Request, context: RuntimeContext): Promise<CommandOutcome> {
  ...
  if (runtime === undefined) throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "The keyless Orca runtime is not installed for this command.",
    { reason: "orca_runtime_unavailable" });
  ...
  if (request.command === "swap.orca.prepare") return operationOutcome(await runtime.prepare(request, context.clock.now()));
  if (request.command === "swap.orca.status") return operationOutcome(await runtime.status(request.operationId, context.clock.now()));
  // Foreground CLI: the typed approval code and the single send are one command, like the Uniswap path.
  if (request.command === "swap.orca.approve") return operationOutcome(await runtime.approveAndExecute(request.operationId, context.clock.now()));
  return operationOutcome(await runtime.execute(request.operationId, context.clock.now()));
}

function data(value: unknown, proofClass: string): CommandOutcome { return { proofClass, data: value, operation: null, receipt: null, nextActions: [] }; }
function operationOutcome(value: Awaited<ReturnType<NonNullable<RuntimeContext["orcaRuntime"]>["status"]>>): CommandOutcome {
