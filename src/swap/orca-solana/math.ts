...
import { TICK_ARRAY_SIZE, type TickArrayState, type WhirlpoolState } from "./accounts.js";
...
export interface WhirlpoolSwapQuote {
  readonly amountInAtomic: string; readonly amountOutAtomic: string; readonly feeAtomic: string;
  readonly sqrtPriceBefore: string; readonly sqrtPriceAfter: string; readonly tickBefore: number; readonly tickAfter: number;
  readonly steps: number; readonly initializedTicksCrossed: number;
...
export function sqrtPriceAtTick(tick: number): bigint {
  if (!Number.isSafeInteger(tick) || tick >= 0 || tick < MIN_TICK) blocked("Whirlpool tick is outside the pinned negative range.", "orca_tick_range");
  const absolute = -tick;
...
export function quoteWhirlpoolExactInAToB(pool: WhirlpoolState, arrays: readonly TickArrayState[], amountIn: bigint): WhirlpoolSwapQuote {
  if (amountIn <= 0n || amountIn > U64_MAX) blocked("Whirlpool input must be a positive u64.", "orca_amount");
  if (arrays.length !== 3) blocked("Whirlpool swap requires exactly three tick arrays.", "orca_tick_array_state");
  const spacing = pool.tickSpacing, span = TICK_ARRAY_SIZE * spacing, fee = BigInt(pool.feeRate);
  if (spacing <= 0 || fee >= FEE_RATE_DENOMINATOR || pool.liquidity <= 0n || pool.liquidity > U128_MAX) blocked("Whirlpool state is not swappable.", "orca_pool_state");
  ...
  while (remaining > 0n) {
    if (++steps > MAX_STEPS) blocked("Whirlpool swap crosses too many ticks for this bounded quote.", "orca_tick_boundary");
    ...
    if (next.sequenceEnd && step.nextPrice === target) blocked("The swap would reach the end of the passed tick arrays.", "orca_tick_boundary");
    remaining -= step.amountIn + step.fee;
    output = checkedU64(output + step.amountOut); feeTotal += step.fee;
    if (step.nextPrice === target) {
      const row = arrays[next.arrayIndex]!.ticks[(next.tick - arrays[next.arrayIndex]!.startTickIndex) / spacing]!;
      if (row.initialized) { liquidity -= row.liquidityNet; crossed += 1; }
      if (liquidity < 0n || liquidity > U128_MAX) blocked("Whirlpool liquidity left its bounds while crossing a tick.", "orca_pool_state");
      tick = next.tick - 1;
    } else if (step.nextPrice !== price) tick = tickAtOrBelow(step.nextPrice, tick);
    price = step.nextPrice; arrayIndex = next.arrayIndex;
    if (liquidity === 0n && remaining > 0n) blocked("Whirlpool has no liquidity for the rest of this swap.", "orca_pool_state");
  }
  if (output <= 0n) blocked("Whirlpool quote returned no output.", "orca_quote_zero");
  ...
  const impact = spot === 0n ? 10_000n : (shortfall * 10_000n + spot - 1n) / spot;
  return { amountInAtomic: amountIn.toString(), amountOutAtomic: output.toString(), feeAtomic: feeTotal.toString(),
    ...
    spotOutputPerSolAtomic: spotOutput(1_000_000_000n, pool.sqrtPrice).toString(), priceImpactBps: Number(impact > 10_000n ? 10_000n : impact) };
}
...
export function tickArrayStart(tick: number, spacing: number): number {
  const span = TICK_ARRAY_SIZE * spacing; return Math.floor(tick / span) * span;
}
/** sqrtPrice^2 / 2^128 is token B atomic per token A atomic. */
export function spotOutput(amountA: bigint, sqrtPrice: bigint): bigint { return amountA * sqrtPrice * sqrtPrice / (Q64 * Q64); }

function computeSwapStep(remaining: bigint, fee: bigint, liquidity: bigint, current: bigint, target: bigint) {
  ...
  if (amountIn + stepFee > remaining) blocked("Whirlpool swap step exceeded the remaining input.", "orca_pool_state");
  return { amountIn, amountOut, fee: stepFee, nextPrice };
}
function deltaA(first: bigint, second: bigint, liquidity: bigint, roundUp: boolean): bigint {
  ...
  if (numerator > U256_MAX || denominator === 0n) blocked("Whirlpool token A delta overflowed.", "orca_pool_state");
  const result = roundUp ? divideUp(numerator, denominator) : numerator / denominator;
  if (result > U128_MAX) blocked("Whirlpool token A delta overflowed.", "orca_pool_state");
  return result;
}
function deltaB(first: bigint, second: bigint, liquidity: bigint): bigint {
  const [lower, upper] = first < second ? [first, second] : [second, first];
  const product = liquidity * (upper - lower);
  if (product > U128_MAX) blocked("Whirlpool token B delta overflowed.", "orca_pool_state");
  return product >> 64n;
}
function nextSqrtPriceFromAInput(price: bigint, liquidity: bigint, amount: bigint): bigint {
  if (amount === 0n) return price;
  const numerator = (liquidity * price) << 64n, denominator = (liquidity << 64n) + price * amount;
  if (numerator > U256_MAX || denominator > U256_MAX) blocked("Whirlpool next price overflowed.", "orca_pool_state");
  const next = divideUp(numerator, denominator);
  if (next < WHIRLPOOL_MIN_SQRT_PRICE) blocked("Whirlpool next price is below the protocol minimum.", "orca_pool_state");
  return next;
}
function nextInitializedTick(arrays: readonly TickArrayState[], tick: number, spacing: number, start: number) {
  let search = tick;
  for (let index = start; index < arrays.length; index += 1) {
    ...
    if (search < lower || search >= upper) blocked("Whirlpool search left the passed tick array.", "orca_tick_boundary");
    for (let offset = Math.floor((search - lower) / spacing); offset >= 0; offset -= 1) {
      if (array.ticks[offset]!.initialized) return { arrayIndex: index, tick: lower + offset * spacing, sequenceEnd: false };
    }
    if (index === arrays.length - 1) return { arrayIndex: index, tick: lower, sequenceEnd: true };
    search = lower - 1;
  }
...
function tickAtOrBelow(price: bigint, from: number): number {
  let tick = from;
  for (let guard = 0; guard < 4_096; guard += 1) { if (sqrtPriceAtTick(tick) <= price) return tick; tick -= 1; }
...
function checkedU64(value: bigint): bigint { if (value < 0n || value > U64_MAX) blocked("Whirlpool amount left u64.", "orca_pool_state"); return value; }
