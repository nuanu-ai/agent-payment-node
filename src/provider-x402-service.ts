...
import {
  captureProviderEvidenceLowerBlock,
  observeProviderSettlement,
  providerX402ReadPort,
} from "./provider-x402-evidence.js";
import {
  providerX402RequestHash,
...
import {
  providerX402TerminalReceipt,
  transitionProviderX402Operation,
  type ProviderX402TransitionExtra,
} from "./provider-x402-state.js";
...
import {
  assertProviderAtomicAmount,
  assertProviderPolicyBalance,
  sameFrozenProviderPolicy,
  sameFrozenProviderProfile,
  soleProviderOffer,
} from "./provider-x402-policy.js";
import { waitForProviderSettlement, type ProviderSettlementWaitProjection } from "./provider-x402-wait.js";
import { boundedX402ReadPort } from "./x402-service-rpc.js";
import {
  assertFrozenProviderX402Policy,
  assertProviderX402RpcBinding,
  canonicalProviderX402RpcUrl,
  observeProviderX402Balance,
  requireProviderX402Adapter,
...
export class ProviderX402Service {
  ...
  async prepare(request: PrepareRequest): Promise<unknown> {
    ...
    return await state.withLocks([
      ...
      try { return publicProviderX402Operation(await this.completePreparation(operation)); }
      catch {
        const durable = await this.repository.loadOperation(profileHash, operationId);
        if (durable !== null) return publicProviderX402Operation(durable);
        throw new ApnError("APN_STATE_CORRUPT", "Staged provider x402 operation disappeared during preparation.");
      }
    });
  }
  private async completePreparation(operation: ProviderX402OperationRecord): Promise<ProviderX402OperationRecord> {
  ...
  async approve(operationIdInput: string): Promise<unknown> {
    ...
    return await this.withOperationLock(found, async (loaded) => {
      ...
      if (current.terminal) return publicProviderX402Operation(current);
      if (current.state !== "awaiting_approval") return publicProviderX402Operation(await this.reconcile(current));
      let operation = current;
      try {
        await this.operations.assertProviderAccountAvailable(operation.provider.providerId,
          operation.provider.accountBindingHash, operation.provider.payer, operation.operationId);
        const bound = await requireProviderX402Profile(this.context, operation.profileHash);
        if (!sameFrozenProviderProfile(bound, operation)) await this.failBeforeEffect(operation, "provider_profile_changed");
        const adapter = requireProviderX402Adapter(this.context, bound);
        ...
        if (lower.rpcOriginHash !== operation.rpcOriginHash) await this.failBeforeEffect(operation, "rpc_origin_changed");
        ...
        const selected = soleProviderOffer(challenge);
        if (
          selected.digest !== operation.requirement.digest || selected.payee !== operation.requirement.payee ||
        ...
        if (error instanceof ApnError && error.code === "APN_OPERATION_BLOCKED") throw error;
        ...
        if (durable === null || durable.state !== "awaiting_approval" || operation.state !== "awaiting_approval") throw error;
  ...
  async resume(operationIdInput: string, waitSeconds?: number): Promise<ProviderSettlementWaitProjection | undefined> {
    ...
    try {
      ...
      if (waitSeconds !== undefined && isCode(error, "APN_STATE_BUSY")) return waitTimeout(waitSeconds, 0);
      throw error;
    }
    if (waitSeconds === undefined) return undefined;
    const current = await this.repository.findOperation(operationId);
    if (current?.terminal === true) return {
      outcome: "completed", requestedSeconds: waitSeconds.toString(), observationCount: "0",
    };
    return await waitForProviderSettlement(this.context.wait, waitSeconds, deadline as number, async () => {
      if (this.context.wait.nowMs() >= (deadline as number)) return false;
      const latest = await this.repository.findOperation(operationId);
      if (latest === null) throw new ApnError("APN_OPERATION_NOT_FOUND", "Operation was not found.");
      try { await this.withOperationLock(latest, async (locked) => await this.reconcile(locked, deadline), deadline); }
      catch (error) {
        if (isCode(error, "APN_STATE_BUSY")) return false;
        throw error;
      }
  ...
  private async reconcile(operation: ProviderX402OperationRecord, deadline?: number): Promise<ProviderX402OperationRecord> {
    ...
    if (operation.settlementEvidence !== undefined) {
      if (operation.state === "ambiguous_effect" && operation.sellerResult !== undefined) return operation;
    ...
    if (rpc === null) throw new ApnError("APN_OPERATION_BLOCKED", "Provider settlement wait requires bounded x402 RPC.");
    const observation = await observeProviderSettlement(operation, rpc);
    if (observation.kind === "pending") return operation;
    if (observation.kind === "ambiguous") {
      ...
      if (operation.state === "ambiguous_effect" && operation.reason === observation.reason && sameUpper) return operation;
  ...
  private async recoverOrphanReceipt(operation: ProviderX402OperationRecord): Promise<ProviderX402OperationRecord> {
    ...
    if (operation.terminal) {
      if (receipt === null) {
        ...
        const reconstructed = providerX402TerminalReceipt(
  ...
  private async withOperationLock<T>(
    ...
    action: (current: ProviderX402OperationRecord) => Promise<T>,
    deadline?: number,
  ): Promise<T> {
    const remaining = deadline === undefined ? undefined : Math.floor(deadline - this.context.wait.nowMs());
  ...
  private async failBeforeEffect(
    operation: ProviderX402OperationRecord,
    reason: string,
    shouldThrow = true,
...
function waitTimeout(seconds: number, observations: number): ProviderSettlementWaitProjection {
