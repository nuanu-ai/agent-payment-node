...
export async function prepareCoinbaseGasless(
  context: RuntimeContext,
  operations: OperationService,
  durable: ProviderDirectState,
  loadProfile: (profileHash: string) => Promise<ProviderProfileRecord>,
  request: Extract<CommandRequest, { command: "gasless.transfer.prepare" }>,
): Promise<unknown> {
  const profile = canonicalProfile(request.profile), gasless = gaslessCommandRequest(request.request);
  if (gasless.chainId !== CHAIN_ID) throw new ApnError("APN_PROVIDER_CAPABILITY_UNAVAILABLE", "Coinbase gasless supports Base USDC only.");
  ...
  if (gross <= 0n || minimum <= 0n || minimum > gross || maximumFee < 0n) throw new ApnError("APN_INVALID_INPUT", "Coinbase gasless amount bounds are invalid.");
  await context.ready();
  ...
  return await state.withLocks([`profile:${profileHash}`, `provider-account:${initialBound.provider_id}:${initialBound.account_binding_hash}`,
    ...
    if (bound.public_address === recipient) throw new ApnError("APN_INVALID_INPUT", "Coinbase gasless sender and recipient must differ.");
    ...
    if (BigInt(snapshot.balanceAtomic) < gross) throw new ApnError("APN_INSUFFICIENT_USDC", "USDC balance is insufficient for the gross transfer.");
    ...
    const providerDirect: ProviderDirectBinding = { schemaVersion: "apn.provider-direct.v1", providerId: bound.provider_id,
      profileRevision: bound.revision, capabilityHash: bound.capability_hash, accountBindingHash: bound.account_binding_hash,
    ...
    const initial = { at: preparedAt.toISOString(), state: "awaiting_approval" as const, terminal: false,
    ...
    await durable.persist(operation); return publicOperation(operation);
  });
}

export async function coinbaseGaslessPreconditionsMatch(context: RuntimeContext, operation: OperationRecord): Promise<boolean> {
...
export async function reobserveCoinbaseGasless(
  ...
  if (observed.status === "not_found" || observed.status === "pending") return await durable.transition(operation,
    "ambiguous_effect", false, observed.reason, "provider_effect_no_replay", { coinbaseGaslessCursor: observed.cursor });
...
function assertCoinbaseProfile(profile: ProviderProfileRecord): void {
