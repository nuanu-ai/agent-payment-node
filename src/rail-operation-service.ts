...
export class RailOperationService {
      ...
      try {
        const committed = await state.withLocks(keys, async () => {
          ...
          if (latest.integrityHash !== operation.integrityHash || saved?.integrityHash !== claim.integrityHash) {
            ...
            return { done: publicRailOperation(latest) } as const;
          }
        ...
        if ("done" in committed) return committed.done;
  ...
  private async executeLocalSolanaClaimed(initial: RailOperationRecord, adapter: DirectRailPort, mayStartSigning = false): Promise<unknown> {
    const keys = [`profile:${initial.profileHash}`, `operation:${initial.operationId}`];
    const state = this.context.state;
    let signed: RailOperationRecord;
    let effect: RailSignedEffect;
    if (initial.state === "signing_started") {
      const recovered = await state.withLocks(keys, async () => {
        ...
        if (latest.integrityHash !== initial.integrityHash) return { done: latest } as const;
        const sealed = await adapter.recoverEffect(binding(latest));
        if (sealed === null) return mayStartSigning
          ? { pending: latest } as const
          : { done: await this.move(latest, "failed_before_effect", "custody_proved_no_sealed_effect", "durable_pre_effect") } as const;
        this.assertEffect(latest, sealed);
        return { signed: await this.move(latest, "signed_not_submitted", "encrypted_effect_recovered", "durable_signed_effect", sealed), effect: sealed } as const;
      });
      if ("done" in recovered) return publicRailOperation(recovered.done);
      if ("signed" in recovered) { signed = recovered.signed!; effect = recovered.effect!; }
      else {
        const pending = recovered.pending;
        try { await this.revalidate(pending, adapter); }
        catch (error) {
        ...
        const sealed = await state.withLocks(keys, async () => {
          const latest = await this.required(initial.operationId);
          if (latest.integrityHash !== pending.integrityHash) return { done: latest } as const;
          try { await this.assertCurrentRailPolicy(latest); }
          catch (error) {
            await this.move(latest, "failed_before_effect", "pre_sign_owner_or_policy_changed", "durable_pre_effect");
            throw error;
          }
          if (adapter.sealRevalidated === undefined) corrupt();
          let value: RailSignedEffect;
          try { value = await adapter.sealRevalidated(binding(latest)); }
          catch (error) {
          ...
          return { signed: await this.move(latest, "signed_not_submitted", "encrypted_effect_bound", "durable_signed_effect", value), effect: value } as const;
        });
        if ("done" in sealed) return publicRailOperation(sealed.done);
        signed = sealed.signed; effect = sealed.effect;
      }
      ...
      const recovered = await state.withLocks(keys, async () => {
        ...
        if (latest.integrityHash !== initial.integrityHash) return { done: latest } as const;
        const sealed = await adapter.recoverEffect(binding(latest));
        if (sealed === null) corrupt();
        this.assertEffect(latest, sealed);
        return { signed: latest, effect: sealed } as const;
      });
      if ("done" in recovered) return publicRailOperation(recovered.done);
      signed = recovered.signed; effect = recovered.effect;
    } else return publicRailOperation(initial);

    try { await this.revalidate(signed, adapter); }
    catch {
    ...
    const committed = await state.withLocks(keys, async () => {
      const latest = await this.required(initial.operationId);
      if (latest.integrityHash !== signed.integrityHash) return { done: latest } as const;
      try { await this.assertCurrentRailPolicy(latest); }
      catch {
        return { done: await this.move(latest, "failed_before_effect", "never_submitted_frozen_effect_no_longer_valid", "durable_pre_effect") } as const;
      }
      // The intent is durable before adapter.submit can make even one physical send attempt.
      return { submitting: await this.move(latest, "submitting", "submission_intent_persisted", "effect_outcome_unknown") } as const;
    });
    if ("done" in committed) return publicRailOperation(committed.done);
    ...
    try {
      ...
      if (result.transactionId !== effect.transactionId) throw new Error("effect mismatch");
    ...
    const acknowledged = await state.withLocks(keys, async () => {
      const latest = await this.required(initial.operationId);
      if (latest.integrityHash !== submitting.integrityHash) return latest;
  ...
  private async assertCurrentRailPolicy(operation: RailOperationRecord): Promise<RailOperationRecord["account"]> {
