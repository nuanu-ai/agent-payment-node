...
export class ProviderDirectState {
  ...
  async inspectReceipt(operation: OperationRecord): Promise<OperationRecord> {
    if (operation.transactionHash === undefined || operation.terminal ||
      !["provider_acknowledged", "evidence_pending", "ambiguous_effect"].includes(operation.state)) return operation;
    const binding = operation.providerDirect;
    if (binding === undefined) return operation;
    let receipt: RpcReceipt | null;
    try {
      if (sha256(`direct-rpc\0${this.context.requireRpcUrl()}`) !== binding.rpcBindingHash) throw new Error("rpc binding drift");
  ...
  async persist(operation: OperationRecord, rpcReceipt?: RpcReceipt): Promise<void> {
  ...
  private async receiptAmbiguous(operation: OperationRecord, reason: string, receipt?: RpcReceipt): Promise<OperationRecord> {
